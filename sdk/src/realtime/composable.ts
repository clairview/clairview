...
/**
 * Creates a client to communicate with a Clairview REST WebSocket.
 *
 * @param config The optional configuration.
 *
 * @returns A Clairview realtime client.
 */
export function realtime(config: WebSocketConfig = {}) {
	return <Schema>(client: ClairviewClient<Schema>) => {
		...
		const debug = (level: keyof ConsoleInterface, ...data: any[]) =>
			config.debug && client.globals.logger[level]('[Clairview SDK]', ...data);
