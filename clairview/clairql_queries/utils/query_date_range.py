...
from clairview.clairql.errors import ImpossibleASTError
from clairview.clairql.parser import ast
from clairview.models.team import Team, WeekStartDay
from clairview.queries.util import get_earliest_timestamp, get_trunc_func_ch
from clairview.schema import DateRange, InsightDateRange, IntervalType
...
# Originally similar to clairview/queries/query_date_range.py but rewritten to be used in ClairQL queries
class QueryDateRange:
    ...
    def interval_period_string_as_clairql_constant(self) -> ast.Expr:
    ...
    def date_to_start_of_interval_clairql(self, date: ast.Expr) -> ast.Call:
    ...
    def date_from_to_start_of_interval_clairql(self) -> ast.Call:
        return self.date_to_start_of_interval_clairql(self.date_from_as_clairql())

    def date_to_to_start_of_interval_clairql(self) -> ast.Call:
        return self.date_to_start_of_interval_clairql(self.date_to_as_clairql())

    def date_to_with_extra_interval_clairql(self) -> ast.Call:
        return ast.Call(
            name="plus",
            args=[self.date_to_start_of_interval_clairql(self.date_to_as_clairql()), self.one_interval_period()],
        )

    def to_placeholders(self) -> dict[str, ast.Expr]:
        return {
            "interval": self.interval_period_string_as_clairql_constant(),
            "one_interval_period": self.one_interval_period(),
            "number_interval_period": self.number_interval_periods(),
            "date_from": self.date_from_as_clairql(),
            "date_to": self.date_to_as_clairql(),
            "date_from_start_of_interval": self.date_from_to_start_of_interval_clairql(),
            "date_to_start_of_interval": self.date_to_to_start_of_interval_clairql(),
            "date_from_with_adjusted_start_of_interval": (
                self.date_from_to_start_of_interval_clairql()
                if self.use_start_of_interval()
                else self.date_from_as_clairql()
            ),
        }
...
class QueryDateRangeWithIntervals(QueryDateRange):
    ...
    def get_start_of_interval_clairql(self, *, source: ast.Expr = None) -> ast.Expr:
