...
class TestActorsQueryRunner(ClickhouseTestMixin, APIBaseTest):
        ...
        runner = self._create_runner(
            ActorsQuery(
                properties=[
                    ...
                    ClairQLPropertyFilter(key="toInt(properties.index) > 5"),
                ]
            )
        )
        ...
        runner = self._create_runner(
            ActorsQuery(
                fixedProperties=[
                    ...
                    ClairQLPropertyFilter(key="toInt(properties.index) < 2"),
                ]
            )
        )
    ...
    def test_persons_query_search_snapshot(self):
        ...
        assert pretty_print_in_tests(runner.to_clairql(), self.team.pk) == self.snapshot
    ...
    def test_source_clairql_query_poe_on(self):
        self.random_uuid = self._create_random_persons()
        source_query = ClairQLQuery(query="SELECT distinct person_id FROM events WHERE event='clicky-4'")
        query = ActorsQuery(
            select=["properties.email"],
            orderBy=["properties.email DESC"],
            source=source_query,
    ...
    def test_source_clairql_query_poe_off(self):
        self.random_uuid = self._create_random_persons()
        source_query = ClairQLQuery(query="SELECT distinct person_id FROM events WHERE event='clicky-4'")
        query = ActorsQuery(
            select=["properties.email"],
