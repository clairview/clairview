...
from clairview.clairql_queries.utils.recordings_helper import RecordingsHelper
from clairview.models import Team, Group
from clairview.schema import ActorsQuery
...
class ActorStrategy:
    ...
    def __init__(self, team: Team, query: ActorsQuery, paginator: ClairQLHasMorePaginator):
        self.team = team
...
class PersonStrategy(ActorStrategy):
    ...
    def get_actors(self, actor_ids, order_by: str = "") -> dict[str, dict]:
        ...
        persons_query = """SELECT clairview_person.id, clairview_person.uuid, clairview_person.properties, clairview_person.is_identified, clairview_person.created_at
            FROM clairview_person
            WHERE clairview_person.uuid = ANY(%(uuids)s)
            AND clairview_person.team_id = %(team_id)s"""
        if order_by:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                """SELECT clairview_persondistinctid.person_id, clairview_persondistinctid.distinct_id
            FROM clairview_persondistinctid
            WHERE clairview_persondistinctid.person_id = ANY(%(people_ids)s)
            AND clairview_persondistinctid.team_id = %(team_id)s""",
