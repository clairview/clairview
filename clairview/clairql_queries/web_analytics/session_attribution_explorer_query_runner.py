...
class SessionAttributionExplorerQueryRunner(QueryRunner):
    ...
    def to_query(self) -> ast.SelectQuery:
        with self.timings.measure("session_attribution_query_runner"):
            # We use string concatenation here, so that the resultant clairql can be opened as its own clairql insight,
            ...
            # execute_clairql_query. Those filters *are* user input, which is why placeholders are used. It helps that
            # placeholders are valid in ClairQL insights too, so it will "just work"!
...
"""
    ...
    def calculate(self):
        response = self.paginator.execute_clairql_query(
            ...
            filters=ClairQLFilters(dateRange=self.query.filters.dateRange, properties=self.query.filters.properties)
