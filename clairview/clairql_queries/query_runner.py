...
from clairview.caching.utils import is_stale, ThresholdMode, cache_target_age, last_refresh_from_cached_result
from clairview.clickhouse.client.execute_async import enqueue_process_query_task, get_query_status, QueryNotFoundError
from clairview.clickhouse.query_tagging import tag_queries, get_query_tag_value
...
from clairview.schema_helpers import to_dict, to_json
from clairview.utils import generate_cache_key, get_from_dict_or_attr
from clairview.clairql.modifiers import create_default_modifiers_for_user

logger = structlog.get_logger(__name__)

QUERY_CACHE_WRITE_COUNTER = Counter(
    "clairview_query_cache_write_total",
    "When a query result was persisted in the cache.",
    labelnames=[LABEL_TEAM_ID],
)

QUERY_CACHE_HIT_COUNTER = Counter(
    "clairview_query_cache_hit_total",
...
def get_query_runner(
    ...
    if kind == "ClairQLQuery":
        from .clairql_query_runner import ClairQLQueryRunner

        return ClairQLQueryRunner(
            query=cast(ClairQLQuery | dict[str, Any], query),
            team=team,
            timings=timings,
            limit_context=limit_context,
            modifiers=modifiers,
...
class QueryRunner(ABC, Generic[Q, R, CR]):
    ...
    def to_clairql(self) -> str:
        with self.timings.measure("to_clairql"):
            return print_ast(
                self.to_query(),
                ClairQLContext(
                    team_id=self.team.pk,
    ...
    def get_cache_payload(self) -> dict:
        return {
            ...
            "team_id": self.team.pk,
            "clairql_modifiers": to_dict(self.modifiers),
    ...
    def apply_dashboard_filters(self, dashboard_filter: DashboardFilter):
        ...
        # Notable exception: `ClairQLQuery`, which has `properties` and `dateRange` within `ClairQLFilters`
