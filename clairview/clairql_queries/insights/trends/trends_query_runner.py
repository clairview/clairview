...
from clairview.clairql_queries.insights.trends.display import TrendsDisplay
from clairview.clairql_queries.insights.trends.series_with_extras import SeriesWithExtras
from clairview.clairql_queries.insights.trends.trends_actors_query_builder import TrendsActorsQueryBuilder
from clairview.clairql_queries.insights.trends.trends_query_builder import TrendsQueryBuilder
from clairview.clairql_queries.query_runner import QueryRunner
from clairview.clairql_queries.utils.formula_ast import FormulaAST
from clairview.clairql_queries.utils.query_compare_to_date_range import QueryCompareToDateRange
from clairview.clairql_queries.utils.query_date_range import QueryDateRange
from clairview.clairql_queries.utils.query_previous_period_date_range import (
...
from clairview.queries.util import correct_result_for_sampling
...
from clairview.utils import format_label_date, multisort
from clairview.warehouse.models.util import get_view_or_table_by_name
...
class TrendsQueryRunner(QueryRunner):
    ...
    def calculate(self):
        ...
        if len(queries) == 1:
            response_clairql_query = queries[0]
        else:
            response_clairql_query = ast.SelectUnionQuery(select_queries=[])
            for query in queries:
                if isinstance(query, ast.SelectQuery):
                    response_clairql_query.select_queries.append(query)
                else:
                    response_clairql_query.select_queries.extend(query.select_queries)

        with self.timings.measure("printing_clairql_for_response"):
            response_clairql = to_printed_clairql(response_clairql_query, self.team, self.modifiers)
        ...
        return TrendsQueryResponse(
            ...
            hasMore=has_more,
            timings=timings,
            clairql=response_clairql,
            modifiers=self.modifiers,
            error=". ".join(debug_errors),
        )

    def build_series_response(self, response: ClairQLQueryResponse, series: SeriesWithExtras, series_count: int):
        def get_value(name: str, val: Any):
            if name not in ["date", "total", "breakdown_value"]:
                raise Exception("Column not found in clairql results")
            if response.columns is None:
                raise Exception("No columns returned from clairql results")
            ...
            if series.aggregate_values:
                series_object = {
                    ...
                    "action": {  # TODO: Populate missing props in `action`
                        ...
                        "math_clairql": series.series.math_clairql,
    ...
    def update_clairql_modifiers(self) -> None:
        if (
    ...
    def _is_breakdown_field_boolean(
        ...
        if breakdown_type == "clairql" or breakdown_type == "cohort" or breakdown_type == "session":
            return False
    ...
    # TODO: Move this to clairview/clairql_queries/legacy_compatibility/query_to_filter.py
