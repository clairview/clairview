...
from clairview.clairql_queries.insights.trends.breakdown import BREAKDOWN_NULL_STRING_LABEL, BREAKDOWN_OTHER_STRING_LABEL
from clairview.models import Cohort, GroupTypeMapping, Team
...
class TestTrendsPersons(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_trends_breakdown_others_persons(self):
        ...
        result = self._get_actors(
            trends_query=source_query, day="2023-04-29", breakdown="$$_clairview_breakdown_other_$$"
        )
    ...
    def test_trends_breakdown_null_persons(self):
        ...
        result = self._get_actors(trends_query=source_query, day="2023-05-06", breakdown="$$_clairview_breakdown_null_$$")

        self.assertEqual(len(result), 1)
        self.assertEqual(get_distinct_id(result[0]), "person1")
        self.assertEqual(get_event_count(result[0]), 1)

    def test_trends_breakdown_clairql_persons(self):
        self._create_events()
        source_query = TrendsQuery(
            series=[EventsNode(event="$pageview")],
            dateRange=InsightDateRange(date_from="-7d"),
            breakdownFilter=BreakdownFilter(breakdown="properties.some_property", breakdown_type=BreakdownType.CLAIRQL),
        )
    ...
    def trends_all_cohort_breakdown_persons(self, inCohortVia: str):
        ...
        source_query.modifiers = ClairQLQueryModifiers(inCohortVia=inCohortVia)
    ...
    def test_trends_multiple_breakdown_null_persons(self):
        ...
        result = self._get_actors(
            trends_query=source_query, day="2023-05-06", breakdown=["$$_clairview_breakdown_null_$$"]
        )
    ...
    def test_trends_multiple_breakdowns_clairql_persons(self):
        self._create_events()
        source_query = TrendsQuery(
            series=[EventsNode(event="$pageview")],
            dateRange=InsightDateRange(date_from="-7d"),
            breakdownFilter=BreakdownFilter(
                breakdowns=[Breakdown(property="properties.some_property", type=BreakdownType.CLAIRQL)],
                breakdown_limit=1,
            ),
        )
