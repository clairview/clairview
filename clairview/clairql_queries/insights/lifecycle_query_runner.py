...
from clairview.clairql.property import property_to_expr, action_to_expr
...
from clairview.utils import format_label_date
...
class LifecycleQueryRunner(QueryRunner):
            ...
            if day is not None:
                exprs.append(
                    ast.CompareOperation(
                        ...
                        right=self.query_date_range.date_to_start_of_interval_clairql(
    ...
    def calculate(self) -> LifecycleQueryResponse:
        ...
        # e.g. the LifecycleQuery with ClairQLQuery, while keeping the chart logic the same.
        ...
        return LifecycleQueryResponse(results=res, timings=response.timings, clairql=clairql, modifiers=self.modifiers)
            ...
            events_query = parse_select(
                ...
                placeholders={
                    **self.query_date_range.to_placeholders(),
                    ...
                    "trunc_timestamp": self.query_date_range.date_to_start_of_interval_clairql(
                        ast.Field(chain=["events", "timestamp"])
                    ),
                    "trunc_created_at": self.query_date_range.date_to_start_of_interval_clairql(
                        ast.Field(chain=["created_at"])
                    ),
                    "trunc_epoch": self.query_date_range.date_to_start_of_interval_clairql(
    ...
    def periods_query(self):
        with self.timings.measure("periods_query"):
            periods_query = parse_select(
                ...
                placeholders={
                    **self.query_date_range.to_placeholders(),
                    "date_to_plus_interval": self.query_date_range.date_to_start_of_interval_clairql(
                        parse_expr(
