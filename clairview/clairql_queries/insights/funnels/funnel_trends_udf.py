...
from clairview.clairql.constants import ClairQLQuerySettings
from clairview.clairql.parser import parse_select
from clairview.clairql_queries.insights.funnels import FunnelTrends
from clairview.clairql_queries.insights.utils.utils import get_start_of_interval_clairql_str
from clairview.schema import BreakdownType, BreakdownAttributionType
from clairview.utils import DATERANGE_MAP
...
class FunnelTrendsUDF(FunnelTrends):
    ...
    def get_query(self) -> ast.SelectQuery:
        ...
        inner_select = cast(
            ...
            SELECT
                arrayJoin({fn}(
                    ...
                    {prop_vals},
                    arraySort(t -> t.1, groupArray(tuple(toFloat(timestamp), {get_start_of_interval_clairql_str(self.context.interval.value, team=self.context.team, source='timestamp')}, {prop_selector}, arrayFilter((x) -> x != 0, [{steps}{exclusions}]))))
                )) as af_tuple,
        ...
        inner_select.settings = ClairQLQuerySettings(date_time_output_format="iso", date_time_input_format="best_effort")
