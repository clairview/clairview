...
from clairview.clairql_queries.actors_query_runner import ActorsQueryRunner
from clairview.clairql_queries.insights.funnels import Funnel
...
from clairview.clairql_queries.insights.funnels.test.breakdown_cases import (
    assert_funnel_results_equal,
    funnel_breakdown_group_test_factory,
    funnel_breakdown_test_factory,
)
from clairview.clairql_queries.insights.funnels.test.conversion_time_cases import (
...
def funnel_test_factory(Funnel, event_factory, person_factory):
    class TestGetFunnel(ClickhouseTestMixin, APIBaseTest):
        ...
        def test_clairql_aggregation(self):
            # first person
            person_factory(
                distinct_ids=["user"],
                team_id=self.team.pk,
            ...
            results = (
                self._basic_funnel(
                    filters={
                        **basic_filters,
                        "funnel_aggregate_by_clairql": "properties.$session_id",
                    }
                )
                .calculate()
                .results
            ...
            # distinct_id
            results = (
                self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_clairql": "distinct_id"})
                .calculate()
                .results
            )
            self.assertEqual(results[0]["count"], 2)
            ...
            # person_id
            results = (
                self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_clairql": "person_id"})
                .calculate()
                .results
            )
            self.assertEqual(results[0]["count"], 2)
            ...
            result = (
                self._basic_funnel(
                    filters={**basic_filters, "funnel_aggregate_by_clairql": "person.properties.common_prop"}
                )
                .calculate()
                .results
            )
...
class TestFunnelStepCountsWithoutAggregationQuery(BaseTest):
    ...
    def test_smoke(self):
        ...
        response = execute_clairql_query(query_type="FunnelsQuery", query=query_ast, team=self.team)
