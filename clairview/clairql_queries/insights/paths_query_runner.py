import itertools
from clairview.clairql.constants import ClairQLGlobalSettings, MAX_BYTES_BEFORE_EXTERNAL_GROUP_BY
...
from clairview.caching.insights_api import BASE_MINIMUM_INSIGHT_REFRESH_INTERVAL, REDUCED_MINIMUM_INSIGHT_REFRESH_INTERVAL
from clairview.constants import PAGEVIEW_EVENT, SCREEN_EVENT, CLAIRQL
...
from clairview.clairql_queries.insights.funnels.utils import funnel_window_interval_unit_to_sql
...
from clairview.schema import PathsQuery
...
class PathsQueryRunner(QueryRunner):
    ...
    def construct_event_clairql(self) -> ast.Expr:
        event_clairql: ast.Expr = parse_expr("event")

        if self._should_query_event(CLAIRQL) and self.query.pathsFilter.pathsClairQLExpression:
            event_clairql = parse_expr(self.query.pathsFilter.pathsClairQLExpression)

        if self._should_query_event(PAGEVIEW_EVENT):
            event_clairql = parse_expr(
                "if(event = {event}, replaceRegexpAll(ifNull(properties.$current_url, ''), '(.)/$', '\\\\1'), {event_clairql})",
                {"event": ast.Constant(value=PAGEVIEW_EVENT), "event_clairql": event_clairql},
            )

        if self._should_query_event(SCREEN_EVENT):
            event_clairql = parse_expr(
                "if(event = {event}, properties.$screen_name, {event_clairql})",
                {"event": ast.Constant(value=SCREEN_EVENT), "event_clairql": event_clairql},
            )

        return event_clairql
    ...
    def funnel_join(self) -> ast.JoinExpr:
        ...
        from clairview.clairql_queries.insights.insight_actors_query_runner import InsightActorsQueryRunner
    ...
    def paths_events_query(self) -> ast.SelectQuery:
        ...
        event_clairql = self.construct_event_clairql()
        event_conditional = parse_expr("ifNull({event_clairql}, '') AS path_item_ungrouped", {"event_clairql": event_clairql})
        ...
        return ast.And(
            exprs=[
                ast.CompareOperation(
                    ...
                    right=self.query_date_range.date_from_to_start_of_interval_clairql(),
                ),
                ast.CompareOperation(
                    op=ast.CompareOperationOp.LtEq,
                    left=field_to_compare,
                    right=self.query_date_range.date_to_as_clairql(),
                ),
            ]
        )
    ...
    def calculate(self) -> PathsQueryResponse:
        query = self.to_query()
        clairql = to_printed_clairql(query, self.team)
        ...
        response = execute_clairql_query(
            ...
            modifiers=self.modifiers,
            limit_context=self.limit_context,
            settings=ClairQLGlobalSettings(
        ...
        return PathsQueryResponse(results=results, timings=response.timings, clairql=clairql, modifiers=self.modifiers)
