...
from clairview.schema import ClairQLQueryModifiers, PersonsArgMaxVersion
...
class TestInsightActorsQueryRunner(ClickhouseTestMixin, APIBaseTest):
    ...
    def select(self, query: str, placeholders: Optional[dict[str, Any]] = None, modifiers: Optional[dict] = None):
        ...
        return execute_clairql_query(
            ...
            modifiers=ClairQLQueryModifiers(**modifiers) if modifiers else None,
        )
                    ...
                    <ActorsQuery select={['properties.name']}>
                        <InsightActorsQuery day='2020-01-09'>
                            <TrendsQuery
                                dateRange={<InsightDateRange date_from='2020-01-09' date_to='2020-01-19' />}
                                series={[<EventsNode event='$pageview' />]}
                                properties={[<PersonPropertyFilter type='person' key='email' value='tom@clairview.com' operator='is_not' />]}
                            />
                        </InsightActorsQuery>
                    </ActorsQuery>
                )
