...
from clairview.test.base import APIBaseTest, _create_event, _create_person
...
class TestStickinessQueryRunner(APIBaseTest):
    ...
    def test_property_filtering_clairql(self):
        self._create_test_events()

        response = self._run_query(properties=[ClairQLPropertyFilter(key="properties.$browser == 'Chrome'")])
    ...
    def test_clairql_aggregations(self):
        self._create_test_events()

        series: list[EventsNode | ActionsNode] = [
            EventsNode(event="$pageview", math="clairql", math_clairql="e.properties.prop")
        ]
