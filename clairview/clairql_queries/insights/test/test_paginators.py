...
class TestClairQLHasMorePaginator(ClickhouseTestMixin, APIBaseTest):
    maxDiff = None
    random_uuid: str
    ...
    def test_response_params_consistency(self):
        ...
        paginator = ClairQLHasMorePaginator(limit=5, offset=10)
        paginator.response = paginator.execute_clairql_query(
    ...
    def test_handle_none_response(self):
        ...
        paginator = ClairQLHasMorePaginator(limit=5, offset=0)
    ...
    def test_limit_context_variations(self):
        ...
        for case in test_cases:
            with self.subTest(case=case):
                paginator = ClairQLHasMorePaginator.from_limit_context(
    ...
    @patch("clairview.clairql_queries.insights.paginators.execute_clairql_query")
    def test_passes_limit_context(self, mock_execute_clairql_query: MagicMock):
        limit_context = LimitContext.EXPORT
        paginator = ClairQLHasMorePaginator.from_limit_context(limit_context=limit_context, limit=5, offset=10)
        paginator.execute_clairql_query(
            query=cast(SelectQuery, parse_select("SELECT * FROM persons")), query_type="query type"
        )
        mock_execute_clairql_query.assert_called_once()
        self.assertEqual(mock_execute_clairql_query.call_args.kwargs["limit_context"], limit_context)
