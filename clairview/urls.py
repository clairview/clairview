...
from clairview.api.early_access_feature import early_access_features
from clairview.api.survey import surveys
from clairview.constants import PERMITTED_FORUM_DOMAINS
from clairview.demo.legacy import demo_route
...
from .year_in_clairview import year_in_clairview
...
def home(request, *args, **kwargs):
    if request.get_host().split(":")[0] == "app.clairview.com" and get_instance_setting("REDIRECT_APP_TO_US"):
        url = "https://us.clairview.com{}".format(request.get_full_path())
        if url_has_allowed_host_and_scheme(url, "us.clairview.com", True):
...
urlpatterns = [
    ...
    *ee_urlpatterns,
    ...
    path("year_in_clairview/2022/<str:user_uuid>", year_in_clairview.render_2022),
    path("year_in_clairview/2022/<str:user_uuid>/", year_in_clairview.render_2022),
    path("year_in_clairview/2023/<str:user_uuid>", year_in_clairview.render_2023),
    path("year_in_clairview/2023/<str:user_uuid>/", year_in_clairview.render_2023),
]
...
if settings.TEST:
    # Used in clairview-js e2e tests
    @csrf_exempt
