...
from clairview.storage.object_storage import (
    health_check,
    read,
    write,
    get_presigned_url,
...
class TestStorage(APIBaseTest):
    ...
    @patch("clairview.storage.object_storage.client")
    ...
    def test_can_generate_presigned_url_for_existing_file(self) -> None:
        with self.settings(OBJECT_STORAGE_ENABLED=True):
            ...
            self.assertRegex(
                presigned_url,
                r"^http://localhost:\d+/clairview/test_storage_bucket/test_can_generate_presigned_url_for_existing_file/.*\?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=.*$",
            )
    ...
    def test_can_generate_presigned_url_for_non_existent_file(self) -> None:
        with self.settings(OBJECT_STORAGE_ENABLED=True):
            ...
            self.assertRegex(
                presigned_url,
                r"^http://localhost:\d+/clairview/test_storage_bucket/test_can_ignore_presigned_url_for_non_existent_file/.*?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=.*$",
            )
