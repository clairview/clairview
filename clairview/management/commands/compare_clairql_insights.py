...
class Command(BaseCommand):
    help = "Test if ClairQL insights match their legacy counterparts"

    # 1133835
    def handle(self, *args, **options):
        import json
        from typing import cast
        from clairview.schema import ClairQLQueryModifiers, ClairQLQueryResponse, MaterializationMode
        from clairview.models import Insight, Filter
        ...
        for insight in insights[0:500]:
            ...
            try:
                ...
                url = f"https://us.clairview.com/project/{insight.team_id}/insights/{insight.short_id}/edit"
            ...
            try:
                ...
                clairql_results = cast(ClairQLQueryResponse, query_runner.calculate()).results or []
            except Exception as e:
                url = f"https://us.clairview.com/project/{insight.team_id}/insights/{insight.short_id}/edit"
                print(f"ClairQL Insight {url} ({insight.id}). ERROR: {e}")  # noqa: T201
                print(json.dumps(insight.filters))  # noqa: T201
                continue
            try:
                all_ok = True
                sorter = lambda x: (
                    "$$_clairview_breakdown_other_$$" if x.get("breakdown_value") == "Other" else x.get("breakdown_value")
                )
                sorted_legacy_results = sorted(
                    legacy_results,
                    key=sorter,
                )
                sorted_clairql_results = sorted(clairql_results, key=lambda x: x.get("breakdown_value"))
                for legacy_result, clairql_result in zip(sorted_legacy_results, sorted_clairql_results):
                    fields = ["label", "count", "aggregated_value", "data", "labels", "days"]
                    for field in fields:
                        legacy_value = legacy_result.get(field)
                        clairql_value = clairql_result.get(field)
                        if field == "count":
                            legacy_value = int(legacy_value or 0)
                            clairql_value = int(clairql_value)
                        if field == "data":
                            legacy_value = [int(x) for x in legacy_value or []]
                            clairql_value = [int(x) for x in clairql_value or []]
                        if legacy_value != clairql_value:
                            if (
                                (field == "days" and clairql_value == [])
                                or (field == "labels" and insight.filters.get("interval") == "month")
                                or (field == "labels" and legacy_value == [] and clairql_value is None)
                                or (
                                    field == "label"
                                    and legacy_value == "Other"
                                    and clairql_value == "$$_clairview_breakdown_other_$$"
                                )
                            ):
                                continue
                            print(  # noqa: T201
                                f"Insight https://us.clairview.com/project/{insight.team_id}/insights/{insight.short_id}/edit"
                                f" ({insight.id}). MISMATCH in {legacy_result.get('status')} row, field {field}"
                            )
                            print("Legacy:", legacy_value)  # noqa: T201
                            print("ClairQL: ", clairql_value)  # noqa: T201
                            print(json.dumps(insight.filters))  # noqa: T201
                            print("")  # noqa: T201
                            all_ok = False
                if all_ok:
