...
class Command(BaseCommand):
    help = "Test if TorQL insights match their legacy counterparts"

    # 1133835
    def handle(self, *args, **options):
        import json
        from typing import cast
        from clairview.schema import TorQLQueryModifiers, TorQLQueryResponse, MaterializationMode
        ...
        for insight in insights[0:500]:
            ...
            try:
                ...
                torql_results = cast(TorQLQueryResponse, query_runner.calculate()).results or []
            except Exception as e:
                url = f"https://us.clairview.com/project/{insight.team_id}/insights/{insight.short_id}/edit"
                print(f"TorQL Insight {url} ({insight.id}). ERROR: {e}")  # noqa: T201
                print(json.dumps(insight.filters))  # noqa: T201
                continue
            try:
                all_ok = True
                ...
                sorted_torql_results = sorted(torql_results, key=lambda x: x.get("breakdown_value"))
                for legacy_result, torql_result in zip(sorted_legacy_results, sorted_torql_results):
                    fields = ["label", "count", "aggregated_value", "data", "labels", "days"]
                    for field in fields:
                        legacy_value = legacy_result.get(field)
                        torql_value = torql_result.get(field)
                        if field == "count":
                            legacy_value = int(legacy_value or 0)
                            torql_value = int(torql_value)
                        if field == "data":
                            legacy_value = [int(x) for x in legacy_value or []]
                            torql_value = [int(x) for x in torql_value or []]
                        if legacy_value != torql_value:
                            if (
                                (field == "days" and torql_value == [])
                                or (field == "labels" and insight.filters.get("interval") == "month")
                                or (field == "labels" and legacy_value == [] and torql_value is None)
                                or (
                                    field == "label"
                                    and legacy_value == "Other"
                                    and torql_value == "$$_clairview_breakdown_other_$$"
                                )
                            ):
                                continue
                            print(  # noqa: T201
                            ...
                            print("TorQL: ", torql_value)  # noqa: T201
                            print(json.dumps(insight.filters))  # noqa: T201
                            print("")  # noqa: T201
                            all_ok = False
                if all_ok:
