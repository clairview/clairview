...
def handle_check(necessary_migrations: Sequence[AsyncMigration]):
            ...
            "\n".join(
                [
                    ...
                    *(f"- {migration.get_name_with_requirements()}" for migration in necessary_migrations),
                    "See more in Docs: https://clairview.com/docs/self-host/configure/async-migrations/overview",
                ]
            )
        )
        exit(1)
    ...
    if running_migrations.exists():
        logger.critical(
            "\n".join(
                [
                    "Stopping ClairView!",
                    f"Async migration {running_migrations[0].name} is currently running. If you're trying to update ClairView, wait for it to finish before proceeding",
                    "See more in Docs: https://clairview.com/docs/self-host/configure/async-migrations/overview",
                ]
            )
        )
    ...
    if errored_migrations.exists():
        logger.error(
            "\n".join(
                [
                    f"Stopping ClairView!",
                    "Some async migrations are currently in an 'Errored' state. If you're trying to update ClairView, please make sure they complete successfully first:",
...
def handle_run(necessary_migrations: Sequence[AsyncMigration]):
    for migration in necessary_migrations:
        ...
        started_successfully = start_async_migration(migration.name, ignore_clairview_version=True)
