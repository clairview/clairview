        ...
        cursor.execute(
            ...
            SELECT clairview_team.id,
                COALESCE(
                    MIN(
                        CASE
                            WHEN clairview_dashboard.pinned THEN clairview_dashboard.id
                            ELSE NULL
                        END
                    ),
                    MIN(
                        CASE
                            WHEN NOT clairview_dashboard.pinned THEN clairview_dashboard.id
                            ELSE NULL
                        END
                    )
                ) AS primary_dashboard_id
            FROM clairview_team
            INNER JOIN clairview_dashboard ON clairview_dashboard.team_id = clairview_team.id
            WHERE clairview_team.primary_dashboard_id IS NULL
            GROUP BY clairview_team.id
            """
        )
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("clairview", "0219_migrate_tags_v2"),
    ]
