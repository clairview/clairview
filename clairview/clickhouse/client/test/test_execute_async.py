...
from clairview.clickhouse.client.async_task_chain import task_chain_context
...
class TestExecuteProcessQuery(TestCase):
    def setUp(self):
        self.user = User.objects.create(email="test@clairview.com")
    ...
    @patch("clairview.clickhouse.client.execute_async.redis.get_client")
    @patch("clairview.api.services.query.process_query_dict")
...
class ClickhouseClientTestCase(TestCase, ClickhouseTestMixin):
    def setUp(self):
        self.user = User.objects.create(email="test@clairview.com", id=1337)
    ...
    def test_async_query_server_errors(self):
        ...
        with patch(
            "clairview.api.services.query.process_query_dict", side_effect=CHQueryErrorTooManySimultaneousQueries("bla")
        ):
            self.assertRaises(
    ...
    @patch("clairview.client.execute_process_query")
    ...
    @patch("clairview.clickhouse.client.async_task_chain.execute_task_chain")
    @patch("django.db.transaction.on_commit")
    def test_context_manager_exit(self, on_commit_mock, execute_task_chain_mock):
        mock_chain: list[Any] = []
        with patch("clairview.clickhouse.client.async_task_chain.get_task_chain", return_value=mock_chain):
    ...
    @patch("clairview.clickhouse.client.execute.get_pool")
