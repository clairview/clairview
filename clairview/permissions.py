...
from clairview.models import Organization, OrganizationMembership, Team, User
from clairview.models.personal_api_key import APIScopeObjectOrNotSupported
from clairview.utils import get_can_create_org
...
class ClairViewFeatureFlagPermission(BasePermission):
    def has_permission(self, request, view) -> bool:
        user = cast(User, request.user)
        organization = get_organization_from_view(view)
        flag = getattr(view, "clairview_feature_flag", None)

        config = {}

        if not flag:
            raise ImproperlyConfigured(
                "ClairViewFeatureFlagPermission requires the view to define the clairview_feature_flag attribute."
            )
        ...
        for required_flag, actions in config.items():
            if "*" in actions or view.action in actions:
                ...
                enabled = clairviewanalytics.feature_enabled(
                    required_flag,
                    user.distinct_id,
