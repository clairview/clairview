...
class HogLanguage(StrEnum):
    ...
    HOG_JSON = "hogJson"
    HOG_QL = "clairQL"
    HOG_QL_EXPR = "clairQLExpr"
    HOG_TEMPLATE = "hogTemplate"


class ClairQLNotice(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
...
class ClairQLQueryModifiers(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
...
class ClairQLVariable(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    code_name: str
...
class NodeKind(StrEnum):
    ...
    HOG_QUERY = "HogQuery"
    HOG_QL_QUERY = "ClairQLQuery"
    HOG_QL_METADATA = "ClairQLMetadata"
    HOG_QL_AUTOCOMPLETE = "ClairQLAutocomplete"
...
class PathsFilter(BaseModel):
    ...
    pathsClairQLExpression: Optional[str] = None
...
class PathsFilterLegacy(BaseModel):
    ...
    paths_clairql_expression: Optional[str] = None
...
class QueryResponseAlternative7(BaseModel):
    ...
    errors: list[ClairQLNotice]
    isValid: Optional[bool] = None
    isValidView: Optional[bool] = None
    notices: list[ClairQLNotice]
    query: Optional[str] = None
    warnings: list[ClairQLNotice]
...
class SessionAttributionExplorerQueryResponse(BaseModel):
    ...
    clairql: Optional[str] = Field(default=None, description="Generated ClairQL query.")
    limit: Optional[int] = None
    modifiers: Optional[ClairQLQueryModifiers] = Field(
...
class TaxonomicFilterGroupType(StrEnum):
    ...
    CLAIRQL_EXPRESSION = "clairql_expression"
    NOTEBOOKS = "notebooks"
...
class ActorsQueryResponse(BaseModel):
    ...
    hasMore: Optional[bool] = None
    clairql: str = Field(..., description="Generated ClairQL query.")
    limit: int
...
class FunnelsFilterLegacy(BaseModel):
    ...
    funnel_aggregate_by_clairql: Optional[str] = None
...
class ClairQLAutocompleteResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
...
class ClairQLMetadataResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    errors: list[ClairQLNotice]
...
class ClairQLPropertyFilter(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    key: str
    label: Optional[str] = None
    type: Literal["clairql"] = "clairql"
    value: Optional[Union[str, float, list[Union[str, float]]]] = None


class ClairQLQueryResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    ...
    clairql: Optional[str] = Field(default=None, description="Generated ClairQL query.")
    limit: Optional[int] = None
    metadata: Optional[ClairQLMetadataResponse] = Field(default=None, description="Query metadata output")
    modifiers: Optional[ClairQLQueryModifiers] = Field(
        default=None, description="Modifiers used when performing the query"
    )
    offset: Optional[int] = None
...
class SavedInsightNode(BaseModel):
    ...
    showClairQLEditor: Optional[bool] = Field(default=None, description="Include a ClairQL query editor above ClairQL tables")
...
class CachedClairQLQueryResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    cache_key: str
...
class DataWarehouseNode(BaseModel):
    ...
    math: Optional[
        Union[BaseMathType, PropertyMathType, CountPerActorMathType, Literal["unique_group"], Literal["clairql"]]
    ] = None
    math_group_type_index: Optional[MathGroupTypeIndex] = None
    math_clairql: Optional[str] = None
...
class ClairQLFilters(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    dateRange: Optional[DateRange] = None
...
class ClairQLQuery(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    explain: Optional[bool] = None
    filters: Optional[ClairQLFilters] = None
    kind: Literal["ClairQLQuery"] = "ClairQLQuery"
    modifiers: Optional[ClairQLQueryModifiers] = Field(
        default=None, description="Modifiers used when performing the query"
    )
    query: str
    response: Optional[ClairQLQueryResponse] = None
    values: Optional[dict[str, Any]] = Field(
        default=None, description="Constant values that can be referenced with the {placeholder} syntax in the query"
    )
    variables: Optional[dict[str, ClairQLVariable]] = Field(
...
class FunnelsFilter(BaseModel):
    ...
    funnelAggregateByClairQL: Optional[str] = None
...
class EventsQuery(BaseModel):
    ...
    where: Optional[list[str]] = Field(default=None, description="ClairQL filters to apply on returned data")
...
class ActorsQuery(BaseModel):
    ...
    fixedProperties: Optional[
        list[Union[PersonPropertyFilter, CohortPropertyFilter, ClairQLPropertyFilter, EmptyPropertyFilter]]
    ] = Field(
        default=None,
        description=(
    ...
    search: Optional[str] = None
    select: Optional[list[str]] = None
    source: Optional[Union[InsightActorsQuery, FunnelsActorsQuery, FunnelCorrelationActorsQuery, ClairQLQuery]] = None
...
class ClairQLMetadata(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    ...
    filters: Optional[ClairQLFilters] = Field(default=None, description="Extra filters applied to query via {filters}")
    globals: Optional[dict[str, Any]] = Field(default=None, description="Extra globals for the query")
    kind: Literal["ClairQLMetadata"] = "ClairQLMetadata"
    language: HogLanguage = Field(..., description="Language to validate")
    modifiers: Optional[ClairQLQueryModifiers] = Field(
        default=None, description="Modifiers used when performing the query"
    )
    query: str = Field(..., description="Query to validate")
    response: Optional[ClairQLMetadataResponse] = None
    sourceQuery: Optional[
        Union[
            EventsNode,
            ActionsNode,
...
class QueryRequest(BaseModel):
    ...
    query: Union[
        ...
        ...,
        description=(
            "Submit a JSON string representing a query for ClairView data analysis, for example a ClairQL query.\n\nExample"
            ' payload:\n\n```\n\n{"query": {"kind": "ClairQLQuery", "query": "select * from events limit'
            ' 100"}}\n\n```\n\nFor more details on ClairQL queries, see the [ClairView ClairQL'
            " documentation](/docs/clairql#api-access)."
        ),
        discriminator="kind",
    )
