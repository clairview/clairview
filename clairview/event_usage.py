...
def alias_invite_id(user: User, invite_id: str) -> None:
    clairviewanalytics.alias(user.distinct_id, f"invite_{invite_id}")
...
def report_user_logged_in(
    ...
    """
    Reports that a user has logged in to ClairView.
    """
    clairviewanalytics.capture(
        user.distinct_id,
        "user logged in",
...
def report_team_action(
    ...
    if properties is None:
        properties = {}
    clairviewanalytics.capture(str(team.uuid), event, properties=properties, groups=groups(team=team))

    if group_properties:
        clairviewanalytics.group_identify("team", str(team.id), properties=group_properties)
...
def report_organization_action(
    ...
    if group_properties:
        clairviewanalytics.group_identify("organization", str(organization.id), properties=group_properties)
