...
from clairview.async_migrations.utils import (
    complete_migration,
    execute_op,
...
from clairview.version_requirement import ServiceVersionRequirement
...
def start_async_migration(
    migration_name: str,
    ignore_clairview_version=False,
    ...
    2. The migration can be run with the current ClairView version
    ...
    """
    send_analytics_to_clairview("Async migration start", {"name": migration_name})
    ...
    if not (
        ignore_clairview_version
        or is_clairview_version_compatible(
            migration_instance.clairview_min_version,
            migration_instance.clairview_max_version,
        )
    ):
        process_error(
            migration_instance,
            f"Migration is not available on this ClairView version",
...
def run_next_migration(candidate: str):
    ...
    migration_in_range = is_clairview_version_compatible(
        migration_instance.clairview_min_version, migration_instance.clairview_max_version
    )
