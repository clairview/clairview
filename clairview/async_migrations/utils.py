...
from clairview.async_migrations.definition import AsyncMigrationOperation
from clairview.async_migrations.setup import DEPENDENCY_TO_ASYNC_MIGRATION
from clairview.celery import app
from clairview.clickhouse.client import sync_execute
from clairview.clickhouse.client.connection import make_ch_pool
from clairview.clickhouse.query_tagging import reset_query_tags, tag_queries
...
from clairview.utils import get_machine_id
...
def send_analytics_to_clairview(event, data):
    clairviewanalytics.project_api_key = "sTMFPsFhdP1Ssg"
    ...
    clairviewanalytics.capture(get_machine_id(), event, data, groups=groups)
...
def _get_all_shard_connections():
    from clairview.clickhouse.client.connection import ch_pool as default_ch_pool
...
def process_error(
    ...
    if alert:
        if async_migrations_emails_enabled():
            from clairview.tasks.email import send_async_migration_errored_email
    ...
    from clairview.async_migrations.runner import attempt_migration_rollback
...
def trigger_migration(migration_instance: AsyncMigration, fresh_start: bool = True):
    from clairview.tasks.async_migrations import run_async_migration
...
def force_stop_migration(
    ...
    2. Our Celery tasks are not essential for the functioning of ClairView, meaning losing a task is not the end of the world
    """
...
def complete_migration(migration_instance: AsyncMigration, email: bool = True):
    ...
    if needs_update:
        ...
        send_analytics_to_clairview("Async migration completed", {"name": migration_instance.name})

        if email and async_migrations_emails_enabled():
            from clairview.tasks.email import send_async_migration_complete_email
    ...
    if get_instance_setting("AUTO_START_ASYNC_MIGRATIONS"):
        ...
        if next_migration:
            from clairview.async_migrations.runner import run_next_migration
