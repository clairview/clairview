...
class EnterpriseFeatureException(APIException):
    ...
    def __init__(self, feature: Optional[str] = None) -> None:
        super().__init__(
            detail=(
                f"{feature.capitalize() if feature else 'This feature'} is part of the premium ClairView offering. "
                + (
                    "To use it, subscribe to ClairView Cloud with a generous free tier."
                    if is_cloud()
                    else "Self-hosted licenses are no longer available for purchase. Please contact sales@clairview.com to discuss options."
                )
            )
        )
...
def generate_exception_response(
    ...
    statsd.incr(
        f"clairview_cloud_raw_endpoint_exception",
