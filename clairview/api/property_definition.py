...
from clairview.constants import GROUP_TYPES_LIMIT, AvailableFeature
from clairview.event_usage import report_user_action
from clairview.exceptions import EnterpriseFeatureException
from clairview.filters import TermSearchFilterBackend, term_search_filter_sql
from clairview.models import EventProperty, PropertyDefinition, User
from clairview.models.activity_logging.activity_log import Detail, log_activity
...
class QueryContext:
    ...
    # the event name filter is used with and without a clairview_eventproperty_table_join_alias qualifier
    event_name_join_filter: str = ""

    clairview_eventproperty_table_join_alias = "check_for_matching_event_property"
    ...
    def with_event_property_filter(
        ...
        if event_names and len(event_names) > 0:
            event_property_field = f"{self.clairview_eventproperty_table_join_alias}.property IS NOT NULL"
    ...
    def _join_on_event_property(self):
        return (
            ...
            {self.event_property_join_type} (
                ...
                FROM clairview_eventproperty
                WHERE team_id = %(team_id)s {self.event_name_join_filter}
            ) {self.clairview_eventproperty_table_join_alias}
            ON {self.clairview_eventproperty_table_join_alias}.property = name
            """
...
EVENTS_HIDDEN_PROPERTY_DEFINITIONS = set(
    [
        ...
        "$set",
        "$set_once",
        # clairview-js used to send it on events and shouldn't have, now it confuses users
        "$initial_referrer",
...
class PropertyDefinitionViewSet(
    ...
    def dangerously_get_queryset(self):
        ...
        property_definition_fields = ", ".join(
            [
                f'clairview_propertydefinition."{f.column}"'
        ...
        query_context = (
            QueryContext(
                ...
                table=(
                    "ee_enterprisepropertydefinition FULL OUTER JOIN clairview_propertydefinition ON clairview_propertydefinition.id=ee_enterprisepropertydefinition.propertydefinition_ptr_id"
                    if use_enterprise_taxonomy
                    else "clairview_propertydefinition"
                ),
                property_definition_fields=property_definition_fields,
                property_definition_table="clairview_propertydefinition",
                limit=limit,
                offset=offset,
            )
            .with_type_filter(
