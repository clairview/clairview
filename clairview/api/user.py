...
from clairview.api.email_verification import EmailVerifier
from clairview.api.organization import OrganizationSerializer
from clairview.api.shared import OrganizationBasicSerializer, TeamBasicSerializer
from clairview.api.utils import (
...
from clairview.auth import (
    PersonalAPIKeyAuthentication,
    SessionAuthentication,
    TemporaryTokenAuthentication,
    authenticate_secondarily,
)
from clairview.constants import PERMITTED_FORUM_DOMAINS
from clairview.email import is_email_available
from clairview.event_usage import (
    report_user_logged_in,
    report_user_updated,
    report_user_verified_email,
)
from clairview.middleware import get_impersonated_session_expires_at
from clairview.models import Dashboard, Team, User, UserScenePersonalisation
from clairview.models.organization import Organization
from clairview.models.user import NOTIFICATION_DEFAULTS, Notifications
from clairview.permissions import APIScopePermission
from clairview.rate_limit import UserAuthenticationThrottle, UserEmailVerificationThrottle
from clairview.tasks import user_identify
from clairview.tasks.email import send_email_change_emails
from clairview.user_permissions import UserPermissions
from clairview.utils import get_js_url
...
def redirect_to_site(request):
    ...
    # see https://github.com/ClairView/clairview/issues/9671
    state = urllib.parse.quote(json.dumps(params), safe="")

    return redirect("{}#__clairview={}".format(app_url, state))
...
def redirect_to_website(request):
    ...
    if request.user.strapi_id is None:
        response = requests.request(
            "POST",
            "https://squeak.clairview.cc/api/auth/local/register",
            json={
        ...
        if response.status_code == 200:
            ...
            if response.text and error_message == "Email or Username are already taken":
                return redirect("https://clairview.com/auth?error=emailIsTaken")
    else:
        token = jwt.encode(
            {
    ...
    return redirect("{}?userData={}&redirect={}".format("https://clairview.com/auth", userData, app_url))
...
def test_slack_webhook(request):
    ...
    message = {"text": "_Greetings_ from ClairView!"}
    try:
