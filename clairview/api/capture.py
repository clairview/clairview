...
from clairview.api.utils import get_data, get_token, safe_clickhouse_string
from clairview.cache_utils import cache_for
from clairview.exceptions import generate_exception_response
from clairview.kafka_client.client import KafkaProducer, session_recording_kafka_producer
...
from clairview.metrics import KLUDGES_COUNTER, LABEL_RESOURCE_TYPE
from clairview.models.utils import UUIDT
from clairview.redis import get_client
from clairview.session_recordings.session_recording_helpers import (
    preprocess_replay_events_for_blob_ingestion,
    split_replay_events,
    byte_size_dict,
)
from clairview.storage import object_storage
from clairview.utils import get_ip_address
from clairview.utils_cors import cors_response
...
# https://github.com/ClairView/clairview/blob/master/plugin-server/src/worker/ingestion/person-state.ts#L22-L33
...
OVERFLOWING_REDIS_KEY = "@clairview/capture-overflow/"
...
def log_event(
    ...
    try:
        ...
        statsd.incr("clairview_cloud_plugin_server_ingestion")
        return future
    except Exception:
...
def _get_sent_at(data, request) -> tuple[Optional[datetime], Any]:
    try:
        if request.GET.get("_"):  # clairview-js
            sent_at = request.GET["_"]
        elif isinstance(data, dict) and data.get("sent_at"):  # clairview-android, clairview-ios
...
def lib_version_from_query_params(request) -> str:
    # url has a ver parameter from clairview-js
    return request.GET.get("ver", "unknown")


@csrf_exempt
@timed("clairview_cloud_event_endpoint")
def get_event(request):
    ...
    with start_span(op="request.authenticate"):
        ...
        if not token:
            return cors_response(
                ...
                generate_exception_response(
                    "capture",
                    "API key not provided. You can find your project API key in ClairView project settings.",
    ...
    with start_span(op="request.process"):
        if isinstance(data, dict):
            if data.get("batch"):  # clairview-python and clairview-ruby
                if not historical:
    ...
    with start_span(op="kafka.produce") as span:
        ...
        for event, event_uuid, distinct_id in processed_events:
            try:
                ...
                statsd.incr("clairview_cloud_raw_endpoint_failure", tags={"endpoint": "capture"})
    ...
    try:
        ...
        # clairview-js will retry when it receives a 504, and it sends `retry_count` in the query params,
    ...
    statsd.incr("clairview_cloud_raw_endpoint_success", tags={"endpoint": "capture"})
...
def replace_with_warning(
    ...
    try:
        ...
        clairview_size_calculation = byte_size_dict(event)
        ...
        try:
            ...
            size_difference = kafka_size - clairview_size_calculation
        except:
            pass
        ...
        logger.info(
            ...
            clairview_calculation=clairview_size_calculation,
            lib_version=lib_version,
        )
...
def sample_replay_data_to_object_storage(
    ...
    """
    try:
        # capture more of clairview message too large since we know we're using latest versions
