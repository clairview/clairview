...
from clairview.async_migrations.runner import (
    MAX_CONCURRENT_ASYNC_MIGRATIONS,
    is_clairview_version_compatible,
)
from clairview.async_migrations.setup import get_async_migration_definition
...
from clairview.permissions import IsStaffUser
...
class AsyncMigrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "started_at",
            "finished_at",
            "clairview_max_version",
            "clairview_min_version",
            "parameters",
            "error_count",
...
class AsyncMigrationsViewset(viewsets.ModelViewSet):
    ...
    def trigger(self, request, **kwargs):
        ...
        if not is_clairview_version_compatible(
            ...
            return response.Response(
                {
                    "success": False,
                    "error": f"Can't run migration. Minimum ClairView version: {migration_instance.clairview_min_version}. Maximum ClairView version: {migration_instance.clairview_max_version}",
                },
                status=400,
            )
