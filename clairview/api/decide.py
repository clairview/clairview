...
from clairview.geoip import get_geoip_properties
from clairview.api.survey import SURVEY_TARGETING_FLAG_PREFIX
from clairview.api.utils import get_project_id, get_token, hostname_in_allowed_url_list, parse_domain
from clairview.database_healthcheck import DATABASE_FOR_FLAG_MATCHING
...
from clairview.models.feature_flag import get_all_feature_flags
from clairview.models.feature_flag.flag_analytics import increment_request_count
from clairview.models.filters.mixins.utils import process_bool
from clairview.models.utils import execute_with_timeout
from clairview.plugins.site import get_decide_site_apps
...
def on_permitted_recording_domain(team: Team, request: HttpRequest) -> bool:
    ...
    is_authorized_mobile_client: bool = user_agent is not None and any(
        ...
        for keyword in ["clairview-android", "clairview-ios", "clairview-react-native", "clairview-flutter"]
    )
...
@csrf_exempt
@timed("clairview_cloud_decide_endpoint")
def get_decide(request: HttpRequest):
    ...
    if request.method == "POST":
        try:
            ...
            # default value added because of bug in clairview-js 1.19.0
            # see https://sentry.io/organizations/clairview2/issues/2738865125/?project=1899813
            # as a tombstone if the below statsd counter hasn't seen errors for N days
            # then it is likely that no clients are running clairview-js 1.19.0
            # and this defaulting could be removed
            statsd.incr(
                f"clairview_cloud_decide_defaulted_api_version_on_value_error",
        ...
        if team is None and token:
            ...
            if not project_id:
                return cors_response(
                    ...
                    generate_exception_response(
                        "decide",
                        "Project API key invalid. You can find your project API key in ClairView project settings.",
        ...
        if team:
            ...
            return cors_response(
                ...
                generate_exception_response(
                    "decide",
                    "No project API key provided. You can find your project API key in ClairView project settings.",
                    code="no_api_key",
                    type="authentication_error",
                    status_code=status.HTTP_401_UNAUTHORIZED,
                ),
    ...
    statsd.incr(f"clairview_cloud_raw_endpoint_success", tags={"endpoint": "decide"})
