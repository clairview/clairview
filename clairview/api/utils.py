...
from clairview.constants import EventDefinitionType
from clairview.exceptions import (
    RequestParsingError,
    UnspecifiedCompressionFallbackParsingError,
    generate_exception_response,
)
from clairview.models import Entity, EventDefinition
from clairview.models.entity import MathType
...
from clairview.utils import load_data_from_request
        ...
        if request.POST.get("api_key"):
            ...
            if isinstance(data, dict):
                if data.get("$token"):
                    ...
                    token = data["api_key"]  # server-side libraries like clairview-python and clairview-ruby
...
# keep in sync with clairview/plugin-server/src/utils/db/utils.ts::safeClickhouseString
...
def create_event_definitions_sql(
    ...
    enterprise_join = (
        "FULL OUTER JOIN ee_enterpriseeventdefinition ON clairview_eventdefinition.id=ee_enterpriseeventdefinition.eventdefinition_ptr_id"
        if is_enterprise
        else ""
    )
