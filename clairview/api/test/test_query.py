...
class TestQuery(ClickhouseTestMixin, APIBaseTest):
    ...
    @patch(
        "clairview.clickhouse.client.execute._annotate_tagged_query", return_value=("SELECT 1&&&", {})
    ...
    def test_select_event_person(self):
        with freeze_time("2020-01-10 12:00:00"):
            person = _create_person(
                properties={"name": "Tom", "email": "tom@clairview.com"},
                distinct_ids=["2", "some-random-uid"],
                team=self.team,
                immediate=True,
            )
...
class TestQueryRetrieve(APIBaseTest):
    def setUp(self):
        ...
        self.redis_get_patch = mock.patch("clairview.redis.get_client", return_value=self.redis_client_mock)
