...
class TestUtils(BaseTest):
    ...
    # keep in sync with clairview/plugin-server/tests/utils.test.ts::safeClickhouseString
    ...
    def test_raise_if_user_provided_url_unsafe(self):
        ...
        raise_if_user_provided_url_unsafe("https://clairview.com")  # Safe
        raise_if_user_provided_url_unsafe("https://clairview.com/foo/bar")  # Safe, with path
        raise_if_user_provided_url_unsafe("https://clairview.com:443")  # Safe, good port
        ...
        self.assertRaisesMessage(
            ValueError,
            "No hostname",
            lambda: raise_if_user_provided_url_unsafe("clairview.com"),
        )
        self.assertRaisesMessage(
            ValueError,
            "Scheme must be either HTTP or HTTPS",
            lambda: raise_if_user_provided_url_unsafe("ftp://clairview.com"),
        )
        self.assertRaisesMessage(
            ValueError,
            "Internal hostname",
    ...
    @parameterized.expand(
        [
            ...
            ("multi element matches", ["http://localhost", "http://clairview.com"], "http://localhost:8123", True),
            ("scheme should be ignored", ["ftp://localhost"], "https://localhost:8123", True),
            ("needle is not in allowlist", ["http://localhost"], "http://clairview.com:8123", False),
            ("regex needle is not in allowlist", ["http://*.com"], "http://localhost:8123", False),
            ("regex needle is in allowlist", ["http://*.com"], "http://clairview.com:8123", True),
        ]
    )
