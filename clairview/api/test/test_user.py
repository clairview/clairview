...
from clairview.api.email_verification import email_verification_token_generator
from clairview.models import Dashboard, Team, User
...
class TestUserAPI(APIBaseTest):
    ...
    def test_can_only_list_yourself(self):
        ...
        user = self._create_user("newtest@clairview.com")
    ...
    def test_creating_users_on_this_endpoint_is_not_supported(self):
        ...
        response = self.client.post("/api/users/", {"first_name": "James", "email": "test+james@clairview.com"})
    ...
    @patch("clairview.tasks.user_identify.identify_task")
    ...
    def test_update_current_user(self, mock_capture, mock_identify_task):
        ...
        user = self._create_user("old@clairview.com", password="12345678")
    ...
    def test_set_scene_personalisation_for_user(self, _mock_capture, _mock_identify_task):
        another_org = Organization.objects.create(name="Another Org")
        another_team = Team.objects.create(name="Another Team", organization=another_org)
        user = self._create_user("the-user@clairview.com", password="12345678")
    ...
    @patch("clairview.api.user.is_email_available", return_value=False)
    @patch("clairview.tasks.email.send_email_change_emails.delay")
    ...
    @patch("clairview.api.user.is_email_available", return_value=True)
    @patch("clairview.tasks.email.send_email_change_emails.delay")
    @patch("clairview.api.email_verification.send_email_verification")
    ...
    def test_user_can_update_password(self, mock_capture, mock_identify):
        user = self._create_user("bob@clairview.com", password="A12345678")
        self.client.force_login(user)
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response_data = response.json()
        self.assertEqual(response_data["email"], "bob@clairview.com")
        self.assertNotIn("password", response_data)
        self.assertNotIn("current_password", response_data)
        ...
        response = self.client.post("/api/login", {"email": "bob@clairview.com", "password": "a_new_password"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    @patch("clairview.tasks.user_identify.identify_task")
    @patch("clairviewanalytics.capture")
    def test_user_with_no_password_set_can_set_password(self, mock_capture, mock_identify):
        user = self._create_user("no_password@clairview.com", password=None)
        self.client.force_login(user)
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response_data = response.json()
        self.assertEqual(response_data["email"], "no_password@clairview.com")
        self.assertNotIn("password", response_data)
        self.assertNotIn("current_password", response_data)
        ...
        response = self.client.post(
            "/api/login",
            {"email": "no_password@clairview.com", "password": "a_new_password"},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_user_with_unusable_password_set_can_set_password(self):
        user = self._create_user("no_password@clairview.com", password="123456789")
    ...
    def test_unauthenticated_user_cannot_update_anything(self):
        ...
        self.user.refresh_from_db()
        self.assertNotEqual(self.user.email, "new@clairview.com")
    ...
    @patch("clairview.api.user.secrets.token_urlsafe")
    ...
    def test_redirect_only_to_allowed_urls(self, patched_token):
        ...
        def assert_allowed_url(url):
            ...
            self.assertEqual(response.status_code, status.HTTP_302_FOUND)
            self.assertTrue(f"{url}#__clairview=" in location)
...
class TestStaffUserAPI(APIBaseTest):
    ...
    def test_update_staff_user(self):
        user = self._create_user("newuser@clairview.com", password="12345678")
    ...
    @patch("clairview.api.user.TOTPDeviceForm")
    def test_add_2fa(self, patch_is_valid):
        patch_is_valid.return_value = Mock()
        self._create_user("newuser@clairview.com", password="12345678")
...
class TestEmailVerificationAPI(APIBaseTest):
    ...
    def test_user_can_request_verification_email(self, mock_capture):
        ...
        with self.settings(CELERY_TASK_ALWAYS_EAGER=True, SITE_URL="https://my.clairview.net"):
        ...
        self.validate_basic_html(
            html_message,
            "https://my.clairview.net",
            preheader="Please follow the link inside to verify your account.",
        )
        link_index = html_message.find("https://my.clairview.net/verify_email")
        reset_link = html_message[link_index : html_message.find('"', link_index)]
        token = reset_link.replace("https://my.clairview.net/verify_email/", "").replace(f"{self.user.uuid}/", "")
