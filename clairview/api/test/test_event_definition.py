...
from clairview.models import Action, EventDefinition, Organization, Team, ActivityLog
...
class TestEventDefinitionAPI(APIBaseTest):
    ...
    def test_event_type_event_clairview(self):
        response = self.client.get("/api/projects/@current/event_definitions/?event_type=event_clairview")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["count"], 1)
        self.assertEqual(response.json()["results"][0]["name"], "$pageview")
