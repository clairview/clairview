...
from clairview.api.authentication import password_reset_token_generator
...
class TestLoginAPI(APIBaseTest):
    ...
    @patch("clairview.api.authentication.is_email_available", return_value=True)
    @patch("clairview.api.authentication.EmailVerifier.create_token_and_send_email_verification")
    ...
    @patch("clairview.api.authentication.is_email_available", return_value=True)
    @patch("clairview.api.authentication.EmailVerifier.create_token_and_send_email_verification")
    @patch("clairview.api.authentication.is_email_verification_disabled", return_value=True)
    ...
    def test_user_cant_login_with_incorrect_email(self, mock_capture):
        response = self.client.post(
            "/api/login",
            {"email": "user2@clairview.com", "password": self.CONFIG_PASSWORD},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.json(), self.ERROR_INVALID_CREDENTIALS)
    ...
    def test_login_endpoint_is_protected_against_brute_force_attempts(self):
        User.objects.create(email="new_user@clairview.com", password="87654321")
        ...
        with self.settings(AXES_ENABLED=True, AXES_FAILURE_LIMIT=3):
            for _ in range(0, 2):
                response = self.client.post(
                    "/api/login",
                    {"email": "new_user@clairview.com", "password": "invalid"},
                )
                self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
                self.assertEqual(response.json(), self.ERROR_INVALID_CREDENTIALS)
            ...
            response = self.client.post("/api/login", {"email": "new_user@clairview.com", "password": "invalid"})
...
class TestPasswordResetAPI(APIBaseTest):
    ...
    def test_anonymous_user_can_request_password_reset(self, mock_capture):
        ...
        self.assertEqual(mail.outbox[0].subject, "Reset your ClairView password")
        ...
        link_index = html_message.find("https://my.clairview.net/reset")
        reset_link = html_message[link_index : html_message.find('"', link_index)]
        self.assertTrue(
            password_reset_token_generator.check_token(
                self.user,
                reset_link.replace("https://my.clairview.net/reset/", "").replace(f"{self.user.uuid}/", ""),
            )
        )
    ...
    def test_reset_with_sso_available(self):
        ...
        self.assertTrue(
            password_reset_token_generator.check_token(
                self.user,
                reset_link.replace(f"https://my.clairview.net/reset/{self.user.uuid}/", ""),
            )
        )

        # check we mention SSO providers
        self.assertIn("Google, GitHub", html_message)
        self.assertIn("https://my.clairview.net/login", html_message)  # CTA link
    ...
    def test_is_rate_limited_on_email_not_ip(self):
        ...
        for email in ["email@clairview.com", "other-email@clairview.com"]:
            for i in range(7):
                with self.settings(CELERY_TASK_ALWAYS_EAGER=True, SITE_URL="https://my.clairview.net"):
