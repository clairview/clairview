...
from clairview.plugins.access import can_configure_plugins, can_globally_manage_plugins, can_install_plugins
from clairview.plugins.test.mock import mocked_plugin_requests_get
from clairview.plugins.test.plugin_archives import (
    HELLO_WORLD_PLUGIN_GITHUB_ATTACHMENT_ZIP,
    HELLO_WORLD_PLUGIN_GITHUB_ZIP,
    HELLO_WORLD_PLUGIN_SECRET_GITHUB_ZIP,
)
from clairview.queries.app_metrics.test.test_app_metrics import create_app_metric
from clairview.test.base import APIBaseTest, QueryMatchingTest, snapshot_postgres_queries
...
@mock.patch("clairview.models.plugin.reload_plugins_on_workers", side_effect=mocked_plugin_reload)
...
class TestPluginAPI(APIBaseTest, QueryMatchingTest):
    ...
    def _create_plugin(
        ...
        params = {"url": "https://github.com/ClairView/helloworldplugin"}
    ...
    def test_create_plugin_auth(self, mock_get, mock_reload):
        repo_url = "https://github.com/ClairView/helloworldplugin"
    ...
    @mock.patch("clairview.models.plugin.PluginSourceFile.objects.sync_from_plugin_archive")
    ...
    def test_create_plugin_commit_url(self, mock_get, mock_reload):
        ...
        response = self.client.post(
            "/api/organizations/@current/plugins/",
            {"url": f"https://github.com/ClairView/helloworldplugin/commit/{HELLO_WORLD_PLUGIN_GITHUB_ZIP[0]}"},
        )
        self.assertEqual(response.status_code, 201)
        self.assertEqual(
            response.json(),
    ...
    def test_create_plugin_other_commit_url(self, mock_get, mock_reload):
        ...
        response2 = self.client.post(
            "/api/organizations/@current/plugins/",
            {
                "url": "https://github.com/ClairView/helloworldplugin/commit/{}".format(
        ...
        self.assertEqual(
            response2.json(),
            {
                ...
                "url": f"https://github.com/ClairView/helloworldplugin/commit/{HELLO_WORLD_PLUGIN_GITHUB_ATTACHMENT_ZIP[0]}",
                "icon": None,
                "config_schema": {
                    "bar": {
                        "name": "What's in the bar?",
    ...
    def test_create_plugin_version_range_gt_current(self, mock_get, mock_reload):
        with self.is_cloud(False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/clairview-plugin/version-greater-than/commit/0.0.0"},
            )
            self.assertEqual(response.status_code, 201)
    ...
    def test_create_plugin_version_range_lt_invalid(self, mock_get, mock_reload):
        with self.is_cloud(False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/clairview-plugin/version-less-than/commit/..."},
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(
                cast(dict[str, str], response.json())["detail"],
                'Invalid ClairView semantic version requirement "< ..."!',
            )
    ...
    @patch("clairview.api.plugin.validate_plugin_job_payload")
    @patch("clairview.api.plugin.connections")
            ...
            [
                {
                    ...
                    "item_id": str(plugin_id),
                    "scope": "Plugin",
                    "user": {"email": "user1@clairview.com", "first_name": ""},
                },
                {
                    "activity": "enabled",
                    "created_at": "2021-08-25T22:09:14.252000Z",
