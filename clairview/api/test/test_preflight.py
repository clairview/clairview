...
class TestPreflight(APIBaseTest, QueryMatchingTest):
    ...
    @patch("clairview.storage.object_storage._client")
    ...
    def test_cloud_preflight_request(self):
        with self.is_cloud(True):
            with self.settings(SITE_URL="https://app.clairview.com", OBJECT_STORAGE_ENABLED=False):
                response = self.client.get("/_preflight/")
                self.assertEqual(response.status_code, status.HTTP_200_OK)
                response = response.json()
                available_timezones = cast(dict, response).pop("available_timezones")
    ...
    def test_ee_preflight_with_users_limit(self):
        try:
            ...
            with self.is_cloud(False):
                ...
                OrganizationInvite.objects.create(organization=self.organization, target_email="invite@clairview.com")
