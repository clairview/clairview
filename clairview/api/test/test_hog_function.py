...
def get_db_field_value(field, model_id):
    ...
    cursor.execute(f"select {field} from clairview_hogfunction where id='{model_id}';")
...
class TestHogFunctionAPI(ClickhouseTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    def test_create_hog_function(self, *args):
        ...
        assert response.json() == {
            ...
            "enabled": False,
            "hog": "fetch(inputs.url);",
            "bytecode": ["_H", CLAIRQL_BYTECODE_VERSION, 32, "url", 32, "inputs", 1, 2, 2, "fetch", 1, 35],
            "inputs_schema": [],
            "inputs": {},
            "filters": {"bytecode": ["_H", CLAIRQL_BYTECODE_VERSION, 29]},
            "icon_url": None,
            "template": None,
            "masking": None,
    ...
    def test_generates_hog_bytecode(self, *args):
        ...
        assert response.json()["bytecode"] == json.loads(
            f'["_H", {CLAIRQL_BYTECODE_VERSION}, 33, 0, 33, 3, 36, 0, 15, 40, 45, 33, 1, 36, 0, 6, 37, 0, 32, "url", 32, "inputs", 1, 2, 32, "headers", 32, "x-count", 36, 0, 42, 1, 32, "body", 32, "payload", 32, "inputs", 1, 2, 32, "method", 32, "method", 32, "inputs", 1, 2, 42, 3, 2, "fetch", 2, 35, 39, -52, 35]'
        ), response.json()
    ...
    def test_generates_inputs_bytecode(self, *args):
        ...
        assert response.json()["inputs"] == {
            ...
            "payload": {
                ...
                "bytecode": {
                    "event": ["_H", CLAIRQL_BYTECODE_VERSION, 32, "event", 1, 1],
                    "groups": ["_H", CLAIRQL_BYTECODE_VERSION, 32, "groups", 1, 1],
                    "nested": {"foo": ["_H", CLAIRQL_BYTECODE_VERSION, 32, "url", 32, "event", 1, 2]},
                    "person": ["_H", CLAIRQL_BYTECODE_VERSION, 32, "person", 1, 1],
                    "event_url": [
                        "_H",
                        CLAIRQL_BYTECODE_VERSION,
                        32,
        ...
        assert response.json()["masking"] == snapshot(
            {
                ...
                "bytecode": ["_H", CLAIRQL_BYTECODE_VERSION, 32, "email", 32, "properties", 32, "person", 1, 3],
            }
        )

    @patch("clairview.permissions.clairviewanalytics.feature_enabled", return_value=True)
    def test_loads_status_when_enabled_and_available(self, *args):
        with patch("clairview.plugins.plugin_server_api.requests.get") as mock_get:
    ...
    def test_patches_status_on_enabled_update(self, *args):
        with patch("clairview.plugins.plugin_server_api.requests.get") as mock_get:
            with patch("clairview.plugins.plugin_server_api.requests.patch") as mock_patch:
                mock_get.return_value.status_code = status.HTTP_200_OK
