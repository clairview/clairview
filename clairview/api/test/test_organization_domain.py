...
from clairview.test.base import APIBaseTest, BaseTest
...
class TestOrganizationDomainsAPI(APIBaseTest):
    ...
    def setUpTestData(cls):
        ...
        cls.domain = OrganizationDomain.objects.create(organization=cls.organization, domain="myclairview.com")

        cls.another_org = Organization.objects.create(name="Another Org")
        Team.objects.create(organization=cls.another_org)
        cls.another_domain = OrganizationDomain.objects.create(organization=cls.another_org, domain="org.clairview.net")
    ...
    def test_can_list_and_retrieve_domains(self):
        ...
        self.assertEqual(item["domain"], "myclairview.com")
    ...
    def test_create_domain(self):
        ...
        response_data = response.json()
        self.assertEqual(response_data["domain"], "the.clairview.com")
        ...
        self.assertEqual(instance.domain, "the.clairview.com")
    ...
    @patch("clairview.models.organization_domain.dns.resolver.resolve")
    def test_can_request_verification_for_unverified_domains(self, mock_dns_query):
        ...
        mock_dns_query.return_value = FakeDNSResponse(
            [
                dns.rrset.from_text(
                    "_clairview-challenge.myclairview.com.",
                    3600,
                    "IN",
                    "TXT",
        ...
        self.assertEqual(response_data["domain"], "myclairview.com")
        self.assertEqual(
    ...
    def test_only_admin_can_create_verified_domains(self):
        count = OrganizationDomain.objects.count()
        response = self.client.post("/api/organizations/@current/domains/", {"domain": "evil.clairview.com"})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(
            response.json(),
