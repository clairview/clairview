...
from clairview.models import Action, Cohort, Dashboard, FeatureFlag, Insight
...
from clairview.test.base import APIBaseTest, override_settings
...
class TestAccessMiddleware(APIBaseTest):
    ...
    def test_ip_range(self):
        ...
        with self.settings(ALLOWED_IP_BLOCKS=["192.168.0.0/31", "127.0.0.0/25", "128.0.0.1"]):
            ...
            self.assertIn(b"ClairView is not available", response.content)
            ...
            self.assertNotIn(b"ClairView is not available", response.content)
...
class TestAutoProjectMiddleware(APIBaseTest):
    ...
    def test_project_redirects_to_clairview_org_style_tokens(self):
        res = self.client.get(
...
@override_settings(CLOUD_DEPLOYMENT="US")  # As ClairView Cloud
class TestClairViewTokenCookieMiddleware(APIBaseTest):
    CONFIG_AUTO_LOGIN = False
    ...
    def test_logged_in_client(self):
        ...
        self.assertEqual(ph_project_token_cookie["domain"], "clairview.com")
        ...
        self.assertEqual(ph_project_name_cookie["domain"], "clairview.com")
        ...
        self.assertEqual(ph_instance_cookie["domain"], "clairview.com")
...
class TestAutoLogoutImpersonateMiddleware(APIBaseTest):
    ...
    def setUp(self):
        ...
        self.other_user = User.objects.create_and_join(
            self.organization, email="other-user@clairview.com", password="123456"
        )
    ...
    def test_staff_user_can_login(self):
        ...
        assert self.client.get("/api/users/@me").json()["email"] == "other-user@clairview.com"
    ...
    def test_after_timeout_api_requests_401(self):
        ...
        with freeze_time(now):
            ...
            assert res.json()["email"] == "other-user@clairview.com"
    ...
    def test_after_timeout_redirects_to_logout_then_admin(self):
        ...
        with freeze_time(now + timedelta(seconds=35)):
            ...
            res = self.client.get("/api/users/@me")
            assert res.status_code == 200
            assert res.json()["email"] == "user1@clairview.com"
