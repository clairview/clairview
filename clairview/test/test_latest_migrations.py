...
class TestLatestMigrations(TestCase):
    def test_clairview_migration_is_in_sync_with_latest(self) -> None:
        """
        regression test
        ...
        """
        latest_manifest_migration = self._get_latest_migration_from_manifest("clairview")
        latest_migration_file = self._get_newest_migration_file(f"{pathlib.Path().resolve()}/clairview/migrations/*")
    ...
    def _get_latest_migration_from_manifest(django_app: str) -> str:
        ...
        clairview_latest_migration = next(line for line in manifest.splitlines() if line.startswith(f"{django_app}: "))

        return clairview_latest_migration.replace(f"{django_app}: ", "")
