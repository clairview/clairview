...
from clairview.models.team import get_team_in_cache, util
...
class TestTeam(BaseTest):
    ...
    def test_preinstalled_are_autoenabled(self, mock_get):
        with self.is_cloud(False):
            with self.settings(PLUGINS_PREINSTALLED_URLS=["https://github.com/ClairView/helloworldplugin/"]):
    ...
    @mock.patch("clairviewanalytics.feature_enabled", return_value=True)
    def test_team_on_cloud_uses_feature_flag_to_determine_person_on_events(self, mock_feature_enabled):
        with self.is_cloud(True):
            with override_instance_config("PERSON_ON_EVENTS_ENABLED", False):
                ...
                # called more than once when evaluating clairql
    ...
    @mock.patch("clairviewanalytics.feature_enabled", return_value=False)
    ...
    @mock.patch("clairview.models.project.Project.objects.create", side_effect=Exception)
