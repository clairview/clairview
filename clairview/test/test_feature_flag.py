...
from clairview.models import Cohort, FeatureFlag, GroupTypeMapping, Person
from clairview.models.feature_flag import get_feature_flags_for_team_in_cache
from clairview.models.feature_flag.flag_matching import (
...
class TestFeatureFlagMatcher(BaseTest, QueryMatchingTest):
    ...
    def test_super_condition_matches_boolean(self):
        Person.objects.create(
            team=self.team,
            distinct_ids=["test_id"],
            properties={"email": "test@clairview.com", "is_enabled": True},
        )
    ...
    def test_super_condition_with_override_properties(self):
        Person.objects.create(
            team=self.team,
            distinct_ids=["test_id"],
            properties={"email": "test@clairview.com", "is_enabled": False},
        )
    ...
    def test_multi_property_filters(self):
        ...
        feature_flag = self.create_feature_flag(
            filters={
                "groups": [
                    {"properties": [{"key": "email", "value": "tim@clairview.com"}]},
        ...
        feature_flag = self.create_feature_flag(
            filters={
                "groups": [
                    {
                        "properties": [
                            {"key": "email", "value": "tim@clairview.com"},
        ...
        with self.assertNumQueries(4):
            ...
            self.assertEqual(
                FeatureFlagMatcher(
                    [feature_flag1, feature_flag2],
                    "example_id",
                    property_value_overrides={"email": "neil@clairview.com"},
                ).get_match(feature_flag1),
                FeatureFlagMatch(True, None, FeatureFlagMatchReason.CONDITION_MATCH, 0),
            )
        ...
        with self.assertNumQueries(4):
            self.assertEqual(
                FeatureFlagMatcher(
                    [feature_flag1, feature_flag2],
                    "random_id",
                    property_value_overrides={"email": "example@clairview.com"},
                ).get_match(feature_flag2),
                FeatureFlagMatch(True, None, FeatureFlagMatchReason.CONDITION_MATCH, 0),
            )
        ...
        with self.assertNumQueries(4):
            ...
            self.assertEqual(
                FeatureFlagMatcher(
                    [feature_flag1],
                    "example_id",
                    property_value_overrides={"id": "second_id", "email": "neil@clairview.com"},
                ).get_match(feature_flag1),
                FeatureFlagMatch(True, None, FeatureFlagMatchReason.CONDITION_MATCH, 0),
            )
    ...
    def test_complex_cohort_filter_with_override_properties(self):
        ...
        # Example: email contains .com ; email contains @ ; email contains clairview
        # -> 3 different filters, all of which match neil@clairview.com.
    ...
    @patch("clairview.models.feature_flag.flag_matching.postgres_healthcheck")
    ...
    def test_legacy_property_filters(self):
        ...
        feature_flag = self.create_feature_flag(
            filters={"properties": [{"key": "email", "value": "tim@clairview.com"}]},
        )
        self.assertEqual(
            FeatureFlagMatcher([feature_flag], "example_id").get_match(feature_flag),
            FeatureFlagMatch(True, None, FeatureFlagMatchReason.CONDITION_MATCH, 0),
    ...
    def test_with_sql_injection_properties_and_other_aliases(self):
        Person.objects.create(
            ...
            properties={
                "number space": 30,
                ";'\" SELECT 1; DROP TABLE clairview_featureflag;": "30",
                "version!!!": "1.24",
            ...
            groups=[
                {
                    "properties": [
                        ...
                        {
                            "key": ";'\" SELECT 1; DROP TABLE clairview_featureflag;",
                            "value": "100",
                            "type": "person",
                            "operator": "gt",
                        },
...
class TestFeatureFlagHashKeyOverrides(BaseTest, QueryMatchingTest):
    ...
    def setUpTestData(cls):
        ...
        cls.person = Person.objects.create(
            team=cls.team,
            distinct_ids=["example_id"],
            properties={"email": "tim@clairview.com", "team": "clairview"},
        )
    ...
    def test_setting_overrides(self):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                f"SELECT hash_key FROM clairview_featureflaghashkeyoverride WHERE team_id = {self.team.pk} AND person_id={self.person.id}"
            )
            res = cursor.fetchall()
    ...
    def test_hash_key_overrides_for_multiple_ids_when_people_are_not_merged(self):
        Person.objects.create(
            team=self.team,
            distinct_ids=["1"],
            properties={"email": "beuk@clairview.com", "team": "clairview"},
        )

        Person.objects.create(
            team=self.team,
            distinct_ids=["2"],
            properties={"email": "beuk2@clairview.com", "team": "clairview"},
        )
        ...
        def insert_fail(execute, sql, *args, **kwargs):
            ...
            if "insert" in sql.lower():
                ...
                raise IntegrityError(
                    """
                    insert or update on table "clairview_featureflaghashkeyoverride" violates foreign key constraint "clairview_featureflagh_person_id_7e517f7c_fk_clairview_p"
                    DETAIL:  Key (person_id)=(1487010281) is not present in table "clairview_person".
                """
                )
            return execute(sql, *args, **kwargs)
