...
from clairview.torql.database.database import TORQL_CHARACTERS_TO_BE_WRAPPED, Database, create_torql_database
...
from clairview.torql.filters import replace_filters
from clairview.torql.functions.mapping import ALL_EXPOSED_FUNCTION_NAMES
from clairview.torql.parser import parse_select, parse_expr, parse_string_template, parse_program
from clairview.torql import ast
from clairview.torql.base import AST, CTE, ConstantType
from clairview.torql.resolver import resolve_types
from clairview.torql.timings import TorQLTimings
from clairview.torql.visitor import TraversingVisitor, clone_expr
from clairview.torql_queries.query_runner import get_query_runner
...
MATCH_ANY_CHARACTER = "$$_MARKETTOR_ANY_$$"
...
def get_table(context: TorQLContext, join_expr: ast.JoinExpr, ctes: Optional[dict[str, CTE]]) -> None | Table:
    ...
    def resolve_fields_on_table(table: Table | None, table_query: ast.SelectQuery) -> Table | None:
        ...
        try:
            node = cast(ast.SelectQuery, resolve_types(node=table_query, dialect="torql", context=context))
            ...
            table_name = table.to_printed_torql()
...
def get_tables_aliases(query: ast.SelectQuery, context: TorQLContext) -> dict[str, ast.Table]:
...
def resolve_table_field_traversers(table: Table, context: TorQLContext) -> Table:
...
def append_table_field_to_response(table: Table, suggestions: list[AutocompleteCompletionItem], language: str) -> None:
    ...
    extend_responses(
        ...
        details=details,
        insert_text=lambda key: f"`{key}`" if any(n in key for n in TORQL_CHARACTERS_TO_BE_WRAPPED) else key,
    )
...
def get_torql_autocomplete(
    query: TorQLAutocomplete, team: Team, database_arg: Optional[Database] = None
) -> TorQLAutocompleteResponse:
    response = TorQLAutocompleteResponse(suggestions=[], incomplete_list=False)
    timings = TorQLTimings()

    if database_arg is not None:
        database = database_arg
    else:
        database = create_torql_database(team_id=team.pk, team_arg=team)
                    ...
                    for index, chain_part in enumerate(node.chain):
                        ...
                        if is_last_part:
                            ...
                            if isinstance(field, StringJSONDatabaseField):
                                if last_table.to_printed_torql() == "events":
                                    if field.name == "person_properties":
                                        property_type = PropertyDefinition.Type.PERSON
                                    else:
                                        property_type = PropertyDefinition.Type.EVENT
                                elif last_table.to_printed_torql() == "persons":
                                    property_type = PropertyDefinition.Type.PERSON
                                elif last_table.to_printed_torql() == "groups":
...
def add_globals_to_suggestions(globalVars: dict, response: TorQLAutocompleteResponse):
