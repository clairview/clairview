...
from clairview.torql.visitor import Visitor
from hogvm.python.operation import (
    Operation,
    TORQL_BYTECODE_IDENTIFIER,
...
def create_bytecode(
    ...
    bytecode: list[Any] = []
    if args is None:
        bytecode.append(TORQL_BYTECODE_IDENTIFIER)
        bytecode.append(TORQL_BYTECODE_VERSION)
...
class BytecodeCompiler(Visitor):
    def __init__(
        ...
        self.context = context or TorQLContext(team_id=None)
    ...
    def visit_call(self, node: ast.Call):
        ...
        # TorQL functions can have two sets of parameters: asd(args) or asd(params)(args)
...
def execute_hog(
    ...
    bytecode = create_bytecode(
        ...
        context=TorQLContext(team_id=team.id if team else None),
    )
