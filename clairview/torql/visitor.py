...
from clairview.torql.base import AST, Expr
...
class TraversingVisitor(Visitor[None]):
    ...
    def visit_torqlx_attribute(self, node: ast.TorQLXAttribute):
...
class CloningVisitor(Visitor[Any]):
    ...
    def visit_torqlx_tag(self, node: ast.TorQLXTag):
        return ast.TorQLXTag(kind=node.kind, attributes=[self.visit(a) for a in node.attributes])

    def visit_torqlx_attribute(self, node: ast.TorQLXAttribute):
        return ast.TorQLXAttribute(name=node.name, value=self.visit(node.value))
