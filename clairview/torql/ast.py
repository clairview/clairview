...
from clairview.torql.base import Type, Expr, CTE, ConstantType, UnknownType, AST
from clairview.torql.constants import ConstantDataType, TorQLQuerySettings
...
from clairview.torql.errors import NotImplementedError, QueryError, ResolutionError
...
class FieldAliasType(Type):
    ...
    def resolve_database_field(self, context: TorQLContext):
    ...
    def resolve_table_type(self, context: TorQLContext):
        if isinstance(self.type, FieldType):
...
class BaseTableType(Type):
    def resolve_database_table(self, context: TorQLContext) -> Table:
...
class FieldType(Type):
    ...
    def resolve_database_field(self, context: TorQLContext) -> Optional[FieldOrTable]:
    ...
    def is_nullable(self, context: TorQLContext) -> bool:
    ...
    def get_child(self, name: str | int, context: TorQLContext) -> Type:
        database_field = self.resolve_database_field(context)
...
class UnresolvedFieldType(Type):
    ...
    def has_child(self, name: str | int, context: TorQLContext) -> bool:
        return False
...
class Alias(Expr):
    ...
    expr: Expr
    """
    Aliases are "hidden" if they're automatically created by TorQL when abstracting fields.
...
class SelectQuery(Expr):
    ...
    settings: Optional[TorQLQuerySettings] = None
...
class TorQLXTag(AST):
    kind: str
    attributes: list[TorQLXAttribute]
