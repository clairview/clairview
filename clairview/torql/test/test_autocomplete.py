...
from clairview.torql.autocomplete import get_torql_autocomplete
from clairview.torql.database.database import Database, create_torql_database
from clairview.torql.database.models import StringDatabaseField
from clairview.torql.database.schema.events import EventsTable
from clairview.torql.database.schema.persons import PERSONS_FIELDS
...
from clairview.schema import TorQLAutocomplete, TorQLAutocompleteResponse, HogLanguage, TorQLQuery, Kind
...
class TestAutocomplete(ClickhouseTestMixin, APIBaseTest):
    ...
    def _select(
        ...
        autocomplete = TorQLAutocomplete(
            kind="TorQLAutocomplete", query=query, language=HogLanguage.HOG_QL, startPosition=start, endPosition=end
        )
        return get_torql_autocomplete(query=autocomplete, team=self.team, database_arg=database)

    def _expr(self, query: str, start: int, end: int, database: Optional[Database] = None) -> TorQLAutocompleteResponse:
        autocomplete = TorQLAutocomplete(
            kind="TorQLAutocomplete",
            query=query,
            language=HogLanguage.HOG_QL_EXPR,
            sourceQuery=TorQLQuery(query="select * from events"),
            startPosition=start,
            endPosition=end,
        )
        return get_torql_autocomplete(query=autocomplete, team=self.team, database_arg=database)
    ...
    def _json(self, query: str, start: int, end: int, database: Optional[Database] = None) -> TorQLAutocompleteResponse:
        autocomplete = TorQLAutocomplete(
            kind="TorQLAutocomplete",
            query=query,
    ...
    def test_autocomplete_events_hidden_field(self):
        database = create_torql_database(team_id=self.team.pk, team_arg=self.team)
