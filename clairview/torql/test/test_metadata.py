...
from clairview.torql.metadata import get_torql_metadata
from clairview.models import PropertyDefinition, Cohort
from clairview.schema import TorQLMetadata, TorQLMetadataResponse, TorQLQuery, HogLanguage
...
class TestMetadata(ClickhouseTestMixin, APIBaseTest):
    ...
    def _expr(self, query: str, table: str = "events", debug=True) -> TorQLMetadataResponse:
        return get_torql_metadata(
            query=TorQLMetadata(
                kind="TorQLMetadata",
                language=HogLanguage.HOG_QL_EXPR,
                query=query,
                sourceQuery=TorQLQuery(query=f"select * from {table}"),
                response=None,
                debug=debug,
            ),
            team=self.team,
    ...
    def _select(self, query: str) -> TorQLMetadataResponse:
        return get_torql_metadata(
            query=TorQLMetadata(kind="TorQLMetadata", language=HogLanguage.HOG_QL, query=query, response=None),
            team=self.team,
        )

    def _program(self, query: str, globals: Optional[dict] = None) -> TorQLMetadataResponse:
        return get_torql_metadata(
            query=TorQLMetadata(
                kind="TorQLMetadata", language=HogLanguage.HOG, query=query, globals=globals, response=None
            ),
            team=self.team,
        )

    def _template(self, query: str) -> TorQLMetadataResponse:
        return get_torql_metadata(
            query=TorQLMetadata(kind="TorQLMetadata", language=HogLanguage.HOG_TEMPLATE, query=query, response=None),
            team=self.team,
        )
