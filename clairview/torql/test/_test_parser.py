...
from clairview.torql.parser import parse_program
from clairview.torql import ast
from clairview.torql.errors import ExposedTorQLError, SyntaxError
from clairview.torql.parser import parse_expr, parse_order_expr, parse_select, parse_string_template
from clairview.torql.visitor import clear_locations
...
def parser_test_factory(backend: Literal["python", "cpp"]):
    ...
    class TestParser(*base_classes):
        ...
        ) -> ast.SelectQuery | ast.SelectUnionQuery | ast.TorQLXTag:
            return cast(
                ast.SelectQuery | ast.SelectUnionQuery | ast.TorQLXTag,
        ...
        def test_select_array_join_errors(self):
            with self.assertRaises(ExposedTorQLError) as e:
        ...
        def test_select_placeholders(self):
            self.assertEqual(
                self._select("select 1 where 1 == {torql_val_1}"),
                ast.SelectQuery(
                    select=[ast.Constant(value=1)],
                    where=ast.CompareOperation(
                        op=ast.CompareOperationOp.Eq,
                        left=ast.Constant(value=1),
                        right=ast.Placeholder(expr=ast.Field(chain=["torql_val_1"])),
                    ),
                ),
            )
            self.assertEqual(
                self._select(
                    "select 1 where 1 == {torql_val_1}",
                    {"torql_val_1": ast.Constant(value="bar")},
                ),
                ast.SelectQuery(
                    select=[ast.Constant(value=1)],
                    where=ast.CompareOperation(
        ...
        def test_placeholder_expressions(self):
            actual = self._select("select 1 where 1 == {1 ? torql_val_1 : torql_val_2}")
                    ...
                    where=ast.CompareOperation(
                        ...
                        right=ast.Placeholder(
                            expr=ast.Call(
                                ...
                                args=[
                                    ast.Constant(value=1),
                                    ast.Field(chain=["torql_val_1"]),
                                    ast.Field(chain=["torql_val_2"]),
                                ],
                            )
                        ),
                    ),
        ...
        def test_visit_torqlx_tag(self):
            node = self._select("select event from <TorQLQuery query='select event from events' />")
            table_node = cast(ast.SelectQuery, node).select_from.table
            assert table_node == ast.TorQLXTag(
                kind="TorQLQuery",
                attributes=[ast.TorQLXAttribute(name="query", value=ast.Constant(value="select event from events"))],
            )

            node2 = self._select("select event from (<TorQLQuery query='select event from events' />)")
            assert node2 == node

        def test_visit_torqlx_tag_nested(self):
            # Basic case
            node = self._select(
                "select event from <OuterQuery><TorQLQuery query='select event from events' /></OuterQuery>"
            )
            table_node = cast(ast.SelectQuery, node).select_from.table
            assert table_node == ast.TorQLXTag(
                kind="OuterQuery",
                attributes=[
                    ast.TorQLXAttribute(
                        ...
                        value=ast.TorQLXTag(
                            kind="TorQLQuery",
                            attributes=[
                                ast.TorQLXAttribute(name="query", value=ast.Constant(value="select event from events"))
                            ],
                        ),
                    )
                ],
            ...
            table_node = cast(ast.SelectQuery, node).select_from.table
            assert table_node == ast.TorQLXTag(kind="OuterQuery", attributes=[])

            # With attribute
            node = self._select(
                "select event from <OuterQuery q='b'><TorQLQuery query='select event from events' /></OuterQuery>"
            )
            table_node = cast(ast.SelectQuery, node).select_from.table
            assert table_node == ast.TorQLXTag(
                kind="OuterQuery",
                attributes=[
                    ast.TorQLXAttribute(name="q", value=ast.Constant(value="b")),
                    ast.TorQLXAttribute(
                        name="source",
                        value=ast.TorQLXTag(
                            kind="TorQLQuery",
            ...
            with self.assertRaises(ExposedTorQLError) as e:
                self._select(
                    "select event from <OuterQuery q='b'><TorQLQuery query='select event from events' /></TorQLQuery>"
                )
            assert str(e.exception) == "Opening and closing TorQLX tags must match. Got OuterQuery and TorQLQuery"

            # With mismatched closing tag
            with self.assertRaises(ExposedTorQLError) as e:
                self._select(
                    "select event from <OuterQuery source='b'><TorQLQuery query='select event from events' /></OuterQuery>"
                )
            assert str(e.exception) == "Nested TorQLX tags cannot have a source attribute"

        def test_visit_torqlx_tag_alias(self):
            node = self._select("select event from <TorQLQuery query='select event from events' /> as a")
            table_node = cast(ast.SelectQuery, node).select_from.table
            ...
            node2 = self._select("select event from <TorQLQuery query='select event from events' /> a")
            assert node2 == node

        def test_visit_torqlx_tag_source(self):
            query = """
                select id, email from (
                    <ActorsQuery
                        select={['id', 'properties.email as email']}
                        source={
                            <TorQLQuery query='select distinct person_id from events' />
                        }
                    />
                )
            """
        ...
        def test_visit_torqlx_tag_column_source(self):
            query = """
                select <a href='https://google.com'>{event}</a> from events
            """
            node = self._select(query)
            assert isinstance(node, ast.SelectQuery) and cast(ast.TorQLXTag, node.select[0]) == ast.TorQLXTag(
                kind="a",
                attributes=[
                    ast.TorQLXAttribute(name="href", value=Constant(value="https://google.com")),
                    ast.TorQLXAttribute(name="source", value=ast.Field(chain=["event"])),
                ],
            )
