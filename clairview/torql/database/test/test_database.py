...
from clairview.torql.database.database import create_torql_database, serialize_database
from clairview.torql.database.models import FieldTraverser, LazyJoin, StringDatabaseField, ExpressionField, Table
...
from clairview.schema import DatabaseSchemaDataWarehouseTable, TorQLQueryModifiers, PersonsOnEventsMode
from clairview.test.base import BaseTest, QueryMatchingTest, FuzzyInt
from clairview.warehouse.models import DataWarehouseTable, DataWarehouseCredential, DataWarehouseSavedQuery
from clairview.torql.query import execute_torql_query
from clairview.torql.test.utils import pretty_print_in_tests
...
from clairview.warehouse.models.join import DataWarehouseJoin
...
class TestDatabase(BaseTest, QueryMatchingTest):
    ...
    def test_serialize_database_no_person_on_events(self):
        with override_settings(PERSON_ON_EVENTS_V2_OVERRIDE=False):
            serialized_database = serialize_database(
                TorQLContext(team_id=self.team.pk, database=create_torql_database(team_id=self.team.pk))
            )
            assert (
                json.dumps(
    ...
    def test_serialize_database_clairview_table(self):
        database = create_torql_database(team_id=self.team.pk)

        serialized_database = serialize_database(TorQLContext(team_id=self.team.pk, database=database))

        tables = database.get_clairview_tables()
    ...
    def test_serialize_database_warehouse_table_s3_with_hyphens(self):
        ...
        DataWarehouseTable.objects.create(
            ...
            url_pattern="https://bucket.s3/data/*",
            columns={
                "id-hype": {"torql": "StringDatabaseField", "clickhouse": "Nullable(String)", "schema_valid": True}
            },
        )
        ...
        assert field.torql_value == "`id-hype`"
    ...
    def test_serialize_database_warehouse_table_source(self):
        ...
        assert field is not None
        assert field.name == "id"
        assert field.torql_value == "id"
        assert field.type == "string"
        assert field.schema_valid is True

    @patch("clairview.torql.query.sync_execute", return_value=([], []))
    @pytest.mark.usefixtures("unittest_snapshot")
    def test_database_with_warehouse_tables(self, patch_execute):
        ...
        create_torql_database(team_id=self.team.pk)
        ...
        self.assertEqual(
            response.clickhouse,
            f"SELECT whatever.id AS id FROM s3(%(torql_val_0_sensitive)s, %(torql_val_3_sensitive)s, %(torql_val_4_sensitive)s, %(torql_val_1)s, %(torql_val_2)s) AS whatever LIMIT 100 SETTINGS readonly=2, max_execution_time=60, allow_experimental_object_type=1, format_csv_allow_double_quotes=0, max_ast_elements=4000000, max_expanded_ast_elements=4000000, max_bytes_before_external_group_by=0",
        )
    ...
    def test_database_group_type_mappings(self):
        ...
        db = create_torql_database(team_id=self.team.pk)
    ...
    def test_database_warehouse_joins_deleted_join(self):
        ...
        sql = "select some_field.key from events"
        with pytest.raises(ExposedTorQLError):
            print_ast(parse_select(sql), context, dialect="clickhouse")
    ...
    def test_selecting_from_persons_ignores_future_persons(self):
        ...
        query = print_ast(parse_select(sql), context, dialect="clickhouse")
        assert (
            "ifNull(less(argMax(toTimeZone(person.created_at, %(torql_val_0)s), person.version), plus(now64(6, %(torql_val_1)s), toIntervalDay(1)))"
            in query
        ), query
    ...
    def test_selecting_persons_from_events_ignores_future_persons(self):
        ...
        context = TorQLContext(
            ...
            modifiers=create_default_modifiers_for_team(
                self.team, TorQLQueryModifiers(personsOnEventsMode=PersonsOnEventsMode.DISABLED)
            ),
        )
    ...
    def test_database_credentials_is_not_n_plus_1(self) -> None:
        for i in range(10):
            ...
            DataWarehouseTable.objects.create(
                ...
                url_pattern="https://bucket.s3/data/*",
                columns={
                    "id": {"torql": "StringDatabaseField", "clickhouse": "Nullable(String)", "schema_valid": True}
                },
            )
