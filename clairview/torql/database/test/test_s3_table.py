from clairview.torql.constants import MAX_SELECT_RETURNED_ROWS
from clairview.torql.context import TorQLContext
from clairview.torql.database.database import create_torql_database
from clairview.torql.database.s3_table import build_function_call
...
from clairview.torql.database.test.tables import create_aapl_stock_s3_table
...
class TestS3Table(BaseTest):
    ...
    def test_s3_table_select(self):
        ...
        self.assertEqual(
            clickhouse,
            "SELECT aapl_stock.Date AS Date, aapl_stock.Open AS Open, aapl_stock.High AS High, aapl_stock.Low AS Low, aapl_stock.Close AS Close, aapl_stock.Volume AS Volume, aapl_stock.OpenInt AS OpenInt FROM s3(%(torql_val_0_sensitive)s, %(torql_val_1)s) AS aapl_stock LIMIT 10",
        )

    def test_s3_table_select_with_alias(self):
        self._init_database()

        torql = self._select(query="SELECT High, Low FROM aapl_stock AS a LIMIT 10", dialect="torql")
        self.assertEqual(torql, "SELECT High, Low FROM aapl_stock AS a LIMIT 10")
        ...
        self.assertEqual(
            clickhouse,
            "SELECT a.High AS High, a.Low AS Low FROM s3(%(torql_val_0_sensitive)s, %(torql_val_1)s) AS a LIMIT 10",
        )
    ...
    def test_s3_table_select_join(self):
        ...
        self.assertEqual(
            clickhouse,
            "SELECT aapl_stock.High AS High, aapl_stock.Low AS Low FROM (SELECT * FROM s3(%(torql_val_0_sensitive)s, %(torql_val_1)s)) AS aapl_stock JOIN (SELECT * FROM s3(%(torql_val_2_sensitive)s, %(torql_val_3)s)) AS aapl_stock_2 ON equals(aapl_stock.High, aapl_stock_2.High) LIMIT 10",
        )
    ...
    def test_s3_table_select_join_with_alias(self):
        ...
        self.assertEqual(
            clickhouse,
            "SELECT a.High AS High, a.Low AS Low FROM (SELECT * FROM s3(%(torql_val_0_sensitive)s, %(torql_val_1)s)) AS a JOIN (SELECT * FROM s3(%(torql_val_2_sensitive)s, %(torql_val_3)s)) AS b ON equals(a.High, b.High) LIMIT 10",
        )
    ...
    def test_s3_table_select_and_non_s3_join(self):
        ...
        self.assertEqual(
            clickhouse,
            f"SELECT aapl_stock.High AS High, aapl_stock.Low AS Low FROM (SELECT * FROM s3(%(torql_val_0_sensitive)s, %(torql_val_1)s)) AS aapl_stock JOIN events ON equals(aapl_stock.High, events.event) WHERE equals(events.team_id, {self.team.pk}) LIMIT 10",
        )
    ...
    def test_s3_table_select_and_non_s3_join_first(self):
        ...
        self.assertEqual(
            clickhouse,
            f"SELECT aapl_stock.High AS High, aapl_stock.Low AS Low FROM events GLOBAL JOIN (SELECT * FROM s3(%(torql_val_0_sensitive)s, %(torql_val_1)s)) AS aapl_stock ON equals(aapl_stock.High, events.event) WHERE equals(events.team_id, {self.team.pk}) LIMIT 10",
        )
    ...
    def test_s3_table_select_in(self):
        ...
        self.assertEqual(
            clickhouse,
            f"SELECT events.uuid AS uuid, events.event AS event FROM events WHERE and(equals(events.team_id, {self.team.pk}), ifNull(globalIn(events.event, (SELECT aapl_stock.Date AS Date FROM s3(%(torql_val_0_sensitive)s, %(torql_val_1)s) AS aapl_stock)), 0)) LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )
    ...
    def test_s3_build_function_call_azure_with_context(self):
        ...
        assert (
            res
            == "azureBlobStorage(%(torql_val_0_sensitive)s, %(torql_val_1_sensitive)s, %(torql_val_2_sensitive)s, %(torql_val_3_sensitive)s, %(torql_val_4_sensitive)s, %(torql_val_5)s, 'auto')"
        )
