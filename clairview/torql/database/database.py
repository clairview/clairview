...
from clairview.torql.database.schema.channel_type import create_initial_channel_type, create_initial_domain_type
from clairview.torql.database.schema.cohort_people import CohortPeople, RawCohortPeople
from clairview.torql.database.schema.events import EventsTable
from clairview.torql.database.schema.groups import GroupsTable, RawGroupsTable
from clairview.torql.database.schema.heatmaps import HeatmapsTable
from clairview.torql.database.schema.log_entries import (
    LogEntriesTable,
    ReplayConsoleLogsLogEntriesTable,
    BatchExportLogEntriesTable,
)
from clairview.torql.database.schema.numbers import NumbersTable
from clairview.torql.database.schema.person_distinct_id_overrides import (
    PersonDistinctIdOverridesTable,
    RawPersonDistinctIdOverridesTable,
    join_with_person_distinct_id_overrides_table,
)
from clairview.torql.database.schema.person_distinct_ids import (
    PersonDistinctIdsTable,
    RawPersonDistinctIdsTable,
)
from clairview.torql.database.schema.persons import (
    PersonsTable,
    RawPersonsTable,
    join_with_persons_table,
)
from clairview.torql.database.schema.session_replay_events import (
    RawSessionReplayEventsTable,
    SessionReplayEventsTable,
    join_replay_table_to_sessions_table_v2,
)
from clairview.torql.database.schema.sessions_v1 import RawSessionsTableV1, SessionsTableV1
from clairview.torql.database.schema.sessions_v2 import (
    SessionsTableV2,
    RawSessionsTableV2,
    join_events_table_to_sessions_table_v2,
)
from clairview.torql.database.schema.static_cohort_people import StaticCohortPeople
from clairview.torql.errors import QueryError, ResolutionError
...
def create_torql_database(
    team_id: int, modifiers: Optional[TorQLQueryModifiers] = None, team_arg: Optional["Team"] = None
) -> Database:
    from clairview.models import Team
    ...
    for saved_query in DataWarehouseSavedQuery.objects.filter(team_id=team.pk).exclude(deleted=True):
        views[saved_query.name] = saved_query.torql_definition(modifiers)
    ...
    for table in (
        ...
        s3_table = table.torql_definition(modifiers)
        ...
        if s3_table.fields.get("properties") is None:
            ...
            class WarehouseProperties(VirtualTable):
                ...
                def to_printed_torql(self):
                    return self.parent_table.to_printed_torql()
                    ...
                    for chain in person_field.chain:
                        if isinstance(table_or_field, ast.LazyJoin):
                            table_or_field = table_or_field.resolve_table(
                                TorQLContext(team_id=team_id, database=database)
                            )
    ...
    for view_name in views:
        ...
        if len(saved_query) != 0:
            tables[view_name] = DatabaseSchemaViewTable(
                ...
                name=view.name,
                query=TorQLQuery(query=saved_query[0].query["query"]),
            )
...
TORQL_CHARACTERS_TO_BE_WRAPPED = ["@", "-", "!", "$", "+"]
...
def serialize_fields(
    ...
    from clairview.torql.database.models import SavedQuery
    from clairview.torql.resolver import resolve_types_from_table
    ...
    for field_key, field in field_input.items():
        try:
            ...
            # We redefine fields on some sourced tables, causing the "torql" and "clickhouse" field names to be intentionally out of sync
            schema_valid = True

        if any(n in field_key for n in TORQL_CHARACTERS_TO_BE_WRAPPED):
            torql_value = f"`{field_key}`"
        else:
            torql_value = str(field_key)
        ...
        if field_key == "team_id" and table_type == "clairview":
            ...
            if isinstance(field, IntegerDatabaseField):
                ...
                field_expr = resolve_types_from_table(field.expr, table_name, context, "torql")
            ...
            field_output.append(
                DatabaseSchemaField(
                    ...
                    type=type,
                    schema_valid=schema_valid,
                    table=field.resolve_table(context).to_printed_torql(),
