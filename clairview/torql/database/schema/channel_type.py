...
from clairview.torql.database.models import ExpressionField
...
if(
    ...
    '$direct',
    torql_lookupDomainType({referring_domain})
)
""",
            {
...
multiIf(
    ...
    coalesce(
        torql_lookupPaidSourceType({source}),
        if(
            match({campaign}, '^(.*(([^a-df-z]|^)shop|shopping).*)$'),
            'Paid Shopping',
            NULL
        ),
        torql_lookupPaidMediumType({medium}),
        torql_lookupPaidSourceType({referring_domain}),
        multiIf (
            {gad_source} = '1',
            'Paid Search',
    ...
    coalesce(
        torql_lookupOrganicSourceType({source}),
        if(
            match({campaign}, '^(.*(([^a-df-z]|^)shop|shopping).*)$'),
            'Organic Shopping',
            NULL
        ),
        torql_lookupOrganicMediumType({medium}),
        torql_lookupOrganicSourceType({referring_domain}),
        multiIf(
            match({campaign}, '^(.*video.*)$'),
            'Organic Video',
