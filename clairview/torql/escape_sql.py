...
def escape_torql_identifier(identifier: str | int) -> str:
    if isinstance(identifier, int):  # In TorQL we allow integers as identifiers to access array elements
        return str(identifier)
    if "%" in identifier:
        raise QueryError(f'The TorQL identifier "{identifier}" is not permitted as it contains the "%" character')
    # TorQL allows dollars in the identifier.
    if re.match(
        r"^[A-Za-z_$][A-Za-z0-9_$]*$", identifier
    ):  # Same regex as the frontend escapePropertyAsTorQlIdentifier
        return identifier
...
def escape_torql_string(
    name: float | int | str | list | tuple | date | datetime | UUID | UUIDT,
    timezone: Optional[str] = None,
) -> str:
    return SQLValueEscaper(timezone=timezone, dialect="torql").visit(name)
...
class SQLValueEscaper:
    ...
    # Unlike clairview.torql.visitor.Visitor, this tiny visitor works on primitives.
    ...
    def visit_datetime(self, value: datetime):
        ...
        if self._dialect == "torql":
            return f"toDateTime({self.visit(datetime_string)})"  # no timezone for torql
