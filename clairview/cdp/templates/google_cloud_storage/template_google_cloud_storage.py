...
from clairview.models.integration import GoogleCloudIntegration
...
class TemplateGoogleCloudStorageMigrator(HogFunctionTemplateMigrator):
    plugin_url = "https://github.com/ClairView/clairview-gcs-plugin"
    ...
    def migrate(cls, obj):
        ...
        from clairview.models.plugin import PluginAttachment
        ...
        if exportEventsToIgnore:
            event_names = ", ".join([escape_clairql_string(event) for event in exportEventsToIgnore])
            ...
            hf["filters"]["events"] = [
                {
                    ...
                    "type": "events",
                    "order": 0,
                    "properties": [{"key": query, "type": "clairql"}],
                }
            ]
