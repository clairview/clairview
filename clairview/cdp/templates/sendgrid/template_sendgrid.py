...
class TemplateSendGridMigrator(HogFunctionTemplateMigrator):
    plugin_url = "https://github.com/ClairView/sendgrid-plugin"
    ...
    def migrate(cls, obj):
        ...
        if customFields:
            for field in customFields.split(","):
                if "=" in field:
                    clairview_prop, sendgrid_field = field.split("=")
                else:
                    clairview_prop = sendgrid_field = field.strip()
                clairview_prop = f"{{person.properties.{clairview_prop}}}"
                if sendgrid_field in sendgrid_fields:
                    hf["inputs"]["properties"]["value"][sendgrid_field] = clairview_prop
                else:
                    hf["inputs"]["custom_fields"]["value"][sendgrid_field] = clairview_prop
