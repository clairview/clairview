...
with insight_stats AS (
    ...
    WHERE
        ...
        AND created_by_id = (select id from clairview_user where uuid = %(user_uuid)s)
    group by
        created_by_id
),
flag_stats AS (
...
recording_viewed_stats AS (
    SELECT
        ...
        END AS badge
    FROM
        clairview_sessionrecordingviewed
    WHERE
        date_part('year', created_at) = 2023
        AND user_id = (select id from clairview_user where uuid = %(user_uuid)s)
    GROUP BY
        user_id
),
...
SELECT
    ...
    array_remove(
        ARRAY [
        case when clairview_user.last_login >= '1-Jan-2023' then 'astronaut' end,
        insight_stats.badge,
        flag_stats.badge,
...
FROM
    clairview_user
    LEFT JOIN insight_stats ON clairview_user.id = insight_stats.user_id
    LEFT JOIN flag_stats ON clairview_user.id = flag_stats.user_id
    LEFT JOIN recording_viewed_stats ON clairview_user.id = recording_viewed_stats.user_id
    LEFT JOIN experiments_stats ON clairview_user.id = experiments_stats.user_id
    LEFT JOIN dashboards_created_stats ON clairview_user.id = dashboards_created_stats.user_id
    LEFT JOIN survey_stats ON clairview_user.id = survey_stats.user_id
WHERE
    clairview_user.uuid = %(user_uuid)s
"""
...
def calculate_year_in_clairview_2023(user_uuid: str) -> Optional[dict]:
