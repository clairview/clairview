...
from clairview.year_in_clairview.calculate_2023 import calculate_year_in_clairview_2023
...
explanation = {
    ...
    "reporter": "This just in: You love asking questions and have a nose for news. Surveys have made you the gossip columnist of ClairView!",
    "champion": "You're unmatched. Unstoppable. You're like the Usain Bolt of hedgehogs! We're grateful to have you as a ClairView power user.",
}
...
def render_2023(request, user_uuid: str) -> HttpResponse:
    ...
    try:
        data = calculate_year_in_clairview_2023(user_uuid)
        ...
        capture_exception(e)
        logger.error("year_in_clairview_2023_error_rendering_2023_page", exc_info=True, exc=e, data=data or "no data")
...
def render_2022(request, user_uuid: str) -> HttpResponse:
    template = get_template("hibernating.html")
    html = template.render({"message": "This is the 2022 Year in ClairView. That's too long ago 🙃"}, request=request)
    return HttpResponse(html)
