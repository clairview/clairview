...
CELERY_TASK_DURATION_HISTOGRAM = Histogram(
    "clairview_celery_task_duration_seconds",
...
def receiver_setup_logging(loglevel, logfile, format, colorize, **kwargs) -> None:
    ...
    from clairview.settings import logs
...
def on_worker_start(**kwargs) -> None:
    ...
    from clairview.settings import sentry_init
...
def prerun_signal_handler(task_id, task, **kwargs):
    ...
    from clairview.clickhouse.client.connection import (
        Workload,
...
def postrun_signal_handler(task_id, task, **kwargs):
    from clairview.clickhouse.query_tagging import reset_query_tags
    ...
    try:
        ...
        if is_cloud():
            create_incident(
                ...
                "clairview.celery.setup_periodic_tasks",
                "critical",
            )
        else:
