...
def process_query_task(
    ...
    """
    from clairview.client import execute_process_query
        ...
        try:
            ...
            with pushed_metrics_registry("celery_pg_table_cache_hit_rate") as registry:
                hit_rate_gauge = Gauge(
                    "clairview_celery_pg_table_cache_hit_rate",
...
POSTGRES_TABLES = ["clairview_personoverride", "clairview_personoverridemapping"]
...
def pg_row_count() -> None:
    with pushed_metrics_registry("celery_pg_row_count") as registry:
        row_count_gauge = Gauge(
            "clairview_celery_pg_table_row_count",
...
def ingestion_lag() -> None:
    ...
    try:
        ...
        with pushed_metrics_registry("celery_ingestion_lag") as registry:
            lag_gauge = Gauge(
                "clairview_celery_observed_ingestion_lag_seconds",
            ...
            for event, lag in results:
                ...
                statsd.gauge(f"clairview_celery_{metric}_lag_seconds_rough_minute_precision", lag)
    ...
    for team in Team.objects.filter(pk__in=team_ids):
        requests.post(
            ...
            json={
                "event": "$heartbeat",
                "distinct_id": "clairview-celery-heartbeat",
    ...
    with connection.cursor() as cursor:
        ...
        with pushed_metrics_registry("celery_graphile_worker_queue_size") as registry:
            processing_lag_gauge = Gauge(
                "clairview_celery_graphile_lag_seconds",
            ...
            waiting_jobs_gauge = Gauge(
                "clairview_celery_graphile_waiting_jobs",
...
def clickhouse_row_count() -> None:
    ...
    with pushed_metrics_registry("celery_clickhouse_row_count") as registry:
        row_count_gauge = Gauge(
            "clairview_celery_clickhouse_table_row_count",
        ...
        for table in CLICKHOUSE_TABLES:
            try:
                ...
                statsd.gauge(
                    f"clairview_celery_clickhouse_table_row_count",
                    rows,
                    tags={"table": table},
                )
            except:
...
def clickhouse_errors_count() -> None:
    ...
    with pushed_metrics_registry("celery_clickhouse_errors") as registry:
        errors_gauge = Gauge(
            "clairview_celery_clickhouse_errors",
...
def clickhouse_part_count() -> None:
    ...
    with pushed_metrics_registry("celery_clickhouse_part_count") as registry:
        parts_count_gauge = Gauge(
            "clairview_celery_clickhouse_table_parts_count",
        ...
        for table, parts in rows:
            ...
            statsd.gauge(
                f"clairview_celery_clickhouse_table_parts_count",
                parts,
                tags={"table": table},
            )
...
def clickhouse_mutation_count() -> None:
    ...
    with pushed_metrics_registry("celery_clickhouse_mutation_count") as registry:
        mutations_count_gauge = Gauge(
            "clairview_celery_clickhouse_table_mutations_count",
    ...
    for table, muts in rows:
        ...
        statsd.gauge(
            f"clairview_celery_clickhouse_table_mutations_count",
            muts,
            tags={"table": table},
        )
...
def clickhouse_clear_removed_data() -> None:
    from clairview.models.async_deletion.delete_cohorts import AsyncCohortDeletion
    from clairview.models.async_deletion.delete_events import AsyncEventDeletion
    from clairview.pagerduty.pd import create_incident
...
def monitoring_check_clickhouse_schema_drift() -> None:
    from clairview.tasks.check_clickhouse_schema_drift import (
...
def calculate_cohort(parallel_count: int) -> None:
    from clairview.tasks.calculate_cohort import calculate_cohorts
...
def poll_query_performance(last_known_run_time_ns: int) -> None:
    ...
    try:
        ...
        from clairview.tasks.poll_query_performance import poll_query_performance as poll_query_performance_nontask
...
def process_scheduled_changes() -> None:
    from clairview.tasks.process_scheduled_changes import process_scheduled_changes
...
def validate_proxy_domains() -> None:
    from clairview.tasks.validate_proxy_domains import validate_proxy_domains
...
def sync_insight_cache_states_task() -> None:
    from clairview.caching.insight_caching_state import sync_insight_cache_states
...
def schedule_cache_updates_task() -> None:
    from clairview.caching.insight_cache import schedule_cache_updates
...
def sync_insight_caching_state(
    ...
    from clairview.caching.insight_caching_state import sync_insight_caching_state
...
def calculate_decide_usage() -> None:
    from clairview.models.feature_flag.flag_analytics import (
...
def demo_reset_master_team() -> None:
    from clairview.tasks.demo_reset_master_team import demo_reset_master_team
...
def sync_all_organization_available_product_features() -> None:
    from clairview.tasks.sync_all_organization_available_product_features import (
...
def check_async_migration_health() -> None:
    from clairview.tasks.async_migrations import check_async_migration_health
...
def verify_persons_data_in_sync() -> None:
    from clairview.tasks.verify_persons_data_in_sync import (
...
def stop_surveys_reached_target() -> None:
    from clairview.tasks.stop_surveys_reached_target import stop_surveys_reached_target
...
def update_survey_iteration() -> None:
    from clairview.tasks.update_survey_iteration import update_survey_iteration
...
def send_org_usage_reports() -> None:
    from clairview.tasks.usage_report import send_all_org_usage_reports
...
def calculate_external_data_rows_synced() -> None:
    try:
        from clairview.tasks.warehouse import capture_external_data_rows_synced
    except ImportError:
        pass
    else:
