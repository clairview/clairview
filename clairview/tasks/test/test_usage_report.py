...
from clairview.schema import EventsQuery
...
from clairview.test.fixtures import create_app_metric2
from clairview.utils import get_machine_id, get_previous_day
from clairview.warehouse.models import ExternalDataJob, ExternalDataSource
...
class UsageReport(APIBaseTest, ClickhouseTestMixin, ClickhouseDestroyTablesMixin):
    ...
    def _test_usage_report(self) -> list[dict]:
        with self.settings(SITE_URL="http://test.clairview.com"):
            ...
            assert report["table_sizes"]["clairview_event"] < 10**7  # <10MB
            assert report["table_sizes"]["clairview_sessionrecordingevent"] < 10**7  # <10MB
    ...
    @patch("clairview.tasks.usage_report.Client")
    @patch("requests.post")
    def test_unlicensed_usage_report(self, mock_post: MagicMock, mock_client: MagicMock) -> None:
        ...
        mock_clairview = MagicMock()
        mock_client.return_value = mock_clairview
        ...
        assert mock_clairview.capture.call_count == 2
        mock_clairview.capture.assert_has_calls(calls, any_order=True)
...
class TestFeatureFlagsUsageReport(ClickhouseDestroyTablesMixin, TestCase, ClickhouseTestMixin):
    ...
    def _setup_teams(self) -> None:
        self.analytics_org = Organization.objects.create(name="ClairView")
    ...
    @snapshot_clickhouse_queries
    @patch("clairview.tasks.usage_report.Client")
    @patch("clairview.tasks.usage_report.send_report_to_billing_service")
    def test_usage_report_decide_requests(self, billing_task_mock: MagicMock, clairview_capture_mock: MagicMock) -> None:
        self._setup_teams()
        for i in range(10):
            _create_event(
                distinct_id="3",
    ...
    def test_usage_report_local_evaluation_requests(
        self, billing_task_mock: MagicMock, clairview_capture_mock: MagicMock
    ) -> None:
        self._setup_teams()
        for i in range(10):
            _create_event(
...
class TestSurveysUsageReport(ClickhouseDestroyTablesMixin, TestCase, ClickhouseTestMixin):
    ...
    @patch("clairview.tasks.usage_report.Client")
    @patch("clairview.tasks.usage_report.send_report_to_billing_service")
    def test_usage_report_survey_responses(self, billing_task_mock: MagicMock, clairview_capture_mock: MagicMock) -> None:
        self._setup_teams()
        for i in range(10):
            _create_event(
                distinct_id="3",
...
class TestHogFunctionUsageReports(ClickhouseDestroyTablesMixin, TestCase, ClickhouseTestMixin):
    ...
    @patch("clairview.tasks.usage_report.Client")
    @patch("clairview.tasks.usage_report.send_report_to_billing_service")
    def test_hog_function_usage_metrics(self, billing_task_mock: MagicMock, clairview_capture_mock: MagicMock) -> None:
        self._setup_teams()
...
class SendUsageTest(LicensedTestMixin, ClickhouseDestroyTablesMixin, APIBaseTest):
    ...
    def test_send_usage(self, mock_post: MagicMock, mock_client: MagicMock) -> None:
        ...
        mock_clairview.capture.assert_any_call(
            get_machine_id(),
            "organization usage report",
    ...
    @freeze_time("2021-10-10T23:01:00Z")
    @patch("clairview.tasks.usage_report.capture_exception")
    @patch("clairview.tasks.usage_report.sync_execute", side_effect=Exception())
    @patch("clairview.tasks.usage_report.Client")
    @patch("requests.post")
