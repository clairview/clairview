...
import clairview.tasks.tasks
from clairview.redis import get_client
from clairview.tasks.poll_query_performance import query_manager_from_initial_query_id, poll_query_performance
from clairview.tasks.tasks import Polling
...
class TestPollQueryPerformance(SimpleTestCase):
    ...
    @patch("clairview.tasks.poll_query_performance.QueryStatusManager")
    @patch("clairview.tasks.poll_query_performance.sync_execute")
...
class TestPollQueryPerformanceTask(SimpleTestCase):
    @patch("clairview.tasks.tasks.logger.error")
    def test_poll_query_performance_does_not_run_if_last_update_does_not_match(
        ...
        clairview.tasks.tasks.poll_query_performance("DIFFERENT TIME")
        mock_logger_error.assert_called_once_with("Poll query performance task terminating: another poller is running")

    @patch("clairview.tasks.tasks.logger.error")
    @patch("clairview.tasks.tasks.poll_query_performance.apply_async")
    def test_poll_query_performance_runs_and_restarts_itself_with_delay(
        ...
        clairview.tasks.tasks.poll_query_performance(key)
    ...
    @patch("clairview.tasks.tasks.logger.error")
    @patch("clairview.tasks.tasks.poll_query_performance.delay")
    ...
    def test_start_poll_query_performance_does_nothing_for_14_seconds(
        ...
        for _ in range(3):
            clairview.tasks.tasks.start_poll_query_performance()
