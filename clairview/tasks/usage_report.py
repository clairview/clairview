...
from clairview import version_requirement
...
from clairview.constants import FlagRequestType
from clairview.logging.timing import timed_log
from clairview.models import GroupTypeMapping, OrganizationMembership, User
...
from clairview.models.plugin import PluginConfig
from clairview.models.team.team import Team
from clairview.models.utils import namedtuplefetchall
from clairview.settings import CLICKHOUSE_CLUSTER, INSTANCE_TAG
...
from clairview.warehouse.models import ExternalDataJob
...
class TableSizes(TypedDict):
    clairview_event: int
    clairview_sessionrecordingevent: int
...
def get_instance_metadata(period: tuple[datetime, datetime]) -> InstanceMetadata:
    ...
    if realm != "cloud":
        ...
        metadata.table_sizes = {
            "clairview_event": fetch_table_size("clairview_event"),
            "clairview_sessionrecordingevent": fetch_table_size("clairview_sessionrecordingevent"),
        }
...
def get_teams_with_billable_event_count_in_period(
    ...
    if count_distinct:
        ...
        # https://github.com/ClairView/clairview/blob/master/clairview/models/event/sql.py#L92
    ...
    results = sync_execute(
        ...
        SELECT
            ...
            multiIf(
                ...
                {lib_expression} = 'clairview-node', 'node_events',
                {lib_expression} = 'clairview-android', 'android_events',
                {lib_expression} = 'clairview-flutter', 'flutter_events',
                {lib_expression} = 'clairview-ios', 'ios_events',
                {lib_expression} = 'clairview-go', 'go_events',
                {lib_expression} = 'clairview-java', 'java_events',
                {lib_expression} = 'clairview-react-native', 'react_native_events',
                {lib_expression} = 'clairview-ruby', 'ruby_events',
                {lib_expression} = 'clairview-python', 'python_events',
                {lib_expression} = 'clairview-php', 'php_events',
                'other'
            ) AS metric,
            count(1) as count
        FROM events
...
def capture_report(
    ...
    try:
        ...
        logger.info(f"UsageReport sent to ClairView for organization {org_id}")
    except Exception as err:
        logger.exception(
            f"UsageReport sent to ClairView for organization {org_id} failed: {str(err)}",
        )
...
def send_all_org_usage_reports(
    ...
    try:
        ...
        logger.info("Sending usage reports to ClairView and Billing...")  # noqa T201
        time_now = datetime.now()
        for org_report in org_reports.values():
            ...
            # First capture the events to ClairView
        ...
        time_since = datetime.now() - time_now
        logger.debug(f"Sending usage reports to ClairView and Billing took {time_since.total_seconds()} seconds.")  # noqa T201
    except Exception as err:
        capture_exception(err)
        raise
