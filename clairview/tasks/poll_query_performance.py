...
from clairview.clickhouse.client.execute_async import QueryStatusManager
from clairview.utils import UUID_REGEX
import re
import math
...
def get_query_results() -> list[Any]:
    SYSTEM_PROCESSES_SQL = r"""
        SELECT
            ...
            query_id
        FROM clusterAllReplicas(clairview, system.processes)
        ...
        UNION ALL SELECT
            ...
            ProfileEvents['OSCPUVirtualTimeMicroseconds'] as OSCPUVirtualTimeMicroseconds,
            query_id
        FROM clusterAllReplicas(clairview, system.query_log)
        WHERE initial_query_id REGEXP '\d+_[0-9a-f]{8}-'
