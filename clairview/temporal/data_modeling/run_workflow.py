...
from clairview.warehouse.models import DataWarehouseModelPath, DataWarehouseSavedQuery
from clairview.warehouse.util import database_sync_to_async
from clairview.warehouse.data_load.create_table import create_table_from_saved_query
from clairview.temporal.data_imports.util import prepare_s3_files_for_querying
...
async def run_dag_activity(inputs: RunDagActivityInputs) -> Results:
    ...
    * ClairView tables (e.g. events, persons, sessions) are assumed to be always available
...
async def build_dag_from_selectors(selector_paths: SelectorPaths, team_id: int) -> DAG:
    ...
    """
    clairview_tables = await get_clairview_tables(team_id)
    dag = {}
        ...
        for path in paths:
            ...
            for index, label in enumerate(path):
                ...
                if (
                    ...
                    and label not in clairview_tables
...
class RunWorkflow(ClairViewWorkflow):
    """A Temporal Workflow to run ClairView data models.
