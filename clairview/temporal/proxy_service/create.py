...
from clairview.models import ProxyRecord
from clairview.temporal.batch_exports.base import ClairViewWorkflow
from clairview.temporal.common.logger import bind_temporal_org_worker_logger

from clairview.temporal.proxy_service.common import (
    get_grpc_client,
...
from clairview.temporal.proxy_service.proto import CreateRequest, StatusRequest, CertificateState_READY
    ...
    try:
        ...
        if is_cloudflare:
            ...
            await update_record_message(
                ...
                message="The DNS record appears to have Cloudflare proxying enabled - please disable this. For more information see [the docs](https://clairview.com/docs/advanced/proxy/managed-reverse-proxy)",
            )
        raise
...
class CreateManagedProxyWorkflow(ClairViewWorkflow):
