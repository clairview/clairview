...
from clairview.batch_exports.service import (
    BatchExportField,
    BatchExportModel,
    BatchExportSchema,
    HttpBatchExportInputs,
)
from clairview.models import BatchExportRun
...
from clairview.temporal.batch_exports.metrics import (
    get_bytes_exported_metric,
    get_rows_exported_metric,
)
from clairview.temporal.batch_exports.temporary_file import (
    BatchExportTemporaryFile,
    json_dumps_bytes,
)
from clairview.temporal.common.clickhouse import get_client
...
async def insert_into_http_activity(inputs: HttpInsertInputs) -> RecordsCompleted:
    ...
    async with get_client(team_id=inputs.team_id) as client:
        ...
        # The HTTP destination currently only supports the ClairView batch capture endpoint. In the
        # future we may support other endpoints, but we'll need a way to template the request body,
        # headers, etc.
        #
        # For now, we write the batch out in ClairView capture format, which means each Batch Export
        ...
        clairview_batch_header = """{{"api_key": "{}","historical_migration":true,"batch": [""".format(inputs.token)
        clairview_batch_footer = "]}"

        with BatchExportTemporaryFile() as batch_file:

            def write_event_to_batch(event):
                if batch_file.records_since_last_reset == 0:
                    batch_file.write(clairview_batch_header)
                else:
                    batch_file.write(",")
            ...
            async def flush_batch_to_http_endpoint(last_uploaded_timestamp: str, session: aiohttp.ClientSession):
                ...
                batch_file.write(clairview_batch_footer)
            ...
            async with aiohttp.ClientSession() as session:
                for record_batch in record_iterator:
                    for row in record_batch.select(columns).to_pylist():
                        # Format result row as ClairView event, write JSON to the batch file.
...
class HttpBatchExportWorkflow(ClairViewWorkflow):
