...
from clairview.batch_exports.service import BatchExportModel, BatchExportSchema
from clairview.temporal.batch_exports.batch_exports import (
    finish_batch_export_run,
    iter_model_records,
    start_batch_export_run,
)
from clairview.temporal.batch_exports.s3_batch_export import (
...
from clairview.temporal.common.clickhouse import ClickHouseClient
from clairview.temporal.tests.batch_exports.utils import mocked_start_batch_export_run
from clairview.temporal.tests.utils.events import (
    generate_test_events_in_clickhouse,
)
from clairview.temporal.tests.utils.models import (
    acreate_batch_export,
    adelete_batch_export,
    afetch_batch_export_runs,
)
from clairview.temporal.tests.utils.s3 import read_parquet_from_s3, read_s3_data_as_json
...
def s3_key_prefix(request):
    ...
    try:
        ...
        return f"clairview-data-{str(uuid.uuid4())}"
...
@pytest.mark.parametrize(
    "s3_key_prefix",
    [
        "clairview-{table}/{year}-{month}-{day}/{hour}:{minute}:{second}",
        "clairview-{table}/{hour}:{minute}:{second}/{year}-{month}-{day}",
        "clairview-{table}/{hour}:{minute}:{second}",
        "clairview/{year}-{month}-{day}/{hour}:{minute}:{second}",
        "{year}-{month}-{day}",
    ],
    indirect=True,
)
...
async def test_s3_export_workflow_with_request_timeouts(
    ...
    async with (
        ...
        with (
            mock.patch("clairview.temporal.batch_exports.s3_batch_export.aioboto3.Session", FakeSession),
            mock.patch("clairview.temporal.batch_exports.batch_exports.RetryPolicy", DoNotRetryPolicy),
        ):
            await activity_environment.client.execute_workflow(
                S3BatchExportWorkflow.run,
                inputs,
