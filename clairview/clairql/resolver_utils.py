...
from clairview.clairql.errors import ResolutionError, SyntaxError
from clairview.clairql.visitor import clone_expr


def lookup_field_by_name(scope: ast.SelectQueryType, name: str, context: ClairQLContext) -> Optional[ast.Type]:
...
def ast_to_query_node(expr: ast.Expr | ast.ClairQLXTag):
    ...
    elif isinstance(expr, ast.ClairQLXTag):
...
def expand_clairqlx_query(node: ast.ClairQLXTag, team_id: Optional[int]):
