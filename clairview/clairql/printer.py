...
from clairview.clairql.constants import (
    MAX_SELECT_RETURNED_ROWS,
    ClairQLGlobalSettings,
)
from clairview.clairql.functions import (
...
from clairview.clairql.database.models import Table, FunctionCallTable, SavedQuery
...
from clairview.clairql.errors import ImpossibleASTError, InternalClairQLError, QueryError, ResolutionError
from clairview.clairql.escape_sql import (
    escape_clickhouse_identifier,
    escape_clickhouse_string,
    escape_clairql_identifier,
    escape_clairql_string,
)
from clairview.clairql.functions.mapping import ALL_EXPOSED_FUNCTION_NAMES, validate_function_args, CLAIRQL_COMPARISON_MAPPING
from clairview.clairql.modifiers import create_default_modifiers_for_team, set_default_in_cohort_via
from clairview.clairql.resolver import resolve_types
from clairview.clairql.resolver_utils import lookup_field_by_name
from clairview.clairql.transforms.in_cohort import resolve_in_cohorts, resolve_in_cohorts_conjoined
from clairview.clairql.transforms.lazy_tables import resolve_lazy_tables
from clairview.clairql.transforms.property_types import build_property_swapper, PropertySwapper
from clairview.clairql.visitor import Visitor, clone_expr
from clairview.models.property import PropertyName, TableColumn
...
def team_id_guard_for_table(table_type: Union[ast.TableType, ast.TableAliasType], context: ClairQLContext) -> ast.Expr:
    """Add a mandatory "and(team_id, ...)" filter around the expression."""
    if not context.team_id:
        raise InternalClairQLError("context.team_id not found")
...
def to_printed_clairql(query: ast.Expr, team: Team, modifiers: Optional[ClairQLQueryModifiers] = None) -> str:
    """Prints the ClairQL query without mutating the node"""
    return print_ast(
        clone_expr(query),
        dialect="clairql",
...
def print_ast(
    node: ast.Expr,
    context: ClairQLContext,
    dialect: Literal["clairql", "clickhouse"],
...
def prepare_ast_for_printing(
    ...
    with context.timings.measure("create_clairql_database"):
        context.database = context.database or create_clairql_database(context.team_id, context.modifiers, context.team)
...
class _Printer(Visitor):
    ...
    def visit_select_query(self, node: ast.SelectQuery):
        if self.dialect == "clickhouse":
            if not self.context.enable_select_queries:
                raise InternalClairQLError("Full SELECT queries are disabled if context.enable_select_queries is False")
            if not self.context.team_id:
                raise InternalClairQLError("Full SELECT queries are disabled if context.team_id is not set")
    ...
    def visit_join_expr(self, node: ast.JoinExpr) -> JoinExprResponse:
        ...
        if isinstance(node.type, ast.TableAliasType) or isinstance(node.type, ast.TableType):
            ...
            if self.dialect == "clickhouse":
                ...
                sql = table_type.table.to_printed_clairql()
            ...
            if isinstance(table_type.table, FunctionCallTable) and not isinstance(table_type.table, S3Table):
                ...
                raise QueryError(f"Table '{table_type.table.to_printed_clairql()}' does not accept arguments")
            ...
            if self.dialect == "clairql":
                join_strings.append(self._print_identifier(node.type.table.to_printed_clairql()))
            else:
                raise ImpossibleASTError(f"Unexpected LazyTableType for: {node.type.table.to_printed_clairql()}")
        else:
            raise QueryError(
    ...
    def visit_tuple_access(self, node: ast.TupleAccess):
        ...
        symbol = "?." if self.dialect == "clairql" and node.nullish else "."
    ...
    def visit_compare_operation(self, node: ast.CompareOperation):
        ...
        # :HACK: until the new type system is out: https://github.com/ClairView/clairview/pull/17267
        ...
        if in_join_constraint or self.dialect == "clairql" or not_nullable:
            return op
    ...
    def visit_field(self, node: ast.Field):
        if node.type is None:
            field = ".".join([self._print_clairql_identifier_or_index(identifier) for identifier in node.chain])
            raise ImpossibleASTError(f"Field {field} has no type")

        if self.dialect == "clairql":
            if node.chain == ["*"]:
                return "*"
            # When printing ClairQL, we print the properties out as a chain as they are.
            return ".".join([self._print_clairql_identifier_or_index(identifier) for identifier in node.chain])
    ...
    def visit_call(self, node: ast.Call):
        ...
        if node.name in CLAIRQL_COMPARISON_MAPPING:
            op = CLAIRQL_COMPARISON_MAPPING[node.name]
            if len(node.args) != 2:
        ...
        elif func_meta := find_clairql_aggregation(node.name):
            ...
            for stack_node in self.stack:
                if stack_node != node and isinstance(stack_node, ast.Call) and find_clairql_aggregation(stack_node.name):
                    raise QueryError(
        ...
        elif func_meta := find_clairql_function(node.name):
        ...
        elif func_meta := find_clairql_clairview_function(node.name):
            validate_function_args(node.args, func_meta.min_args, func_meta.max_args, node.name)
            args = [self.visit(arg) for arg in node.args]

            if self.dialect in ("clairql", "clickhouse"):
                if node.name == "clairql_lookupDomainType":
                ...
                elif node.name == "clairql_lookupPaidSourceType":
                ...
                elif node.name == "clairql_lookupPaidMediumType":
                    return (
                        f"dictGetOrNull('channel_definition_dict', 'type_if_paid', (coalesce({args[0]}, ''), 'medium'))"
                    )
                elif node.name == "clairql_lookupOrganicSourceType":
                ...
                elif node.name == "clairql_lookupOrganicMediumType":
                    return f"dictGetOrNull('channel_definition_dict', 'type_if_organic', (coalesce({args[0]}, ''), 'medium'))"
            raise QueryError(f"Unexpected unresolved ClairQL function '{node.name}(...)'")
        else:
    ...
    def visit_table_type(self, type: ast.TableType):
        if self.dialect == "clickhouse":
            ...
            return type.table.to_printed_clairql()
    ...
    def visit_field_type(self, type: ast.FieldType):
        ...
        if (
            ...
            # :KLUDGE: Legacy person properties handling. Only used within non-ClairQL queries, such as insights.
            if (
                self.context.within_non_clairql_query
                ...
                if self.context.within_non_clairql_query and type_with_name_in_scope == type:
                    # Do not prepend table name in non-clairql context. We don't know what it actually is.
                    return field_sql
            ...
            # :KLUDGE: Legacy person properties handling. Only used within non-ClairQL queries, such as insights.
            if self.context.within_non_clairql_query and field_sql == "events__pdi__person.properties":
                if self.context.modifiers.personsOnEventsMode != PersonsOnEventsMode.DISABLED:
                    field_sql = "person_properties"
                else:
                    field_sql = "person_props"
    ...
    def __get_all_materialized_property_sources(
        ...
        if isinstance(table, ast.TableType):
            if self.dialect == "clickhouse":
                ...
                table_name = table.table.to_printed_clairql()
            if field is None:
    ...
    def _print_identifier(self, name: str) -> str:
        ...
        return escape_clairql_identifier(name)

    def _print_clairql_identifier_or_index(self, name: str | int) -> str:
    ...
    def _print_escaped_string(self, name: float | int | str | list | tuple | datetime | date) -> str:
        ...
        return escape_clairql_string(name, timezone=self._get_timezone())
