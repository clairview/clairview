...
from clairview.schema import ClairQLVariable

T = TypeVar("T", bound=ast.Expr)


def replace_variables(node: T, variables: list[ClairQLVariable], team: Team) -> T:
...
class ReplaceVariables(CloningVisitor):
    ...
    def __init__(self, variables: list[ClairQLVariable], team: Team):
        super().__init__()
