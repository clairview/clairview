...
def escape_clairql_identifier(identifier: str | int) -> str:
    if isinstance(identifier, int):  # In ClairQL we allow integers as identifiers to access array elements
        return str(identifier)
    if "%" in identifier:
        raise QueryError(f'The ClairQL identifier "{identifier}" is not permitted as it contains the "%" character')
    # ClairQL allows dollars in the identifier.
    if re.match(
        r"^[A-Za-z_$][A-Za-z0-9_$]*$", identifier
    ):  # Same regex as the frontend escapePropertyAsClairQlIdentifier
        return identifier
...
def escape_clairql_string(
    name: float | int | str | list | tuple | date | datetime | UUID | UUIDT,
    timezone: Optional[str] = None,
) -> str:
    return SQLValueEscaper(timezone=timezone, dialect="clairql").visit(name)
...
class SQLValueEscaper:
    ...
    # Unlike clairview.clairql.visitor.Visitor, this tiny visitor works on primitives.
    ...
    def visit_datetime(self, value: datetime):
        ...
        if self._dialect == "clairql":
            return f"toDateTime({self.visit(datetime_string)})"  # no timezone for clairql
