...
from clairview.clairql.ast import ConstantType, FieldTraverserType
...
from clairview.clairql.database.schema.persons import PersonsTable
from clairview.clairql.errors import ImpossibleASTError, QueryError, ResolutionError
from clairview.clairql.functions import find_clairql_clairview_function
from clairview.clairql.functions.action import matches_action
from clairview.clairql.functions.cohort import cohort_query_node
from clairview.clairql.functions.mapping import CLAIRQL_CLICKHOUSE_FUNCTIONS, compare_types, validate_function_args
from clairview.clairql.functions.recording_button import recording_button
from clairview.clairql.functions.sparkline import sparkline
from clairview.clairql.clairqlx import CLAIRQLX_COMPONENTS, convert_to_hx
from clairview.clairql.parser import parse_select
from clairview.clairql.resolver_utils import expand_clairqlx_query, lookup_cte_by_name, lookup_field_by_name
from clairview.clairql.visitor import CloningVisitor, TraversingVisitor, clone_expr
...
def resolve_types_from_table(
    expr: ast.Expr, table_name: str, context: ClairQLContext, dialect: Literal["clairql", "clickhouse"]
) -> ast.Expr:
...
class Resolver(CloningVisitor):
    ...
    def visit_join_expr(self, node: ast.JoinExpr):
        ...
        if isinstance(node.table, ast.ClairQLXTag):
            node.table = expand_clairqlx_query(node.table, self.context.team_id)
    ...
    def visit_clairqlx_tag(self, node: ast.ClairQLXTag):
        if node.kind in CLAIRQLX_COMPONENTS:
            return self.visit(convert_to_hx(node))
        return self.visit(expand_clairqlx_query(node, self.context.team_id))
    ...
    def visit_call(self, node: ast.Call):
        ...
        if func_meta := find_clairql_clairview_function(node.name):
        ...
        if node.name in CLAIRQL_CLICKHOUSE_FUNCTIONS:
            signatures = CLAIRQL_CLICKHOUSE_FUNCTIONS[node.name].signatures
            if signatures:
        ...
        if return_type is None:
            ...
            # Uncomment once all clairql mappings are complete with signatures
    ...
    def visit_expr_call(self, node: ast.ExprCall):
        raise QueryError("You can only call simple functions in ClairQL, not expressions")

    def visit_block(self, node: ast.Block):
        raise QueryError("You can not use blocks in ClairQL")
