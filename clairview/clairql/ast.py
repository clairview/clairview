...
from clairview.clairql.errors import NotImplementedError, QueryError, ResolutionError
...
class FieldAliasType(Type):
    ...
    def get_child(self, name: str, context: ClairQLContext) -> Type:
        return self.type.get_child(name, context)

    def has_child(self, name: str, context: ClairQLContext) -> bool:
        return self.type.has_child(name, context)

    def resolve_constant_type(self, context: ClairQLContext) -> "ConstantType":
        return self.type.resolve_constant_type(context)

    def resolve_database_field(self, context: ClairQLContext):
    ...
    def resolve_table_type(self, context: ClairQLContext):
        if isinstance(self.type, FieldType):
...
class BaseTableType(Type):
    def resolve_database_table(self, context: ClairQLContext) -> Table:
...
class SelectQueryType(Type):
    ...
    def resolve_column_constant_type(self, name: str, context: ClairQLContext) -> "ConstantType":
...
class FieldType(Type):
    ...
    def resolve_database_field(self, context: ClairQLContext) -> Optional[FieldOrTable]:
    ...
    def is_nullable(self, context: ClairQLContext) -> bool:
    ...
    def get_child(self, name: str | int, context: ClairQLContext) -> Type:
        database_field = self.resolve_database_field(context)
...
class UnresolvedFieldType(Type):
    ...
    def has_child(self, name: str | int, context: ClairQLContext) -> bool:
        return False
...
class Alias(Expr):
    ...
    expr: Expr
    """
    Aliases are "hidden" if they're automatically created by ClairQL when abstracting fields.
...
class SelectQuery(Expr):
    ...
    settings: Optional[ClairQLQuerySettings] = None
...
@dataclass(kw_only=True)
class ClairQLXAttribute(AST):
    name: str
    value: Any


@dataclass(kw_only=True)
class ClairQLXTag(AST):
    kind: str
    attributes: list[ClairQLXAttribute]
