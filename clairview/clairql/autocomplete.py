...
from clairview.clairql.database.database import CLAIRQL_CHARACTERS_TO_BE_WRAPPED, Database, create_clairql_database
...
from clairview.clairql.functions.mapping import ALL_EXPOSED_FUNCTION_NAMES
from clairview.clairql.parser import parse_select, parse_expr, parse_string_template, parse_program
from clairview.clairql import ast
from clairview.clairql.base import AST, CTE, ConstantType
from clairview.clairql.resolver import resolve_types
from clairview.clairql.timings import ClairQLTimings
from clairview.clairql.visitor import TraversingVisitor, clone_expr
...
def get_table(context: ClairQLContext, join_expr: ast.JoinExpr, ctes: Optional[dict[str, CTE]]) -> None | Table:
    ...
    def resolve_fields_on_table(table: Table | None, table_query: ast.SelectQuery) -> Table | None:
        ...
        try:
            node = cast(ast.SelectQuery, resolve_types(node=table_query, dialect="clairql", context=context))
            ...
            table_name = table.to_printed_clairql()
...
def get_tables_aliases(query: ast.SelectQuery, context: ClairQLContext) -> dict[str, ast.Table]:
...
def resolve_table_field_traversers(table: Table, context: ClairQLContext) -> Table:
...
def append_table_field_to_response(table: Table, suggestions: list[AutocompleteCompletionItem], language: str) -> None:
    ...
    extend_responses(
        ...
        details=details,
        insert_text=lambda key: f"`{key}`" if any(n in key for n in CLAIRQL_CHARACTERS_TO_BE_WRAPPED) else key,
    )
...
def get_clairql_autocomplete(
    query: ClairQLAutocomplete, team: Team, database_arg: Optional[Database] = None
) -> ClairQLAutocompleteResponse:
    response = ClairQLAutocompleteResponse(suggestions=[], incomplete_list=False)
    timings = ClairQLTimings()

    if database_arg is not None:
        database = database_arg
    else:
        database = create_clairql_database(team_id=team.pk, team_arg=team)
                    ...
                    for index, chain_part in enumerate(node.chain):
                        ...
                        if is_last_part:
                            ...
                            if isinstance(field, StringJSONDatabaseField):
                                if last_table.to_printed_clairql() == "events":
                                    if field.name == "person_properties":
                                        property_type = PropertyDefinition.Type.PERSON
                                    else:
                                        property_type = PropertyDefinition.Type.EVENT
                                elif last_table.to_printed_clairql() == "persons":
                                    property_type = PropertyDefinition.Type.PERSON
                                elif last_table.to_printed_clairql() == "groups":
...
def add_globals_to_suggestions(globalVars: dict, response: ClairQLAutocompleteResponse):
