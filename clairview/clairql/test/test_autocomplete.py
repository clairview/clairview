...
from clairview.clairql.database.models import StringDatabaseField
from clairview.clairql.database.schema.events import EventsTable
from clairview.clairql.database.schema.persons import PERSONS_FIELDS
...
from clairview.schema import ClairQLAutocomplete, ClairQLAutocompleteResponse, HogLanguage, ClairQLQuery, Kind
...
class TestAutocomplete(ClickhouseTestMixin, APIBaseTest):
    ...
    def _select(
        ...
        autocomplete = ClairQLAutocomplete(
            kind="ClairQLAutocomplete", query=query, language=HogLanguage.HOG_QL, startPosition=start, endPosition=end
        )
        return get_clairql_autocomplete(query=autocomplete, team=self.team, database_arg=database)

    def _expr(self, query: str, start: int, end: int, database: Optional[Database] = None) -> ClairQLAutocompleteResponse:
        autocomplete = ClairQLAutocomplete(
            kind="ClairQLAutocomplete",
            query=query,
            language=HogLanguage.HOG_QL_EXPR,
            sourceQuery=ClairQLQuery(query="select * from events"),
            startPosition=start,
            endPosition=end,
        )
        return get_clairql_autocomplete(query=autocomplete, team=self.team, database_arg=database)
    ...
    def _json(self, query: str, start: int, end: int, database: Optional[Database] = None) -> ClairQLAutocompleteResponse:
        autocomplete = ClairQLAutocomplete(
            kind="ClairQLAutocomplete",
            query=query,
    ...
    def test_autocomplete_events_hidden_field(self):
        database = create_clairql_database(team_id=self.team.pk, team_arg=self.team)
