...
from clairview.clickhouse.client.execute import sync_execute
from clairview.clairql import ast
from clairview.clairql.constants import MAX_SELECT_RETURNED_ROWS, ClairQLQuerySettings, ClairQLGlobalSettings
from clairview.clairql.context import ClairQLContext
from clairview.clairql.database.database import Database
from clairview.clairql.database.models import DateDatabaseField, StringDatabaseField
from clairview.clairql.errors import ExposedClairQLError, QueryError
from clairview.clairql.parser import parse_select, parse_expr
from clairview.clairql.printer import print_ast, to_printed_clairql, prepare_ast_for_printing, print_prepared_ast
from clairview.models import PropertyDefinition
from clairview.models.team.team import WeekStartDay
...
from clairview.test.base import BaseTest, _create_event, cleanup_materialized_columns
...
class TestPrinter(BaseTest):
    ...
    # Helper to always translate ClairQL with a blank context
    def _expr(
        self,
        query: str,
        ...
        context = context or ClairQLContext(team_id=self.team.pk, enable_select_queries=True)
    ...
    def _select(
        ...
        return print_ast(
            ...
            context or ClairQLContext(team_id=self.team.pk, enable_select_queries=True),
            "clickhouse",
        )
    ...
    def _assert_expr_error(
        ...
        with self.assertRaises(ExposedClairQLError) as context:
    ...
    def test_to_printed_clairql(self):
        expr = parse_select("select 1 + 2, 3 from events")
        repsponse = to_printed_clairql(expr, self.team)
        self.assertEqual(
    ...
    def test_literals(self):
        ...
        self.assertEqual(self._expr("'string'"), "%(clairql_val_0)s")
    ...
    def test_array_access(self):
        ...
        self.assertEqual(
            ...
            "replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_0)s), ''), 'null'), '^\"|\"$', '')",
        )
        self.assertEqual(self._expr("events.event[1 + 2]"), "events.event[plus(1, 2)]")

        self.assertEqual(self._expr("[1,2,3]?.[1]", dialect="clairql"), "[1, 2, 3]?.[1]")
    ...
    def test_tuple_access(self):
        self.assertEqual(self._expr("(1,2)?.2", dialect="clairql"), "tuple(1, 2)?.2")
    ...
    def test_fields_and_properties(self):
        ...
        self.assertEqual(
            ...
            "replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_0)s, %(clairql_val_1)s), ''), 'null'), '^\"|\"$', '')",
        )
        context = ClairQLContext(team_id=self.team.pk)
        self.assertEqual(
        ...
        with override_settings(PERSON_ON_EVENTS_V2_OVERRIDE=False):
            context = ClairQLContext(
                ...
                modifiers=ClairQLQueryModifiers(personsOnEventsMode=PersonsOnEventsMode.DISABLED),
            )
            self.assertEqual(
                self._expr("person.properties.bla", context),
                "replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(person_props, %(clairql_val_0)s), ''), 'null'), '^\"|\"$', '')",
            )
            context = ClairQLContext(team_id=self.team.pk)
            self.assertEqual(
                self._expr("person.properties.bla", context),
        ...
        with override_settings(PERSON_ON_EVENTS_OVERRIDE=True):
            ...
            self.assertEqual(
                self._expr("person.properties.bla", context),
                "replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(person_properties, %(clairql_val_0)s), ''), 'null'), '^\"|\"$', '')",
            )
            context = ClairQLContext(team_id=self.team.pk)
            self.assertEqual(
                self._expr("person.properties.bla", context),
                "replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.person_properties, %(clairql_val_0)s), ''), 'null'), '^\"|\"$', '')",
            )

    def test_clairql_properties(self):
        self.assertEqual(
            self._expr("event", ClairQLContext(team_id=self.team.pk), "clairql"),
            "event",
        )
        self.assertEqual(
            self._expr("person", ClairQLContext(team_id=self.team.pk), "clairql"),
            "person",
        )
        self.assertEqual(
            self._expr(
                "person.properties.$browser",
                ClairQLContext(team_id=self.team.pk),
                "clairql",
            ),
            "person.properties.$browser",
        )
        self.assertEqual(
            self._expr("properties.$browser", ClairQLContext(team_id=self.team.pk), "clairql"),
            "properties.$browser",
        )
        self.assertEqual(
            self._expr(
        ...
        self.assertEqual(
            self._expr("properties.1", ClairQLContext(team_id=self.team.pk), "clairql"),
            "properties.1",
        )
        self.assertEqual(
            self._expr("properties.`1`", ClairQLContext(team_id=self.team.pk), "clairql"),
    ...
    def test_clairql_properties_json(self):
        context = ClairQLContext(team_id=self.team.pk)
        self.assertEqual(
            self._expr("properties.nomat.json.yet", context),
            "replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_0)s, %(clairql_val_1)s, %(clairql_val_2)s), ''), 'null'), '^\"|\"$', '')",
        )
        self.assertEqual(
            context.values,
            {"clairql_val_0": "nomat", "clairql_val_1": "json", "clairql_val_2": "yet"},
        )

    def test_clairql_properties_materialized_json_access(self):
        try:
            from ee.clickhouse.materialized_columns.analyze import materialize
        ...
        self.assertEqual(
            ...
            "replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(nullIf(nullIf(events.mat_withmat, ''), 'null'), %(clairql_val_0)s, %(clairql_val_1)s), ''), 'null'), '^\"|\"$', '')",
        )
        self.assertEqual(context.values, {"clairql_val_0": "json", "clairql_val_1": "yet"})
    ...
    def test_property_groups(self):
        ...
        self.assertEqual(
            ...
            "has(events.properties_group_custom, %(clairql_val_0)s) ? events.properties_group_custom[%(clairql_val_0)s] : null",
        )
        self.assertEqual(context.values["clairql_val_0"], "foo")
    ...
    def _test_property_group_comparison(
        ...
        def build_context(property_groups_mode: PropertyGroupsMode) -> ClairQLContext:
            return ClairQLContext(
                team_id=self.team.pk,
                modifiers=ClairQLQueryModifiers(
                    materializationMode=MaterializationMode.AUTO,
    ...
    def test_property_groups_optimized_basic_equality_comparisons(self) -> None:
        ...
        self._test_property_group_comparison(
            ...
            "equals(events.properties_group_custom[%(clairql_val_0)s], %(clairql_val_1)s)",
            {"clairql_val_0": "key", "clairql_val_1": "value"},
    ...
    def test_property_groups_optimized_boolean_equality_comparisons(self) -> None:
        ...
        self._test_property_group_comparison(
            ...
            "equals(events.properties_group_custom[%(clairql_val_0)s], 'true')",
            {"clairql_val_0": "is_boolean"},
            expected_skip_indexes_used={"properties_group_custom_keys_bf", "properties_group_custom_values_bf"},
        )

        self._test_property_group_comparison(
            "properties.is_boolean = false",
            "equals(events.properties_group_custom[%(clairql_val_0)s], 'false')",
            {"clairql_val_0": "is_boolean"},
            expected_skip_indexes_used={"properties_group_custom_keys_bf", "properties_group_custom_values_bf"},
        )
    ...
    def test_property_groups_optimized_empty_string_equality_comparisons(self) -> None:
        ...
        self._test_property_group_comparison(
            "properties.key = ''",
            "and(has(events.properties_group_custom, %(clairql_val_0)s), equals(events.properties_group_custom[%(clairql_val_0)s], %(clairql_val_1)s))",
            {"clairql_val_0": "key", "clairql_val_1": ""},
    ...
    def test_property_groups_optimized_null_comparisons(self) -> None:
        ...
        self._test_property_group_comparison(
            ...
            "has(events.properties_group_custom, %(clairql_val_0)s)",
            {"clairql_val_0": "key"},
            expected_skip_indexes_used={"properties_group_custom_keys_bf"},
        )
        self._test_property_group_comparison(
        ...
        self._test_property_group_comparison(
            ...
            "not(has(events.properties_group_custom, %(clairql_val_0)s))",
            {"clairql_val_0": "key"},
        )
        self._test_property_group_comparison(
            "properties.key = null",
    ...
    def test_property_groups_optimized_in_comparisons(self) -> None:
        ...
        self._test_property_group_comparison(
            ...
            "in(events.properties_group_custom[%(clairql_val_0)s], tuple(%(clairql_val_1)s, %(clairql_val_2)s))",
            {"clairql_val_0": "key", "clairql_val_1": "a", "clairql_val_2": "b"},
            expected_skip_indexes_used={"properties_group_custom_keys_bf", "properties_group_custom_values_bf"},
        )
        self._test_property_group_comparison(
            "properties.key IN 'a'",  # strange, but syntactically valid
            "in(events.properties_group_custom[%(clairql_val_0)s], %(clairql_val_1)s)",
            {"clairql_val_0": "key", "clairql_val_1": "a"},
            expected_skip_indexes_used={"properties_group_custom_keys_bf", "properties_group_custom_values_bf"},
        )
        self._test_property_group_comparison(
            "properties.key IN ('a', 'b', '')",
            (
                "or("
                "in(events.properties_group_custom[%(clairql_val_0)s], tuple(%(clairql_val_1)s, %(clairql_val_2)s)), "
                "and(has(events.properties_group_custom, %(clairql_val_0)s), equals(events.properties_group_custom[%(clairql_val_0)s], %(clairql_val_3)s))"
                ")"
            ),
            {"clairql_val_0": "key", "clairql_val_1": "a", "clairql_val_2": "b", "clairql_val_3": ""},
            expected_skip_indexes_used={"properties_group_custom_keys_bf"},
        )
        self._test_property_group_comparison(
    ...
    def test_property_groups_select_with_aliases(self):
        ...
        assert printed == (
            "SELECT has(events.properties_group_custom, %(clairql_val_0)s) ? events.properties_group_custom[%(clairql_val_0)s] : null AS ft "
            "FROM events "
            f"WHERE and(equals(events.team_id, {self.team.pk}), equals(events.properties_group_custom[%(clairql_val_1)s], %(clairql_val_2)s)) "
            "LIMIT 50000"
        )
    ...
    def test_functions(self):
        context = ClairQLContext(team_id=self.team.pk)  # inline values
        self.assertEqual(self._expr("abs(1)"), "abs(1)")
        self.assertEqual(self._expr("max2(1,2)"), "max2(1, 2)")
        self.assertEqual(self._expr("toInt('1')", context), "accurateCastOrNull(%(clairql_val_0)s, %(clairql_val_1)s)")
        self.assertEqual(self._expr("toFloat('1.3')", context), "accurateCastOrNull(%(clairql_val_2)s, %(clairql_val_3)s)")
        self.assertEqual(
            self._expr("toUUID('470f9b15-ff43-402a-af9f-2ed7c526a6cf')", context),
            "accurateCastOrNull(%(clairql_val_4)s, %(clairql_val_5)s)",
        )
        self.assertEqual(
            self._expr("toDecimal('3.14')", context), "accurateCastOrNull(%(clairql_val_6)s, %(clairql_val_7)s)"
        )
    ...
    def test_expr_parse_errors(self):
        ...
        self._assert_expr_error(
            "event as `as%d`",
            'The ClairQL identifier "as%d" is not permitted as it contains the "%" character',
        )
    ...
    def test_expr_syntax_errors(self):
        ...
        self._assert_expr_error("b.a(bla)", "You can only call simple functions in ClairQL, not expressions")
        self._assert_expr_error("a -> { print(2) }", "You can not use expressions inside placeholders")

    def test_logic(self):
        self.assertEqual(
            self._expr("event or timestamp"),
            "or(events.event, toTimeZone(events.timestamp, %(clairql_val_0)s))",
        )
        self.assertEqual(
            self._expr("properties.bla and properties.bla2"),
            "and(replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_0)s), ''), 'null'), '^\"|\"$', ''), replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_1)s), ''), 'null'), '^\"|\"$', ''))",
        )
        self.assertEqual(
            self._expr("event or timestamp or true or count()"),
            "or(events.event, toTimeZone(events.timestamp, %(clairql_val_0)s), 1, count())",
        )
        self.assertEqual(
            self._expr("event or not timestamp"),
            "or(events.event, not(toTimeZone(events.timestamp, %(clairql_val_0)s)))",
        )

    def test_comparisons(self):
        context = ClairQLContext(team_id=self.team.pk)
        self.assertEqual(self._expr("event == 'E'", context), "equals(events.event, %(clairql_val_0)s)")
        self.assertEqual(
            self._expr("event != 'E'", context),
            "notEquals(events.event, %(clairql_val_1)s)",
        )
        self.assertEqual(self._expr("event > 'E'", context), "greater(events.event, %(clairql_val_2)s)")
        self.assertEqual(
            self._expr("event >= 'E'", context),
            "greaterOrEquals(events.event, %(clairql_val_3)s)",
        )
        self.assertEqual(self._expr("event < 'E'", context), "less(events.event, %(clairql_val_4)s)")
        self.assertEqual(
            self._expr("event <= 'E'", context),
            "lessOrEquals(events.event, %(clairql_val_5)s)",
        )
        self.assertEqual(self._expr("event like 'E'", context), "like(events.event, %(clairql_val_6)s)")
        self.assertEqual(
            self._expr("event not like 'E'", context),
            "notLike(events.event, %(clairql_val_7)s)",
        )
        self.assertEqual(
            self._expr("event ilike 'E'", context),
            "ilike(events.event, %(clairql_val_8)s)",
        )
        self.assertEqual(
            self._expr("event not ilike 'E'", context),
            "notILike(events.event, %(clairql_val_9)s)",
        )
        self.assertEqual(self._expr("event in 'E'", context), "in(events.event, %(clairql_val_10)s)")
        self.assertEqual(
            self._expr("event not in 'E'", context),
            "notIn(events.event, %(clairql_val_11)s)",
        )
        self.assertEqual(self._expr("event ~ 'E'", context), "match(events.event, %(clairql_val_12)s)")
        self.assertEqual(self._expr("event =~ 'E'", context), "match(events.event, %(clairql_val_13)s)")
        self.assertEqual(
            self._expr("event !~ 'E'", context),
            "not(match(events.event, %(clairql_val_14)s))",
        )
        self.assertEqual(
            self._expr("event ~* 'E'", context),
            "match(events.event, concat('(?i)', %(clairql_val_15)s))",
        )
        self.assertEqual(
            self._expr("event =~* 'E'", context),
            "match(events.event, concat('(?i)', %(clairql_val_16)s))",
        )
        self.assertEqual(
            self._expr("event !~* 'E'", context),
            "not(match(events.event, concat('(?i)', %(clairql_val_17)s)))",
        )
    ...
    def test_values(self):
        context = ClairQLContext(team_id=self.team.pk)
        self.assertEqual(self._expr("event == 'E'", context), "equals(events.event, %(clairql_val_0)s)")
        self.assertEqual(context.values, {"clairql_val_0": "E"})
        self.assertEqual(
            self._expr("coalesce(4.2, 5, 'lol', 'hoo')", context),
            "coalesce(4.2, 5, %(clairql_val_1)s, %(clairql_val_2)s)",
        )
        self.assertEqual(
            context.values,
            {"clairql_val_0": "E", "clairql_val_1": "lol", "clairql_val_2": "hoo"},
        )
    ...
    def test_select_order_by(self):
        ...
        self.assertEqual(
            ...
            f"SELECT events.event AS event FROM events WHERE equals(events.team_id, {self.team.pk}) ORDER BY events.event DESC, toTimeZone(events.timestamp, %(clairql_val_0)s) ASC LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )
    ...
    def test_select_group_by(self):
        self.assertEqual(
            ...
            f"SELECT events.event AS event FROM events WHERE equals(events.team_id, {self.team.pk}) GROUP BY events.event, toTimeZone(events.timestamp, %(clairql_val_0)s) LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )

    def test_select_distinct(self):
        self.assertEqual(
            self._select("select distinct event from events group by event, timestamp"),
            f"SELECT DISTINCT events.event AS event FROM events WHERE equals(events.team_id, {self.team.pk}) GROUP BY events.event, toTimeZone(events.timestamp, %(clairql_val_0)s) LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )

    def test_select_subquery(self):
        self.assertEqual(
            self._select("SELECT event from (select distinct event from events group by event, timestamp)"),
            f"SELECT event AS event FROM (SELECT DISTINCT events.event AS event FROM events WHERE equals(events.team_id, {self.team.pk}) GROUP BY events.event, toTimeZone(events.timestamp, %(clairql_val_0)s)) LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )
        self.assertEqual(
            self._select("SELECT event from (select distinct event from events group by event, timestamp) e"),
            f"SELECT e.event AS event FROM (SELECT DISTINCT events.event AS event FROM events WHERE equals(events.team_id, {self.team.pk}) GROUP BY events.event, toTimeZone(events.timestamp, %(clairql_val_0)s)) AS e LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )
    ...
    def test_select_sample(self):
        ...
        with override_settings(PERSON_ON_EVENTS_V2_OVERRIDE=False):
            context = ClairQLContext(
                team_id=self.team.pk,
                enable_select_queries=True,
                modifiers=ClairQLQueryModifiers(personsArgMaxVersion=PersonsArgMaxVersion.V2),
            )
            query = self._select(
            ...
            self.assertEqual(
                ...
                "ifNull(less(argMax(toTimeZone(person.created_at, %(clairql_val_0)s), person.version), "
                "plus(now64(6, %(clairql_val_1)s), toIntervalDay(1))), 0)))), 0)) "
        ...
        with override_settings(PERSON_ON_EVENTS_OVERRIDE=True, PERSON_ON_EVENTS_V2_OVERRIDE=False):
            ...
            self.assertEqual(
                ...
                f"%(clairql_val_0)s), person.version), plus(now64(6, %(clairql_val_1)s), toIntervalDay(1))), 0)))), 0)) SETTINGS optimize_aggregation_in_order=1) "
    ...
    def test_count_if_distinct(self):
        self.assertEqual(
            ...
            f"SELECT countIf(DISTINCT events.event, like(events.event, %(clairql_val_0)s)) FROM events WHERE equals(events.team_id, {self.team.pk}) LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )
    ...
    def test_print_timezone(self):
        ...
        self.assertEqual(
            ...
            f"SELECT now64(6, %(clairql_val_0)s), toDateTime(toTimeZone(events.timestamp, %(clairql_val_1)s), %(clairql_val_2)s), toDate(events.test_date), parseDateTime64BestEffortOrNull(%(clairql_val_3)s, 6, %(clairql_val_4)s) FROM events WHERE equals(events.team_id, {self.team.pk}) LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )
        self.assertEqual(
            context.values,
            {
    ...
    def test_print_timezone_custom(self):
        ...
        self.assertEqual(
            ...
            f"SELECT now64(6, %(clairql_val_0)s), toDateTime(toTimeZone(events.timestamp, %(clairql_val_1)s), %(clairql_val_2)s), parseDateTime64BestEffortOrNull(%(clairql_val_3)s, 6, %(clairql_val_4)s) FROM events WHERE equals(events.team_id, {self.team.pk}) LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )
        self.assertEqual(
            context.values,
            {
                "clairql_val_0": "Europe/Brussels",
                "clairql_val_1": "Europe/Brussels",
                "clairql_val_2": "Europe/Brussels",
                "clairql_val_3": "2020-02-02",
                "clairql_val_4": "Europe/Brussels",
            },
        )

    def test_print_timezone_gibberish(self):
        self.team.timezone = "Europe/ClairViewLandia"
        self.team.save()
        ...
        self.assertEqual(str(error_context.exception), "Unknown timezone: 'Europe/ClairViewLandia'")
    ...
    def test_nullish_concat(self):
        self.assertEqual(
            ...
            f"concat('', %(clairql_val_0)s, toString(3), toString(4), '')",
        )

    def test_concat_pipes(self):
        self.assertEqual(
            self._expr("'a' || 'b' || 3 || timestamp"),
            f"concat(%(clairql_val_0)s, %(clairql_val_1)s, toString(3), ifNull(toString(toTimeZone(events.timestamp, %(clairql_val_2)s)), ''))",
        )
    ...
    def test_to_start_of_week_gets_mode(self):
        ...
        default_week_context = ClairQLContext(team_id=self.team.pk, database=Database(None, None))
        sunday_week_context = ClairQLContext(team_id=self.team.pk, database=Database(None, 0))  # 0 == WeekStartDay.SUNDAY
        monday_week_context = ClairQLContext(team_id=self.team.pk, database=Database(None, 1))  # 1 == WeekStartDay.MONDAY

        self.assertEqual(
            self._expr("toStartOfWeek(timestamp)", default_week_context),  # Sunday is the default
            f"toStartOfWeek(toTimeZone(events.timestamp, %(clairql_val_0)s), 0)",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            f"toStartOfWeek(toTimeZone(events.timestamp, %(clairql_val_0)s), 3)",
        )

    def test_functions_expecting_datetime_arg(self):
        self.assertEqual(
            self._expr("tumble(toDateTime('2023-06-12'), toIntervalDay('1'))"),
            f"tumble(assumeNotNull(toDateTime(parseDateTime64BestEffortOrNull(%(clairql_val_0)s, 6, %(clairql_val_1)s))), toIntervalDay(%(clairql_val_2)s))",
        )
        self.assertEqual(
            self._expr("tumble(now(), toIntervalDay('1'))"),
            f"tumble(toDateTime(now64(6, %(clairql_val_0)s), 'UTC'), toIntervalDay(%(clairql_val_1)s))",
        )
        self.assertEqual(
            self._expr("tumble(parseDateTime('2021-01-04+23:00:00', '%Y-%m-%d+%H:%i:%s'), toIntervalDay('1'))"),
            f"tumble(assumeNotNull(toDateTime(parseDateTimeOrNull(%(clairql_val_0)s, %(clairql_val_1)s, %(clairql_val_2)s))), toIntervalDay(%(clairql_val_3)s))",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            f"SELECT tumble(toDateTime(toTimeZone(events.timestamp, %(clairql_val_0)s), 'UTC'), toIntervalDay(%(clairql_val_1)s)) FROM events WHERE equals(events.team_id, {self.team.pk}) LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )
    ...
    def test_field_nullable_equals(self):
        ...
        assert generated_sql_statements1 == (
            ...
            f"ifNull(equals(session_replay_events.start_time, toStartOfMonth(now64(6, %(clairql_val_1)s))), "
            f"isNull(session_replay_events.start_time) and isNull(toStartOfMonth(now64(6, %(clairql_val_1)s)))), "
            # now() = now() (also two nullable fields)
            f"ifNull(equals(now64(6, %(clairql_val_2)s), now64(6, %(clairql_val_3)s)), isNull(now64(6, %(clairql_val_2)s)) and isNull(now64(6, %(clairql_val_3)s))), "
            # 1 = now()
            f"ifNull(equals(1, now64(6, %(clairql_val_4)s)), 0), "
            # now() = 1
            f"ifNull(equals(now64(6, %(clairql_val_5)s), 1), 0), "
            # 1 = 1
            f"1, "
            # click_count = 1
            ...
            f"isNull(session_replay_events.click_count) "
            # ...
            f"FROM (SELECT min(toTimeZone(session_replay_events.min_first_timestamp, %(clairql_val_0)s)) AS start_time, sum(session_replay_events.click_count) AS click_count, sum(session_replay_events.keypress_count) AS keypress_count FROM session_replay_events WHERE equals(session_replay_events.team_id, {self.team.pk})) AS session_replay_events LIMIT {MAX_SELECT_RETURNED_ROWS}"
        )
    ...
    def test_field_nullable_not_equals(self):
        ...
        assert generated_sql1 == (
            ...
            # start_time = toStartOfMonth(now())
            # (the return of toStartOfMonth() is treated as "potentially nullable" since we yet have full typing support)
            f"ifNull(notEquals(session_replay_events.start_time, toStartOfMonth(now64(6, %(clairql_val_1)s))), "
            f"isNotNull(session_replay_events.start_time) or isNotNull(toStartOfMonth(now64(6, %(clairql_val_1)s)))), "
            # now() = now() (also two nullable fields)
            f"ifNull(notEquals(now64(6, %(clairql_val_2)s), now64(6, %(clairql_val_3)s)), isNotNull(now64(6, %(clairql_val_2)s)) or isNotNull(now64(6, %(clairql_val_3)s))), "
            # 1 = now()
            f"ifNull(notEquals(1, now64(6, %(clairql_val_4)s)), 1), "
            # now() = 1
            f"ifNull(notEquals(now64(6, %(clairql_val_5)s), 1), 1), "
            # 1 = 1
            f"0, "
            # click_count = 1
    ...
    def test_field_nullable_boolean(self):
        ...
        assert generated_sql_statements1 == (
            f"SELECT "
            "ifNull(equals(transform(nullIf(nullIf(events.mat_is_boolean, ''), 'null'), %(clairql_val_0)s, %(clairql_val_1)s, NULL), 1), 0), "
            "ifNull(equals(transform(nullIf(nullIf(events.mat_is_boolean, ''), 'null'), %(clairql_val_2)s, %(clairql_val_3)s, NULL), 0), 0), "
            "isNull(transform(nullIf(nullIf(events.mat_is_boolean, ''), 'null'), %(clairql_val_4)s, %(clairql_val_5)s, NULL)) "
            f"FROM events WHERE equals(events.team_id, {self.team.pk}) LIMIT {MAX_SELECT_RETURNED_ROWS}"
        )
        assert context.values == {
            "clairql_val_0": ["true", "false"],
            "clairql_val_1": [True, False],
            "clairql_val_2": ["true", "false"],
            "clairql_val_3": [True, False],
            "clairql_val_4": ["true", "false"],
            "clairql_val_5": [True, False],
        }

    def test_field_nullable_like(self):
        context = ClairQLContext(team_id=self.team.pk, enable_select_queries=True, database=Database())
        ...
        assert generated_sql_statements1 == (
            f"SELECT "
            # event like 'a',
            "ifNull(like(events.nullable_field, %(clairql_val_0)s), 0), "
            # event like null,
            "isNull(events.nullable_field), "
            # null like event,
            "isNull(events.nullable_field), "
            # null like 'a',
            "ifNull(like(NULL, %(clairql_val_1)s), 0), "
            # 'a' like event,
            "ifNull(like(%(clairql_val_2)s, events.nullable_field), 0), "
            # 'a' like null
            "isNull(%(clairql_val_3)s) "
            f"FROM events WHERE equals(events.team_id, {self.team.pk}) LIMIT {MAX_SELECT_RETURNED_ROWS}"
        )
    ...
    def test_field_nullable_not_like(self):
        ...
        assert generated_sql_statements1 == (
            f"SELECT "
            # event like 'a',
            "ifNull(notLike(events.nullable_field, %(clairql_val_0)s), 1), "
            # event like null,
            "isNotNull(events.nullable_field), "
            # null like event,
            "isNotNull(events.nullable_field), "
            # null like 'a',
            "ifNull(notLike(NULL, %(clairql_val_1)s), 1), "
            # 'a' like event,
            "ifNull(notLike(%(clairql_val_2)s, events.nullable_field), 1), "
            # 'a' like null
            "isNotNull(%(clairql_val_3)s) "
            f"FROM events WHERE equals(events.team_id, {self.team.pk}) LIMIT {MAX_SELECT_RETURNED_ROWS}"
        )
    ...
    def test_print_global_settings(self):
        ...
        printed = print_ast(
            ...
            ClairQLContext(team_id=self.team.pk, enable_select_queries=True),
            dialect="clickhouse",
            settings=ClairQLGlobalSettings(max_execution_time=10),
        )
        self.assertEqual(
            printed,
    ...
    def test_print_query_level_settings(self):
        query = parse_select("SELECT 1 FROM events")
        query.settings = ClairQLQuerySettings(optimize_aggregation_in_order=True)
        printed = print_ast(
            query,
            ClairQLContext(team_id=self.team.pk, enable_select_queries=True),
            "clickhouse",
    ...
    def test_print_hidden_aliases_timestamp(self):
        ...
        self.assertEqual(
            printed,
            f"SELECT timestamp AS timestamp FROM (SELECT toTimeZone(events.timestamp, %(clairql_val_0)s), "
            f"toTimeZone(events.timestamp, %(clairql_val_1)s) AS timestamp FROM events WHERE equals(events.team_id, {self.team.pk})) "
    ...
    def test_print_hidden_aliases_column_override(self):
        ...
        self.assertEqual(
            printed,
            f"SELECT event AS event FROM (SELECT toTimeZone(events.timestamp, %(clairql_val_0)s) AS event, "
    ...
    def test_lookup_domain_type(self):
        query = parse_select("select clairql_lookupDomainType('www.google.com') from events")
        printed = print_ast(
            query,
            ClairQLContext(team_id=self.team.pk, enable_select_queries=True),
            dialect="clickhouse",
        ...
        self.assertEqual(
            (
                ...
                "(coalesce(%(clairql_val_0)s, ''), 'source')), "
                "dictGetOrNull('channel_definition_dict', 'domain_type', "
                "(cutToFirstSignificantSubdomain(coalesce(%(clairql_val_0)s, '')), 'source'))) "
    ...
    def test_lookup_paid_source_type(self):
        query = parse_select("select clairql_lookupPaidSourceType('google') from events")
        printed = print_ast(
            query,
            ClairQLContext(team_id=self.team.pk, enable_select_queries=True),
            dialect="clickhouse",
    ...
    def test_lookup_paid_medium_type(self):
        query = parse_select("select clairql_lookupPaidMediumType('social') from events")
        printed = print_ast(
            query,
            ClairQLContext(team_id=self.team.pk, enable_select_queries=True),
            dialect="clickhouse",
        ...
        self.assertEqual(
            (
                ...
                "(coalesce(%(clairql_val_0)s, ''), 'medium')) "
    ...
    def test_lookup_organic_source_type(self):
        query = parse_select("select clairql_lookupOrganicSourceType('google') from events")
        printed = print_ast(
            query,
            ClairQLContext(team_id=self.team.pk, enable_select_queries=True),
            dialect="clickhouse",
    ...
    def test_lookup_organic_medium_type(self):
        query = parse_select("select clairql_lookupOrganicMediumType('social') from events")
        printed = print_ast(
            query,
            ClairQLContext(team_id=self.team.pk, enable_select_queries=True),
            dialect="clickhouse",
    ...
    def test_override_timezone(self):
        ...
        self.assertEqual(
            ...
            f"SELECT toDateTime(toTimeZone(events.timestamp, %(clairql_val_0)s), %(clairql_val_1)s), toDateTime(toTimeZone(events.timestamp, %(clairql_val_2)s), %(clairql_val_3)s), now64(6, %(clairql_val_4)s), now64(6, %(clairql_val_5)s) FROM events WHERE equals(events.team_id, {self.team.pk}) LIMIT {MAX_SELECT_RETURNED_ROWS}",
        )
        self.assertEqual(
            context.values,
            {
                ...
                "clairql_val_1": "UTC",
                "clairql_val_2": "UTC",
                "clairql_val_3": "US/Pacific",
                "clairql_val_4": "UTC",
                "clairql_val_5": "US/Pacific",
            },
        )
    ...
    def test_trim_leading_trailing_both(self):
        ...
        assert printed == (
            f"SELECT trim(LEADING %(clairql_val_1)s FROM %(clairql_val_0)s), trim(TRAILING %(clairql_val_3)s FROM %(clairql_val_2)s), trim(BOTH %(clairql_val_5)s FROM %(clairql_val_4)s) LIMIT {MAX_SELECT_RETURNED_ROWS} SETTINGS "
            "readonly=2, max_execution_time=10, allow_experimental_object_type=1, format_csv_allow_double_quotes=0, max_ast_elements=4000000, max_expanded_ast_elements=4000000, max_bytes_before_external_group_by=0"
        )
