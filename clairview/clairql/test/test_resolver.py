...
from clairview.clairql.test.utils import pretty_dataclasses
from clairview.clairql.visitor import clone_expr
from clairview.clairql.parser import parse_select
from clairview.clairql.printer import print_ast, print_prepared_ast
from clairview.clairql.resolver import ResolutionError, resolve_types
...
class TestResolver(BaseTest):
    ...
    def _print_clairql(self, select: str):
    ...
    def setUp(self):
        ...
        self.context = ClairQLContext(database=self.database, team_id=self.team.pk, enable_select_queries=True)
    ...
    def test_unresolved_field_type(self):
        ...
        with self.assertRaises(QueryError):
            resolve_types(self._select(query), self.context, dialect="clickhouse")
        # does not raise with ClairQL
        select = self._select(query)
        select = cast(ast.SelectQuery, resolve_types(select, self.context, dialect="clairql"))
    ...
    def test_ctes_loop(self):
        with self.assertRaises(QueryError) as e:
            self._print_clairql("with cte as (select * from cte) select * from cte")
        self.assertIn("Too many CTE expansions (50+). Probably a CTE loop.", str(e.exception))

    def test_ctes_basic_column(self):
        expr = self._print_clairql("with 1 as cte select cte from events")
        expected = self._print_clairql("select 1 from events")
        self.assertEqual(expr, expected)

    def test_ctes_recursive_column(self):
        self.assertEqual(
            self._print_clairql("with 1 as cte, cte as soap select soap from events"),
            self._print_clairql("select 1 from events"),
        )

    def test_ctes_field_access(self):
        with self.assertRaises(QueryError) as e:
            self._print_clairql("with properties as cte select cte.$browser from events")
        self.assertIn("Cannot access fields on CTE cte yet", str(e.exception))

    def test_ctes_subqueries(self):
        self.assertEqual(
            self._print_clairql("with my_table as (select * from events) select * from my_table"),
            self._print_clairql("select * from (select * from events) my_table"),
        )

        self.assertEqual(
            self._print_clairql("with my_table as (select * from events) select my_table.timestamp from my_table"),
            self._print_clairql("select my_table.timestamp from (select * from events) my_table"),
        )

        self.assertEqual(
            self._print_clairql("with my_table as (select * from events) select timestamp from my_table"),
            self._print_clairql("select timestamp from (select * from events) my_table"),
        )
    ...
    def test_ctes_with_aliases(self):
        self.assertEqual(
            ...
            self._print_clairql("SELECT a FROM (SELECT 1 AS a) AS new_alias WHERE new_alias.a=1"),
        )
    ...
    def test_visit_clairqlx_tag_alias(self):
        node = self._select("select event from <ClairQLQuery query='select event from events' /> a")
        node = cast(ast.SelectQuery, resolve_types(node, self.context, dialect="clickhouse"))
    ...
    def test_visit_clairqlx_tag_source(self):
        ...
        """
        node = cast(ast.SelectQuery, resolve_types(self._select(query), self.context, dialect="clairql"))
        clairql = print_prepared_ast(node, ClairQLContext(team_id=self.team.pk, enable_select_queries=True), "clairql")
        expected = (
            "SELECT id, email FROM "
        ...
        assert clairql == expected

    def test_visit_clairqlx_recording_button(self):
    ...
    def test_visit_clairqlx_sparkline(self):
    ...
    def test_visit_clairqlx_object(self):
    ...
    def test_globals(self):
        ...
        query = print_prepared_ast(node, context, "clairql")
