...
from clairview.schema import ClairQLMetadata, ClairQLMetadataResponse, ClairQLQuery, HogLanguage
...
class TestMetadata(ClickhouseTestMixin, APIBaseTest):
    ...
    def _expr(self, query: str, table: str = "events", debug=True) -> ClairQLMetadataResponse:
        return get_clairql_metadata(
            query=ClairQLMetadata(
                kind="ClairQLMetadata",
                language=HogLanguage.HOG_QL_EXPR,
                query=query,
                sourceQuery=ClairQLQuery(query=f"select * from {table}"),
                response=None,
                debug=debug,
            ),
            team=self.team,
    ...
    def _select(self, query: str) -> ClairQLMetadataResponse:
        return get_clairql_metadata(
            query=ClairQLMetadata(kind="ClairQLMetadata", language=HogLanguage.HOG_QL, query=query, response=None),
            team=self.team,
        )

    def _program(self, query: str, globals: Optional[dict] = None) -> ClairQLMetadataResponse:
        return get_clairql_metadata(
            query=ClairQLMetadata(
                kind="ClairQLMetadata", language=HogLanguage.HOG, query=query, globals=globals, response=None
            ),
            team=self.team,
        )

    def _template(self, query: str) -> ClairQLMetadataResponse:
        return get_clairql_metadata(
            query=ClairQLMetadata(kind="ClairQLMetadata", language=HogLanguage.HOG_TEMPLATE, query=query, response=None),
            team=self.team,
        )
