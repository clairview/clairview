...
class TestModifiers(BaseTest):
    ...
    def test_create_default_modifiers_for_team_init(self):
        ...
        modifiers = create_default_modifiers_for_team(
            self.team,
            ClairQLQueryModifiers(personsOnEventsMode=PersonsOnEventsMode.PERSON_ID_NO_OVERRIDE_PROPERTIES_ON_EVENTS),
        )
        assert modifiers.personsOnEventsMode == PersonsOnEventsMode.PERSON_ID_NO_OVERRIDE_PROPERTIES_ON_EVENTS
        modifiers = create_default_modifiers_for_team(
            self.team,
            ClairQLQueryModifiers(personsOnEventsMode=PersonsOnEventsMode.PERSON_ID_OVERRIDE_PROPERTIES_ON_EVENTS),
        )
    ...
    def test_modifiers_persons_on_events_mode_mapping(self):
        ...
        test_cases: list[TestCase] = [
            ...
            TestCase(
                ...
                [
                    ...
                    "toTimeZone(events.person_created_at, %(clairql_val_0)s) AS created_at",
                ],
            ),
            TestCase(
        ...
        for test_case in test_cases:
            clickhouse_query = execute_clairql_query(
                query,
                team=self.team,
                modifiers=ClairQLQueryModifiers(personsOnEventsMode=test_case.mode),
                pretty=False,
            ).clickhouse
    ...
    def test_modifiers_persons_argmax_version_v2(self):
        ...
        response = execute_clairql_query(
            query,
            team=self.team,
            modifiers=ClairQLQueryModifiers(personsArgMaxVersion=PersonsArgMaxVersion.V1),
        )
    ...
    def test_modifiers_persons_argmax_version_auto(self):
        ...
        response = execute_clairql_query(
            ...
            team=self.team,
            modifiers=ClairQLQueryModifiers(personsArgMaxVersion=PersonsArgMaxVersion.AUTO),
        )
        assert "in(tuple(person.id, person.version)" in response.clickhouse
    ...
    def test_modifiers_in_cohort_join(self):
        ...
        response = execute_clairql_query(
            f"select * from persons where id in cohort {cohort.pk}",
            team=self.team,
            modifiers=ClairQLQueryModifiers(inCohortVia="leftjoin"),
        )
    ...
    def test_modifiers_materialization_mode(self):
        ...
        response = execute_clairql_query(
            ...
            team=self.team,
            modifiers=ClairQLQueryModifiers(materializationMode=MaterializationMode.AUTO),
            pretty=False,
        )
        assert (
        ...
        response = execute_clairql_query(
            "SELECT properties.$browser FROM events",
            team=self.team,
            modifiers=ClairQLQueryModifiers(materializationMode=MaterializationMode.LEGACY_NULL_AS_NULL),
            pretty=False,
        )
        assert (
            "SELECT nullIf(nullIf(events.`mat_$browser`, ''), 'null') AS `$browser` FROM events" in response.clickhouse
        ...
        response = execute_clairql_query(
            "SELECT properties.$browser FROM events",
            team=self.team,
            modifiers=ClairQLQueryModifiers(materializationMode=MaterializationMode.LEGACY_NULL_AS_STRING),
            pretty=False,
        )
        ...
        response = execute_clairql_query(
            "SELECT properties.$browser FROM events",
            team=self.team,
            modifiers=ClairQLQueryModifiers(materializationMode=MaterializationMode.DISABLED),
            pretty=False,
        )
        assert (
            "SELECT replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_0)s), ''), 'null'), '^\"|\"$', '') AS `$browser` FROM events"
    ...
    def test_optimize_joined_filters(self):
        ...
        response = execute_clairql_query(
            ...
            team=self.team,
            modifiers=ClairQLQueryModifiers(optimizeJoinedFilters=False),
        )
        ...
        response = execute_clairql_query(
            f"select event from events where person.properties.$browser ilike '%Chrome%'",
            team=self.team,
            modifiers=ClairQLQueryModifiers(optimizeJoinedFilters=True),
        )
