...
from clairview.clairql.clairqlx import convert_tag_to_hx, convert_dict_to_hx, convert_to_hx
from clairview.test.base import BaseTest


class TestClairQLX(BaseTest):
    def test_convert_tag_to_hx(self):
        tag = ast.ClairQLXTag(kind="Sparkline", attributes=[])
        self.assertEqual(
            convert_tag_to_hx(tag), ast.Tuple(exprs=[ast.Constant(value="__hx_tag"), ast.Constant(value="Sparkline")])
        )

        tag = ast.ClairQLXTag(
            kind="Sparkline", attributes=[ast.ClairQLXAttribute(name="color", value=ast.Constant(value="red"))]
        )
        self.assertEqual(
    ...
    def test_convert_to_hx(self):
        ...
        self.assertEqual(
            convert_to_hx(ast.ClairQLXTag(kind="Sparkline", attributes=[])),
