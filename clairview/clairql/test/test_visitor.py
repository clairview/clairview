...
from clairview.clairql.ast import UUIDType, ClairQLXTag, ClairQLXAttribute
from clairview.clairql.errors import InternalClairQLError
from clairview.clairql.parser import parse_expr
from clairview.clairql.visitor import CloningVisitor, Visitor, TraversingVisitor
...
class TestVisitor(BaseTest):
    ...
    def test_unknown_error_visitor(self):
        ...
        with self.assertRaises(InternalClairQLError) as e:
    ...
    def test_clairql_exception_start_end(self):
        class EternalVisitor(TraversingVisitor):
            def visit_constant(self, node: ast.Constant):
                if node.value == 616:
                    raise InternalClairQLError("You tried accessing a forbidden number, perish!")
    ...
    def test_clairql_visitor_naming_exceptions(self):
        class NamingCheck(Visitor):
            def visit_uuid_type(self, node: ast.Constant):
                return "visit_uuid_type"

            def visit_clairqlx_tag(self, node: ast.Constant):
                return "visit_clairqlx_tag"

            def visit_clairqlx_attribute(self, node: ast.Constant):
                return "visit_clairqlx_attribute"

        assert NamingCheck().visit(UUIDType()) == "visit_uuid_type"
        assert NamingCheck().visit(ClairQLXAttribute(name="a", value="a")) == "visit_clairqlx_attribute"
        assert NamingCheck().visit(ClairQLXTag(kind="", attributes=[])) == "visit_clairqlx_tag"
