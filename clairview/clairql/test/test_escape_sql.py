...
class TestPrintString(BaseTest):
    def test_sanitize_clairql_identifier(self):
        self.assertEqual(escape_clairql_identifier("a"), "a")
        self.assertEqual(escape_clairql_identifier("$browser"), "$browser")
        self.assertEqual(escape_clairql_identifier("0asd"), "`0asd`")
        self.assertEqual(escape_clairql_identifier("123"), "`123`")
        self.assertEqual(escape_clairql_identifier("event"), "event")
        self.assertEqual(escape_clairql_identifier("a b c"), "`a b c`")
        self.assertEqual(escape_clairql_identifier("a.b.c"), "`a.b.c`")
        self.assertEqual(escape_clairql_identifier("a-b-c"), "`a-b-c`")
        self.assertEqual(escape_clairql_identifier("a#$#"), "`a#$#`")
        self.assertEqual(escape_clairql_identifier("back`tick"), "`back\\`tick`")
        self.assertEqual(escape_clairql_identifier("single'quote"), "`single'quote`")
        self.assertEqual(escape_clairql_identifier('double"quote'), '`double"quote`')
        self.assertEqual(
            escape_clairql_identifier("other escapes: \b \f \n \t \0 \a \v \\"),
            "`other escapes: \\b \\f \\n \\t \\0 \\a \\v \\\\`",
        )
    ...
    def test_sanitize_clairql_string(self):
        self.assertEqual(escape_clairql_string("a"), "'a'")
        self.assertEqual(escape_clairql_string("$browser"), "'$browser'")
        self.assertEqual(escape_clairql_string("a b c"), "'a b c'")
        self.assertEqual(escape_clairql_string("a#$%#"), "'a#$%#'")
        self.assertEqual(escape_clairql_string("back`tick"), "'back`tick'")
        self.assertEqual(escape_clairql_string("single'quote"), "'single\\'quote'")
        self.assertEqual(escape_clairql_string('double"quote'), "'double\"quote'")
        self.assertEqual(
            escape_clairql_string("other escapes: \b \f \n \t \0 \a \v \\"),
            "'other escapes: \\b \\f \\n \\t \\0 \\a \\v \\\\'",
        )
        self.assertEqual(escape_clairql_string(["list", "things", []]), "['list', 'things', []]")
        self.assertEqual(escape_clairql_string(("tuple", "things", ())), "('tuple', 'things', ())")
        uuid = UUIDT()
        self.assertEqual(escape_clairql_string(uuid), f"toUUID('{str(uuid)}')")
        date = datetime.fromisoformat("2020-02-02 02:02:02")
        self.assertEqual(escape_clairql_string(date), "toDateTime('2020-02-02 02:02:02.000000')")
        self.assertEqual(
            escape_clairql_string(date, timezone="Europe/Brussels"),
            "toDateTime('2020-02-02 03:02:02.000000')",
        )
        self.assertEqual(escape_clairql_string(date.date()), "toDate('2020-02-02')")
        self.assertEqual(escape_clairql_string(1), "1")
        self.assertEqual(escape_clairql_string(-1), "-1")
        self.assertEqual(escape_clairql_string(float("inf")), "Inf")
        self.assertEqual(escape_clairql_string(float("nan")), "NaN")
        self.assertEqual(escape_clairql_string(float("-inf")), "-Inf")
        self.assertEqual(escape_clairql_string(float("123")), "123.0")
        self.assertEqual(escape_clairql_string(float("123.123")), "123.123")
        self.assertEqual(escape_clairql_string(float("-123.123")), "-123.123")
        self.assertEqual(escape_clairql_string(float("0.000000000000000001")), "1e-18")
        self.assertEqual(
            escape_clairql_string(float("234732482374928374923")),
            "2.3473248237492837e+20",
        )

    def test_escape_clairql_identifier_errors(self):
        with self.assertRaises(QueryError) as context:
            escape_clairql_identifier("with % percent")
        self.assertTrue(
            'The ClairQL identifier "with % percent" is not permitted as it contains the "%" character'
