...
from clairview.clairql.errors import ExposedClairQLError, SyntaxError
from clairview.clairql.parser import parse_expr, parse_order_expr, parse_select, parse_string_template
from clairview.clairql.visitor import clear_locations
from clairview.test.base import BaseTest, MemoryLeakTestMixin
...
def parser_test_factory(backend: Literal["python", "cpp"]):
    ...
    class TestParser(*base_classes):
        ...
        ) -> ast.SelectQuery | ast.SelectUnionQuery | ast.ClairQLXTag:
            return cast(
                ast.SelectQuery | ast.SelectUnionQuery | ast.ClairQLXTag,
        ...
        def test_select_array_join_errors(self):
            with self.assertRaises(ExposedClairQLError) as e:
        ...
        def test_select_placeholders(self):
            self.assertEqual(
                self._select("select 1 where 1 == {clairql_val_1}"),
                ast.SelectQuery(
                    select=[ast.Constant(value=1)],
                    where=ast.CompareOperation(
                        op=ast.CompareOperationOp.Eq,
                        left=ast.Constant(value=1),
                        right=ast.Placeholder(expr=ast.Field(chain=["clairql_val_1"])),
                    ),
                ),
            )
            self.assertEqual(
                self._select(
                    "select 1 where 1 == {clairql_val_1}",
                    {"clairql_val_1": ast.Constant(value="bar")},
                ),
                ast.SelectQuery(
                    select=[ast.Constant(value=1)],
                    where=ast.CompareOperation(
        ...
        def test_placeholder_expressions(self):
            actual = self._select("select 1 where 1 == {1 ? clairql_val_1 : clairql_val_2}")
                    ...
                    where=ast.CompareOperation(
                        ...
                        right=ast.Placeholder(
                            expr=ast.Call(
                                ...
                                args=[
                                    ast.Constant(value=1),
                                    ast.Field(chain=["clairql_val_1"]),
                                    ast.Field(chain=["clairql_val_2"]),
                                ],
                            )
                        ),
                    ),
        ...
        def test_visit_clairqlx_tag(self):
            node = self._select("select event from <ClairQLQuery query='select event from events' />")
            table_node = cast(ast.SelectQuery, node).select_from.table
            assert table_node == ast.ClairQLXTag(
                kind="ClairQLQuery",
                attributes=[ast.ClairQLXAttribute(name="query", value=ast.Constant(value="select event from events"))],
            )

            node2 = self._select("select event from (<ClairQLQuery query='select event from events' />)")
            assert node2 == node

        def test_visit_clairqlx_tag_nested(self):
            # Basic case
            node = self._select(
                "select event from <OuterQuery><ClairQLQuery query='select event from events' /></OuterQuery>"
            )
            table_node = cast(ast.SelectQuery, node).select_from.table
            assert table_node == ast.ClairQLXTag(
                kind="OuterQuery",
                attributes=[
                    ast.ClairQLXAttribute(
                        name="source",
                        value=ast.ClairQLXTag(
                            kind="ClairQLQuery",
                            attributes=[
                                ast.ClairQLXAttribute(name="query", value=ast.Constant(value="select event from events"))
                            ],
                        ),
                    )
                ],
            ...
            table_node = cast(ast.SelectQuery, node).select_from.table
            assert table_node == ast.ClairQLXTag(kind="OuterQuery", attributes=[])

            # With attribute
            node = self._select(
                "select event from <OuterQuery q='b'><ClairQLQuery query='select event from events' /></OuterQuery>"
            )
            table_node = cast(ast.SelectQuery, node).select_from.table
            assert table_node == ast.ClairQLXTag(
                kind="OuterQuery",
                attributes=[
                    ast.ClairQLXAttribute(name="q", value=ast.Constant(value="b")),
                    ast.ClairQLXAttribute(
                        name="source",
                        value=ast.ClairQLXTag(
                            kind="ClairQLQuery",
            ...
            with self.assertRaises(ExposedClairQLError) as e:
                self._select(
                    "select event from <OuterQuery q='b'><ClairQLQuery query='select event from events' /></ClairQLQuery>"
                )
            assert str(e.exception) == "Opening and closing ClairQLX tags must match. Got OuterQuery and ClairQLQuery"

            # With mismatched closing tag
            with self.assertRaises(ExposedClairQLError) as e:
                self._select(
                    "select event from <OuterQuery source='b'><ClairQLQuery query='select event from events' /></OuterQuery>"
                )
            assert str(e.exception) == "Nested ClairQLX tags cannot have a source attribute"

        def test_visit_clairqlx_tag_alias(self):
            node = self._select("select event from <ClairQLQuery query='select event from events' /> as a")
            table_node = cast(ast.SelectQuery, node).select_from.table
            ...
            node2 = self._select("select event from <ClairQLQuery query='select event from events' /> a")
            assert node2 == node

        def test_visit_clairqlx_tag_source(self):
            query = """
                select id, email from (
                    <ActorsQuery
                        select={['id', 'properties.email as email']}
                        source={
                            <ClairQLQuery query='select distinct person_id from events' />
                        }
                    />
                )
            """
        ...
        def test_visit_clairqlx_tag_column_source(self):
            query = """
                select <a href='https://google.com'>{event}</a> from events
            """
            node = self._select(query)
            assert isinstance(node, ast.SelectQuery) and cast(ast.ClairQLXTag, node.select[0]) == ast.ClairQLXTag(
                kind="a",
                attributes=[
                    ast.ClairQLXAttribute(name="href", value=Constant(value="https://google.com")),
                    ast.ClairQLXAttribute(name="source", value=ast.Field(chain=["event"])),
                ],
            )
