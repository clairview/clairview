...
from clairview.clairql.property import property_to_expr
from clairview.clairql.query import execute_clairql_query
from clairview.clairql.test.utils import pretty_print_in_tests, pretty_print_response_in_tests
from clairview.models import Cohort
from clairview.models.cohort.util import recalculate_cohortpeople
from clairview.models.utils import UUIDT, uuid7
from clairview.session_recordings.queries.test.session_replay_sql import (
    produce_replay_summary,
)
from clairview.schema import ClairQLFilters, EventPropertyFilter, DateRange, QueryTiming, SessionPropertyFilter
...
class TestQuery(ClickhouseTestMixin, APIBaseTest):
    ...
    def _create_random_events(self) -> str:
        ...
        _create_person(
            properties={"sneaky_mail": "tim@clairview.com", "random_uuid": random_uuid},
            team=self.team,
            distinct_ids=["bla"],
            is_identified=True,
        )
    ...
    def test_extended_query_time(self):
        self.assertEqual(CLAIRQL_INCREASED_MAX_EXECUTION_TIME, 600)
    ...
    def test_query_distinct(self):
        with freeze_time("2020-01-10"):
            ...
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            self.assertEqual(response.results, [("tim@clairview.com",)])
    ...
    def test_query_joins_simple(self):
        with freeze_time("2020-01-10"):
            ...
            self.assertEqual(response.results[0][4], "tim@clairview.com")
    ...
    def test_query_joins_pdi_person_properties(self):
        with freeze_time("2020-01-10"):
            ...
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            self.assertEqual(response.results[0][0], "bla")
            self.assertEqual(response.results[0][1], "tim@clairview.com")
    ...
    def test_query_joins_events_pdi_person_properties(self):
        with freeze_time("2020-01-10"):
            ...
            self.assertEqual(response.results[0][0], "random event")
            self.assertEqual(response.results[0][2], "bla")
            self.assertEqual(response.results[0][3], "tim@clairview.com")
    ...
    def test_query_joins_events_person_properties(self):
        with freeze_time("2020-01-10"):
            ...
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            self.assertEqual(response.results[0][0], "random event")
            self.assertEqual(response.results[0][2], "tim@clairview.com")
    ...
    def test_query_joins_events_person_properties_in_aggregration(self):
        with freeze_time("2020-01-10"):
            ...
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            self.assertEqual(response.results[0][0], "tim@clairview.com")
    ...
    def test_join_with_property_materialized_session_id(self):
        with freeze_time("2020-01-10"):
            ...
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            assert pretty_print_in_tests(response.clairql, self.team.pk) == self.snapshot
    ...
    def test_window_functions_simple(self):
        ...
        """
        response = execute_clairql_query(query, team=self.team)
    ...
    def test_property_access_with_arrays(self):
        with freeze_time("2020-01-10"):
            ...
            self.assertEqual(
                f"SELECT "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_0)s), ''), 'null'), '^\"|\"$', '') AS string, "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_1)s, %(clairql_val_2)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_3)s, %(clairql_val_4)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_5)s, %(clairql_val_6)s, %(clairql_val_7)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_8)s, %(clairql_val_9)s, %(clairql_val_10)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_11)s, %(clairql_val_12)s, %(clairql_val_13)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_14)s, %(clairql_val_15)s, %(clairql_val_16)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_17)s, %(clairql_val_18)s, %(clairql_val_19)s, %(clairql_val_20)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_21)s, %(clairql_val_22)s, %(clairql_val_23)s, %(clairql_val_24)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_25)s, %(clairql_val_26)s, %(clairql_val_27)s, %(clairql_val_28)s, %(clairql_val_29)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_30)s, %(clairql_val_31)s, %(clairql_val_32)s, %(clairql_val_33)s, %(clairql_val_34)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_35)s, %(clairql_val_36)s, %(clairql_val_37)s, %(clairql_val_38)s, %(clairql_val_39)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_40)s, %(clairql_val_41)s, %(clairql_val_42)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_43)s, %(clairql_val_44)s, %(clairql_val_45)s), ''), 'null'), '^\"|\"$', '') "
                f"FROM events "
                f"WHERE and(equals(events.team_id, {self.team.pk}), ifNull(equals(replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(clairql_val_46)s), ''), 'null'), '^\"|\"$', ''), %(clairql_val_47)s), 0)) "
                f"LIMIT 100 "
    ...
    def test_property_access_with_arrays_zero_index_error(self):
        ...
        with self.assertRaises(QueryError) as e:
            execute_clairql_query(query, team=self.team)
    ...
    def test_clairql_query_filters(self):
        with freeze_time("2020-01-10"):
            ...
            response = execute_clairql_query(query, team=self.team, filters=filters, placeholders=placeholders)
            self.assertEqual(len(response.results), 1)

    def test_clairql_query_filters_empty_true(self):
        ...
        self.assertEqual(response.clairql, "SELECT event FROM events WHERE true LIMIT 100")

    def test_clairql_query_filters_double_error(self):
        query = "SELECT event from events where {filters}"
    ...
    def test_clairql_query_filters_alias(self):
        with freeze_time("2020-01-10"):
            random_uuid = self._create_random_events()
    ...
    @pytest.mark.usefixtures("unittest_snapshot")
    def test_clairql_union_all_limits(self):
    ...
    @pytest.mark.usefixtures("unittest_snapshot")
    def test_clairql_query_session_filters(self):
        with freeze_time("2024-07-05"):
            ...
            assert response.clairql is not None
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            assert pretty_print_in_tests(response.clairql, self.team.pk) == self.snapshot
            self.assertEqual(response.results, [(s1, "https://example.com/1")])

    @pytest.mark.usefixtures("unittest_snapshot")
    def test_clairql_query_filters_session_date_range(self):
        with freeze_time("2024-07-05"):
            s1 = str(uuid7("2024-07-03", 42))
            ...
            query = "SELECT session_id, $entry_current_url from sessions WHERE {filters}"
            filters = ClairQLFilters(dateRange=DateRange(date_from="2024-07-04", date_to="2024-07-06"))
            response = execute_clairql_query(
                query,
                team=self.team,
                filters=filters,
