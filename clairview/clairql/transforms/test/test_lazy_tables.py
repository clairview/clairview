...
from clairview.schema import ClairQLQueryModifiers, PersonsOnEventsMode
...
class TestLazyJoins(BaseTest):
    ...
    def _print_select(self, select: str, modifiers: ClairQLQueryModifiers | None = None):
        ...
        query = print_ast(
            ...
            ClairQLContext(
                ...
                modifiers=modifiers if modifiers is not None else ClairQLQueryModifiers(),
            ),
            "clickhouse",
        )
    ...
    def test_resolve_lazy_table_indirectly_referenced(self):
        ...
        printed = self._print_select(
            ...
            ClairQLQueryModifiers(personsOnEventsMode=PersonsOnEventsMode.PERSON_ID_OVERRIDE_PROPERTIES_JOINED),
        )
        assert printed == self.snapshot
