...
from clairview.schema import ClairQLQueryModifiers, InCohortVia
...
class TestInCohort(BaseTest):
    ...
    def test_in_cohort_dynamic(self):
        ...
        response = execute_clairql_query(
            ...
            self.team,
            modifiers=ClairQLQueryModifiers(inCohortVia=InCohortVia.LEFTJOIN),
            pretty=False,
        )
    ...
    def test_in_cohort_error(self):
        with self.assertRaises(QueryError) as e:
            execute_clairql_query(
                ...
                self.team,
                modifiers=ClairQLQueryModifiers(inCohortVia=InCohortVia.SUBQUERY),
                pretty=False,
            )
    ...
    def test_in_cohort_conjoined_string(self):
        ...
        response = execute_clairql_query(
            f"SELECT event FROM events WHERE person_id IN COHORT 'my cohort'",
            self.team,
            modifiers=ClairQLQueryModifiers(inCohortVia=InCohortVia.LEFTJOIN_CONJOINED),
            pretty=False,
        )
        assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot  # type: ignore
