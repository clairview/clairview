...
from clairview.clairql.database.models import LazyTableToAdd, LazyJoinToAdd
from clairview.clairql.errors import ResolutionError
from clairview.clairql.resolver import resolve_types
from clairview.clairql.resolver_utils import get_long_table_name
...
class LazyTableResolver(TraversingVisitor):
    ...
    def __init__(
        ...
        self.dialect: Literal["clairql", "clickhouse"] = dialect
    ...
    def visit_property_type(self, node: ast.PropertyType):
        ...
        if isinstance(table_type, ast.LazyJoinType) or isinstance(table_type, ast.LazyTableType):
            if self.context and self.context.within_non_clairql_query:
                # If we're in a non-ClairQL query, traverse deeper, just like we normally would have.
