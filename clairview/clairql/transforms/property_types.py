...
from clairview.clairql.escape_sql import escape_clairql_identifier
from clairview.clairql.visitor import CloningVisitor, TraversingVisitor
...
def build_property_swapper(node: ast.Expr, context: ClairQLContext) -> None:
    ...
    def visit_property_type(self, node: ast.PropertyType):
        if node.field_type.name == "properties" and len(node.chain) == 1:
            if isinstance(node.field_type.table_type, ast.BaseTableType):
                ...
                table_name = table_type.resolve_database_table(self.context).to_printed_clairql()
...
class PropertySwapper(CloningVisitor):
    ...
    def visit_field(self, node: ast.Field):
        ...
        if isinstance(type, ast.PropertyType) and type.field_type.name == "person_properties" and len(type.chain) == 1:
            ...
            if isinstance(type.field_type.table_type, ast.BaseTableType):
                table = type.field_type.table_type.resolve_database_table(self.context).to_printed_clairql()
                if table == "events":
                    if property_name in self.person_properties:
                        return self._convert_string_property_to_type(node, "person", property_name)
    ...
    def _convert_string_property_to_type(
        ...
        if property_type == "person":
            clairview_field_type = self.person_properties.get(property_name)
        elif property_type == "group":
            clairview_field_type = self.group_properties.get(property_name)
        else:
            clairview_field_type = self.event_properties.get(property_name)

        field_type = "Float" if clairview_field_type == "Numeric" else clairview_field_type or "String"
    ...
    def _add_notice(self, node: ast.Field, message: str):
        ...
        self.context.add_notice(
            start=max(node.start, node.end - len(escape_clairql_identifier(node.chain[-1]))),
            end=node.end,
            message=message,
        )
