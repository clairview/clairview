...
from clairview.clairql.database.schema.channel_type import create_initial_channel_type, create_initial_domain_type
from clairview.clairql.database.schema.cohort_people import CohortPeople, RawCohortPeople
from clairview.clairql.database.schema.events import EventsTable
from clairview.clairql.database.schema.groups import GroupsTable, RawGroupsTable
from clairview.clairql.database.schema.heatmaps import HeatmapsTable
from clairview.clairql.database.schema.log_entries import (
    LogEntriesTable,
    ReplayConsoleLogsLogEntriesTable,
    BatchExportLogEntriesTable,
)
from clairview.clairql.database.schema.numbers import NumbersTable
from clairview.clairql.database.schema.person_distinct_id_overrides import (
    PersonDistinctIdOverridesTable,
    RawPersonDistinctIdOverridesTable,
    join_with_person_distinct_id_overrides_table,
)
from clairview.clairql.database.schema.person_distinct_ids import (
    PersonDistinctIdsTable,
    RawPersonDistinctIdsTable,
)
from clairview.clairql.database.schema.persons import (
    PersonsTable,
    RawPersonsTable,
    join_with_persons_table,
)
from clairview.clairql.database.schema.session_replay_events import (
    RawSessionReplayEventsTable,
    SessionReplayEventsTable,
    join_replay_table_to_sessions_table_v2,
)
from clairview.clairql.database.schema.sessions_v1 import RawSessionsTableV1, SessionsTableV1
from clairview.clairql.database.schema.sessions_v2 import (
    SessionsTableV2,
    RawSessionsTableV2,
    join_events_table_to_sessions_table_v2,
)
from clairview.clairql.database.schema.static_cohort_people import StaticCohortPeople
from clairview.clairql.errors import QueryError, ResolutionError
...
from clairview.warehouse.models.external_data_job import ExternalDataJob
from clairview.warehouse.models.external_data_schema import ExternalDataSchema
from clairview.warehouse.models.external_data_source import ExternalDataSource
from clairview.warehouse.models.table import (
    DataWarehouseTable,
...
class Database(BaseModel):
    ...
    def get_clairview_tables(self) -> list[str]:
...
def create_clairql_database(
    team_id: int, modifiers: Optional[ClairQLQueryModifiers] = None, team_arg: Optional["Team"] = None
) -> Database:
    from clairview.models import Team
    ...
    for saved_query in DataWarehouseSavedQuery.objects.filter(team_id=team.pk).exclude(deleted=True):
        views[saved_query.name] = saved_query.clairql_definition(modifiers)
    ...
    for table in (
        ...
        s3_table = table.clairql_definition(modifiers)
        ...
        if s3_table.fields.get("properties") is None:
            ...
            class WarehouseProperties(VirtualTable):
                ...
                def to_printed_clairql(self):
                    return self.parent_table.to_printed_clairql()
                    ...
                    for chain in person_field.chain:
                        if isinstance(table_or_field, ast.LazyJoin):
                            table_or_field = table_or_field.resolve_table(
                                ClairQLContext(team_id=team_id, database=database)
                            )
...
DatabaseSchemaTable: TypeAlias = DatabaseSchemaClairViewTable | DatabaseSchemaDataWarehouseTable | DatabaseSchemaViewTable
...
def serialize_database(
    ...
    # ClairView Tables
    clairview_tables = context.database.get_clairview_tables()
    for table_key in clairview_tables:
        ...
        fields = serialize_fields(field_input, context, table_key, table_type="clairview")
        fields_dict = {field.name: field for field in fields}
        tables[table_key] = DatabaseSchemaClairViewTable(fields=fields_dict, id=table_key, name=table_key)
    ...
    for view_name in views:
        ...
        if len(saved_query) != 0:
            tables[view_name] = DatabaseSchemaViewTable(
                ...
                name=view.name,
                query=ClairQLQuery(query=saved_query[0].query["query"]),
            )
...
CLAIRQL_CHARACTERS_TO_BE_WRAPPED = ["@", "-", "!", "$", "+"]
...
def serialize_fields(
    ...
    table_type: Literal["clairview"] | Literal["external"] = "clairview",
) -> list[DatabaseSchemaField]:
    from clairview.clairql.database.models import SavedQuery
    from clairview.clairql.resolver import resolve_types_from_table
    ...
    for field_key, field in field_input.items():
        try:
            ...
            # We redefine fields on some sourced tables, causing the "clairql" and "clickhouse" field names to be intentionally out of sync
            schema_valid = True

        if any(n in field_key for n in CLAIRQL_CHARACTERS_TO_BE_WRAPPED):
            clairql_value = f"`{field_key}`"
        else:
            clairql_value = str(field_key)

        if field_key == "team_id" and table_type == "clairview":
            pass
            ...
            if isinstance(field, IntegerDatabaseField):
                ...
                field_expr = resolve_types_from_table(field.expr, table_name, context, "clairql")
            ...
            field_output.append(
                DatabaseSchemaField(
                    ...
                    type=type,
                    schema_valid=schema_valid,
                    table=field.resolve_table(context).to_printed_clairql(),
            ...
            field_output.append(
                DatabaseSchemaField(
                    ...
                    schema_valid=schema_valid,
                    table=field.to_printed_clairql(),
