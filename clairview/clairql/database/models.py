...
from clairview.clairql.errors import ResolutionError, NotImplementedError

if TYPE_CHECKING:
    from clairview.clairql.context import ClairQLContext
    from clairview.clairql.ast import SelectQuery, LazyJoinType
    from clairview.clairql.base import ConstantType
...
class DatabaseField(FieldOrTable):
    ...
    def get_constant_type(self) -> "ConstantType":
        from clairview.clairql.ast import UnknownType
...
class IntegerDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from clairview.clairql.ast import IntegerType
...
class FloatDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from clairview.clairql.ast import FloatType
...
class StringDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from clairview.clairql.ast import StringType
...
class DateDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from clairview.clairql.ast import DateType
...
class DateTimeDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from clairview.clairql.ast import DateTimeType
...
class BooleanDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from clairview.clairql.ast import BooleanType
...
class Table(FieldOrTable):
    ...
    def to_printed_clickhouse(self, context: "ClairQLContext") -> str:
        raise NotImplementedError("Table.to_printed_clickhouse not overridden")

    def to_printed_clairql(self) -> str:
        raise NotImplementedError("Table.to_printed_clairql not overridden")
...
class LazyJoin(FieldOrTable):
    ...
    join_function: Callable[["LazyJoinToAdd", "ClairQLContext", "SelectQuery"], Any]
    join_table: Table | str
    from_field: list[str | int]
    to_field: Optional[list[str | int]] = None

    def resolve_table(self, context: "ClairQLContext") -> Table:
...
class LazyTable(Table):
    """
    A table that is replaced with a subquery returned from `lazy_select(requested_fields: Dict[name, chain], modifiers: ClairQLQueryModifiers, node: SelectQuery)`
    """
...
class SavedQuery(Table):
    ...
    def to_printed_clickhouse(self, context):
        from clairview.warehouse.models import validate_saved_query_name
