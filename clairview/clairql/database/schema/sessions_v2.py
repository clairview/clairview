...
from clairview.clairql.database.schema.channel_type import create_channel_type_expr, POSSIBLE_CHANNEL_TYPES
from clairview.clairql.database.schema.sessions_v1 import null_if_empty
...
from clairview.schema import BounceRatePageViewMode
...
class RawSessionsTableV2(Table):
    ...
    def avoid_asterisk_fields(self) -> list[str]:
        return [
            "session_id_v7",  # ClairQL insights currently don't support returning uint128s due to json serialisation
...
def select_from_sessions_table_v2(
    requested_fields: dict[str, list[str | int]], node: ast.SelectQuery, context: ClairQLContext
):
    from clairview.clairql import ast
...
def join_events_table_to_sessions_table_v2(
    join_to_add: LazyJoinToAdd, context: ClairQLContext, node: ast.SelectQuery
) -> ast.JoinExpr:
    from clairview.clairql import ast
