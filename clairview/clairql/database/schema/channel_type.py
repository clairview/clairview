...
from clairview.clairql.database.models import ExpressionField
...
if(
    ...
    '$direct',
    clairql_lookupDomainType({referring_domain})
)
""",
            {
...
def create_channel_type_expr(
    ...
    # This logic is referenced in our docs https://clairview.com/docs/data/channel-type, be sure to update both if you
    # update either.
    return parse_expr(
        """
multiIf(
    ...
    coalesce(
        clairql_lookupPaidSourceType({source}),
        if(
            match({campaign}, '^(.*(([^a-df-z]|^)shop|shopping).*)$'),
            'Paid Shopping',
            NULL
        ),
        clairql_lookupPaidMediumType({medium}),
        clairql_lookupPaidSourceType({referring_domain}),
        multiIf (
            {gad_source} = '1',
            'Paid Search',
    ...
    coalesce(
        clairql_lookupOrganicSourceType({source}),
        if(
            match({campaign}, '^(.*(([^a-df-z]|^)shop|shopping).*)$'),
            'Organic Shopping',
            NULL
        ),
        clairql_lookupOrganicMediumType({medium}),
        clairql_lookupOrganicSourceType({referring_domain}),
        multiIf(
            match({campaign}, '^(.*video.*)$'),
            'Organic Video',
