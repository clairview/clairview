...
from clairview.clairql.database.models import FunctionCallTable
...
def build_function_call(
    ...
    if re.match(r"^https:\/\/.+\.blob\.core\.windows\.net\/", url):
        ...
        if regex_result is None:
            raise ExposedClairQLError("Can't parse Azure blob storage URL")
        ...
        if not access_key or not access_secret:
            raise ExposedClairQLError("Azure blob storage has no access key or secret")
...
class S3Table(FunctionCallTable):
    ...
    def to_printed_clairql(self):
        return escape_clairql_identifier(self.name)
