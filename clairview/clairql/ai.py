...
from .database.database import create_clairql_database, serialize_database
...
IDENTITY_MESSAGE = "ClairQL is ClairView's variant of SQL. It supports most of ClickHouse SQL. You write ClairQL based on a prompt. You don't help with other knowledge."

CLAIRQL_EXAMPLE_MESSAGE = """Example ClairQL query for prompt "weekly active users that performed event ACTIVATION_EVENT on example.com/foo/ 3 times or more, by week":
...
REQUEST_MESSAGE = (
    "I need a robust ClairQL query to get the following results: {prompt}\n"
...
def write_sql_from_prompt(prompt: str, *, current_query: Optional[str] = None, team: "Team", user: "User") -> str:
    database = create_clairql_database(team.pk)
    ...
    messages: list[openai.types.chat.ChatCompletionMessageParam] = [
        ...
        {
            "role": "system",
            "content": CLAIRQL_EXAMPLE_MESSAGE,
        },
        {
            "role": "user",
    ...
    generated_valid_clairql = False
    attempt_count = 0
    prompt_tokens_last, completion_tokens_last = 0, 0
    prompt_tokens_total, completion_tokens_total = 0, 0
    for _ in range(3):  # Try up to 3 times in case the generated SQL is not valid ClairQL
        attempt_count += 1
        ...
        try:
            ...
            generated_valid_clairql = True
            break

    report_user_action(
        user,
        "generated ClairQL with AI",
        {
            "prompt": prompt,
            "response": candidate_sql or error,
            "result": ("valid_clairql" if generated_valid_clairql else "invalid_clairql")
            if candidate_sql
            else "prompt_unclear",
