...
def create_default_modifiers_for_user(
    user: "User", team: "Team", modifiers: Optional[ClairQLQueryModifiers] = None
) -> ClairQLQueryModifiers:
    ...
    modifiers.useMaterializedViews = clairviewanalytics.feature_enabled(
        "data-modeling",
...
def create_default_modifiers_for_team(
    team: "Team", modifiers: Optional[ClairQLQueryModifiers] = None
) -> ClairQLQueryModifiers:
    if modifiers is None:
        modifiers = ClairQLQueryModifiers()
    else:
...
def set_default_modifier_values(modifiers: ClairQLQueryModifiers, team: "Team"):
    ...
    if (
        ...
        and is_cloud()
        and clairviewanalytics.feature_enabled(
            "clairql-optimized-property-groups-mode-enabled",
            str(team.uuid),
...
def set_default_in_cohort_via(modifiers: ClairQLQueryModifiers) -> ClairQLQueryModifiers:
