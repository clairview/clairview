...
from clairview.clairql.base import AST, Expr
...
class TraversingVisitor(Visitor[None]):
    ...
    def visit_clairqlx_attribute(self, node: ast.ClairQLXAttribute):
...
class CloningVisitor(Visitor[Any]):
    ...
    def visit_clairqlx_tag(self, node: ast.ClairQLXTag):
        return ast.ClairQLXTag(kind=node.kind, attributes=[self.visit(a) for a in node.attributes])

    def visit_clairqlx_attribute(self, node: ast.ClairQLXAttribute):
        return ast.ClairQLXAttribute(name=node.name, value=self.visit(node.value))
