...
def create_bytecode(
    ...
    bytecode: list[Any] = []
    if args is None:
        bytecode.append(CLAIRQL_BYTECODE_IDENTIFIER)
        bytecode.append(CLAIRQL_BYTECODE_VERSION)
...
class BytecodeCompiler(Visitor):
    def __init__(
        ...
        self.context = context or ClairQLContext(team_id=None)
    ...
    def visit_call(self, node: ast.Call):
        ...
        # ClairQL functions can have two sets of parameters: asd(args) or asd(params)(args)
...
def execute_hog(
    ...
    bytecode = create_bytecode(
        ...
        context=ClairQLContext(team_id=team.id if team else None),
    )
