...
from clairview.constants import NON_TIME_SERIES_DISPLAY_TYPES, TRENDS_CUMULATIVE
from clairview.models.filters.filter import Filter
from clairview.models.team import Team
from clairview.queries.breakdown_props import get_breakdown_cohort_name
from clairview.queries.insight import insight_sync_execute
from clairview.queries.trends.util import parse_response

# Regex for adding the formula variable index to all params, except ClairQL params
PARAM_DISAMBIGUATION_REGEX = re.compile(r"%\((?!clairql_)")
...
class TrendsFormula:
    def _run_formula_query(self, filter: Filter, team: Team):
        ...
        params.update(filter.clairql_context.values)
