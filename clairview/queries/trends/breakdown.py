...
from clairview.queries.groups_join_query import GroupsJoinQuery
...
from clairview.queries.query_date_range import TIME_IN_SECONDS, QueryDateRange
from clairview.schema import PersonsOnEventsMode
from clairview.session_recordings.queries.session_query import SessionQuery
from clairview.queries.trends.sql import (
...
from clairview.queries.trends.util import (
...
from clairview.queries.util import (
    alias_poe_mode_for_legacy,
    get_interval_func_ch,
    get_person_properties_mode,
    get_start_of_interval_sql,
)
from clairview.utils import encode_get_request_params, generate_short_id
from clairview.queries.person_on_events_v2_sql import PERSON_DISTINCT_ID_OVERRIDES_JOIN_SQL

BREAKDOWN_OTHER_DISPLAY = "Other (i.e. all remaining values)"
BREAKDOWN_NULL_DISPLAY = "None (i.e. no value)"


BREAKDOWN_OTHER_STRING_LABEL = "$$_clairview_breakdown_other_$$"
BREAKDOWN_OTHER_NUMERIC_LABEL = 9007199254740991  # pow(2, 53) - 1, for JS compatibility
BREAKDOWN_NULL_STRING_LABEL = "$$_clairview_breakdown_null_$$"
...
class TrendsBreakdown:
    ...
    def _props_to_filter(self) -> tuple[str, dict]:
        ...
        return parse_prop_grouped_clauses(
            ...
            clairql_context=self.filter.clairql_context,
        )
    ...
    def get_query(self) -> tuple[str, dict, Callable]:
        ...
        if len(_params["values"]) == 0:
            ...
            # See https://github.com/ClairView/clairview/pull/5674 for context.
            return (
    ...
    def _get_breakdown_value(self, breakdown: str) -> str:
        if self.filter.breakdown_type == "clairql":
            from clairview.clairql.clairql import translate_clairql

            breakdown_value = translate_clairql(breakdown, self.filter.clairql_context)
