...
from clairview.constants import FILTER_TEST_ACCOUNTS, INSIGHT_FUNNELS
...
from clairview.queries.funnels import ClickhouseFunnel, ClickhouseFunnelActors
from clairview.queries.funnels.test.breakdown_cases import (
    assert_funnel_results_equal,
    funnel_breakdown_test_factory,
)
from clairview.queries.funnels.test.conversion_time_cases import (
...
def funnel_test_factory(Funnel, event_factory, person_factory):
    class TestGetFunnel(ClickhouseTestMixin, APIBaseTest):
        ...
        def test_breakdown_values_is_set_on_the_query_with_fewer_than_two_entities(self):
            """
            failing test for https://sentry.io/organizations/clairview/issues/2807609211/?project=1899813&referrer=slack
            """
        ...
        def test_clairql_aggregation(self):
            ...
            # distinct_id
            result = self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_clairql": "distinct_id"}).run()
            self.assertEqual(result[0]["count"], 2)
            self.assertEqual(result[1]["count"], 1)
            self.assertEqual(result[2]["count"], 1)

            # person_id
            result = self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_clairql": "person_id"}).run()
            self.assertEqual(result[0]["count"], 2)
            self.assertEqual(result[1]["count"], 1)
            self.assertEqual(result[2]["count"], 1)
