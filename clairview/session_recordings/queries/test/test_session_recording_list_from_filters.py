...
from clairview.clickhouse.log_entries import TRUNCATE_LOG_ENTRIES_TABLE_SQL
from clairview.constants import AvailableFeature
from clairview.models import Cohort, GroupTypeMapping, Person
...
class TestSessionRecordingsListFromFilters(ClickhouseTestMixin, APIBaseTest):
    ...
    def _filter_recordings_by(self, recordings_filter: dict) -> SessionRecordingQueryResult:
        ...
        session_recording_list_instance = SessionRecordingListFromFilters(
            filter=the_filter, team=self.team, clairql_query_modifiers=None
        )
    ...
    def test_operand_or_person_filters(self):
        ...
        Person.objects.create(team=self.team, distinct_ids=[user], properties={"email": "test@clairview.com"})

        second_user = "test_operand_or_filter-second_user"
        Person.objects.create(team=self.team, distinct_ids=[second_user], properties={"email": "david@clairview.com"})
    ...
    @snapshot_clickhouse_queries
    def test_event_filter_with_clairql_properties(self):
        user = "test_event_filter_with_clairql_properties-user"

        Person.objects.create(team=self.team, distinct_ids=[user], properties={"email": "bla"})

        session_id = f"test_event_filter_with_clairql_properties-1-{str(uuid4())}"
        self.create_event(
            user,
            self.an_hour_ago,
            properties={
                ...
                "events": [
                    {
                        ...
                        "name": "$pageview",
                        "properties": [
                            {"key": "properties.$browser == 'Chrome'", "type": "clairql"},
                        ],
                    }
                ]
            }
                ...
                "events": [
                    {
                        ...
                        "order": 0,
                        "name": "$pageview",
                        "properties": [{"key": "properties.$browser == 'Firefox'", "type": "clairql"}],
                    }
                ]
            }
        )
    ...
    @snapshot_clickhouse_queries
    def test_event_filter_with_clairql_person_properties(self):
        user = "test_event_filter_with_clairql_properties-user"
    ...
    @freeze_time("2021-01-21T20:00:00.000Z")
    @snapshot_clickhouse_queries
    def test_event_filter_with_clairql_event_properties_test_accounts_excluded(self):
        self.team.test_account_filters = [
            {"key": "properties.$browser == 'Chrome'", "type": "clairql"},
        ]
        self.team.save()
        ...
        (session_recordings, _, _) = self._filter_recordings_by(
            {
                # pageview that matches the clairql test_accounts filter
                "events": [
                    {
                        "id": "$pageview",
                        "type": "events",
        ...
        self.team.test_account_filters = [
            {"key": "person.properties.email == 'bla'", "type": "clairql"},
        ]
        self.team.save()

        (session_recordings, _, _) = self._filter_recordings_by(
            {
                # only 1 pageview that matches the clairql test_accounts filter
                "events": [
                    {
                        "id": "$pageview",
                        "type": "events",
    ...
    @freeze_time("2021-01-21T20:00:00.000Z")
    @snapshot_clickhouse_queries
    def test_top_level_clairql_event_property_test_account_filter(self):
        """
        This is a regression test. A user with an $ip test account filter
        reported the filtering wasn't working.
        ...
        """
        self.team.test_account_filters = [
            {"key": "properties.is_internal_user == 'true'", "type": "clairql"},
        ]
        self.team.save()
    ...
    @freeze_time("2021-01-21T20:00:00.000Z")
    @snapshot_clickhouse_queries
    def test_top_level_clairql_person_property_test_account_filter(self):
        """
        This is a regression test. A user with an $ip test account filter
        reported the filtering wasn't working.
    ...
    def test_top_level_event_host_property_test_account_filter(self):
        """
        This is a regression test. See: https://clairviewhelp.zendesk.com/agent/tickets/18059
        """
        self.team.test_account_filters = [
