...
from clairview.settings.base_variables import DEBUG, IS_COLLECT_STATIC, TEST
...
if TEST or DEBUG:
    ...
    PG_USER: str = os.getenv("PGUSER", "clairview")
    PG_PASSWORD: str = os.getenv("PGPASSWORD", "clairview")
    PG_PORT: str = os.getenv("PGPORT", "5432")
    PG_DATABASE: str = os.getenv("PGDATABASE", "clairview")
...
# Clickhouse Settings
CLICKHOUSE_TEST_DB: str = "clairview" + SUFFIX
...
CLICKHOUSE_CLUSTER: str = os.getenv("CLICKHOUSE_CLUSTER", "clairview")
...
if not CDP_FUNCTION_EXECUTOR_API_URL:
    CDP_FUNCTION_EXECUTOR_API_URL = (
        "http://localhost:6738" if DEBUG else "http://ingestion-cdp-function-callbacks.clairview.svc.cluster.local"
    )
...
CACHES = {
    "default": {
        ...
        "OPTIONS": {
            ...
            "COMPRESSOR": "clairview.caching.tolerant_zlib_compressor.TolerantZlibCompressor",
        },
        "KEY_PREFIX": "clairview",
    }
}
