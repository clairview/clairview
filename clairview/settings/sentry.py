...
from clairview.git import get_git_commit_full

from clairview.settings import get_from_env
...
def traces_sampler(sampling_context: dict) -> float:
    ...
    if op == "http.server":
        ...
        if task in (
            "clairview.celery.redis_heartbeat",
            "clairview.celery.redis_celery_queue_depth",
        ):
            return 0.0001  # 0.01%
        ...
        task = sampling_context.get("celery_job", {}).get("task")
        if task == "clairview.tasks.calculate_cohort.insert_cohort_from_feature_flag":
