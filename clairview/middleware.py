...
from clairview.api.capture import get_event
from clairview.api.decide import get_decide
from clairview.api.shared import UserBasicSerializer
from clairview.clickhouse.client.execute import clickhouse_query_counter
from clairview.clickhouse.query_tagging import QueryCounter, reset_query_tags, tag_queries
...
from clairview.models import Action, Cohort, Dashboard, FeatureFlag, Insight, Notebook, User, Team
from clairview.rate_limit import DecideRateThrottle
from clairview.settings import SITE_URL, DEBUG, PROJECT_SWITCHING_TOKEN_ALLOWLIST
        ...
        if ip:
            if settings.ALLOWED_IP_BLOCKS:
                ...
                if get_geoip_properties(ip).get("$geoip_country_code", None) not in settings.BLOCKED_GEOIP_REGIONS:
                    return response
        return HttpResponse(
            "ClairView is not available in your region. If you think this is in error, please contact tim@clairview.com.",
            status=403,
        )
...
class AutoProjectMiddleware:
    ...
    Sometimes you get sent a link to ClairView that points to an item from a different project than the one you currently
...
def per_request_logging_context_middleware(
    ...
    def middleware(request: HttpRequest) -> HttpResponse:
        ...
        # that do not use Host header app.clairview.com. This is important as we
        # roll out CloudFront in front of app.clairview.com. We can get the host
...
class ClairViewTokenCookieMiddleware(SessionMiddleware):
    """
