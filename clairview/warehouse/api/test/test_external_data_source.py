...
from clairview.temporal.data_imports.pipelines.stripe.settings import ENDPOINTS
from clairview.test.base import APIBaseTest
from clairview.warehouse.models import ExternalDataSource, ExternalDataSchema
import uuid
...
from clairview.warehouse.models.external_data_schema import sync_frequency_interval_to_sync_frequency
...
class TestExternalDataSource(APIBaseTest):
    ...
    @patch("clairview.warehouse.api.external_data_source.trigger_external_data_source_workflow")
    ...
    def test_database_schema(self):
        ...
        with postgres_connection.cursor() as cursor:
            cursor.execute(
                """
                CREATE TABLE IF NOT EXISTS clairview_test (
        ...
        self.assertTrue("clairview_test" in table_names)

        with postgres_connection.cursor() as cursor:
            cursor.execute(
                """
                DROP TABLE clairview_test;
                """
            )
            postgres_connection.commit()
    ...
    def test_database_schema_stripe_credentials(self):
        with patch(
            "clairview.warehouse.api.external_data_source.validate_stripe_credentials"
    ...
    def test_database_schema_zendesk_credentials(self):
        with patch(
            "clairview.warehouse.api.external_data_source.validate_zendesk_credentials"
        ) as validate_credentials_mock:
            validate_credentials_mock.return_value = True
    ...
    @patch(
        "clairview.warehouse.api.external_data_source.get_sql_schemas_for_source_type",
