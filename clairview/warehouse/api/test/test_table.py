...
from clairview.warehouse.models import DataWarehouseTable
...
class TestTable(APIBaseTest):
    @patch(
        ...
        return_value={
            "id": {"clickhouse": "Nullable(String)", "clairql": "StringDatabaseField", "valid": True},
            "a_column": {"clickhouse": "Nullable(String)", "clairql": "StringDatabaseField", "valid": True},
        },
    )
    @patch(
        "clairview.warehouse.models.table.DataWarehouseTable.validate_column_type",
        return_value=True,
    )
    @patch("clairview.tasks.warehouse.get_ph_client")
    ...
    def test_create_columns_invalid_schema(self, patch_get_columns, patch_validate_column_type, patch_get_ph_client):
        ...
        assert table.name == "whatever"
        assert table.columns == {
            "id": {"clickhouse": "Nullable(String)", "clairql": "StringDatabaseField", "valid": False},
            "a_column": {"clickhouse": "Nullable(String)", "clairql": "StringDatabaseField", "valid": False},
        }

        assert table.credential.access_key, "_accesskey"
        assert table.credential.access_secret, "_accesssecret"

    @patch("clairview.warehouse.models.table.DataWarehouseTable.get_columns")
    ...
    def test_update_schema_200_old_column_style(self, patch_validate_column_type):
        ...
        assert table.columns["id"] == {"clickhouse": "Nullable(Float64)", "clairql": "FloatDatabaseField", "valid": True}
    ...
    def test_update_schema_200_new_column_style(self, patch_validate_column_type):
        table = DataWarehouseTable.objects.create(
            ...
            columns={"id": {"clickhouse": "Nullable(Int64)", "clairql": "IntegerDatabaseField"}},
        )
        response = self.client.post(
            f"/api/projects/{self.team.pk}/warehouse_tables/{table.id}/update_schema", {"updates": {"id": "float"}}
        )
