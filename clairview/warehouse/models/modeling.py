...
UPDATE_PATHS_QUERY = """\
insert into clairview_datawarehousemodelpath (
  id,
  team_id,
  table_id,
  saved_query_id,
  ...
  select
    ...
    now() as updated_at
  from
    clairview_datawarehousemodelpath as model_path,
    (
      select
        path
      from clairview_datawarehousemodelpath
...
delete from clairview_datawarehousemodelpath
where
  team_id = %(team_id)s
  and id in (
    select id
...
class UnknownParentError(Exception):
    ...
    def __init__(self, parent: str, query: str):
        super().__init__(
            f"The parent name {parent} does not correspond to an existing ClairView table, Data Warehouse Table, or Data Warehouse Saved Query."
        )
        self.query = query
...
class DataWarehouseModelPathManager(models.Manager["DataWarehouseModelPath"]):
    ...
    def get_or_create_root_path_for_clairview_source(
        self, clairview_source_name: str, team: Team
        ...
        Raises:
            ValueError: If the provided `clairview_source_name` is not a ClairView table.
    ...
    def get_or_create_query_parent_paths(self, query: str, team: Team) -> list["DataWarehouseModelPath"]:
        ...
        for parent in get_parents_from_model_query(query):
            try:
                parent_path, _ = self.get_or_create_root_path_for_clairview_source(parent, team)
            except ValueError:
                pass
            else:
    ...
    def get_longest_common_ancestor_path(
        ...
        query = "select lca(array_agg(path)) from clairview_datawarehousemodelpath where path ? %(lqueries)s"
...
class DataWarehouseModelPath(CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    ...
    ClairView tables and external data source tables.
    """
