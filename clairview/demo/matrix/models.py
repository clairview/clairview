...
if TYPE_CHECKING:
    from clairview.demo.matrix.matrix import Cluster, Matrix
...
class SimBrowserClient(SimClient):
    ...
    def capture(self, event: str, properties: Optional[Properties] = None):
        """Capture an arbitrary event. Similar to JS `clairview.capture()`."""
        combined_properties: Properties = {
    ...
    def identify(self, distinct_id: Optional[str], set_properties: Optional[Properties] = None):
        """Identify person in active client. Similar to JS `clairview.identify()`.

        Use with distinct_id=None for `clairview.people.set()`-like behavior."""
    ...
    def group(
        ...
        """Link the person to the specified group. Similar to JS `clairview.group()`."""
        if set_properties is None:
            set_properties = {}
    ...
    def reset(self):
        """Reset active client, for instance when the user logs out. Similar to JS `clairview.reset()`."""
        self.active_distinct_id = self.device_id
        self.is_logged_in = True

    def register(self, super_properties: Properties):
        """Register super properties. Similar to JS `clairview.register()`."""
        self.super_properties.update(super_properties)

    def unregister(self, *super_property_keys: str):
        """Removes super properties. Similar to JS `clairview.unregister()`."""
...
class SimPerson(ABC):
    ...
    in_clairview_id: Optional[UUID]  # ClairView person ID (must be a UUID)
    country_code: str
    timezone: str
    ...
    def _append_event(
        ...
        *,
        ...
        if self.in_clairview_id is None:
            self.in_clairview_id = self.cluster.roll_uuidt()
        ...
        sim_event = SimEvent(
            ...
            properties=properties,
            timestamp=timestamp,
            person_id=self.in_clairview_id,
