...
from clairview.constants import MAX_SLUG_LENGTH
...
class UUIDT(uuid.UUID):
    ...
    well before that, is still in use in ClairView, but should not be used for new columns / models / features / etc.
...
class CreatedMetaFields(models.Model):
    created_by = models.ForeignKey("clairview.User", on_delete=models.SET_NULL, null=True, blank=True)
