...
from clairview.models.person import Person, PersonDistinctId
from clairview.models.property import GroupTypeIndex, GroupTypeName
from clairview.models.property.property import Property
from clairview.models.cohort import Cohort, CohortOrEmpty
from clairview.models.utils import execute_with_timeout
from clairview.queries.base import match_property, properties_to_Q, sanitize_property_key
from clairview.database_healthcheck import (
    postgres_healthcheck,
    DATABASE_FOR_FLAG_MATCHING,
)
from clairview.utils import label_for_team_id_to_track
                ...
                with execute_with_timeout(FLAG_MATCHING_QUERY_TIMEOUT_MS, DATABASE_FOR_FLAG_MATCHING) as cursor:
                    ...
                    query = """
                        WITH target_person_ids AS (
                            SELECT team_id, person_id FROM clairview_persondistinctid WHERE team_id = %(team_id)s AND
                        ...
                        SELECT key FROM clairview_featureflag WHERE team_id = %(team_id)s AND ensure_experience_continuity = TRUE AND active = TRUE AND deleted = FALSE
            ...
            with execute_with_timeout(FLAG_MATCHING_QUERY_TIMEOUT_MS) as cursor:
                query = """
                    ...
                    INSERT INTO clairview_featureflaghashkeyoverride (team_id, person_id, feature_flag_key, hash_key)
                        ...
                        WHERE EXISTS (SELECT 1 FROM clairview_person WHERE id = person_id AND team_id = %(team_id)s)
