...
from clairview.plugins.access import can_configure_plugins, can_install_plugins
from clairview.plugins.plugin_server_api import populate_plugin_capabilities_on_workers, reload_plugins_on_workers
...
def update_validated_data_from_url(validated_data: dict[str, Any], url: str) -> dict[str, Any]:
    ...
    if url.startswith("file:"):
        ...
        clairview_version = plugin_json.get("clairviewVersion", None)
    ...
    if clairview_version and not is_cloud():
        # Legacy: ClairView is no longer versioned
        try:
...
class Plugin(models.Model):
    ...
    # DEPRECATED: this was used when syncing clairview.json with the db on app start
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                ...
                name="clairview_unique_plugin_storage_key",
            )
        ]
...
def transpile(input_string: str, type: Literal["site", "frontend"] = "site") -> Optional[str]:
    from clairview.settings.base_variables import BASE_DIR
