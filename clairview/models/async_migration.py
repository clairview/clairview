...
class AsyncMigration(models.Model):
    ...
    clairview_min_version = models.CharField(max_length=20, null=True, blank=True)
    clairview_max_version = models.CharField(max_length=20, null=True, blank=True)
    ...
    def get_name_with_requirements(self) -> str:
        return (
            f"{self.name} - must be ran on ClairView version {self.clairview_min_version} up to {self.clairview_max_version}"
        )
