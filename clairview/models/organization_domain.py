...
class OrganizationDomain(UUIDModel):
    ...
    )  # verification (through DNS) is only used for ClairView Cloud; on self-hosted we take all domains as verified
    ...
    )  # currently only used for ClairView Cloud; SSO enforcement on self-hosted is set by env var
    ...
    def attempt_verification(self) -> tuple["OrganizationDomain", bool]:
        ...
        try:
            ...
            dns_response = dns.resolver.resolve(f"_clairview-challenge.{self.domain}", "TXT")
