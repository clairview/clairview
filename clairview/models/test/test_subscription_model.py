...
class TestSubscription(BaseTest):
    def _create_insight_subscription(self, **kwargs):
        ...
        params = {
            ...
            "insight": insight,
            "target_type": "email",
            "target_value": "tests@clairview.com",
            "frequency": "weekly",
            "interval": 2,
    ...
    def test_generating_token(self):
        subscription = self._create_insight_subscription(
            target_value="test1@clairview.com,test2@clairview.com,test3@clairview.com"
        )
        subscription.save()

        token = get_unsubscribe_token(subscription, "test2@clairview.com")
        ...
        assert info["email"] == "test2@clairview.com"
    ...
    def test_unsubscribe_using_token_succeeds(self):
        ...
        assert subscription.target_value == "test1@clairview.com,test3@clairview.com"
    ...
    def test_unsubscribe_using_token_fails_if_too_old(self):
        ...
        with freeze_time(datetime(2022, 1, 1) + timedelta(days=UNSUBSCRIBE_TOKEN_EXP_DAYS - 1)):
            subscription = unsubscribe_using_token(token)
            assert "test2@clairview.com" not in subscription.target_value

    def test_unsubscribe_does_nothing_if_already_unsubscribed(self):
        subscription = self._create_insight_subscription(target_value="test1@clairview.com,test3@clairview.com")
        subscription.save()
    ...
    def test_unsubscribe_deletes_subscription_if_last_subscriber(self):
        subscription = self._create_insight_subscription(target_value="test1@clairview.com,test2@clairview.com")
        subscription.save()

        assert not subscription.deleted
        token = get_unsubscribe_token(subscription, "test1@clairview.com")
        subscription = unsubscribe_using_token(token)
        assert not subscription.deleted
        token = get_unsubscribe_token(subscription, "test2@clairview.com")
        subscription = unsubscribe_using_token(token)
