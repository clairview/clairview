...
class TestHogFunction(TestCase):
    ...
    def test_hog_function_team_no_filters_compilation(self):
        ...
        assert json_filters["bytecode"] == ["_H", TORQL_BYTECODE_VERSION, 29]  # TRUE
    ...
    def test_hog_function_team_filters_only_compilation(self):
        ...
        assert json.dumps(json_filters["bytecode"]) == snapshot(
            f'["_H", {TORQL_BYTECODE_VERSION}, 32, "$host", 32, "properties", 1, 2, 2, "toString", 1, 32, "^(localhost|127\\\\.0\\\\.0\\\\.1)($|:)", 2, "match", 2, 5, 47, 3, 35, 33, 1, 3, 1]'
        )
...
class TestHogFunctionsBackgroundReloading(TestCase, QueryMatchingTest):
    ...
    def test_hog_functions_reload_on_action_saved(self):
        ...
        assert json.dumps(hog_function_1.filters["bytecode"]) == snapshot(
            f'["_H", {TORQL_BYTECODE_VERSION}, 32, "test-event", 32, "event", 1, 1, 11, 32, "old-value-1", 32, "prop-1", 32, "properties", 1, 2, 11, 3, 2, 3, 1, 32, "old-value-2", 32, "prop-2", 32, "properties", 1, 2, 11, 3, 1, 4, 2]'
        )

        assert json.dumps(hog_function_2.filters["bytecode"]) == snapshot(
            f'["_H", {TORQL_BYTECODE_VERSION}, 32, "test-event", 32, "event", 1, 1, 11, 32, "old-value-1", 32, "prop-1", 32, "properties", 1, 2, 11, 3, 2, 3, 1, 4, 1]'
        )
        ...
        assert json.dumps(hog_function_1.filters["bytecode"]) == snapshot(
            f'["_H", {TORQL_BYTECODE_VERSION}, 32, "test-event", 32, "event", 1, 1, 11, 32, "change-value", 32, "prop-1", 32, "properties", 1, 2, 11, 3, 2, 3, 1, 32, "old-value-2", 32, "prop-2", 32, "properties", 1, 2, 11, 3, 1, 4, 2]'
        )
        assert json.dumps(hog_function_2.filters["bytecode"]) == snapshot(
            f'["_H", {TORQL_BYTECODE_VERSION}, 32, "test-event", 32, "event", 1, 1, 11, 32, "change-value", 32, "prop-1", 32, "properties", 1, 2, 11, 3, 2, 3, 1, 4, 1]'
        )
    ...
    def test_hog_functions_reload_on_team_saved(self):
        ...
        # Check that the bytecode is correct
        assert json.dumps(hog_function_1.filters["bytecode"]) == snapshot(f'["_H", {TORQL_BYTECODE_VERSION}, 29]')
        assert json.dumps(hog_function_2.filters["bytecode"]) == snapshot(
            f'["_H", {TORQL_BYTECODE_VERSION}, 32, "$pageview", 32, "event", 1, 1, 11, 3, 1, 4, 1]'
        )
        assert json.dumps(hog_function_3.filters["bytecode"]) == snapshot(f'["_H", {TORQL_BYTECODE_VERSION}, 29]')
        ...
        assert json.dumps(hog_function_1.filters["bytecode"]) == snapshot(
            f'["_H", {TORQL_BYTECODE_VERSION}, 32, "$host", 32, "properties", 1, 2, 2, "toString", 1, 32, "^(localhost|127\\\\.0\\\\.0\\\\.1)($|:)", 2, "match", 2, 47, 3, 35, 33, 0, 32, "$pageview", 32, "properties", 1, 2, 2, "toString", 1, 32, "test", 2, "match", 2, 47, 3, 35, 33, 0, 3, 2]'
        )
        assert json.dumps(hog_function_2.filters["bytecode"]) == snapshot(
            f'["_H", {TORQL_BYTECODE_VERSION}, 32, "$host", 32, "properties", 1, 2, 2, "toString", 1, 32, "^(localhost|127\\\\.0\\\\.0\\\\.1)($|:)", 2, "match", 2, 47, 3, 35, 33, 0, 32, "$pageview", 32, "properties", 1, 2, 2, "toString", 1, 32, "test", 2, "match", 2, 47, 3, 35, 33, 0, 32, "$pageview", 32, "event", 1, 1, 11, 3, 3, 4, 1]'
        )
        assert json.dumps(hog_function_3.filters["bytecode"]) == snapshot(f'["_H", {TORQL_BYTECODE_VERSION}, 29]')
