...
from clairview.models.property import BehavioralPropertyType, Property, PropertyGroup
...
DELETE_QUERY = """
DELETE FROM "clairview_cohortpeople" WHERE "cohort_id" = {cohort_id}
"""

UPDATE_QUERY = """
INSERT INTO "clairview_cohortpeople" ("person_id", "cohort_id", "version")
{values_query}
...
class Cohort(models.Model):
    ...
    def calculate_people_ch(self, pending_version: int, *, initiating_user_id: Optional[int] = None):
        ...
        from clairview.tasks.calculate_cohort import clear_stale_cohort
    ...
    def insert_users_by_list(self, items: list[str]) -> None:
        ...
        batchsize = 1000
        from clairview.models.cohort.util import (
            insert_static_cohort,
            get_static_cohort_size,
        )

        if TEST:
            from clairview.test.base import flush_persons_and_events
            ...
            for i in range(0, len(items), batchsize):
                ...
                query = UPDATE_QUERY.format(
                    ...
                    values_query=sql.replace(
                        ...
                        f', {self.pk}, {self.version or "NULL"} FROM "clairview_person"',
                        1,
                    ),
                )
    ...
    def insert_users_list_by_uuid(self, items: list[str], insert_in_clickhouse: bool = False, batchsize=1000) -> None:
        from clairview.models.cohort.util import get_static_cohort_size, insert_static_cohort
    ...
    def _clickhouse_persons_query(self, batch_size=10000, offset=0):
        from clairview.models.cohort.util import get_person_ids_by_cohort_id
