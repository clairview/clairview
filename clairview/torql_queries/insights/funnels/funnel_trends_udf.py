...
from clairview.torql.constants import TorQLQuerySettings
from clairview.torql.parser import parse_select
from clairview.torql_queries.insights.funnels import FunnelTrends
from clairview.torql_queries.insights.utils.utils import get_start_of_interval_torql_str
...
class FunnelTrendsUDF(FunnelTrends):
    ...
    def get_query(self) -> ast.SelectQuery:
        ...
        inner_select = cast(
            ...
            SELECT
                arrayJoin({fn}(
                    ...
                    {prop_vals},
                    arraySort(t -> t.1, groupArray(tuple(toFloat(timestamp), {get_start_of_interval_torql_str(self.context.interval.value, team=self.context.team, source='timestamp')}, {prop_selector}, arrayFilter((x) -> x != 0, [{steps}{exclusions}]))))
                )) as af_tuple,
        ...
        inner_select.settings = TorQLQuerySettings(date_time_output_format="iso", date_time_input_format="best_effort")
