...
from clairview.torql.errors import ImpossibleASTError
from clairview.torql.parser import ast
...
# Originally similar to clairview/queries/query_date_range.py but rewritten to be used in TorQL queries
class QueryDateRange:
    ...
    def interval_period_string_as_torql_constant(self) -> ast.Expr:
    ...
    def date_to_start_of_interval_torql(self, date: ast.Expr) -> ast.Call:
    ...
    def date_from_to_start_of_interval_torql(self) -> ast.Call:
        return self.date_to_start_of_interval_torql(self.date_from_as_torql())

    def date_to_to_start_of_interval_torql(self) -> ast.Call:
        return self.date_to_start_of_interval_torql(self.date_to_as_torql())

    def date_to_with_extra_interval_torql(self) -> ast.Call:
        return ast.Call(
            name="plus",
            args=[self.date_to_start_of_interval_torql(self.date_to_as_torql()), self.one_interval_period()],
        )

    def to_placeholders(self) -> dict[str, ast.Expr]:
        return {
            "interval": self.interval_period_string_as_torql_constant(),
            "one_interval_period": self.one_interval_period(),
            "number_interval_period": self.number_interval_periods(),
            "date_from": self.date_from_as_torql(),
            "date_to": self.date_to_as_torql(),
            "date_from_start_of_interval": self.date_from_to_start_of_interval_torql(),
            "date_to_start_of_interval": self.date_to_to_start_of_interval_torql(),
            "date_from_with_adjusted_start_of_interval": (
                self.date_from_to_start_of_interval_torql()
                if self.use_start_of_interval()
                else self.date_from_as_torql()
            ),
        }
...
class QueryDateRangeWithIntervals(QueryDateRange):
    ...
    def get_start_of_interval_torql(self, *, source: ast.Expr = None) -> ast.Expr:
