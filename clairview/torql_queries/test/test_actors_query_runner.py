...
class TestActorsQueryRunner(ClickhouseTestMixin, APIBaseTest):
        ...
        runner = self._create_runner(
            ActorsQuery(
                properties=[
                    ...
                    TorQLPropertyFilter(key="toInt(properties.index) > 5"),
                ]
            )
        )
        ...
        runner = self._create_runner(
            ActorsQuery(
                fixedProperties=[
                    ...
                    TorQLPropertyFilter(key="toInt(properties.index) < 2"),
                ]
            )
        )
    ...
    def test_persons_query_search_email(self):
        ...
        runner = self._create_runner(ActorsQuery(search=f"jacob4@{self.random_uuid}.clairview"))
    ...
    def test_persons_query_search_snapshot(self):
        ...
        assert pretty_print_in_tests(runner.to_torql(), self.team.pk) == self.snapshot
    ...
    def test_persons_query_order_by(self):
        ...
        results = runner.calculate().results
        self.assertEqual(results[0], [f"jacob9@{self.random_uuid}.clairview.com"])
    ...
    def test_persons_query_order_by_with_aliases(self):
        ...
        results = runner.calculate().results
        self.assertEqual(results[0], [f"jacob0@{self.random_uuid}.clairview.com"])
    ...
    def test_persons_query_limit(self):
        ...
        response = runner.calculate()
        self.assertEqual(response.results, [[f"jacob9@{self.random_uuid}.clairview.com"]])
        ...
        response = runner.calculate()
        self.assertEqual(response.results, [[f"jacob7@{self.random_uuid}.clairview.com"]])
        self.assertEqual(response.hasMore, True)

    @override_settings(PERSON_ON_EVENTS_OVERRIDE=True, PERSON_ON_EVENTS_V2_OVERRIDE=True)
    def test_source_torql_query_poe_on(self):
        self.random_uuid = self._create_random_persons()
        source_query = TorQLQuery(query="SELECT distinct person_id FROM events WHERE event='clicky-4'")
        query = ActorsQuery(
            select=["properties.email"],
            orderBy=["properties.email DESC"],
            source=source_query,
        ...
        response = runner.calculate()
        self.assertEqual(response.results, [[f"jacob4@{self.random_uuid}.clairview.com"]])

    @override_settings(PERSON_ON_EVENTS_OVERRIDE=False, PERSON_ON_EVENTS_V2_OVERRIDE=False)
    def test_source_torql_query_poe_off(self):
        self.random_uuid = self._create_random_persons()
        source_query = TorQLQuery(query="SELECT distinct person_id FROM events WHERE event='clicky-4'")
        query = ActorsQuery(
            select=["properties.email"],
    ...
    def test_persons_query_grouping(self):
        ...
        _create_person(
            properties={
                "email": f"jacob0@{random_uuid}.clairview.com",
                "name": f"Mr Jacob {random_uuid}",
                "random_uuid": random_uuid,
                "index": 0,
            },
        ...
        flush_persons_and_events()
        runner = self._create_runner(ActorsQuery(search="clairview.com"))
