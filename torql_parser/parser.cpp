...
#include "TorQLLexer.h"
#include "TorQLParser.h"
#include "TorQLParserBaseVisitor.h"

#include "error.h"
#include "parser.h"
#include "string.h"

#define VISIT(RULE) any visit##RULE(TorQLParser::RULE##Context* ctx) override
#define VISIT_UNSUPPORTED(RULE)                                \
  VISIT(RULE) {                                                \
    throw NotImplementedError("Unsupported rule: " #RULE); \
  }

#define HANDLE_TORQL_ERROR(TYPE, OTHER_CLEANUP)                                                     \
  (const TYPE& e) {                                                                                     \
...
class TorQLParseTreeConverter : public TorQLParserBaseVisitor {
 private:
  parser_state* state;
  bool is_internal;
...
#define IS_AST_NODE_INSTANCE_IMPL(TORQL_MODULE, TYPE_NAME)                               \
  PyObject* node_type = PyObject_GetAttrString(state->TORQL_MODULE##_module, TYPE_NAME); \
...
#undef IS_AST_NODE_INSTANCE_IMPL
 ...
 public:
  TorQLParseTreeConverter(parser_state* state, bool is_internal) : state(state), is_internal(is_internal) {}
  ...
  PyObject* visitAsPyObjectFinal(antlr4::tree::ParseTree* tree) {
    ...
    } catch HANDLE_TORQL_ERROR(SyntaxError, ) catch HANDLE_TORQL_ERROR(
        NotImplementedError,
    ) catch HANDLE_TORQL_ERROR(ParsingError, ) catch (const PyInternalError& e) {
      return NULL;
  ...
  VISIT(Select) {
    ...
    return visit(ctx->torqlxTagElement());
  }
...
#undef IS_NODE_A_CONCAT_CALL
  ...
  VISIT(ColumnExprTagElement) { return visit(ctx->torqlxTagElement()); }
  ...
  VISIT(TableExprTag) { return visit(ctx->torqlxTagElement()); }
  ...
  VISIT(TorqlxTagAttribute) {
    string name = visitAsString(ctx->identifier());

    auto column_expr_ctx = ctx->columnExpr();
    if (column_expr_ctx) {
      RETURN_NEW_AST_NODE(
          "TorQLXAttribute", "{s:s#,s:N}", "name", name.data(), name.size(), "value", visitAsPyObject(column_expr_ctx)
      );
    }
    ...
    if (string_ctx) {
      ...
      if (!value) throw PyInternalError();
      RETURN_NEW_AST_NODE("TorQLXAttribute", "{s:s#,s:N}", "name", name.data(), name.size(), "value", value);
    }
  ...
  VISIT(TorqlxTagElementClosed) {
    string kind = visitAsString(ctx->identifier());
    RETURN_NEW_AST_NODE(
        "TorQLXTag", "{s:s#,s:N}", "kind", kind.data(), kind.size(), "attributes",
        visitPyListOfObjects(ctx->torqlxTagAttribute())
    );
  }

  VISIT(TorqlxTagElementNested) {
    string opening = visitAsString(ctx->identifier(0));
    string closing = visitAsString(ctx->identifier(1));
    if (opening != closing) {
      throw SyntaxError("Opening and closing TorQLX tags must match. Got " + opening + " and " + closing);
    }

    auto tag_element_ctx = ctx->torqlxTagElement();
    auto column_expr_ctx = ctx->columnExpr();
    auto tag_attribute_ctx = ctx->torqlxTagAttribute();
    ...
    if (tag_element_ctx) {
      if (found_source) {
        Py_DECREF(attributes);
        throw SyntaxError("Nested TorQLX tags cannot have a source attribute");
      }
      PyObject* source_attribute = build_ast_node(
          "TorQLXAttribute", "{s:s#,s:N}", "name", "source", 6, "value", visitAsPyObject(ctx->torqlxTagElement())
      );
      if (!source_attribute) {
        Py_DECREF(attributes);
        throw PyInternalError();
      ...
      PyObject* source_attribute = build_ast_node(
          "TorQLXAttribute", "{s:s#,s:N}", "name", "source", 6, "value", visitAsPyObject(ctx->columnExpr())
      );
      if (!source_attribute) {
        Py_DECREF(attributes);
        throw PyInternalError();
    ...
    RETURN_NEW_AST_NODE("TorQLXTag", "{s:s#,s:N}", "kind", opening.data(), opening.size(), "attributes", attributes);
  }
...
class TorQLErrorListener : public antlr4::BaseErrorListener {
 public:
  string input;

  TorQLErrorListener(string input) : input(input) {}
...
#define METHOD_PARSE_NODE(PASCAL_CASE, CAMEL_CASE, SNAKE_CASE)                                                         \
  static PyObject* method_parse_##SNAKE_CASE(PyObject* self, PyObject* args, PyObject* kwargs) {                       \
    ...
    auto lexer = new TorQLLexer(input_stream);                                                                         \
    auto stream = new antlr4::CommonTokenStream(lexer);                                                                \
    auto parser = new TorQLParser(stream);                                                                             \
    parser->removeErrorListeners();                                                                                    \
    auto error_listener = new TorQLErrorListener(str);                                                                 \
    parser->addErrorListener(error_listener);                                                                          \
    TorQLParser::PASCAL_CASE##Context* parse_tree;                                                                     \
    try {                                                                                                              \
      parse_tree = parser->CAMEL_CASE();                                                                               \
    } catch HANDLE_TORQL_ERROR(SyntaxError, delete error_listener; delete parser; delete stream; delete lexer;         \
                                  delete input_stream;)                                                                \
    ...
    TorQLParseTreeConverter converter = TorQLParseTreeConverter(state, internal == 1);                                 \
...
static PyObject* method_parse_string_literal_text(PyObject* self, PyObject* args) {
  ...
  } catch HANDLE_TORQL_ERROR(SyntaxError, );
...
static PyMethodDef parser_methods[] = {
    {.ml_name = "parse_expr",
     ...
     .ml_doc = "Parse the TorQL expression string into an AST"},
    ...
    {.ml_name = "parse_select",
     ...
     .ml_flags = METH_VARARGS | METH_KEYWORDS,
     .ml_doc = "Parse the TorQL SELECT statement string into an AST"},
...
PyMODINIT_FUNC PyInit_torql_parser(void) {
