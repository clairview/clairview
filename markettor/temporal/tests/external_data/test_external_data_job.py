...
from markettor.temporal.data_imports.external_data_job import (
...
from markettor.temporal.data_imports.workflow_activities.check_billing_limits import check_billing_limits_activity
from markettor.temporal.data_imports.workflow_activities.create_job_model import (
    CreateExternalDataJobModelActivityInputs,
    create_external_data_job_model_activity,
)
from markettor.temporal.data_imports.workflow_activities.import_data import ImportDataActivityInputs, import_data_activity
from markettor.warehouse.external_data_source.jobs import create_external_data_job
...
from markettor.temporal.data_imports.pipelines.schemas import (
    PIPELINE_TYPE_SCHEMA_DEFAULT_MAPPING,
)
from markettor.models import Team
from markettor.temporal.data_imports.pipelines.pipeline import DataImportPipeline
...
from markettor.warehouse.models.external_data_schema import get_all_schemas_for_source_id
...
async def test_update_external_job_activity_with_non_retryable_error(activity_environment, team, **kwargs):
    ...
    with mock.patch("markettor.warehouse.models.external_data_schema.external_data_workflow_exists", return_value=False):
        await activity_environment.run(update_external_data_job_model, inputs)
...
async def test_external_data_job_workflow_with_schema(team, **kwargs):
    ...
    with (
        mock.patch("markettor.warehouse.models.table.DataWarehouseTable.get_columns", return_value={"id": "string"}),
...
async def test_run_postgres_job(
    ...
    await postgres_connection.execute(
        "CREATE TABLE IF NOT EXISTS {schema}.markettor_test (id integer)".format(schema=postgres_config["schema"])
    )
    await postgres_connection.execute(
        "INSERT INTO {schema}.markettor_test (id) VALUES (1)".format(schema=postgres_config["schema"])
    )
    ...
    async def setup_job_1():
        ...
        markettor_test_schema = await _create_schema("markettor_test", new_source, team)
        ...
        inputs = ImportDataActivityInputs(
            team_id=team.id, run_id=str(new_job.pk), source_id=new_source.pk, schema_id=markettor_test_schema.id
        )
    ...
    with (
        ...
        job_1_team_objects = await minio_client.list_objects_v2(
            Bucket=BUCKET_NAME, Prefix=f"{folder_path}/markettor_test/"
        )
...
async def test_check_schedule_activity_with_missing_schema_id_but_with_schedule(activity_environment, team, **kwargs):
    ...
    with (
        mock.patch(
            "markettor.temporal.data_imports.external_data_job.a_external_data_workflow_exists", return_value=True
        ),
        mock.patch(
            "markettor.temporal.data_imports.external_data_job.a_delete_external_data_schedule", return_value=True
        ),
        mock.patch(
            "markettor.temporal.data_imports.external_data_job.a_trigger_external_data_workflow"
...
async def test_check_schedule_activity_with_missing_schema_id_and_no_schedule(activity_environment, team, **kwargs):
    ...
    with (
        mock.patch(
            "markettor.temporal.data_imports.external_data_job.a_external_data_workflow_exists", return_value=False
        ),
        mock.patch(
            "markettor.temporal.data_imports.external_data_job.a_delete_external_data_schedule", return_value=True
        ),
        mock.patch(
            "markettor.temporal.data_imports.external_data_job.a_sync_external_data_job_workflow"
