...
from markettor.batch_exports.service import BatchExportModel, BatchExportSchema
from markettor.temporal.batch_exports.batch_exports import (
    finish_batch_export_run,
    iter_model_records,
    start_batch_export_run,
)
from markettor.temporal.batch_exports.s3_batch_export import (
...
from markettor.temporal.common.clickhouse import ClickHouseClient
from markettor.temporal.tests.batch_exports.utils import mocked_start_batch_export_run
from markettor.temporal.tests.utils.events import (
    generate_test_events_in_clickhouse,
)
from markettor.temporal.tests.utils.models import (
    acreate_batch_export,
    adelete_batch_export,
    afetch_batch_export_runs,
)
from markettor.temporal.tests.utils.s3 import read_parquet_from_s3, read_s3_data_as_json
...
def s3_key_prefix(request):
    ...
    try:
        ...
        return f"markettor-data-{str(uuid.uuid4())}"
...
@pytest.mark.parametrize(
    "s3_key_prefix",
    [
        "markettor-{table}/{year}-{month}-{day}/{hour}:{minute}:{second}",
        "markettor-{table}/{hour}:{minute}:{second}/{year}-{month}-{day}",
        "markettor-{table}/{hour}:{minute}:{second}",
        "markettor/{year}-{month}-{day}/{hour}:{minute}:{second}",
        "{year}-{month}-{day}",
    ],
    indirect=True,
)
...
async def test_s3_export_workflow_with_request_timeouts(
    ...
    async with (
        ...
        with (
            mock.patch("markettor.temporal.batch_exports.s3_batch_export.aioboto3.Session", FakeSession),
            mock.patch("markettor.temporal.batch_exports.batch_exports.RetryPolicy", DoNotRetryPolicy),
        ):
            await activity_environment.client.execute_workflow(
                S3BatchExportWorkflow.run,
                inputs,
