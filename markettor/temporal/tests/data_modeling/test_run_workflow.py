...
from markettor.temporal.tests.utils.events import generate_test_events_in_clickhouse
...
from markettor.warehouse.models.modeling import DataWarehouseModelPath
...
async def test_run_dag_activity_activity_materialize_mocked(activity_environment, ateam, dag, markettor_tables):
    ...
    with unittest.mock.patch("markettor.temporal.data_modeling.run_workflow.materialize_model", new=magic_mock):
        async with asyncio.timeout(10):
            results = await activity_environment.run(run_dag_activity, run_dag_activity_inputs)

        models_materialized = [model for model in dag.keys() if model not in markettor_tables]
...
async def test_run_dag_activity_activity_skips_if_ancestor_failed_mocked(
    activity_environment, ateam, dag, make_fail, markettor_tables
):
    ...
    """
    run_dag_activity_inputs = RunDagActivityInputs(team_id=ateam.pk, dag=dag)
    assert all(model not in markettor_tables for model in make_fail), "MarketTor tables cannot fail"
    ...
    with unittest.mock.patch("markettor.temporal.data_modeling.run_workflow.materialize_model", new=magic_mock):
        ...
        models_materialized = [model for model in expected_failed | expected_completed if model not in markettor_tables]
