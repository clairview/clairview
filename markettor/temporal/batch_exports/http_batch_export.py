...
from markettor.batch_exports.service import (
    BatchExportField,
    BatchExportModel,
    BatchExportSchema,
    HttpBatchExportInputs,
)
from markettor.models import BatchExportRun
...
from markettor.temporal.batch_exports.metrics import (
    get_bytes_exported_metric,
    get_rows_exported_metric,
)
from markettor.temporal.batch_exports.temporary_file import (
    BatchExportTemporaryFile,
    json_dumps_bytes,
)
from markettor.temporal.common.clickhouse import get_client
...
async def insert_into_http_activity(inputs: HttpInsertInputs) -> RecordsCompleted:
    ...
    async with get_client(team_id=inputs.team_id) as client:
        ...
        # The HTTP destination currently only supports the MarketTor batch capture endpoint. In the
        # future we may support other endpoints, but we'll need a way to template the request body,
        # headers, etc.
        #
        # For now, we write the batch out in MarketTor capture format, which means each Batch Export
        ...
        markettor_batch_header = """{{"api_key": "{}","historical_migration":true,"batch": [""".format(inputs.token)
        markettor_batch_footer = "]}"

        with BatchExportTemporaryFile() as batch_file:

            def write_event_to_batch(event):
                if batch_file.records_since_last_reset == 0:
                    batch_file.write(markettor_batch_header)
                else:
                    batch_file.write(",")
            ...
            async def flush_batch_to_http_endpoint(last_uploaded_timestamp: str, session: aiohttp.ClientSession):
                ...
                batch_file.write(markettor_batch_footer)
            ...
            async with aiohttp.ClientSession() as session:
                for record_batch in record_iterator:
                    for row in record_batch.select(columns).to_pylist():
                        # Format result row as MarketTor event, write JSON to the batch file.
...
class HttpBatchExportWorkflow(MarketTorWorkflow):
