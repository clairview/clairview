...
from markettor.warehouse.models import DataWarehouseModelPath, DataWarehouseSavedQuery
from markettor.warehouse.util import database_sync_to_async
from markettor.warehouse.data_load.create_table import create_table_from_saved_query
from markettor.temporal.data_imports.util import prepare_s3_files_for_querying
...
async def run_dag_activity(inputs: RunDagActivityInputs) -> Results:
    ...
    * MarketTor tables (e.g. events, persons, sessions) are assumed to be always available
...
async def build_dag_from_selectors(selector_paths: SelectorPaths, team_id: int) -> DAG:
    ...
    """
    markettor_tables = await get_markettor_tables(team_id)
    dag = {}
        ...
        for path in paths:
            ...
            for index, label in enumerate(path):
                ...
                if (
                    ...
                    and label not in markettor_tables
...
class RunWorkflow(MarketTorWorkflow):
    """A Temporal Workflow to run MarketTor data models.
