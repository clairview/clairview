...
class TestDataImportPipeline(APIBaseTest):
    ...
    async def test_pipeline_non_incremental(self):
        ...
        with (
            ...
            patch(
                "markettor.temporal.data_imports.pipelines.pipeline.validate_schema_and_update_table"
            ) as mock_validate_schema_and_update_table,
            patch("markettor.temporal.data_imports.pipelines.pipeline.get_delta_tables"),
            patch("markettor.temporal.data_imports.pipelines.pipeline.update_last_synced_at"),
        ):
