...
from markettor.torql.placeholders import find_placeholders
...
class TorQLQueryRunner(QueryRunner):
    query: TorQLQuery
    response: TorQLQueryResponse
    cached_response: CachedTorQLQueryResponse
    ...
    def calculate(self) -> TorQLQueryResponse:
        ...
        if isinstance(query, ast.SelectQuery) and not query.limit:
            paginator = TorQLHasMorePaginator.from_limit_context(limit_context=self.limit_context)
        func = cast(
            Callable[..., TorQLQueryResponse],
            execute_torql_query if paginator is None else paginator.execute_torql_query,
        )
        response = func(
    ...
    def apply_dashboard_filters(self, dashboard_filter: DashboardFilter):
        self.query.filters = self.query.filters or TorQLFilters()
