...
from markettor.torql_queries.utils.recordings_helper import RecordingsHelper
from markettor.models import Team, Group
from markettor.schema import ActorsQuery
...
class ActorStrategy:
    ...
    def __init__(self, team: Team, query: ActorsQuery, paginator: TorQLHasMorePaginator):
        self.team = team
...
class PersonStrategy(ActorStrategy):
    ...
    def get_actors(self, actor_ids, order_by: str = "") -> dict[str, dict]:
        ...
        persons_query = """SELECT markettor_person.id, markettor_person.uuid, markettor_person.properties, markettor_person.is_identified, markettor_person.created_at
            FROM markettor_person
            WHERE markettor_person.uuid = ANY(%(uuids)s)
            AND markettor_person.team_id = %(team_id)s"""
        if order_by:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                """SELECT markettor_persondistinctid.person_id, markettor_persondistinctid.distinct_id
            FROM markettor_persondistinctid
            WHERE markettor_persondistinctid.person_id = ANY(%(people_ids)s)
            AND markettor_persondistinctid.team_id = %(team_id)s""",
