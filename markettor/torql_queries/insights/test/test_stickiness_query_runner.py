...
from markettor.test.base import APIBaseTest, _create_event, _create_person
...
class TestStickinessQueryRunner(APIBaseTest):
    ...
    def test_property_filtering_torql(self):
        self._create_test_events()

        response = self._run_query(properties=[TorQLPropertyFilter(key="properties.$browser == 'Chrome'")])
    ...
    def test_torql_aggregations(self):
        self._create_test_events()

        series: list[EventsNode | ActionsNode] = [
            EventsNode(event="$pageview", math="torql", math_torql="e.properties.prop")
        ]
