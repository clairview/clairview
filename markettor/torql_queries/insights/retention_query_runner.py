...
from markettor.torql.property import entity_to_expr
...
from markettor.torql_queries.utils.query_date_range import QueryDateRangeWithIntervals
...
from markettor.schema import RetentionQuery, RetentionType
...
class RetentionQueryRunner(QueryRunner):
        ...
        return ast.And(
            exprs=[
                ast.CompareOperation(
                    ...
                    right=self.query_date_range.get_start_of_interval_torql(),
                ),
                ast.CompareOperation(
    ...
    def actor_query(self, breakdown_values_filter: Optional[int] = None, cumulative: bool = False) -> ast.SelectQuery:
        start_of_interval_sql = self.query_date_range.get_start_of_interval_torql(
        ...
        if event_query_type == RetentionQueryType.TARGET_FIRST_TIME:
            target_timestamps = parse_expr(
                ...
                """,
                {
                    ...
                    "min_timestamp": self.query_date_range.date_to_start_of_interval_torql(
                        parse_expr(
                    ...
                    """,
                        {
                            ...
                            "date_from_start_of_interval": self.query_date_range.date_from_to_start_of_interval_torql(),
    ...
    def calculate(self) -> RetentionQueryResponse:
        ...
        response = execute_torql_query(
            ...
            settings=TorQLGlobalSettings(max_bytes_before_external_group_by=MAX_BYTES_BEFORE_EXTERNAL_GROUP_BY),
        )
        ...
        return RetentionQueryResponse(results=results, timings=response.timings, torql=torql, modifiers=self.modifiers)
