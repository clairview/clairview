...
from markettor.torql_queries.insights.trends.display import TrendsDisplay
from markettor.torql_queries.insights.trends.series_with_extras import SeriesWithExtras
from markettor.torql_queries.insights.trends.trends_actors_query_builder import TrendsActorsQueryBuilder
from markettor.torql_queries.insights.trends.trends_query_builder import TrendsQueryBuilder
from markettor.torql_queries.query_runner import QueryRunner
from markettor.torql_queries.utils.formula_ast import FormulaAST
from markettor.torql_queries.utils.query_compare_to_date_range import QueryCompareToDateRange
from markettor.torql_queries.utils.query_date_range import QueryDateRange
from markettor.torql_queries.utils.query_previous_period_date_range import (
...
from markettor.queries.util import correct_result_for_sampling
...
from markettor.utils import format_label_date, multisort
from markettor.warehouse.models.util import get_view_or_table_by_name
...
class TrendsQueryRunner(QueryRunner):
    ...
    def calculate(self):
        ...
        if len(queries) == 1:
            response_torql_query = queries[0]
        else:
            response_torql_query = ast.SelectUnionQuery(select_queries=[])
            for query in queries:
                if isinstance(query, ast.SelectQuery):
                    response_torql_query.select_queries.append(query)
                else:
                    response_torql_query.select_queries.extend(query.select_queries)

        with self.timings.measure("printing_torql_for_response"):
            response_torql = to_printed_torql(response_torql_query, self.team, self.modifiers)
        ...
        return TrendsQueryResponse(
            ...
            hasMore=has_more,
            timings=timings,
            torql=response_torql,
            modifiers=self.modifiers,
            error=". ".join(debug_errors),
        )

    def build_series_response(self, response: TorQLQueryResponse, series: SeriesWithExtras, series_count: int):
        def get_value(name: str, val: Any):
            if name not in ["date", "total", "breakdown_value"]:
                raise Exception("Column not found in torql results")
            if response.columns is None:
                raise Exception("No columns returned from torql results")
            ...
            if series.aggregate_values:
                series_object = {
                    ...
                    "action": {  # TODO: Populate missing props in `action`
                        ...
                        "math_torql": series.series.math_torql,
    ...
    def update_torql_modifiers(self) -> None:
        if (
    ...
    def _is_breakdown_field_boolean(
        ...
        if breakdown_type == "torql" or breakdown_type == "cohort" or breakdown_type == "session":
            return False
    ...
    # TODO: Move this to markettor/torql_queries/legacy_compatibility/query_to_filter.py
