...
from markettor.torql_queries.insights.trends.utils import get_properties_chain
...
def torql_to_string(expr: ast.Expr) -> ast.Call:
...
class Breakdown:
        ...
        if (
            ...
            return [
                ast.Alias(
                    ...
                    expr=torql_to_string(ast.Field(chain=["__in_cohort", "cohort_id"])),
                )
            ]
    ...
    def _get_breakdown_col_expr(
        ...
        if breakdown_type == "cohort":
            ...
            return ast.Alias(
                alias=alias,
                expr=torql_to_string(ast.Constant(value=cohort_breakdown)),
            )
