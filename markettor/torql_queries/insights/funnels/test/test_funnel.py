...
from markettor.api.instance_settings import get_instance_setting
from markettor.clickhouse.client.execute import sync_execute
from markettor.constants import INSIGHT_FUNNELS, FunnelOrderType, FunnelVizType
...
from markettor.torql_queries.actors_query_runner import ActorsQueryRunner
from markettor.torql_queries.insights.funnels import Funnel
from markettor.torql_queries.insights.funnels.funnel_query_context import FunnelQueryContext
from markettor.torql_queries.insights.funnels.funnels_query_runner import FunnelsQueryRunner
from markettor.torql_queries.insights.funnels.test.breakdown_cases import (
    assert_funnel_results_equal,
    funnel_breakdown_group_test_factory,
    funnel_breakdown_test_factory,
)
from markettor.torql_queries.insights.funnels.test.conversion_time_cases import (
...
from markettor.models import Action, Element
...
from markettor.queries.funnels import ClickhouseFunnelActors
...
@patch("markettoranalytics.feature_enabled", new=Mock(return_value=False))
    ...
    class TestGetFunnel(ClickhouseTestMixin, APIBaseTest):
        ...
        def test_funnel_person_prop(self):
            ...
            person_factory(
                distinct_ids=["with_property"],
                team_id=self.team.pk,
                properties={"email": "hello@markettor.com"},
            )
        ...
        def test_torql_aggregation(self):
            # first person
            person_factory(
                distinct_ids=["user"],
                team_id=self.team.pk,
                properties={"email": "lembitu@markettor.com", "common_prop": "yes"},
            )
            ...
            person_factory(
                distinct_ids=["second"],
                team_id=self.team.pk,
                properties={"email": "toomas@markettor.com", "common_prop": "yes"},
            )
            ...
            results = (
                self._basic_funnel(
                    filters={
                        **basic_filters,
                        "funnel_aggregate_by_torql": "properties.$session_id",
                    }
                )
                .calculate()
                .results
            ...
            # distinct_id
            results = (
                self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_torql": "distinct_id"})
                .calculate()
                .results
            )
            self.assertEqual(results[0]["count"], 2)
            ...
            # person_id
            results = (
                self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_torql": "person_id"})
                .calculate()
                .results
            )
            self.assertEqual(results[0]["count"], 2)
            ...
            result = (
                self._basic_funnel(
                    filters={**basic_filters, "funnel_aggregate_by_torql": "person.properties.common_prop"}
                )
                .calculate()
                .results
            )
...
class TestFunnelStepCountsWithoutAggregationQuery(BaseTest):
    ...
    def test_smoke(self):
        ...
        response = execute_torql_query(query_type="FunnelsQuery", query=query_ast, team=self.team)
