import itertools
from markettor.torql.constants import TorQLGlobalSettings, MAX_BYTES_BEFORE_EXTERNAL_GROUP_BY
...
from markettor.caching.insights_api import BASE_MINIMUM_INSIGHT_REFRESH_INTERVAL, REDUCED_MINIMUM_INSIGHT_REFRESH_INTERVAL
from markettor.constants import PAGEVIEW_EVENT, SCREEN_EVENT, TORQL
...
from markettor.torql_queries.insights.funnels.funnels_query_runner import FunnelsQueryRunner
from markettor.torql_queries.insights.funnels.utils import funnel_window_interval_unit_to_sql
...
from markettor.schema import PathsQuery
...
class PathsQueryRunner(QueryRunner):
    ...
    def construct_event_torql(self) -> ast.Expr:
        event_torql: ast.Expr = parse_expr("event")

        if self._should_query_event(TORQL) and self.query.pathsFilter.pathsTorQLExpression:
            event_torql = parse_expr(self.query.pathsFilter.pathsTorQLExpression)

        if self._should_query_event(PAGEVIEW_EVENT):
            event_torql = parse_expr(
                "if(event = {event}, replaceRegexpAll(ifNull(properties.$current_url, ''), '(.)/$', '\\\\1'), {event_torql})",
                {"event": ast.Constant(value=PAGEVIEW_EVENT), "event_torql": event_torql},
            )

        if self._should_query_event(SCREEN_EVENT):
            event_torql = parse_expr(
                "if(event = {event}, properties.$screen_name, {event_torql})",
                {"event": ast.Constant(value=SCREEN_EVENT), "event_torql": event_torql},
            )
    ...
    def funnel_join(self) -> ast.JoinExpr:
        ...
        from markettor.torql_queries.insights.insight_actors_query_runner import InsightActorsQueryRunner
    ...
    def paths_events_query(self) -> ast.SelectQuery:
        ...
        event_torql = self.construct_event_torql()
        event_conditional = parse_expr("ifNull({event_torql}, '') AS path_item_ungrouped", {"event_torql": event_torql})
        ...
        return ast.And(
            exprs=[
                ast.CompareOperation(
                    ...
                    right=self.query_date_range.date_from_to_start_of_interval_torql(),
                ),
                ast.CompareOperation(
                    op=ast.CompareOperationOp.LtEq,
                    left=field_to_compare,
                    right=self.query_date_range.date_to_as_torql(),
                ),
            ]
        )
    ...
    def calculate(self) -> PathsQueryResponse:
        ...
        return PathsQueryResponse(results=results, timings=response.timings, torql=torql, modifiers=self.modifiers)
