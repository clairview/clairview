...
from markettor.torql_queries.utils.query_previous_period_date_range import QueryPreviousPeriodDateRange
...
class StickinessQueryRunner(QueryRunner):
    ...
    def _aggregation_expressions(self, series: EventsNode | ActionsNode | DataWarehouseNode) -> ast.Expr:
        if series.math == "torql" and series.math_torql is not None:
            return parse_expr(series.math_torql)
    ...
    def _events_query(self, series_with_extra: SeriesWithExtras) -> ast.SelectQuery:
        num_intervals_column_expr = ast.Alias(
            ...
            expr=ast.Call(
                distinct=True,
                name="count",
                args=[self.query_date_range.date_to_start_of_interval_torql(ast.Field(chain=["e", "timestamp"]))],
            ),
        )
    ...
    def to_actors_query(self, interval_num: Optional[int] = None) -> ast.SelectQuery | ast.SelectUnionQuery:
        ...
        for series in self.series:
            ...
            if series.series.math == "torql" and series.series.math_torql is not None:
                aggregation_alias = "actor_id"
