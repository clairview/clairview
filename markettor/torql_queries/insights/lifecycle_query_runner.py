...
from markettor.torql.property import property_to_expr, action_to_expr
...
from markettor.utils import format_label_date
...
class LifecycleQueryRunner(QueryRunner):
            ...
            if day is not None:
                exprs.append(
                    ast.CompareOperation(
                        ...
                        right=self.query_date_range.date_to_start_of_interval_torql(
    ...
    def calculate(self) -> LifecycleQueryResponse:
        ...
        # e.g. the LifecycleQuery with TorQLQuery, while keeping the chart logic the same.
        ...
        return LifecycleQueryResponse(results=res, timings=response.timings, torql=torql, modifiers=self.modifiers)
            ...
            events_query = parse_select(
                ...
                placeholders={
                    **self.query_date_range.to_placeholders(),
                    ...
                    "trunc_timestamp": self.query_date_range.date_to_start_of_interval_torql(
                        ast.Field(chain=["events", "timestamp"])
                    ),
                    "trunc_created_at": self.query_date_range.date_to_start_of_interval_torql(
                        ast.Field(chain=["created_at"])
                    ),
                    "trunc_epoch": self.query_date_range.date_to_start_of_interval_torql(
    ...
    def periods_query(self):
        with self.timings.measure("periods_query"):
            periods_query = parse_select(
                ...
                placeholders={
                    **self.query_date_range.to_placeholders(),
                    "date_to_plus_interval": self.query_date_range.date_to_start_of_interval_torql(
                        parse_expr(
