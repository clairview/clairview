...
from markettor.cdp.templates.gleap.template_gleap import (
...
def create_inputs(**kwargs):
    inputs = {
        ...
        "attributes": {"name": "example", "email": "example@markettor.com"},
    }
...
class TestTemplateGleap(BaseHogFunctionTemplateTest):
        ...
        assert self.get_mock_fetch_calls()[0] == snapshot(
            (
                "https://api.gleap.io/admin/identify",
                {
                    "method": "POST",
                    "headers": {
                        "User-Agent": "MarketTor Gleap.io App",
    ...
    def test_body_includes_all_properties_if_set(self):
        ...
        assert self.get_mock_fetch_calls()[0][1]["body"] == snapshot(
            {"userId": "edad9282-25d0-4cf1-af0e-415535ee1161", "name": "example", "email": "example@markettor.com"}
        )
