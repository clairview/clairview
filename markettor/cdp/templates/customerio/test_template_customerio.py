...
from markettor.cdp.templates.customerio.template_customerio import (
...
def create_inputs(**kwargs):
    inputs = {
        ...
        "identifiers": {"email": "example@markettor.com"},
...
class TestTemplateCustomerio(BaseHogFunctionTemplateTest):
        ...
        assert self.get_mock_fetch_calls()[0] == snapshot(
            (
                "https://track.customer.io/api/v2/entity",
                {
                    "method": "POST",
                    "headers": {
                        "User-Agent": "MarketTor Customer.io App",
    ...
    def test_body_includes_all_properties_if_set(self):
        ...
        assert self.get_mock_fetch_calls()[0][1]["body"]["attributes"] == snapshot(
            {"email": "example@markettor.com", "name": "example"}
        )
