...
class TemplateSendGridMigrator(HogFunctionTemplateMigrator):
    plugin_url = "https://github.com/MarketTor/sendgrid-plugin"
    ...
    def migrate(cls, obj):
        ...
        if customFields:
            for field in customFields.split(","):
                if "=" in field:
                    markettor_prop, sendgrid_field = field.split("=")
                else:
                    markettor_prop = sendgrid_field = field.strip()
                markettor_prop = f"{{person.properties.{markettor_prop}}}"
                if sendgrid_field in sendgrid_fields:
                    hf["inputs"]["properties"]["value"][sendgrid_field] = markettor_prop
                else:
                    hf["inputs"]["custom_fields"]["value"][sendgrid_field] = markettor_prop
