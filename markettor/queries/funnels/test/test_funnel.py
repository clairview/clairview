...
from markettor.constants import FILTER_TEST_ACCOUNTS, INSIGHT_FUNNELS
...
from markettor.queries.funnels import ClickhouseFunnel, ClickhouseFunnelActors
from markettor.queries.funnels.test.breakdown_cases import (
    assert_funnel_results_equal,
    funnel_breakdown_test_factory,
)
from markettor.queries.funnels.test.conversion_time_cases import (
...
def funnel_test_factory(Funnel, event_factory, person_factory):
    class TestGetFunnel(ClickhouseTestMixin, APIBaseTest):
        ...
        def test_breakdown_values_is_set_on_the_query_with_fewer_than_two_entities(self):
            """
            failing test for https://sentry.io/organizations/markettor/issues/2807609211/?project=1899813&referrer=slack
            """
        ...
        def test_torql_aggregation(self):
            ...
            # distinct_id
            result = self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_torql": "distinct_id"}).run()
            self.assertEqual(result[0]["count"], 2)
            self.assertEqual(result[1]["count"], 1)
            self.assertEqual(result[2]["count"], 1)

            # person_id
            result = self._basic_funnel(filters={**basic_filters, "funnel_aggregate_by_torql": "person_id"}).run()
            self.assertEqual(result[0]["count"], 2)
            self.assertEqual(result[1]["count"], 1)
            self.assertEqual(result[2]["count"], 1)
