...
from markettor.settings.base_variables import DEBUG, IS_COLLECT_STATIC, TEST
...
if TEST or DEBUG:
    ...
    PG_USER: str = os.getenv("PGUSER", "markettor")
    PG_PASSWORD: str = os.getenv("PGPASSWORD", "markettor")
    PG_PORT: str = os.getenv("PGPORT", "5432")
    PG_DATABASE: str = os.getenv("PGDATABASE", "markettor")
...
if read_host:
    ...
    DATABASE_ROUTERS = ["markettor.dbrouter.ReplicaRouter"]
...
# Clickhouse Settings
CLICKHOUSE_TEST_DB: str = "markettor" + SUFFIX
...
CLICKHOUSE_CLUSTER: str = os.getenv("CLICKHOUSE_CLUSTER", "markettor")
...
if not REDIS_URL:
    raise ImproperlyConfigured(
        ...
        "If upgrading from MarketTor 1.0.10 or earlier, see here: "
        "https://markettor.com/docs/deployment/upgrading-markettor#upgrading-from-before-1011"
    )
...
if not CDP_FUNCTION_EXECUTOR_API_URL:
    CDP_FUNCTION_EXECUTOR_API_URL = (
        "http://localhost:6738" if DEBUG else "http://ingestion-cdp-function-callbacks.markettor.svc.cluster.local"
    )
...
CACHES = {
    "default": {
        ...
        "OPTIONS": {
            ...
            "COMPRESSOR": "markettor.caching.tolerant_zlib_compressor.TolerantZlibCompressor",
        },
        "KEY_PREFIX": "markettor",
    }
}
