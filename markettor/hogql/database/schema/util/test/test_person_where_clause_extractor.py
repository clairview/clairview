...
from markettor.hogql.parser import parse_select, parse_expr
from markettor.hogql.printer import prepare_ast_for_printing, print_ast
from markettor.hogql.visitor import clone_expr, CloningVisitor
from markettor.models import PropertyDefinition
from markettor.schema import PersonsOnEventsMode, PersonsArgMaxVersion
...
class TestPersonWhereClauseExtractor(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_person_properties(self):
        actual = self.get_clause("SELECT * FROM events WHERE person.properties.email = 'jimmy@markettor.com'")
        expected = _expr("properties.email = 'jimmy@markettor.com'")
        assert actual == expected

    def test_person_properties_andor_1(self):
        actual = self.get_clause("SELECT * FROM events WHERE person.properties.email = 'jimmy@markettor.com' or false")
        expected = _expr("properties.email = 'jimmy@markettor.com'")
        assert actual == expected

    def test_person_properties_andor_2(self):
        actual = self.get_clause("SELECT * FROM events WHERE person.properties.email = 'jimmy@markettor.com' and false")
        assert actual is None

    def test_person_properties_andor_3(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE person.properties.email = 'jimmy@markettor.com' and person.properties.email = 'timmy@markettor.com'"
        )
        expected = _expr("properties.email = 'jimmy@markettor.com' and properties.email = 'timmy@markettor.com'")
        assert actual == expected

    def test_person_properties_andor_4(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE person.properties.email = 'jimmy@markettor.com' or person.properties.email = 'timmy@markettor.com'"
        )
        expected = _expr("properties.email = 'jimmy@markettor.com' or properties.email = 'timmy@markettor.com'")
        assert actual == expected

    def test_person_properties_andor_5(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE person.properties.email = 'jimmy@markettor.com' or (1 and person.properties.email = 'timmy@markettor.com')"
        )
        expected = _expr("properties.email = 'jimmy@markettor.com' or properties.email = 'timmy@markettor.com'")
        assert actual == expected
    ...
    def test_person_properties_andor_6(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE person.properties.email = 'jimmy@markettor.com' or (0 or person.properties.email = 'timmy@markettor.com')"
        )
        expected = _expr("properties.email = 'jimmy@markettor.com' or properties.email = 'timmy@markettor.com'")
        assert actual == expected
    ...
    def test_person_properties_andor_7(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE person.properties.email = 'jimmy@markettor.com' or (1 or person.properties.email = 'timmy@markettor.com')"
        )
        assert actual is None

    def test_person_properties_andor_8(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE event == '$pageview' and person.properties.email = 'jimmy@markettor.com'"
        )
        expected = _expr("properties.email = 'jimmy@markettor.com'")
        assert actual == expected
    ...
    def test_person_properties_andor_9(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE event == '$pageview' or person.properties.email = 'jimmy@markettor.com'"
        )
        assert actual is None

    def test_person_properties_andor_10(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE properties.email = 'bla@markettor.com' or person.properties.email = 'jimmy@markettor.com'"
        )
        assert actual is None

    def test_person_properties_andor_11(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE properties.email = 'bla@markettor.com' and person.properties.email = 'jimmy@markettor.com'"
        )
        expected = _expr("properties.email = 'jimmy@markettor.com'")
        assert actual == expected
    ...
    def test_person_properties_function_calls(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE properties.email = 'bla@markettor.com' and toString(person.properties.email) = 'jimmy@markettor.com'"
        )
        expected = _expr("toString(properties.email) = 'jimmy@markettor.com'")
        assert actual == expected

    def test_person_properties_function_call_args(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE properties.email = 'bla@markettor.com' and substring(person.properties.email, 10) = 'jimmy@markettor.com'"
        )
        expected = _expr("substring(properties.email, 10) = 'jimmy@markettor.com'")
        assert actual == expected

    def test_person_properties_function_call_args_complex(self):
        actual = self.get_clause(
            "SELECT * FROM events WHERE properties.email = 'bla@markettor.com' and substring(person.properties.email, event = 'bla') = 'jimmy@markettor.com'"
        )
        assert actual is None
