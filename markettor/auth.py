...
from markettor.jwt import MarkettorJwtAudience, decode_jwt
from markettor.models.personal_api_key import (
    PersonalAPIKey,
    hash_key_value,
...
class PersonalAPIKeyAuthentication(authentication.BaseAuthentication):
    ...
    def validate_key(cls, personal_api_key_with_source):
        from markettor.models import PersonalAPIKey
        ...
        if (
            ...
            if not request.GET.get("temporary_token"):
                raise AuthenticationFailed(
                    ...
                    + "See https://markettor.com/docs/deployment/running-behind-proxy for more information."
                )
        if request.GET.get("temporary_token"):
            User = apps.get_model(app_label="markettor", model_name="User")
