...
from markettor.api.person import MinimalPersonSerializer
from markettor.api.routing import TeamAndOrgViewSetMixin
from markettor.api.utils import action, safe_clickhouse_string
from markettor.auth import PersonalAPIKeyAuthentication, SharingAccessTokenAuthentication
from markettor.cloud_utils import is_cloud
from markettor.constants import SESSION_RECORDINGS_FILTER_IDS
...
from markettor.models.person.person import PersonDistinctId
...
from markettor.session_recordings.queries.session_recording_properties import (
    SessionRecordingProperties,
)
from markettor.session_recordings.queries.session_replay_events import SessionReplayEvents
from markettor.session_recordings.realtime_snapshots import (
    get_realtime_snapshots,
    publish_subscription,
)
from markettor.session_recordings.snapshots.convert_legacy_snapshots import (
...
class SessionRecordingViewSet(TeamAndOrgViewSetMixin, viewsets.GenericViewSet, UpdateModelMixin):
    ...
    def persist(self, request: request.Request, *args: Any, **kwargs: Any) -> Response:
        ...
        if not settings.EE_AVAILABLE:
            raise exceptions.ValidationError("LTS persistence is only available in the full version of MarketTor")
    ...
    def summarize(self, request: request.Request, **kwargs):
        ...
        if not environment_is_allowed or not has_openai_api_key:
            raise exceptions.ValidationError("session summary is only supported in MarketTor Cloud")

        if not markettoranalytics.feature_enabled("ai-session-summary", str(user.distinct_id)):
        ...
        markettoranalytics.capture(event="session summarized", distinct_id=str(user.distinct_id), properties=summary)
    ...
    def similar_sessions(self, request: request.Request, **kwargs):
        ...
        if not markettoranalytics.feature_enabled("session-replay-similar-recordings", str(user.distinct_id)):
    ...
    def error_clusters(self, request: request.Request, **kwargs):
        ...
        if not markettoranalytics.feature_enabled("session-replay-error-clustering", str(user.distinct_id)):
    ...
    def _stream_blob_to_client(
        ...
        with STREAM_RESPONSE_TO_CLIENT_HISTOGRAM.time():
            ...
            with stream_from(url=url, headers=headers) as streaming_response:
                ...
                # but it helps avoid network latency from the client to MarketTor, then to object storage, and back again
...
def safely_read_modifiers_overrides(distinct_id: str, team: Team) -> HogQLQueryModifiers:
    ...
    try:
        ...
        flags_n_bags = markettoranalytics.get_all_flags_and_payloads(
            distinct_id,
            groups=groups,
        )
        # this loads nothing whereas the payload is available
        # modifier_overrides = markettoranalytics.get_feature_flag_payload(
        #     flag_key,
        #     distinct_id,
        #     groups=groups,
        # )
