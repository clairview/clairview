...
from markettor.api.early_access_feature import early_access_features
from markettor.api.survey import surveys
from markettor.constants import PERMITTED_FORUM_DOMAINS
from markettor.demo.legacy import demo_route
...
from .year_in_markettor import year_in_markettor
...
def home(request, *args, **kwargs):
    if request.get_host().split(":")[0] == "app.markettor.com" and get_instance_setting("REDIRECT_APP_TO_US"):
        url = "https://us.markettor.com{}".format(request.get_full_path())
        if url_has_allowed_host_and_scheme(url, "us.markettor.com", True):
...
urlpatterns = [
    ...
    *ee_urlpatterns,
    ...
    path("year_in_markettor/2022/<str:user_uuid>", year_in_markettor.render_2022),
    path("year_in_markettor/2022/<str:user_uuid>/", year_in_markettor.render_2022),
    path("year_in_markettor/2023/<str:user_uuid>", year_in_markettor.render_2023),
    path("year_in_markettor/2023/<str:user_uuid>/", year_in_markettor.render_2023),
]
...
if settings.TEST:
    # Used in markettor-js e2e tests
    @csrf_exempt
