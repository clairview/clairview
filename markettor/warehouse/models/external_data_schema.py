...
from markettor.models.utils import CreatedMetaFields, DeletedMetaFields, UUIDModel, UpdatedMetaFields, sane_repr
import uuid
import psycopg2
import pymysql
...
from markettor.warehouse.types import IncrementalFieldType
...
class ExternalDataSchema(CreatedMetaFields, UpdatedMetaFields, UUIDModel, DeletedMetaFields):
    ...
    source = models.ForeignKey("markettor.ExternalDataSource", related_name="schemas", on_delete=models.CASCADE)
    table = models.ForeignKey("markettor.DataWarehouseTable", on_delete=models.SET_NULL, null=True, blank=True)
...
def get_mssql_schemas(
    ...
    def get_schemas(postgres_host: str, postgres_port: int):
        # Importing pymssql requires mssql drivers to be installed locally - see markettor/warehouse/README.md
        import pymssql
