...
UPDATE_PATHS_QUERY = """\
insert into markettor_datawarehousemodelpath (
  id,
  team_id,
  table_id,
  saved_query_id,
  ...
  select
    ...
    now() as updated_at
  from
    markettor_datawarehousemodelpath as model_path,
    (
      select
        path
      from markettor_datawarehousemodelpath
...
delete from markettor_datawarehousemodelpath
where
  team_id = %(team_id)s
  and id in (
    select id
...
class UnknownParentError(Exception):
    ...
    def __init__(self, parent: str, query: str):
        super().__init__(
            f"The parent name {parent} does not correspond to an existing MarketTor table, Data Warehouse Table, or Data Warehouse Saved Query."
        )
        self.query = query
...
class DataWarehouseModelPathManager(models.Manager["DataWarehouseModelPath"]):
    ...
    def get_or_create_root_path_for_markettor_source(
        self, markettor_source_name: str, team: Team
        ...
        Raises:
            ValueError: If the provided `markettor_source_name` is not a MarketTor table.
    ...
    def get_or_create_query_parent_paths(self, query: str, team: Team) -> list["DataWarehouseModelPath"]:
        ...
        for parent in get_parents_from_model_query(query):
            try:
                parent_path, _ = self.get_or_create_root_path_for_markettor_source(parent, team)
            except ValueError:
                pass
            else:
    ...
    def get_longest_common_ancestor_path(
        ...
        query = "select lca(array_agg(path)) from markettor_datawarehousemodelpath where path ? %(lqueries)s"
...
class DataWarehouseModelPath(CreatedMetaFields, UpdatedMetaFields, UUIDModel):
    ...
    MarketTor tables and external data source tables.
    """
