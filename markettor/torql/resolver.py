...
from markettor.torql.ast import ConstantType, FieldTraverserType
...
from markettor.torql.database.schema.persons import PersonsTable
from markettor.torql.errors import ImpossibleASTError, QueryError, ResolutionError
from markettor.torql.functions import find_torql_markettor_function
from markettor.torql.functions.action import matches_action
from markettor.torql.functions.cohort import cohort_query_node
from markettor.torql.functions.mapping import TORQL_CLICKHOUSE_FUNCTIONS, compare_types, validate_function_args
from markettor.torql.functions.recording_button import recording_button
from markettor.torql.functions.sparkline import sparkline
from markettor.torql.torqlx import TORQLX_COMPONENTS, convert_to_hx
from markettor.torql.parser import parse_select
from markettor.torql.resolver_utils import expand_torqlx_query, lookup_cte_by_name, lookup_field_by_name
from markettor.torql.visitor import CloningVisitor, TraversingVisitor, clone_expr
...
def resolve_types_from_table(
    expr: ast.Expr, table_name: str, context: TorQLContext, dialect: Literal["torql", "clickhouse"]
) -> ast.Expr:
...
class Resolver(CloningVisitor):
    ...
    def visit_join_expr(self, node: ast.JoinExpr):
        ...
        if isinstance(node.table, ast.TorQLXTag):
            node.table = expand_torqlx_query(node.table, self.context.team_id)
    ...
    def visit_torqlx_tag(self, node: ast.TorQLXTag):
        if node.kind in TORQLX_COMPONENTS:
            return self.visit(convert_to_hx(node))
        return self.visit(expand_torqlx_query(node, self.context.team_id))
    ...
    def visit_call(self, node: ast.Call):
        ...
        if func_meta := find_torql_markettor_function(node.name):
        ...
        if node.name in TORQL_CLICKHOUSE_FUNCTIONS:
            signatures = TORQL_CLICKHOUSE_FUNCTIONS[node.name].signatures
            if signatures:
        ...
        if return_type is None:
            ...
            # Uncomment once all torql mappings are complete with signatures
    ...
    def visit_expr_call(self, node: ast.ExprCall):
        raise QueryError("You can only call simple functions in TorQL, not expressions")

    def visit_block(self, node: ast.Block):
        raise QueryError("You can not use blocks in TorQL")
