...
from markettor.torql.torqlx import convert_tag_to_hx, convert_dict_to_hx, convert_to_hx
from markettor.test.base import BaseTest


class TestTorQLX(BaseTest):
    def test_convert_tag_to_hx(self):
        tag = ast.TorQLXTag(kind="Sparkline", attributes=[])
        self.assertEqual(
            convert_tag_to_hx(tag), ast.Tuple(exprs=[ast.Constant(value="__hx_tag"), ast.Constant(value="Sparkline")])
        )

        tag = ast.TorQLXTag(
            kind="Sparkline", attributes=[ast.TorQLXAttribute(name="color", value=ast.Constant(value="red"))]
        )
        self.assertEqual(
    ...
    def test_convert_to_hx(self):
        ...
        self.assertEqual(
            convert_to_hx(ast.TorQLXTag(kind="Sparkline", attributes=[])),
