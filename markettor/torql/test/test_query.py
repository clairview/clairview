...
from markettor.torql.property import property_to_expr
from markettor.torql.query import execute_torql_query
from markettor.torql.test.utils import pretty_print_in_tests, pretty_print_response_in_tests
from markettor.models import Cohort
from markettor.models.cohort.util import recalculate_cohortpeople
from markettor.models.utils import UUIDT, uuid7
from markettor.session_recordings.queries.test.session_replay_sql import (
    produce_replay_summary,
)
from markettor.schema import TorQLFilters, EventPropertyFilter, DateRange, QueryTiming, SessionPropertyFilter
...
class TestQuery(ClickhouseTestMixin, APIBaseTest):
    ...
    def _create_random_events(self) -> str:
        ...
        _create_person(
            properties={"sneaky_mail": "tim@markettor.com", "random_uuid": random_uuid},
            team=self.team,
            distinct_ids=["bla"],
            is_identified=True,
        )
    ...
    def test_extended_query_time(self):
        self.assertEqual(TORQL_INCREASED_MAX_EXECUTION_TIME, 600)
    ...
    def test_query_distinct(self):
        with freeze_time("2020-01-10"):
            ...
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            self.assertEqual(response.results, [("tim@markettor.com",)])
    ...
    def test_query_joins_simple(self):
        with freeze_time("2020-01-10"):
            ...
            self.assertEqual(response.results[0][4], "tim@markettor.com")
    ...
    def test_query_joins_pdi_person_properties(self):
        with freeze_time("2020-01-10"):
            ...
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            self.assertEqual(response.results[0][0], "bla")
            self.assertEqual(response.results[0][1], "tim@markettor.com")
    ...
    def test_query_joins_events_pdi_person_properties(self):
        with freeze_time("2020-01-10"):
            ...
            self.assertEqual(response.results[0][0], "random event")
            self.assertEqual(response.results[0][2], "bla")
            self.assertEqual(response.results[0][3], "tim@markettor.com")
    ...
    def test_query_joins_events_person_properties(self):
        with freeze_time("2020-01-10"):
            ...
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            self.assertEqual(response.results[0][0], "random event")
            self.assertEqual(response.results[0][2], "tim@markettor.com")
    ...
    def test_query_joins_events_person_properties_in_aggregration(self):
        with freeze_time("2020-01-10"):
            ...
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            self.assertEqual(response.results[0][0], "tim@markettor.com")
    ...
    def test_join_with_property_materialized_session_id(self):
        with freeze_time("2020-01-10"):
            ...
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            assert pretty_print_in_tests(response.torql, self.team.pk) == self.snapshot
    ...
    def test_window_functions_simple(self):
        ...
        """
        response = execute_torql_query(query, team=self.team)
    ...
    def test_property_access_with_arrays(self):
        with freeze_time("2020-01-10"):
            ...
            self.assertEqual(
                f"SELECT "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_0)s), ''), 'null'), '^\"|\"$', '') AS string, "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_1)s, %(torql_val_2)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_3)s, %(torql_val_4)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_5)s, %(torql_val_6)s, %(torql_val_7)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_8)s, %(torql_val_9)s, %(torql_val_10)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_11)s, %(torql_val_12)s, %(torql_val_13)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_14)s, %(torql_val_15)s, %(torql_val_16)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_17)s, %(torql_val_18)s, %(torql_val_19)s, %(torql_val_20)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_21)s, %(torql_val_22)s, %(torql_val_23)s, %(torql_val_24)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_25)s, %(torql_val_26)s, %(torql_val_27)s, %(torql_val_28)s, %(torql_val_29)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_30)s, %(torql_val_31)s, %(torql_val_32)s, %(torql_val_33)s, %(torql_val_34)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_35)s, %(torql_val_36)s, %(torql_val_37)s, %(torql_val_38)s, %(torql_val_39)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_40)s, %(torql_val_41)s, %(torql_val_42)s), ''), 'null'), '^\"|\"$', ''), "
                f"replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_43)s, %(torql_val_44)s, %(torql_val_45)s), ''), 'null'), '^\"|\"$', '') "
                f"FROM events "
                f"WHERE and(equals(events.team_id, {self.team.pk}), ifNull(equals(replaceRegexpAll(nullIf(nullIf(JSONExtractRaw(events.properties, %(torql_val_46)s), ''), 'null'), '^\"|\"$', ''), %(torql_val_47)s), 0)) "
                f"LIMIT 100 "
    ...
    def test_property_access_with_arrays_zero_index_error(self):
        ...
        with self.assertRaises(QueryError) as e:
            execute_torql_query(query, team=self.team)
    ...
    def test_torql_query_filters(self):
        with freeze_time("2020-01-10"):
            ...
            response = execute_torql_query(query, team=self.team, filters=filters, placeholders=placeholders)
            self.assertEqual(len(response.results), 1)

    def test_torql_query_filters_empty_true(self):
        ...
        self.assertEqual(response.torql, "SELECT event FROM events WHERE true LIMIT 100")

    def test_torql_query_filters_double_error(self):
        query = "SELECT event from events where {filters}"
    ...
    def test_torql_query_filters_alias(self):
        with freeze_time("2020-01-10"):
            random_uuid = self._create_random_events()
    ...
    @pytest.mark.usefixtures("unittest_snapshot")
    def test_torql_union_all_limits(self):
    ...
    @pytest.mark.usefixtures("unittest_snapshot")
    def test_torql_query_session_filters(self):
        with freeze_time("2024-07-05"):
            ...
            assert response.torql is not None
            assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot
            assert pretty_print_in_tests(response.torql, self.team.pk) == self.snapshot
            self.assertEqual(response.results, [(s1, "https://example.com/1")])

    @pytest.mark.usefixtures("unittest_snapshot")
    def test_torql_query_filters_session_date_range(self):
        with freeze_time("2024-07-05"):
            s1 = str(uuid7("2024-07-03", 42))
            ...
            query = "SELECT session_id, $entry_current_url from sessions WHERE {filters}"
            filters = TorQLFilters(dateRange=DateRange(date_from="2024-07-04", date_to="2024-07-06"))
            response = execute_torql_query(
                query,
                team=self.team,
                filters=filters,
