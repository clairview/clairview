...
from markettor.torql.ast import UUIDType, TorQLXTag, TorQLXAttribute
from markettor.torql.errors import InternalTorQLError
from markettor.torql.parser import parse_expr
from markettor.torql.visitor import CloningVisitor, Visitor, TraversingVisitor
...
class TestVisitor(BaseTest):
    ...
    def test_unknown_error_visitor(self):
        ...
        with self.assertRaises(InternalTorQLError) as e:
    ...
    def test_torql_exception_start_end(self):
        class EternalVisitor(TraversingVisitor):
            def visit_constant(self, node: ast.Constant):
                if node.value == 616:
                    raise InternalTorQLError("You tried accessing a forbidden number, perish!")
    ...
    def test_torql_visitor_naming_exceptions(self):
        class NamingCheck(Visitor):
            def visit_uuid_type(self, node: ast.Constant):
                return "visit_uuid_type"

            def visit_torqlx_tag(self, node: ast.Constant):
                return "visit_torqlx_tag"

            def visit_torqlx_attribute(self, node: ast.Constant):
                return "visit_torqlx_attribute"

        assert NamingCheck().visit(UUIDType()) == "visit_uuid_type"
        assert NamingCheck().visit(TorQLXAttribute(name="a", value="a")) == "visit_torqlx_attribute"
        assert NamingCheck().visit(TorQLXTag(kind="", attributes=[])) == "visit_torqlx_tag"
