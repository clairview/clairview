...
from .database.database import create_torql_database, serialize_database
...
IDENTITY_MESSAGE = "TorQL is MarketTor's variant of SQL. It supports most of ClickHouse SQL. You write TorQL based on a prompt. You don't help with other knowledge."

TORQL_EXAMPLE_MESSAGE = """Example TorQL query for prompt "weekly active users that performed event ACTIVATION_EVENT on example.com/foo/ 3 times or more, by week":
...
REQUEST_MESSAGE = (
    "I need a robust TorQL query to get the following results: {prompt}\n"
...
def write_sql_from_prompt(prompt: str, *, current_query: Optional[str] = None, team: "Team", user: "User") -> str:
    database = create_torql_database(team.pk)
    ...
    generated_valid_torql = False
    attempt_count = 0
    prompt_tokens_last, completion_tokens_last = 0, 0
    prompt_tokens_total, completion_tokens_total = 0, 0
    for _ in range(3):  # Try up to 3 times in case the generated SQL is not valid TorQL
        attempt_count += 1
        ...
        try:
            ...
            generated_valid_torql = True
            break

    report_user_action(
        user,
        "generated TorQL with AI",
        {
            "prompt": prompt,
            "response": candidate_sql or error,
            "result": ("valid_torql" if generated_valid_torql else "invalid_torql")
            if candidate_sql
            else "prompt_unclear",
