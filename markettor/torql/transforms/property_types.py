...
from markettor.torql.escape_sql import escape_torql_identifier
from markettor.torql.visitor import CloningVisitor, TraversingVisitor
...
def build_property_swapper(node: ast.Expr, context: TorQLContext) -> None:
    ...
    def visit_property_type(self, node: ast.PropertyType):
        if node.field_type.name == "properties" and len(node.chain) == 1:
            if isinstance(node.field_type.table_type, ast.BaseTableType):
                ...
                table_name = table_type.resolve_database_table(self.context).to_printed_torql()
...
class PropertySwapper(CloningVisitor):
    ...
    def visit_field(self, node: ast.Field):
        ...
        if isinstance(type, ast.PropertyType) and type.field_type.name == "person_properties" and len(type.chain) == 1:
            ...
            if isinstance(type.field_type.table_type, ast.BaseTableType):
                table = type.field_type.table_type.resolve_database_table(self.context).to_printed_torql()
                if table == "events":
                    if property_name in self.person_properties:
                        return self._convert_string_property_to_type(node, "person", property_name)
    ...
    def _convert_string_property_to_type(
        ...
        if property_type == "person":
            markettor_field_type = self.person_properties.get(property_name)
        elif property_type == "group":
            markettor_field_type = self.group_properties.get(property_name)
        else:
            markettor_field_type = self.event_properties.get(property_name)

        field_type = "Float" if markettor_field_type == "Numeric" else markettor_field_type or "String"
    ...
    def _add_notice(self, node: ast.Field, message: str):
        ...
        self.context.add_notice(
            start=max(node.start, node.end - len(escape_torql_identifier(node.chain[-1]))),
            end=node.end,
            message=message,
        )
