...
from markettor.torql.database.models import LazyTableToAdd, LazyJoinToAdd
from markettor.torql.errors import ResolutionError
from markettor.torql.resolver import resolve_types
from markettor.torql.resolver_utils import get_long_table_name
from markettor.torql.transforms.property_types import PropertySwapper
...
class LazyTableResolver(TraversingVisitor):
    ...
    def __init__(
        ...
        self.dialect: Literal["torql", "clickhouse"] = dialect
    ...
    def visit_property_type(self, node: ast.PropertyType):
        ...
        if isinstance(table_type, ast.LazyJoinType) or isinstance(table_type, ast.LazyTableType):
            if self.context and self.context.within_non_torql_query:
                # If we're in a non-TorQL query, traverse deeper, just like we normally would have.
