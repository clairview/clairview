...
from markettor.torql.test.utils import pretty_print_response_in_tests
...
from markettor.schema import TorQLQueryModifiers, InCohortVia
...
class TestInCohort(BaseTest):
    ...
    def test_in_cohort_dynamic(self):
        ...
        response = execute_torql_query(
            ...
            self.team,
            modifiers=TorQLQueryModifiers(inCohortVia=InCohortVia.LEFTJOIN),
            pretty=False,
        )
    ...
    def test_in_cohort_error(self):
        with self.assertRaises(QueryError) as e:
            execute_torql_query(
                ...
                self.team,
                modifiers=TorQLQueryModifiers(inCohortVia=InCohortVia.SUBQUERY),
                pretty=False,
            )
    ...
    def test_in_cohort_conjoined_string(self):
        ...
        response = execute_torql_query(
            f"SELECT event FROM events WHERE person_id IN COHORT 'my cohort'",
            self.team,
            modifiers=TorQLQueryModifiers(inCohortVia=InCohortVia.LEFTJOIN_CONJOINED),
            pretty=False,
        )
        assert pretty_print_response_in_tests(response, self.team.pk) == self.snapshot  # type: ignore
