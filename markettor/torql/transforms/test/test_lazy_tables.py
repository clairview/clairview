...
from markettor.schema import TorQLQueryModifiers, PersonsOnEventsMode
...
class TestLazyJoins(BaseTest):
    ...
    def _print_select(self, select: str, modifiers: TorQLQueryModifiers | None = None):
        ...
        query = print_ast(
            ...
            TorQLContext(
                ...
                modifiers=modifiers if modifiers is not None else TorQLQueryModifiers(),
            ),
            "clickhouse",
        )
    ...
    def test_resolve_lazy_table_indirectly_referenced(self):
        ...
        printed = self._print_select(
            ...
            TorQLQueryModifiers(personsOnEventsMode=PersonsOnEventsMode.PERSON_ID_OVERRIDE_PROPERTIES_JOINED),
        )
        assert printed == self.snapshot
