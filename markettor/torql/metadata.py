...
from markettor.torql.parser import parse_select, parse_program, parse_expr, parse_string_template
...
from markettor.schema import TorQLMetadataResponse, TorQLMetadata, TorQLNotice, HogLanguage
from markettor.torql import ast


def get_torql_metadata(
    query: TorQLMetadata,
    team: Team,
) -> TorQLMetadataResponse:
    response = TorQLMetadataResponse(
        isValid=True,
        isValidView=False,
        query=query.query,
        errors=[],
    ...
    try:
        ...
        if query.language == HogLanguage.HOG:
            ...
            create_bytecode(program, supported_functions={"fetch", "markettorCapture"}, args=[], context=context)
        elif query.language == HogLanguage.HOG_TEMPLATE:
            string = parse_string_template(query.query)
            create_bytecode(string, supported_functions={"fetch", "markettorCapture"}, args=[], context=context)
        ...
        if isinstance(e, ExposedTorQLError):
            error = str(e)
            if "mismatched input '<EOF>' expecting" in error:
                error = "Unexpected end of query"
            if e.end and e.start and e.end < e.start:
                response.errors.append(TorQLNotice(message=error, start=e.end, end=e.start))
            else:
                response.errors.append(TorQLNotice(message=error, start=e.start, end=e.end))
        elif not settings.DEBUG:
            # We don't want to accidentally expose too much data via errors
            response.errors.append(TorQLNotice(message=f"Unexpected {e.__class__.__name__}"))
