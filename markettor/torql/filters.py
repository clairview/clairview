...
from markettor.torql.visitor import CloningVisitor
from markettor.models import Team
from markettor.schema import TorQLFilters, SessionPropertyFilter
...
def replace_filters(node: T, filters: Optional[TorQLFilters], team: Team) -> T:
    return ReplaceFilters(filters, team).visit(node)


class ReplaceFilters(CloningVisitor):
    def __init__(self, filters: Optional[TorQLFilters], team: Team = None):
        super().__init__()
