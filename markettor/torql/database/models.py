...
from markettor.torql.errors import ResolutionError, NotImplementedError

if TYPE_CHECKING:
    from markettor.torql.context import TorQLContext
    from markettor.torql.ast import SelectQuery, LazyJoinType
    from markettor.torql.base import ConstantType
...
class DatabaseField(FieldOrTable):
    ...
    def get_constant_type(self) -> "ConstantType":
        from markettor.torql.ast import UnknownType
...
class IntegerDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from markettor.torql.ast import IntegerType
...
class FloatDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from markettor.torql.ast import FloatType
...
class StringDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from markettor.torql.ast import StringType
...
class DateDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from markettor.torql.ast import DateType
...
class DateTimeDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from markettor.torql.ast import DateTimeType
...
class BooleanDatabaseField(DatabaseField):
    def get_constant_type(self) -> "ConstantType":
        from markettor.torql.ast import BooleanType
...
class Table(FieldOrTable):
    ...
    def to_printed_clickhouse(self, context: "TorQLContext") -> str:
        raise NotImplementedError("Table.to_printed_clickhouse not overridden")

    def to_printed_torql(self) -> str:
        raise NotImplementedError("Table.to_printed_torql not overridden")
...
class LazyJoin(FieldOrTable):
    ...
    join_function: Callable[["LazyJoinToAdd", "TorQLContext", "SelectQuery"], Any]
    join_table: Table | str
    from_field: list[str | int]
    to_field: Optional[list[str | int]] = None

    def resolve_table(self, context: "TorQLContext") -> Table:
...
class LazyTable(Table):
    """
    A table that is replaced with a subquery returned from `lazy_select(requested_fields: Dict[name, chain], modifiers: TorQLQueryModifiers, node: SelectQuery)`
    """
...
class SavedQuery(Table):
    ...
    def to_printed_clickhouse(self, context):
        from markettor.warehouse.models import validate_saved_query_name
