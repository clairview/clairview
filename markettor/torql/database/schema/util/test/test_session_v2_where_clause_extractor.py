...
from markettor.torql.database.schema.util.where_clause_extractor import SessionMinTimestampWhereClauseExtractorV2
...
from markettor.test.base import ClickhouseTestMixin, APIBaseTest
...
SELECT
    ...
    def test_not_like(self):
        # based on a bug here: https://markettor.slack.com/archives/C05LJK1N3CP/p1719916566421079
        where = ast.And(
            exprs=[
...
class TestSessionsV2QueriesTorQLToClickhouse(ClickhouseTestMixin, APIBaseTest):
...
FROM
    (SELECT
        ...
        min(toTimeZone(raw_sessions.min_timestamp, %(torql_val_0)s)) AS `$start_timestamp`,
        raw_sessions.session_id_v7 AS session_id_v7
    FROM
        raw_sessions
    WHERE
        and(equals(raw_sessions.team_id, <TEAM_ID>), ifNull(greaterOrEquals(plus(fromUnixTimestamp(intDiv(toUInt64(bitShiftRight(raw_sessions.session_id_v7, 80)), 1000)), toIntervalDay(3)), %(torql_val_1)s), 0))
    GROUP BY
        raw_sessions.session_id_v7,
        raw_sessions.session_id_v7) AS sessions
WHERE
    ifNull(greater(sessions.`$start_timestamp`, %(torql_val_2)s), 0)
LIMIT 50000\
""")
...
FROM
    ...
    FROM
        raw_sessions
    WHERE
        and(equals(raw_sessions.team_id, <TEAM_ID>), ifNull(greaterOrEquals(plus(fromUnixTimestamp(intDiv(toUInt64(bitShiftRight(raw_sessions.session_id_v7, 80)), 1000)), toIntervalDay(3)), %(torql_val_0)s), 0))
    GROUP BY
        raw_sessions.session_id_v7,
        raw_sessions.session_id_v7) AS sessions ON equals(events.`$session_id`, sessions.session_id)
WHERE
    and(equals(events.team_id, <TEAM_ID>), greater(toTimeZone(events.timestamp, %(torql_val_1)s), %(torql_val_2)s))
GROUP BY
    sessions.session_id
LIMIT 50000\
""")
...
FROM
    events
    LEFT JOIN (SELECT
        dateDiff(%(torql_val_0)s, min(toTimeZone(raw_sessions.min_timestamp, %(torql_val_1)s)), max(toTimeZone(raw_sessions.max_timestamp, %(torql_val_2)s))) AS `$session_duration`,
        raw_sessions.session_id_v7 AS session_id_v7
    FROM
        raw_sessions
    WHERE
    ...
    GROUP BY
        raw_sessions.session_id_v7,
        raw_sessions.session_id_v7) AS events__session ON equals(toUInt128(accurateCastOrNull(events.`$session_id`, %(torql_val_3)s)), events__session.session_id_v7)
WHERE
    and(equals(events.team_id, <TEAM_ID>), less(toTimeZone(events.timestamp, %(torql_val_4)s), today()))
LIMIT 50000\
""")
...
SELECT
    count(DISTINCT e.`$session_id`) AS total,
    toStartOfDay(toTimeZone(e.timestamp, %(torql_val_8)s)) AS day_start,
    multiIf(and(ifNull(greaterOrEquals(e__session.`$session_duration`, 2.0), 0), ifNull(less(e__session.`$session_duration`, 4.5), 0)), %(torql_val_9)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 4.5), 0), ifNull(less(e__session.`$session_duration`, 27.0), 0)), %(torql_val_10)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 27.0), 0), ifNull(less(e__session.`$session_duration`, 44.0), 0)), %(torql_val_11)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 44.0), 0), ifNull(less(e__session.`$session_duration`, 48.0), 0)), %(torql_val_12)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 48.0), 0), ifNull(less(e__session.`$session_duration`, 57.5), 0)), %(torql_val_13)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 57.5), 0), ifNull(less(e__session.`$session_duration`, 61.0), 0)), %(torql_val_14)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 61.0), 0), ifNull(less(e__session.`$session_duration`, 74.0), 0)), %(torql_val_15)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 74.0), 0), ifNull(less(e__session.`$session_duration`, 90.0), 0)), %(torql_val_16)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 90.0), 0), ifNull(less(e__session.`$session_duration`, 98.5), 0)), %(torql_val_17)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 98.5), 0), ifNull(less(e__session.`$session_duration`, 167.01), 0)), %(torql_val_18)s, %(torql_val_19)s) AS breakdown_value
FROM
    events AS e SAMPLE 1
    ...
    FROM
        raw_sessions
    WHERE
        and(equals(raw_sessions.team_id, <TEAM_ID>), ifNull(greaterOrEquals(plus(fromUnixTimestamp(intDiv(toUInt64(bitShiftRight(raw_sessions.session_id_v7, 80)), 1000)), toIntervalDay(3)), toStartOfDay(assumeNotNull(parseDateTime64BestEffortOrNull(%(torql_val_3)s, 6, %(torql_val_4)s)))), 0), ifNull(lessOrEquals(minus(fromUnixTimestamp(intDiv(toUInt64(bitShiftRight(raw_sessions.session_id_v7, 80)), 1000)), toIntervalDay(3)), assumeNotNull(parseDateTime64BestEffortOrNull(%(torql_val_5)s, 6, %(torql_val_6)s))), 0))
    GROUP BY
        raw_sessions.session_id_v7,
        raw_sessions.session_id_v7) AS e__session ON equals(toUInt128(accurateCastOrNull(e.`$session_id`, %(torql_val_7)s)), e__session.session_id_v7)
WHERE
    and(equals(e.team_id, <TEAM_ID>), and(greaterOrEquals(toTimeZone(e.timestamp, %(torql_val_20)s), toStartOfDay(assumeNotNull(parseDateTime64BestEffortOrNull(%(torql_val_21)s, 6, %(torql_val_22)s)))), lessOrEquals(toTimeZone(e.timestamp, %(torql_val_23)s), assumeNotNull(parseDateTime64BestEffortOrNull(%(torql_val_24)s, 6, %(torql_val_25)s))), equals(e.event, %(torql_val_26)s), ifNull(in(if(not(empty(e__override.distinct_id)), e__override.person_id, e.person_id), (SELECT
...
SELECT
    ...
    min(toTimeZone(s.min_first_timestamp, %(torql_val_5)s)) AS start_time
FROM
    session_replay_events AS s
    LEFT JOIN (SELECT
        path(nullIf(nullIf(argMinMerge(raw_sessions.entry_url), %(torql_val_0)s), %(torql_val_1)s)) AS `$entry_pathname`,
        raw_sessions.session_id_v7 AS session_id_v7
    FROM
        raw_sessions
    WHERE
        and(equals(raw_sessions.team_id, <TEAM_ID>), ifNull(greaterOrEquals(plus(fromUnixTimestamp(intDiv(toUInt64(bitShiftRight(raw_sessions.session_id_v7, 80)), 1000)), toIntervalDay(3)), %(torql_val_2)s), 0), ifNull(lessOrEquals(minus(fromUnixTimestamp(intDiv(toUInt64(bitShiftRight(raw_sessions.session_id_v7, 80)), 1000)), toIntervalDay(3)), now64(6, %(torql_val_3)s)), 0))
    GROUP BY
        raw_sessions.session_id_v7,
        raw_sessions.session_id_v7) AS s__session ON equals(toUInt128(accurateCastOrNull(s.session_id, %(torql_val_4)s)), s__session.session_id_v7)
WHERE
    and(equals(s.team_id, <TEAM_ID>), ifNull(equals(s__session.`$entry_pathname`, %(torql_val_6)s), 0), ifNull(greaterOrEquals(toTimeZone(s.min_first_timestamp, %(torql_val_7)s), %(torql_val_8)s), 0), ifNull(less(toTimeZone(s.min_first_timestamp, %(torql_val_9)s), now64(6, %(torql_val_10)s)), 0))
GROUP BY
    s.session_id
LIMIT 50000\
""")
...
FROM
    ...
    FROM
        raw_sessions
    WHERE
        and(equals(raw_sessions.team_id, <TEAM_ID>), ifNull(greaterOrEquals(plus(fromUnixTimestamp(intDiv(toUInt64(bitShiftRight(raw_sessions.session_id_v7, 80)), 1000)), toIntervalDay(3)), minus(now64(6, %(torql_val_1)s), toIntervalDay(7))), 0))
    GROUP BY
        raw_sessions.session_id_v7,
        raw_sessions.session_id_v7) AS sessions
WHERE
    ifNull(greaterOrEquals(sessions.`$start_timestamp`, minus(now64(6, %(torql_val_2)s), toIntervalDay(7))), 0)
LIMIT 50000\
""")
