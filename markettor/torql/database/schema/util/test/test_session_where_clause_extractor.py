...
from markettor.torql.database.schema.util.where_clause_extractor import SessionMinTimestampWhereClauseExtractorV1
...
class TestSessionsQueriesTorQLToClickhouse(ClickhouseTestMixin, APIBaseTest):
...
FROM
    (SELECT
        sessions.session_id AS session_id,
        min(toTimeZone(sessions.min_timestamp, %(torql_val_0)s)) AS `$start_timestamp`
    FROM
        sessions
    WHERE
        and(equals(sessions.team_id, <TEAM_ID>), ifNull(greaterOrEquals(plus(toTimeZone(sessions.min_timestamp, %(torql_val_1)s), toIntervalDay(3)), %(torql_val_2)s), 0))
    GROUP BY
        sessions.session_id,
        sessions.session_id) AS sessions
WHERE
    ifNull(greater(sessions.`$start_timestamp`, %(torql_val_3)s), 0)
LIMIT 50000\
"""
        )
...
FROM
    ...
    FROM
        sessions
    WHERE
        and(equals(sessions.team_id, <TEAM_ID>), ifNull(greaterOrEquals(plus(toTimeZone(sessions.min_timestamp, %(torql_val_0)s), toIntervalDay(3)), %(torql_val_1)s), 0))
    GROUP BY
        sessions.session_id,
        sessions.session_id) AS sessions ON equals(events.`$session_id`, sessions.session_id)
WHERE
    and(equals(events.team_id, <TEAM_ID>), greater(toTimeZone(events.timestamp, %(torql_val_2)s), %(torql_val_3)s))
GROUP BY
    sessions.session_id
LIMIT 50000\
"""
...
FROM
    events
    LEFT JOIN (SELECT
        dateDiff(%(torql_val_0)s, min(toTimeZone(sessions.min_timestamp, %(torql_val_1)s)), max(toTimeZone(sessions.max_timestamp, %(torql_val_2)s))) AS `$session_duration`,
        sessions.session_id AS session_id
    FROM
        sessions
    WHERE
        and(equals(sessions.team_id, <TEAM_ID>), ifNull(lessOrEquals(minus(toTimeZone(sessions.min_timestamp, %(torql_val_3)s), toIntervalDay(3)), today()), 0))
    GROUP BY
        sessions.session_id,
...
SELECT
    count(DISTINCT e.`$session_id`) AS total,
    toStartOfDay(toTimeZone(e.timestamp, %(torql_val_9)s)) AS day_start,
    multiIf(and(ifNull(greaterOrEquals(e__session.`$session_duration`, 2.0), 0), ifNull(less(e__session.`$session_duration`, 4.5), 0)), %(torql_val_10)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 4.5), 0), ifNull(less(e__session.`$session_duration`, 27.0), 0)), %(torql_val_11)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 27.0), 0), ifNull(less(e__session.`$session_duration`, 44.0), 0)), %(torql_val_12)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 44.0), 0), ifNull(less(e__session.`$session_duration`, 48.0), 0)), %(torql_val_13)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 48.0), 0), ifNull(less(e__session.`$session_duration`, 57.5), 0)), %(torql_val_14)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 57.5), 0), ifNull(less(e__session.`$session_duration`, 61.0), 0)), %(torql_val_15)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 61.0), 0), ifNull(less(e__session.`$session_duration`, 74.0), 0)), %(torql_val_16)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 74.0), 0), ifNull(less(e__session.`$session_duration`, 90.0), 0)), %(torql_val_17)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 90.0), 0), ifNull(less(e__session.`$session_duration`, 98.5), 0)), %(torql_val_18)s, and(ifNull(greaterOrEquals(e__session.`$session_duration`, 98.5), 0), ifNull(less(e__session.`$session_duration`, 167.01), 0)), %(torql_val_19)s, %(torql_val_20)s) AS breakdown_value
FROM
    events AS e SAMPLE 1
    ...
    FROM
        sessions
    WHERE
        and(equals(sessions.team_id, <TEAM_ID>), ifNull(greaterOrEquals(plus(toTimeZone(sessions.min_timestamp, %(torql_val_3)s), toIntervalDay(3)), toStartOfDay(assumeNotNull(parseDateTime64BestEffortOrNull(%(torql_val_4)s, 6, %(torql_val_5)s)))), 0), ifNull(lessOrEquals(minus(toTimeZone(sessions.min_timestamp, %(torql_val_6)s), toIntervalDay(3)), assumeNotNull(parseDateTime64BestEffortOrNull(%(torql_val_7)s, 6, %(torql_val_8)s))), 0))
    GROUP BY
        sessions.session_id,
        sessions.session_id) AS e__session ON equals(e.`$session_id`, e__session.session_id)
WHERE
    and(equals(e.team_id, <TEAM_ID>), and(greaterOrEquals(toTimeZone(e.timestamp, %(torql_val_21)s), toStartOfDay(assumeNotNull(parseDateTime64BestEffortOrNull(%(torql_val_22)s, 6, %(torql_val_23)s)))), lessOrEquals(toTimeZone(e.timestamp, %(torql_val_24)s), assumeNotNull(parseDateTime64BestEffortOrNull(%(torql_val_25)s, 6, %(torql_val_26)s))), equals(e.event, %(torql_val_27)s), ifNull(in(if(not(empty(e__override.distinct_id)), e__override.person_id, e.person_id), (SELECT
...
SELECT
    ...
    min(toTimeZone(s.min_first_timestamp, %(torql_val_6)s)) AS start_time
FROM
    session_replay_events AS s
    LEFT JOIN (SELECT
        path(nullIf(nullIf(argMinMerge(sessions.entry_url), %(torql_val_0)s), %(torql_val_1)s)) AS `$entry_pathname`,
        sessions.session_id AS session_id
    FROM
        sessions
    WHERE
        and(equals(sessions.team_id, <TEAM_ID>), ifNull(greaterOrEquals(plus(toTimeZone(sessions.min_timestamp, %(torql_val_2)s), toIntervalDay(3)), %(torql_val_3)s), 0), ifNull(lessOrEquals(minus(toTimeZone(sessions.min_timestamp, %(torql_val_4)s), toIntervalDay(3)), now64(6, %(torql_val_5)s)), 0))
    GROUP BY
        sessions.session_id,
        sessions.session_id) AS s__session ON equals(s.session_id, s__session.session_id)
WHERE
    and(equals(s.team_id, <TEAM_ID>), ifNull(equals(s__session.`$entry_pathname`, %(torql_val_7)s), 0), ifNull(greaterOrEquals(toTimeZone(s.min_first_timestamp, %(torql_val_8)s), %(torql_val_9)s), 0), ifNull(less(toTimeZone(s.min_first_timestamp, %(torql_val_10)s), now64(6, %(torql_val_11)s)), 0))
GROUP BY
    s.session_id
LIMIT 50000\
"""
