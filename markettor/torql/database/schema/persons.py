...
from markettor.torql.ast import SelectQuery, And, CompareOperation, CompareOperationOp, Field, JoinExpr
...
from markettor.torql.database.argmax import argmax_select
...
from markettor.torql.database.schema.util.where_clause_extractor import WhereClauseExtractor
from markettor.torql.database.schema.persons_pdi import PersonsPDITable, persons_pdi_join
...
from markettor.schema import PersonsArgMaxVersion
...
def select_from_persons_table(
    ...
    *,
    ...
    if version == PersonsArgMaxVersion.V2:
        ...
        select.settings = TorQLQuerySettings(optimize_aggregation_in_order=True)
...
def join_with_persons_table(
    ...
    use_inner_join = (
        markettoranalytics.feature_enabled(
