...
from markettor.torql.database.s3_table import build_function_call
...
from markettor.torql.database.test.tables import create_aapl_stock_s3_table
from markettor.torql.errors import ExposedTorQLError
from markettor.warehouse.models.table import DataWarehouseTable
...
class TestS3Table(BaseTest):
    ...
    def test_s3_build_function_call_azure(self):
        res = build_function_call(
            "https://tommarkettortest.blob.core.windows.net/somecontainer/path/to/file.parquet",
        ...
        assert (
            res
            == "azureBlobStorage('https://tommarkettortest.blob.core.windows.net', 'somecontainer', 'path/to/file.parquet', 'tommarkettortest', 'blah', 'Parquet', 'auto', 'some structure')"
        )
    ...
    def test_s3_build_function_call_azure_without_structure(self):
        ...
        assert (
            res
            == "azureBlobStorage('https://tommarkettortest.blob.core.windows.net', 'somecontainer', 'path/to/file.parquet', 'tommarkettortest', 'blah', 'Parquet', 'auto')"
        )
