...
CELERY_TASK_DURATION_HISTOGRAM = Histogram(
    "markettor_celery_task_duration_seconds",
...
def receiver_setup_logging(loglevel, logfile, format, colorize, **kwargs) -> None:
    ...
    from markettor.settings import logs
...
def on_worker_start(**kwargs) -> None:
    ...
    from markettor.settings import sentry_init
...
def prerun_signal_handler(task_id, task, **kwargs):
    ...
    from markettor.clickhouse.client.connection import (
        Workload,
...
def postrun_signal_handler(task_id, task, **kwargs):
    from markettor.clickhouse.query_tagging import reset_query_tags
...
def setup_periodic_tasks(sender: Celery, **kwargs):
    ...
    from markettor.tasks.scheduled import setup_periodic_tasks
    ...
    try:
        ...
        if is_cloud():
            create_incident(
                ...
                "markettor.celery.setup_periodic_tasks",
                "critical",
            )
        else:
