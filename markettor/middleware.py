...
from markettor.api.capture import get_event
from markettor.api.decide import get_decide
from markettor.api.shared import UserBasicSerializer
from markettor.clickhouse.client.execute import clickhouse_query_counter
from markettor.clickhouse.query_tagging import QueryCounter, reset_query_tags, tag_queries
...
from markettor.models import Action, Cohort, Dashboard, FeatureFlag, Insight, Notebook, User, Team
from markettor.rate_limit import DecideRateThrottle
from markettor.settings import SITE_URL, DEBUG, PROJECT_SWITCHING_TOKEN_ALLOWLIST
        ...
        if ip:
            if settings.ALLOWED_IP_BLOCKS:
                ...
                if get_geoip_properties(ip).get("$geoip_country_code", None) not in settings.BLOCKED_GEOIP_REGIONS:
                    return response
        return HttpResponse(
            "MarketTor is not available in your region. If you think this is in error, please contact tim@markettor.com.",
            status=403,
        )
...
class AutoProjectMiddleware:
    ...
    Sometimes you get sent a link to MarketTor that points to an item from a different project than the one you currently
    ...
    def switch_team_if_allowed(self, new_team: Team, request: HttpRequest):
        ...
        user.save()
        # Information for MARKETTOR_APP_CONTEXT
...
def per_request_logging_context_middleware(
    ...
    def middleware(request: HttpRequest) -> HttpResponse:
        ...
        # that do not use Host header app.markettor.com. This is important as we
        # roll out CloudFront in front of app.markettor.com. We can get the host
...
class MarketTorTokenCookieMiddleware(SessionMiddleware):
    """
