...
from markettor.models import Organization, OrganizationMembership, Team, User
from markettor.models.personal_api_key import APIScopeObjectOrNotSupported
from markettor.utils import get_can_create_org
...
class MarketTorFeatureFlagPermission(BasePermission):
    def has_permission(self, request, view) -> bool:
        user = cast(User, request.user)
        organization = get_organization_from_view(view)
        flag = getattr(view, "markettor_feature_flag", None)

        config = {}

        if not flag:
            raise ImproperlyConfigured(
                "MarketTorFeatureFlagPermission requires the view to define the markettor_feature_flag attribute."
            )
        ...
        for required_flag, actions in config.items():
            if "*" in actions or view.action in actions:
                ...
                enabled = markettoranalytics.feature_enabled(
                    required_flag,
                    user.distinct_id,
