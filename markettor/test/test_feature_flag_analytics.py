...
import pytest
import uuid
from markettor.api.feature_flag import _create_usage_dashboard
...
from markettor.test.base import BaseTest, QueryMatchingTest, snapshot_postgres_queries_context
from markettor import redis
import datetime
import concurrent.futures
from markettor.test.base import _create_event, flush_persons_and_events
...
class TestFeatureFlagAnalytics(BaseTest, QueryMatchingTest):
    ...
    def test_increment_request_count_adds_requests_to_appropriate_buckets(self):
        ...
        with freeze_time("2022-05-07 12:23:07") as frozen_datetime:
            ...
            self.assertEqual(
                client.hgetall(f"markettor:decide_requests:{team_id}"),
                {b"165192618": b"10", b"165192619": b"5"},
            )
            self.assertEqual(
                client.hgetall(f"markettor:decide_requests:{other_team_id}"),
                {b"165192618": b"7", b"165192619": b"3"},
            )
            self.assertEqual(client.hgetall(f"markettor:decide_requests:other"), {})
    ...
    def test_no_interference_between_different_types_of_new_incoming_increments(self):
        ...
        with (
            ...
            self.assertEqual(
                client.hgetall(f"markettor:local_evaluation_requests:{team_id}"),
                {b"165192620": b"8"},
            )
            self.assertEqual(client.hgetall(f"markettor:decide_requests:{other_team_id}"), {})
            self.assertEqual(client.hgetall(f"markettor:local_evaluation_requests:{other_team_id}"), {})
