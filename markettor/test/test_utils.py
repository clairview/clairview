...
from markettor.api.test.mock_sentry import mock_sentry_context_for_tagging
...
class TestAbsoluteUrls(TestCase):
    def test_format_absolute_url(self) -> None:
        ...
        absolute_urls_test_cases = [
            ...
            (
                regression_11204,
                "https://app.markettor.com",
                f"https://app.markettor.com/{regression_11204}",
            ),
            (
                "https://app.markettor.com",
                "https://app.markettor.com",
            ...
            (
                "https://app.markettor.com/some/path?=something",
                "https://app.markettor.com",
                "https://app.markettor.com/some/path?=something",
            ),
            (
                "an.external.domain.com/something-outside-markettor",
                "https://app.markettor.com",
                "https://app.markettor.com/an.external.domain.com/something-outside-markettor",
            ),
    ...
    def test_absolute_uri_can_not_escape_out_host(self) -> None:
        with self.settings(SITE_URL="https://app.markettor.com"):
            with pytest.raises(PotentialSecurityProblemException):
                (absolute_uri("https://an.external.domain.com/something-outside-markettor"),)

    def test_absolute_uri_can_not_escape_out_host_on_different_scheme(self) -> None:
        with self.settings(SITE_URL="https://app.markettor.com"):
            with pytest.raises(PotentialSecurityProblemException):
                (absolute_uri("ftp://an.external.domain.com/something-outside-markettor"),)
...
class TestLoadDataFromRequest(TestCase):
    ...
    @patch("markettor.utils.configure_scope")
    ...
    @patch("markettor.utils.gzip")
    def test_can_decompress_gzipped_body_received_with_no_compression_flag(self, patched_gzip):
        # see https://sentry.io/organizations/markettor2/issues/3136510367
...
class TestShouldRefresh(TestCase):
    ...
    def test_can_get_period_to_compare_when_interval_is_day(self) -> None:
        """
        regression test see https://sentry.io/organizations/markettor/issues/3719740579/events/latest/?project=1899813&referrer=latest-event
        """
...
class TestUtilities(TestCase):
    def test_base64_decode(self):
        ...
        # from: https://markettor.sentry.io/issues/5680826999/events/ee804fe6ffd148559180c61d7c822766/
        ...
        # from: https://markettor.sentry.io/issues/5680826999/events/2ec7bfd975594873a84ce8153388c6e2/
