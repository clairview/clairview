...
from markettor import models, rate_limit
...
class TestUserAPI(APIBaseTest):
    ...
    @patch("markettor.rate_limit.SustainedRateThrottle.rate", new="5/hour")
    @patch("markettor.rate_limit.statsd.incr")
    @patch("markettor.rate_limit.is_rate_limit_enabled", return_value=True)
    ...
    @patch("markettor.rate_limit.ClickHouseBurstRateThrottle.rate", new="5/minute")
    @patch("markettor.rate_limit.statsd.incr")
    @patch("markettor.rate_limit.is_rate_limit_enabled", return_value=True)
    ...
    def test_rate_limits_are_based_on_api_key_not_user(self, rate_limit_enabled_mock, incr_mock):
        ...
        # Create a new user
        new_user = create_user(email="test@markettor.com", password="1234", organization=self.organization)
        ...
        new_user = create_user(email="test2@markettor.com", password="1234", organization=self.organization)
        new_personal_api_key = generate_random_token_personal()
        PersonalAPIKey.objects.create(label="X", user=new_user, secure_value=hash_key_value(new_personal_api_key))
    ...
    @patch("markettor.rate_limit.BurstRateThrottle.rate", new="5/minute")
    @patch("markettor.rate_limit.statsd.incr")
    @patch("markettor.rate_limit.is_rate_limit_enabled", return_value=False)
