...
from markettor.models import Action, Cohort, Dashboard, FeatureFlag, Insight
...
from markettor.test.base import APIBaseTest, override_settings
...
class TestAccessMiddleware(APIBaseTest):
    ...
    def test_ip_range(self):
        ...
        with self.settings(ALLOWED_IP_BLOCKS=["192.168.0.0/31", "127.0.0.0/25", "128.0.0.1"]):
            ...
            self.assertIn(b"MarketTor is not available", response.content)
            ...
            self.assertNotIn(b"MarketTor is not available", response.content)
...
class TestAutoProjectMiddleware(APIBaseTest):
    ...
    def test_project_redirects_to_markettor_org_style_tokens(self):
        res = self.client.get(
...
@override_settings(CLOUD_DEPLOYMENT="US")  # As MarketTor Cloud
class TestMarketTorTokenCookieMiddleware(APIBaseTest):
    CONFIG_AUTO_LOGIN = False
    ...
    def test_logged_in_client(self):
        ...
        self.assertEqual(ph_project_token_cookie["domain"], "markettor.com")
        ...
        self.assertEqual(ph_project_name_cookie["domain"], "markettor.com")
        ...
        self.assertEqual(ph_instance_cookie["domain"], "markettor.com")
...
class TestAutoLogoutImpersonateMiddleware(APIBaseTest):
    ...
    def setUp(self):
        ...
        self.other_user = User.objects.create_and_join(
            self.organization, email="other-user@markettor.com", password="123456"
        )
    ...
    def test_staff_user_can_login(self):
        ...
        assert self.client.get("/api/users/@me").json()["email"] == "other-user@markettor.com"
    ...
    def test_after_timeout_api_requests_401(self):
        ...
        with freeze_time(now):
            ...
            assert res.json()["email"] == "other-user@markettor.com"
    ...
    def test_after_timeout_redirects_to_logout_then_admin(self):
        ...
        with freeze_time(now + timedelta(seconds=35)):
            ...
            res = self.client.get("/api/users/@me")
            assert res.status_code == 200
            assert res.json()["email"] == "user1@markettor.com"
