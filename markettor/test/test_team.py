...
from markettor.models.team import get_team_in_cache, util
...
class TestTeam(BaseTest):
    ...
    def test_preinstalled_are_autoenabled(self, mock_get):
        with self.is_cloud(False):
            with self.settings(PLUGINS_PREINSTALLED_URLS=["https://github.com/MarketTor/helloworldplugin/"]):
    ...
    @mock.patch("markettoranalytics.feature_enabled", return_value=True)
    ...
    @mock.patch("markettoranalytics.feature_enabled", return_value=False)
    ...
    @mock.patch("markettor.models.project.Project.objects.create", side_effect=Exception)
