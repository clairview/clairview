...
from markettor.api.utils import get_data, get_token, safe_clickhouse_string
from markettor.cache_utils import cache_for
from markettor.exceptions import generate_exception_response
from markettor.kafka_client.client import KafkaProducer, session_recording_kafka_producer
from markettor.kafka_client.topics import (
...
from markettor.metrics import KLUDGES_COUNTER, LABEL_RESOURCE_TYPE
from markettor.models.utils import UUIDT
from markettor.redis import get_client
from markettor.session_recordings.session_recording_helpers import (
    preprocess_replay_events_for_blob_ingestion,
    split_replay_events,
    byte_size_dict,
)
from markettor.storage import object_storage
from markettor.utils import get_ip_address
from markettor.utils_cors import cors_response
...
# https://github.com/MarketTor/markettor/blob/master/plugin-server/src/worker/ingestion/person-state.ts#L22-L33
...
OVERFLOWING_REDIS_KEY = "@markettor/capture-overflow/"
...
def log_event(
    ...
    try:
        ...
        statsd.incr("markettor_cloud_plugin_server_ingestion")
        return future
    except Exception:
...
def _get_sent_at(data, request) -> tuple[Optional[datetime], Any]:
    try:
        if request.GET.get("_"):  # markettor-js
            sent_at = request.GET["_"]
        elif isinstance(data, dict) and data.get("sent_at"):  # markettor-android, markettor-ios
...
def lib_version_from_query_params(request) -> str:
    # url has a ver parameter from markettor-js
    return request.GET.get("ver", "unknown")


@csrf_exempt
@timed("markettor_cloud_event_endpoint")
def get_event(request):
    ...
    with start_span(op="request.authenticate"):
        ...
        if not token:
            return cors_response(
                ...
                generate_exception_response(
                    "capture",
                    "API key not provided. You can find your project API key in MarketTor project settings.",
    ...
    with start_span(op="request.process"):
        if isinstance(data, dict):
            if data.get("batch"):  # markettor-python and markettor-ruby
                if not historical:
    ...
    with start_span(op="kafka.produce") as span:
        ...
        for event, event_uuid, distinct_id in processed_events:
            try:
                ...
                statsd.incr("markettor_cloud_raw_endpoint_failure", tags={"endpoint": "capture"})
    ...
    try:
        ...
        # markettor-js will retry when it receives a 504, and it sends `retry_count` in the query params,
    ...
    statsd.incr("markettor_cloud_raw_endpoint_success", tags={"endpoint": "capture"})
...
def replace_with_warning(
    ...
    try:
        ...
        markettor_size_calculation = byte_size_dict(event)
        ...
        try:
            ...
            size_difference = kafka_size - markettor_size_calculation
        except:
            pass
        ...
        logger.info(
            ...
            markettor_calculation=markettor_size_calculation,
            lib_version=lib_version,
        )
...
def sample_replay_data_to_object_storage(
    ...
    """
    try:
        # capture more of markettor message too large since we know we're using latest versions
