...
from markettor.plugins.site import get_site_config_from_schema, get_transpiled_site_source


@csrf_exempt
@timed("markettor_cloud_site_app_endpoint")
def get_site_app(request: HttpRequest, id: int, token: str, hash: str) -> HttpResponse:
    try:
        ...
        response = f"{source}().inject({{config:{json.dumps(config)},markettor:window['__$$ph_site_app_{id}']}})"

        statsd.incr(f"markettor_cloud_raw_endpoint_success", tags={"endpoint": "site_app"})
        return HttpResponse(content=response, content_type="application/javascript")
    except Exception as e:
        capture_exception(e, {"data": {"id": id, "token": token}})
        statsd.incr("markettor_cloud_raw_endpoint_failure", tags={"endpoint": "site_app"})
