...
from markettor.geoip import get_geoip_properties
from markettor.api.survey import SURVEY_TARGETING_FLAG_PREFIX
from markettor.api.utils import get_project_id, get_token, hostname_in_allowed_url_list, parse_domain
from markettor.database_healthcheck import DATABASE_FOR_FLAG_MATCHING
...
from markettor.models.feature_flag import get_all_feature_flags
from markettor.models.feature_flag.flag_analytics import increment_request_count
from markettor.models.filters.mixins.utils import process_bool
from markettor.models.utils import execute_with_timeout
from markettor.plugins.site import get_decide_site_apps
...
def on_permitted_recording_domain(team: Team, request: HttpRequest) -> bool:
    ...
    is_authorized_mobile_client: bool = user_agent is not None and any(
        ...
        for keyword in ["markettor-android", "markettor-ios", "markettor-react-native", "markettor-flutter"]
    )
...
@csrf_exempt
@timed("markettor_cloud_decide_endpoint")
def get_decide(request: HttpRequest):
    ...
    if request.method == "POST":
        try:
            ...
            # default value added because of bug in markettor-js 1.19.0
            # see https://sentry.io/organizations/markettor2/issues/2738865125/?project=1899813
            # as a tombstone if the below statsd counter hasn't seen errors for N days
            # then it is likely that no clients are running markettor-js 1.19.0
            # and this defaulting could be removed
            statsd.incr(
                f"markettor_cloud_decide_defaulted_api_version_on_value_error",
        ...
        if team is None and token:
            ...
            if not project_id:
                return cors_response(
                    ...
                    generate_exception_response(
                        "decide",
                        "Project API key invalid. You can find your project API key in MarketTor project settings.",
        ...
        if team:
            if team.id in settings.DECIDE_SHORT_CIRCUITED_TEAM_IDS:
                return cors_response(
                    ...
                    generate_exception_response(
                        ...
                        f"Please contact us at hey@markettor.com",
            ...
            return cors_response(
                ...
                generate_exception_response(
                    "decide",
                    "No project API key provided. You can find your project API key in MarketTor project settings.",
                    code="no_api_key",
                    type="authentication_error",
                    status_code=status.HTTP_401_UNAUTHORIZED,
                ),
    ...
    statsd.incr(f"markettor_cloud_raw_endpoint_success", tags={"endpoint": "decide"})
