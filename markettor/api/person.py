...
from markettor.models.person.missing_person import MissingPerson
...
from markettor.api.capture import capture_internal
from markettor.api.documentation import PersonPropertiesSerializer, extend_schema
from markettor.api.routing import TeamAndOrgViewSetMixin
from markettor.api.utils import format_paginated_url, get_pk_or_uuid, get_target_entity
...
from markettor.models.activity_logging.activity_page import activity_page_response
from markettor.models.async_deletion import AsyncDeletion, DeletionType
from markettor.models.cohort.util import get_all_cohort_ids_by_person_uuid
from markettor.models.filters.lifecycle_filter import LifecycleFilter
...
from markettor.models.person.util import delete_person
from markettor.queries.actor_base_query import (
    ActorBaseQuery,
    get_serialized_people,
)
from markettor.queries.funnels import ClickhouseFunnelActors, ClickhouseFunnelTrendsActors
...
from markettor.queries.paths import PathsActors
from markettor.queries.person_query import PersonQuery
from markettor.queries.properties_timeline import PropertiesTimeline
from markettor.queries.property_values import get_person_property_values_for_key
from markettor.queries.retention import Retention
from markettor.queries.stickiness import Stickiness
from markettor.queries.trends.lifecycle import Lifecycle
from markettor.queries.trends.trends_actors import TrendsActors
...
from markettor.tasks.split_person import split_person
from markettor.utils import (
    convert_property_value,
    format_query_params_absolute_url,
    is_anonymous_id,
)
from prometheus_client import Counter
from markettor.metrics import LABEL_TEAM_ID
        ...
        return {
            ...
            "properties": {
                "next": {
                    ...
                    "nullable": True,
                    "format": "uri",
                    "example": "https://app.markettor.com/api/projects/{project_id}/accounts/?offset=400&limit=100",
                },
                "previous": {
                    "type": "string",
                    "nullable": True,
...
class PersonViewSet(TeamAndOrgViewSetMixin, viewsets.ModelViewSet):
    """
    This endpoint is meant for reading and deleting persons. To create or update persons, we recommend using the [capture API](https://markettor.com/docs/api/capture), the `$set` and `$unset` [properties](https://markettor.com/docs/product-analytics/user-properties), or one of our SDKs.
    """
    ...
    def list(self, request: request.Request, *args: Any, **kwargs: Any) -> response.Response:
        ...
        # This is extra time and DB load, so we only do this when necessary, which is in MarketTor 3000 navigation
        # TODO: Use a more scalable solution before MarketTor 3000 navigation is released, and remove this param
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                "ids",
                OpenApiTypes.OBJECT,
                description="A list of MarketTor person IDs, up to 100 of them. We'll delete all the persons listed.",
            ),
        ],
    )
    ...
    def bulk_delete(self, request: request.Request, pk=None, **kwargs):
        """
        This endpoint allows you to bulk delete persons, either by the MarketTor person IDs or by distinct IDs. You can pass in a maximum of 100 IDs per call.
        """
    ...
    def create(self, *args, **kwargs):
        raise MethodNotAllowed(
            method="POST",
            detail="Creating persons via this API is not allowed. Please create persons by sending an $identify event. See https://markettor.com/docs/product-analytics/identify for details.",
        )
