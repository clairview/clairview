...
from markettor.cdp.templates.slack.template_slack import template as template_slack
...
def get_db_field_value(field, model_id):
    ...
    cursor.execute(f"select {field} from markettor_hogfunction where id='{model_id}';")
...
class TestHogFunctionAPI(ClickhouseTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    @patch("markettor.permissions.markettoranalytics.feature_enabled", return_value=True)
    def test_loads_status_when_enabled_and_available(self, *args):
        with patch("markettor.plugins.plugin_server_api.requests.get") as mock_get:
    ...
    def test_patches_status_on_enabled_update(self, *args):
        with patch("markettor.plugins.plugin_server_api.requests.get") as mock_get:
            with patch("markettor.plugins.plugin_server_api.requests.patch") as mock_patch:
                mock_get.return_value.status_code = status.HTTP_200_OK
