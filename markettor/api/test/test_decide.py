...
from markettor import redis
from markettor.api.decide import get_decide, label_for_team_id_to_track
from markettor.api.test.test_feature_flag import QueryTimeoutWrapper
from markettor.database_healthcheck import postgres_healthcheck
from markettor.exceptions import RequestParsingError, UnspecifiedCompressionFallbackParsingError
...
from markettor.models.feature_flag.feature_flag import FeatureFlagHashKeyOverride
from markettor.models.group.group import Group
from markettor.models.organization import Organization, OrganizationMembership
from markettor.models.person import PersonDistinctId
from markettor.models.personal_api_key import hash_key_value
from markettor.models.plugin import sync_team_inject_web_apps
from markettor.models.team.team import Team
from markettor.models.user import User
from markettor.models.utils import generate_random_token_personal
from markettor.test.base import BaseTest, QueryMatchingTest, snapshot_postgres_queries, snapshot_postgres_queries_context


@patch(
    "markettor.models.feature_flag.flag_matching.postgres_healthcheck.is_connected",
    return_value=True,
)
class TestDecide(BaseTest, QueryMatchingTest):
    ...
    def _post_decide(
        ...
        return self.client.post(
            ...
            HTTP_ORIGIN=origin,
            REMOTE_ADDR=ip,
            HTTP_USER_AGENT=user_agent or "MarketTor test",
        )
    ...
    def test_defaults_to_v2_if_conflicting_parameters(self, *args):
        """
        regression test for https://sentry.io/organizations/markettor2/issues/2738865125/?project=1899813
        markettor-js version 1.19.0 (but not versions before or after)
    ...
    def test_user_session_recording_allowed_for_android(self, *args) -> None:
        ...
        response = self._post_decide(origin="any.site.com", user_agent="markettor-android/3.1.0").json()
        assert response["sessionRecording"] == {
            "endpoint": "/s/",
            "recorderVersion": "v2",
            "consoleLogRecordingEnabled": True,
    ...
    def test_user_session_recording_allowed_for_ios(self, *args) -> None:
        ...
        response = self._post_decide(origin="any.site.com", user_agent="markettor-ios/3.1.0").json()
        assert response["sessionRecording"] == {
            "endpoint": "/s/",
            "recorderVersion": "v2",
            "consoleLogRecordingEnabled": True,
    ...
    def test_feature_flags(self, *args):
        ...
        Person.objects.create(
            ...
            properties={"email": "tim@markettor.com"},
        )
        ...
        FeatureFlag.objects.create(
            team=self.team,
            filters={"properties": [{"key": "email", "value": "tim@markettor.com", "type": "person"}]},
            rollout_percentage=50,
    ...
    def test_feature_flags_v2_consistent_flags(self, *args):
        ...
        # person.delete()
        # person2 = Person.objects.create(team=self.team, distinct_ids=["example_id", "other_id"], properties={"email": "tim@markettor.com"})
    ...
    def test_feature_flags_v3_consistent_flags_with_numeric_distinct_ids(self, *args):
        ...
        Person.objects.create(team=self.team, distinct_ids=[1], properties={"email": "tim@markettor.com"})
        Person.objects.create(
            team=self.team,
    ...
    def test_feature_flags_v2_consistent_flags_with_ingestion_delays(self, *args):
        ...
        # person = Person.objects.create(team=self.team, distinct_ids=["example_id"], properties={"email": "tim@markettor.com"})
    ...
    def test_feature_flags_v2_consistent_flags_with_merged_persons(self, *args):
        ...
        # https://github.com/MarketTor/markettor/blob/master/plugin-server/src/utils/db/db.ts (updateCohortsAndFeatureFlagsForMerge)
        # at which point we run the query
        query = f"""
            WITH deletions AS (
                    DELETE FROM markettor_featureflaghashkeyoverride WHERE team_id = {self.team.pk} AND person_id = {old_person_id}
    ...
    def test_feature_flags_v2_complex(self, *args):
        ...
        Person.objects.create(
            team=self.team,
            distinct_ids=["example_id"],
            properties={"email": "tim@markettor.com", "realm": "cloud"},
        )
        Person.objects.create(
            team=self.team,
            distinct_ids=["hosted_id"],
            properties={"email": "sam@markettor.com", "realm": "hosted"},
        )
        FeatureFlag.objects.create(
            team=self.team,
            filters={"groups": [{"properties": [], "rollout_percentage": None}]},
    ...
    @patch("markettor.models.feature_flag.flag_matching.FLAG_EVALUATION_ERROR_COUNTER")
    ...
    @patch("markettor.models.feature_flag.flag_matching.FLAG_HASH_KEY_WRITES_COUNTER")
    @patch("markettor.api.decide.FLAG_EVALUATION_COUNTER")
    @patch("markettor.models.feature_flag.flag_matching.FLAG_EVALUATION_ERROR_COUNTER")
    ...
    def test_feature_flags_v3_consistent_flags_with_database_errors(self, *args):
        ...
        with self.assertNumQueries(5):
            ...
            # E   3. SELECT "markettor_persondistinctid"."person_id", "markettor_persondistinctid"."distinct_id" FROM "markettor_persondistinctid"
            #           WHERE ("markettor_persondistinctid"."distinct_id" IN ('example_id') AND "markettor_persondistinctid"."team_id" = 1)
            # E   4. SELECT "markettor_featureflaghashkeyoverride"."feature_flag_key", "markettor_featureflaghashkeyoverride"."hash_key", "markettor_featureflaghashkeyoverride"."person_id" FROM "markettor_featureflaghashkeyoverride"
            #            WHERE ("markettor_featureflaghashkeyoverride"."person_id" IN (7) AND "markettor_featureflaghashkeyoverride"."team_id" = 1)
    ...
    def test_feature_flags_v2_with_groups(self, *args):
        # More in-depth tests in markettor/api/test/test_feature_flag.py
    ...
    def test_flag_with_invalid_cohort_filter_condition(self, *args):
        ...
        # Create a cohort with an invalid filter condition (tis broken filter came from this issue: https://github.com/MarketTor/markettor/issues/23213)
    ...
    def test_personal_api_key_without_project_id(self, *args):
        ...
        self.assertEqual(
            response.json(),
            {
                ...
                "detail": "Project API key invalid. You can find your project API key in MarketTor project settings.",
                "attr": None,
            },
        )
    ...
    def test_short_circuited_team(self, *args):
        ...
        with self.settings(DECIDE_SHORT_CIRCUITED_TEAM_IDS=[short_circuited_team.id]):
            ...
            self.assertEqual(
                ...
                f"Team with ID {short_circuited_team.id} cannot access the /decide endpoint.Please contact us at hey@markettor.com",
            )
    ...
    @patch("markettor.models.feature_flag.flag_analytics.CACHE_BUCKET_SIZE", 10)
    def test_decide_analytics_only_fires_when_enabled(self, *args):
        ...
        with self.settings(DECIDE_BILLING_SAMPLING_RATE=0):
            ...
            self.assertEqual(client.hgetall(f"markettor:decide_requests:{self.team.pk}"), {})
        ...
        with self.settings(DECIDE_BILLING_SAMPLING_RATE=1), freeze_time("2022-05-07 12:23:07"):
            ...
            self.assertEqual(
                client.hgetall(f"markettor:decide_requests:{self.team.pk}"),
                {b"165192618": b"1"},
            )
    ...
    def test_decide_analytics_only_fires_with_non_survey_targeting_flags(self, *args):
        ...
        response = req_client.post(
            ...
            data={
                ...
                "conditions": {"url": "https://app.markettor.com/notebooks"},
            },
            format="json",
            content_type="application/json",
        )
...
class TestDecideUsesReadReplica(TransactionTestCase):
    ...
    psql -U markettor
    CREATE USER markettor2 WITH PASSWORD 'markettor';
    # this user will be the one used to connect to the replica^^

    # switch to the db on which you want to give permissions
    \c test_markettor
    GRANT SELECT ON ALL TABLES IN SCHEMA public TO markettor2;
    ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO markettor2;

    then run this test:

    MARKETTOR_DB_NAME='markettor' READ_REPLICA_OPT_IN='decide,PersonalAPIKey,local_evaluation' MARKETTOR_POSTGRES_READ_HOST='localhost'  MARKETTOR_DB_PASSWORD='markettor' MARKETTOR_DB_USER='markettor'  ./bin/tests markettor/api/test/test_decide.py::TestDecideUsesReadReplica
    ...
    MARKETTOR_DB_NAME='markettor' READ_REPLICA_OPT_IN='decide' MARKETTOR_POSTGRES_READ_HOST='localhost'  MARKETTOR_DB_PASSWORD='markettor' MARKETTOR_DB_USER='markettor' MARKETTOR_DB_PASSWORD_READ_REPLICA='password' MARKETTOR_DB_USER_READ_REPLICA='markettor2' ./bin/start
    ...
    def setup_user_and_team_in_db(self, dbname: str = "default"):
        ...
        user = User.objects.db_manager(dbname).create(
            email=f"test-{random.randint(1, 100000)}@markettor.com",
            password="password",
    ...
    def test_healthcheck_uses_read_replica(self):
        ...
        with (
            ...
            # E   1. SELECT 1
            # Main DB queries:
            # E   1. SELECT "markettor_featureflag"."id", -- fill up feature flags cache
    ...
    def test_decide_uses_read_replica(self, mock_is_connected):
        ...
        persons = [{"distinct_ids": ["example_id"], "properties": {"email": "tim@markettor.com"}}]
        flags = [
            {
                "rollout_percentage": 50,
                "name": "Beta feature",
    ...
    def test_decide_uses_read_replica_for_cohorts_based_flags(self, mock_is_connected):
        ...
        persons = [
            ...
            },
            {
                ...
                "properties": {"email": "tim2@markettor.com"},
            },
            {
                "distinct_ids": ["cohort_secondary"],
                "properties": {"email": "tim3@markettor.com"},
            },
        ]
        flags = [
            {
        ...
        with self.assertNumQueries(5, using="replica"), self.assertNumQueries(0, using="default"):
            ...
            # Replica queries:
            # E   1. SET LOCAL statement_timeout = 600
            # E   2. SELECT "markettor_cohort"."id", "markettor_cohort"."name", -- i.e. select all cohorts
            # E   3. SELECT EXISTS(SELECT (1) AS "a" FROM "markettor_cohortpeople" U0 WHERE (U0."cohort_id" = 28 AND U0."cohort_id" = 28 AND U0."person_id" = "markettor_person"."id") LIMIT 1) AS "flag_47_condition_0",  -- a.k.a flag selection query
    ...
    def test_feature_flags_v3_consistent_flags(self, mock_is_connected):
        ...
        with self.assertNumQueries(9, using="replica"), self.assertNumQueries(0, using="default"):
            ...
            # E   2. SELECT "markettor_persondistinctid"."person_id", "markettor_persondistinctid"."distinct_id" FROM "markettor_persondistinctid"
            #           WHERE ("markettor_persondistinctid"."distinct_id" IN ('example_id') AND "markettor_persondistinctid"."team_id" = 1)
            # E   3. SELECT "markettor_featureflaghashkeyoverride"."feature_flag_key", "markettor_featureflaghashkeyoverride"."hash_key", "markettor_featureflaghashkeyoverride"."person_id" FROM "markettor_featureflaghashkeyoverride"
            #            WHERE ("markettor_featureflaghashkeyoverride"."person_id" IN (7) AND "markettor_featureflaghashkeyoverride"."team_id" = 1)
        ...
        with self.assertNumQueries(13, using="replica"), self.assertNumQueries(0, using="default"):
            ...
            # E   1. SET LOCAL statement_timeout = 300
            # E   2. WITH some CTEs,
            #           SELECT key FROM markettor_featureflag WHERE team_id = 13
            # E         AND key NOT IN (SELECT feature_flag_key FROM existing_overrides)
            # E   3. SET LOCAL statement_timeout = 300
            # E   4. SELECT "markettor_persondistinctid"."person_id", "markettor_persondistinctid"."distinct_id" FROM "markettor_persondistinctid" -- a.k.a select the person ids.
            #        We select person overrides from replica DB when no inserts happened
            # E   5. SELECT "markettor_featureflaghashkeyoverride"."feature_flag_key",  - a.k.a select the flag overrides
        ...
        with (
            ...
            # E         AND key NOT IN (SELECT feature_flag_key FROM existing_overrides)
            # E   3. SET LOCAL statement_timeout = 300
            # E   4. SELECT "markettor_persondistinctid"."person_id", -- i.e person from distinct ids
            # E   5. SELECT "markettor_featureflaghashkeyoverride"."feature_flag_key", -- i.e. hash key overrides (note this would've gone to main db if insert did not fail)
            # E   6. SET LOCAL statement_timeout = 600
    ...
    def test_feature_flags_v3_consistent_flags_with_write_on_hash_key_overrides(self, mock_is_connected):
        ...
        with self.assertNumQueries(9, using="replica"), self.assertNumQueries(0, using="default"):
            ...
            # E   2. SELECT "markettor_persondistinctid"."person_id", "markettor_persondistinctid"."distinct_id" FROM "markettor_persondistinctid"
            #           WHERE ("markettor_persondistinctid"."distinct_id" IN ('example_id') AND "markettor_persondistinctid"."team_id" = 1)
            # E   3. SELECT "markettor_featureflaghashkeyoverride"."id", "markettor_featureflaghashkeyoverride"."team_id", -- hash key overrides
        ...
        with self.assertNumQueries(8, using="replica"), self.assertNumQueries(9, using="default"):
            ...
            # E                       WITH some CTEs,
            # E                       INSERT INTO markettor_featureflaghashkeyoverride (team_id, person_id, feature_flag_key, hash_key)
            # E                           SELECT team_id, person_id, key, 'example_id'
            # E                           FROM flags_to_override, target_person_ids
            # E                           WHERE EXISTS (SELECT 1 FROM markettor_person WHERE id = person_id AND team_id = 7)
            # E                           ON CONFLICT DO NOTHING

            # E   3. SET LOCAL statement_timeout = 300
            # E   4. SELECT "markettor_persondistinctid"."person_id", "markettor_persondistinctid"."distinct_id" FROM "markettor_persondistinctid" -- a.k.a select the person for overrides.
            #        We select person overrides from main DB in this case to prevent replication lag from giving us the wrong override values.
            # E   5. SELECT "markettor_featureflaghashkeyoverride"."feature_flag_key",  -- a.k.a get hash key overrides
    ...
    def test_feature_flags_v2_with_groups(self, mock_is_connected):
        ...
        Group.objects.db_manager("replica").create(
            ...
            group_type_index=0,
            group_key="foo",
            group_properties={"email": "a@markettor.com"},
            version=0,
        )

        with self.assertNumQueries(4, using="replica"), self.assertNumQueries(0, using="default"):
            # E   1. SET LOCAL statement_timeout = 300
            # E   2. SELECT "markettor_grouptypemapping"."id", -- a.k.a. get group type mappings
        ...
        with self.assertNumQueries(9, using="replica"), self.assertNumQueries(0, using="default"):
            # E   1. SET LOCAL statement_timeout = 300
            # E   2. SELECT "markettor_grouptypemapping"."id", "markettor_grouptypemapping"."team_id", -- a.k.a get group type mappings

            # E   3. SET LOCAL statement_timeout = 600
            # E   4. SELECT (UPPER(("markettor_group"."group_properties" ->> 'email')::text) AS "flag_182_condition_0" FROM "markettor_group" -- a.k.a get group0 conditions
            # E   5. SELECT (true) AS "flag_181_condition_0" FROM "markettor_group" WHERE ("markettor_group"."team_id" = 91 -- a.k.a get group1 conditions
            response = self._post_decide(
                distinct_id="example_id",
        ...
        with self.assertNumQueries(9, using="replica"), self.assertNumQueries(0, using="default"):
            ...
            # E   3. SELECT "markettor_grouptypemapping"."id", "markettor_grouptypemapping"."team_id", -- a.k.a get group type mappings

            # E   6. SET LOCAL statement_timeout = 600
            # E   7. SELECT (UPPER(("markettor_group"."group_properties" ->> 'email')::text) AS "flag_182_condition_0" FROM "markettor_group" -- a.k.a get group0 conditions
            # E   8. SELECT (true) AS "flag_181_condition_0" FROM "markettor_group" WHERE ("markettor_group"."team_id" = 91 -- a.k.a get group1 conditions
            response = self._post_decide(
                distinct_id="example_id",
    ...
    @patch("markettor.api.feature_flag.report_user_action")
    @patch("markettor.rate_limit.is_rate_limit_enabled", return_value=True)
    def test_local_evaluation(self, mock_rate_limit, mock_capture):
        ...
        with self.assertNumQueries(3, using="replica"), self.assertNumQueries(5, using="default"):
            ...
            # E   1. UPDATE "markettor_personalapikey" SET "last_used_at" = '2023-08-01T11:26:50.728057+00:00'
            # E   2. SELECT "markettor_team"."id", "markettor_team"."uuid", "markettor_team"."organization_id"
            # E   3. SELECT "markettor_organizationmembership"."id", "markettor_organizationmembership"."organization_id", - user org permissions check
            # Captured queries for replica DB:
            # E   1. SELECT "markettor_personalapikey"."id", "markettor_personalapikey"."user_id", "markettor_personalapikey"."label", "markettor_personalapikey"."value", -- check API key, joined with user
            # E   2. SELECT "markettor_featureflag"."id", "markettor_featureflag"."key", "markettor_featureflag"."name", "markettor_featureflag"."filters", -- get flags
            # E   3. SELECT "markettor_grouptypemapping"."id", "markettor_grouptypemapping"."team_id", -- get groups
    ...
    def test_local_evaluation_for_cohorts(self, mock_rate_limit, mock_capture):
        ...
        with self.assertNumQueries(4, using="replica"), self.assertNumQueries(5, using="default"):
            ...
            # E   1. SELECT "markettor_personalapikey"."id", "markettor_personalapikey"."user_id", "markettor_personalapikey"."label", "markettor_personalapikey"."value", -- check API key, joined with user
            # E   2. SELECT "markettor_featureflag"."id", "markettor_featureflag"."key", "markettor_featureflag"."name", "markettor_featureflag"."filters", -- get flags
            # E   3. SELECT "markettor_cohort"."id", "markettor_cohort"."name", "markettor_cohort"."description", -- select all cohorts
            # E   5. SELECT "markettor_grouptypemapping"."id", "markettor_grouptypemapping"."team_id", -- get groups
...
class TestDecideExceptions(TestCase):
    @patch("markettor.api.decide.capture_exception")
    @patch("markettor.api.decide.load_data_from_request")
