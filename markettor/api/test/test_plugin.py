...
from markettor.constants import FROZEN_MARKETTOR_VERSION

from markettor.models import Plugin, PluginAttachment, PluginConfig, PluginSourceFile
...
from markettor.plugins.access import can_configure_plugins, can_globally_manage_plugins, can_install_plugins
from markettor.plugins.test.mock import mocked_plugin_requests_get
from markettor.plugins.test.plugin_archives import (
    HELLO_WORLD_PLUGIN_GITHUB_ATTACHMENT_ZIP,
    HELLO_WORLD_PLUGIN_GITHUB_ZIP,
    HELLO_WORLD_PLUGIN_SECRET_GITHUB_ZIP,
)
from markettor.queries.app_metrics.test.test_app_metrics import create_app_metric
from markettor.test.base import APIBaseTest, QueryMatchingTest, snapshot_postgres_queries
...
@mock.patch("markettor.models.plugin.reload_plugins_on_workers", side_effect=mocked_plugin_reload)
...
class TestPluginAPI(APIBaseTest, QueryMatchingTest):
    ...
    def _create_plugin(
        ...
        params = {"url": "https://github.com/MarketTor/helloworldplugin"}
    ...
    def test_create_plugin_auth(self, mock_get, mock_reload):
        repo_url = "https://github.com/MarketTor/helloworldplugin"
    ...
    @mock.patch("markettor.models.plugin.PluginSourceFile.objects.sync_from_plugin_archive")
    ...
    def test_create_plugin_commit_url(self, mock_get, mock_reload):
        ...
        response = self.client.post(
            "/api/organizations/@current/plugins/",
            {"url": f"https://github.com/MarketTor/helloworldplugin/commit/{HELLO_WORLD_PLUGIN_GITHUB_ZIP[0]}"},
        )
        self.assertEqual(response.status_code, 201)
        self.assertEqual(
            response.json(),
    ...
    def test_create_plugin_other_commit_url(self, mock_get, mock_reload):
        ...
        response2 = self.client.post(
            "/api/organizations/@current/plugins/",
            {
                "url": "https://github.com/MarketTor/helloworldplugin/commit/{}".format(
        ...
        self.assertEqual(
            response2.json(),
            {
                ...
                "url": f"https://github.com/MarketTor/helloworldplugin/commit/{HELLO_WORLD_PLUGIN_GITHUB_ATTACHMENT_ZIP[0]}",
                "icon": None,
                "config_schema": {
                    "bar": {
                        "name": "What's in the bar?",
    ...
    def test_create_plugin_version_range_eq_current(self, mock_get, mock_reload):
        with self.is_cloud(False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/markettor-plugin/version-equals/commit/{FROZEN_MARKETTOR_VERSION}"},
            )
            self.assertEqual(response.status_code, 201)
    ...
    def test_create_plugin_version_range_eq_next_minor(self, mock_get, mock_reload):
        with self.is_cloud(False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {
                    "url": f"https://github.com/markettor-plugin/version-equals/commit/{FROZEN_MARKETTOR_VERSION.next_minor()}"
                },
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(
                cast(dict[str, str], response.json())["detail"],
                f'Currently running MarketTor version {FROZEN_MARKETTOR_VERSION} does not match this plugin\'s semantic version requirement "{FROZEN_MARKETTOR_VERSION.next_minor()}".',
            )
    ...
    def test_create_plugin_version_range_gt_current(self, mock_get, mock_reload):
        with self.is_cloud(False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/markettor-plugin/version-greater-than/commit/0.0.0"},
            )
            self.assertEqual(response.status_code, 201)
    ...
    def test_create_plugin_version_range_gt_next_major(self, mock_get, mock_reload):
        with self.is_cloud(False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {
                    "url": f"https://github.com/markettor-plugin/version-greater-than/commit/{FROZEN_MARKETTOR_VERSION.next_major()}"
                },
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(
                cast(dict[str, str], response.json())["detail"],
                f'Currently running MarketTor version {FROZEN_MARKETTOR_VERSION} does not match this plugin\'s semantic version requirement ">= {FROZEN_MARKETTOR_VERSION.next_major()}".',
            )
    ...
    def test_create_plugin_version_range_lt_current(self, mock_get, mock_reload):
        with self.is_cloud(False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/markettor-plugin/version-less-than/commit/{FROZEN_MARKETTOR_VERSION}"},
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(
                cast(dict[str, str], response.json())["detail"],
                f'Currently running MarketTor version {FROZEN_MARKETTOR_VERSION} does not match this plugin\'s semantic version requirement "< {FROZEN_MARKETTOR_VERSION}".',
            )
    ...
    def test_create_plugin_version_range_lt_next_major(self, mock_get, mock_reload):
        with self.is_cloud(False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {
                    "url": f"https://github.com/markettor-plugin/version-less-than/commit/{FROZEN_MARKETTOR_VERSION.next_major()}"
                },
            )
            self.assertEqual(response.status_code, 201)
    ...
    def test_create_plugin_version_range_lt_invalid(self, mock_get, mock_reload):
        with self.is_cloud(False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/markettor-plugin/version-less-than/commit/..."},
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(
                cast(dict[str, str], response.json())["detail"],
                'Invalid MarketTor semantic version requirement "< ..."!',
            )
    ...
    @patch("markettor.api.plugin.validate_plugin_job_payload")
    @patch("markettor.api.plugin.connections")
            ...
            [
                {
                    ...
                    "item_id": str(plugin_id),
                    "scope": "Plugin",
                    "user": {"email": "user1@markettor.com", "first_name": ""},
                },
                {
                    "activity": "enabled",
                    "created_at": "2021-08-25T22:09:14.252000Z",
