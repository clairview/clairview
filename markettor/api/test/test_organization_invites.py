...
class TestOrganizationInvitesAPI(APIBaseTest):
    def helper_generate_bulk_invite_payload(self, count: int):
        ...
        for i in range(0, count):
            payload.append(
                {
                    "target_email": f"test+{random.randint(1000000, 9999999)}@markettor.com",
    ...
    def test_cant_list_invites_for_an_alien_organization(self):
        ...
        invite = OrganizationInvite.objects.create(target_email="siloed@markettor.com", organization=org)
    ...
    def test_allow_bulk_creating_invites(self, mock_capture):
        ...
        with self.settings(EMAIL_ENABLED=True, SITE_URL="http://test.markettor.com"):
            response = self.client.post(
                ...
                payload,
                format="json",
                headers={"X-Markettor-Session-Id": "123", "Referer": "http://test.markettor.com/my-url"},
            )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        response_data = response.json()
        ...
        mock_capture.assert_any_call(
            ...
            properties={
                ...
                "email_available": True,
                "$session_id": "123",
                "$current_url": "http://test.markettor.com/my-url",
            },
            groups={
                "instance": ANY,
                "organization": str(self.team.organization_id),
