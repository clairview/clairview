...
class TestInstanceStatus(APIBaseTest):
    ...
    @patch("markettor.api.instance_status.is_postgres_alive")
    @patch("markettor.api.instance_status.is_redis_alive")
    @patch("markettor.api.instance_status.is_plugin_server_alive")
    # patched at the module level because it is locally imported in the target code
    @patch("markettor.clickhouse.system_status.dead_letter_queue_ratio_ok_cached")
    @patch("markettor.api.instance_status.async_migrations_ok")
