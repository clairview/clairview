...
from markettor.constants import ENTITY_ID, ENTITY_MATH, ENTITY_TYPE, TRENDS_CUMULATIVE
from markettor.models import Action, Cohort, Organization
...
class TestPersonTrends(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_trends_people_endpoint_filters_search(self):
        _create_person(
            team_id=self.team.pk,
            distinct_ids=["p1"],
            properties={"email": "ben@markettor.com"},
        )
        _create_event(
            team=self.team,
            event="$pageview",
        ...
        _create_person(
            team_id=self.team.pk,
            distinct_ids=["p2"],
            properties={"email": "neil@markettor.com"},
        )
        _create_event(
            team=self.team,
            event="$pageview",
