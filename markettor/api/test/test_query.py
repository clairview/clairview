...
from markettor.hogql.query import LimitContext
...
class TestQuery(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_select_hogql_expressions(self):
        with freeze_time("2020-01-10 12:00:00"):
            _create_person(
                properties={"email": "tom@markettor.com"},
    ...
    @patch(
        "markettor.clickhouse.client.execute._annotate_tagged_query", return_value=("SELECT 1&&&", {})
    ...
    def test_select_event_person(self):
        with freeze_time("2020-01-10 12:00:00"):
            person = _create_person(
                properties={"name": "Tom", "email": "tom@markettor.com"},
                distinct_ids=["2", "some-random-uid"],
                team=self.team,
                immediate=True,
            )
    ...
    @patch("markettor.hogql.constants.DEFAULT_RETURNED_ROWS", 10)
    @patch("markettor.hogql.constants.MAX_SELECT_RETURNED_ROWS", 15)
...
class TestQueryRetrieve(APIBaseTest):
    def setUp(self):
        ...
        self.redis_get_patch = mock.patch("markettor.redis.get_client", return_value=self.redis_client_mock)
