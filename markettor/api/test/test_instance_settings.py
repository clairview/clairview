...
from markettor.api.instance_settings import (
...
from markettor.settings import CONSTANCE_CONFIG
    ...
    def test_updating_email_settings(self):
        ...
        with self.settings(SITE_URL="http://localhost:8000", CELERY_TASK_ALWAYS_EAGER=True):
            response = self.client.patch(
                ...
                {"value": "hellohello@markettor.com"},
            )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["value"], "hellohello@markettor.com")

        self.assertEqual(mail.outbox[0].from_email, "hellohello@markettor.com")
        self.assertEqual(mail.outbox[0].subject, "This is a test email of your MarketTor instance")
