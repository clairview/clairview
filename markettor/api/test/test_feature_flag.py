...
from markettor.api.cohort import get_cohort_actors_for_feature_flag
from markettor.api.feature_flag import FeatureFlagSerializer
from markettor.constants import AvailableFeature
from markettor.models import FeatureFlag, GroupTypeMapping, User
from markettor.models.cohort import Cohort
from markettor.models.dashboard import Dashboard
from markettor.models.early_access_feature import EarlyAccessFeature
from markettor.models.feature_flag import (
...
from markettor.models.personal_api_key import PersonalAPIKey, hash_key_value
...
class TestFeatureFlag(APIBaseTest, ClickhouseTestMixin):
        ...
        for operator in invalid_operators:
            ...
            self.assertEqual(
                response.json(),
                {
                    "type": "validation_error",
                    "code": "invalid_value",
                    "detail": f"Invalid value for operator {operator}: ['@markettor.com']",
                    "attr": "filters",
                },
            )
                            ...
                            "properties": [
                                {
                                    "key": "email",
                                    "type": "person",
                                    "value": '["@markettor.com"]',  # fine as long as a string
                                    "operator": "not_regex",
                                }
                            ],
                        }
    ...
    def test_hard_deleting_feature_flag_is_forbidden(self):
        new_user = User.objects.create_and_join(self.organization, "new_annotations@markettor.com", None)
    ...
    def test_get_feature_flag_activity(self):
        new_user = User.objects.create_and_join(
            ...
            email="person_acting_and_then_viewing_activity@markettor.com",
            password=None,
            first_name="Potato",
        )
        self.client.force_login(new_user)
    ...
    def test_get_feature_flag_activity_only_from_own_team(self):
        ...
        _, org_one_team, org_one_user = User.objects.bootstrap(
            organization_name="Org 1", email="org1@markettor.com", password=None
        )

        _, org_two_team, org_two_user = User.objects.bootstrap(
            organization_name="Org 2", email="org2@markettor.com", password=None
        )
    ...
    def test_get_flags_with_specified_token(self):
        _, _, user = User.objects.bootstrap("Test", "team2@markettor.com", None)
    ...
    def test_my_flags(self, mock_capture):
        ...
        distinct_id_user = User.objects.create_and_join(self.organization, "distinct_id_user@markettor.com", None)
        ...
        distinct_id_0_user = User.objects.create_and_join(self.organization, "distinct_id_0_user@markettor.com", None)
    ...
    def test_local_evaluation_for_invalid_cohorts(self, mock_capture):
        ...
        with self.assertNumQueries(12):
            # E   1. SAVEPOINT
            # E   2. SELECT "markettor_personalapikey"."id"
            # E   3. RELEASE SAVEPOINT
            # E   4. UPDATE "markettor_personalapikey" SET "last_used_at" = '2024-01-31T13:01:37.394080+00:00'
            # E   5. SELECT "markettor_team"."id", "markettor_team"."uuid"
            # E   6. SELECT "markettor_organizationmembership"."id", "markettor_organizationmembership"."organization_id"
            # E   7. SELECT "markettor_cohort"."id"  -- all cohorts
            # E   8. SELECT "markettor_featureflag"."id", "markettor_featureflag"."key", -- all flags
            # E   9. SELECT "markettor_cohort". id = 99999
            # E  10. SELECT "markettor_cohort". id = deleted cohort
            # E  11. SELECT "markettor_cohort". id = cohort from other team
            # E  12. SELECT "markettor_grouptypemapping"."id", -- group type mapping
    ...
    def test_local_evaluation_billing_analytics(self):
        ...
        with freeze_time("2022-05-07 12:23:07"):
            ...
            response = self.client.get(f"/api/feature_flag/local_evaluation?token={self.team.api_token}")
            self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
            self.assertEqual(client.hgetall(f"markettor:local_evaluation_requests:{self.team.pk}"), {})
            ...
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assertEqual(
                client.hgetall(f"markettor:local_evaluation_requests:{self.team.pk}"),
                {b"165192618": b"1"},
            )
            ...
            filters={
                "properties": {
                    ...
                    "values": [
                        ...
                        {"key": "email", "value": "test@markettor.com", "type": "person"},
                    ],
                }
            },
            name="cohort2",
    ...
    @patch("markettor.api.feature_flag.FeatureFlagThrottle.rate", new="7/minute")
    @patch("markettor.rate_limit.BurstRateThrottle.rate", new="5/minute")
    @patch("markettor.rate_limit.statsd.incr")
    ...
    def test_cant_create_flag_with_data_that_fails_to_query(self):
        ...
        with snapshot_postgres_queries_context(self, custom_query_matcher=lambda query: "markettor_person" in query):
            response = self.client.post(
                f"/api/projects/{self.team.id}/feature_flags",
                {
                    "name": "Beta feature",
    ...
    def test_cant_create_flag_with_group_data_that_fails_to_query(self):
        ...
        # Only snapshot flag evaluation queries
        with snapshot_postgres_queries_context(self, custom_query_matcher=lambda query: "markettor_group" in query):
...
class TestResiliency(TransactionTestCase, QueryMatchingTest):
        ...
        with snapshot_postgres_queries_context(self), connection.execute_wrapper(QueryTimeoutWrapper()):
            ...
            with self.assertNumQueries(0):
                all_flags, _, _, errors = get_all_feature_flags(
                    team_id,
                    "random",
                    property_value_overrides={"email": "tim@markettor.com"},
                )
                self.assertTrue(all_flags["property-flag"])
                self.assertTrue(all_flags["default-flag"])
                self.assertFalse(errors)
            ...
            with self.assertNumQueries(0):
                all_flags, _, _, errors = get_all_feature_flags(
                    team_id,
                    "example_id",
                    property_value_overrides={"email": "tom@markettor.com"},
                )
    ...
    def test_feature_flags_v3_with_a_working_slow_db(self, mock_postgres_check):
        ...
        with (
            ...
            patch(
                "markettor.models.feature_flag.flag_matching.FLAG_MATCHING_QUERY_TIMEOUT_MS",
                500,
            ),
        ):
