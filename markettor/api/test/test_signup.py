...
from markettor.models import Dashboard, Organization, Team, User
...
from markettor.models.organization_invite import OrganizationInvite
from markettor.test.base import APIBaseTest
from markettor.utils import get_instance_realm

MOCK_GITLAB_SSO_RESPONSE = {
    "access_token": "123",
    "email": "testemail@markettor.com",
    "name": "John Doe",
}
...
class TestSignupAPI(APIBaseTest):
    ...
    def test_api_sign_up(self, mock_capture):
        ...
        Organization.objects.create(name="MarketTor Internal Metrics", for_internal_metrics=True)
        ...
        self.assertEqual(user.email, "hedgehog@markettor.com")
        ...
        # Assert that the sign up event & identify calls were sent to MarketTor analytics
        ...
        self.assertEqual(response.json()["email"], "hedgehog@markettor.com")

        # Assert that the password was correctly saved
        self.assertTrue(user.check_password(VALID_TEST_PASSWORD))

    @patch("markettor.api.signup.is_email_available", return_value=True)
    @patch("markettor.api.signup.EmailVerifier.create_token_and_send_email_verification")
    ...
    @patch("markettor.api.signup.is_email_available", return_value=True)
    @patch("markettor.api.signup.EmailVerifier.create_token_and_send_email_verification")
    @patch("markettor.api.signup.is_email_verification_disabled", return_value=True)
    ...
    def test_signup_disallowed_on_email_collision(self):
        ...
        User.objects.create(email="fake@markettor.com", first_name="Jane")

        response = self.client.post(
            "/api/signup/",
            {
                "first_name": "John",
                "email": "fake@markettor.com",
                "password": VALID_TEST_PASSWORD,
            },
        )
    ...
    def test_signup_allowed_on_self_hosted_with_env_var(self):
        try:
            ...
            User.objects.create(first_name="name", email="email@markettor.com")
            ...
            with self.is_cloud(False):
                with self.settings(MULTI_ORG_ENABLED=True):
                    response = self.client.post(
                        ...
                        {
                            "first_name": "Jane",
                            "email": "hedgehog4@markettor.com",
                            "password": VALID_TEST_PASSWORD,
                        },
                    )
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
            self.assertEqual(response.json()["email"], "hedgehog4@markettor.com")
    ...
    def test_signup_minimum_attrs(self, mock_capture):
        ...
        self.assertEqual(user.email, "hedgehog2@markettor.com")
        ...
        response = self.client.get("/api/users/@me/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["email"], "hedgehog2@markettor.com")
    ...
    def test_cant_sign_up_with_short_password(self):
        ...
        response = self.client.post(
            "/api/signup/",
            {"first_name": "Jane", "email": "failed@markettor.com", "password": "123"},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.json(),
    ...
    def test_cant_sign_up_with_weak_passwords(self):
        ...
        for password, detail in cases:
            res = self.client.post(
                "/api/signup/",
                {"first_name": "Jane", "email": "failed@markettor.com", "password": password},
            )
    ...
    def test_default_dashboard_is_created_on_signup(self):
        ...
        response = self.client.post(
            ...
            {
                "first_name": "Jane",
                "email": "hedgehog75@markettor.com",
                "password": VALID_TEST_PASSWORD,
            },
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_api_can_use_social_login_to_create_organization_if_enabled(self, mock_request):
        ...
        try:
            ...
            self.assertRedirects(
                response,
                "/organization/confirm-creation?organization_name=&first_name=John%20Doe&email=testemail%40markettor.com",
    ...
    def test_api_social_login_to_create_organization(self, mock_request):
        ...
        self.assertRedirects(
            response,
            "/organization/confirm-creation?organization_name=HogFlix&first_name=John%20Doe&email=testemail%40markettor.com",
        )  # page where user will create a new org

    @mock.patch("social_core.backends.base.BaseAuth.request")
    @mock.patch("markettor.api.authentication.get_instance_available_sso_providers")
    @pytest.mark.skip_on_multitenancy
    ...
    def run_test_for_allowed_domain(
        ...
        if use_invite:
            ...
            invite = OrganizationInvite.objects.create(
                target_email="jane@hogflix.markettor.com",
                organization=new_org,
                first_name="Jane",
        ...
        mock_request.return_value.json.return_value = {
            "access_token": "123",
            "email": "jane@hogflix.markettor.com",
        }
        ...
        self.assertEqual(user.email, "jane@hogflix.markettor.com")
        ...
        if use_invite and not expired_invite:
            ...
            self.assertEqual(
                OrganizationInvite.objects.filter(
                    organization=new_org, target_email="jane@hogflix.markettor.com"
                ).count(),
                0,
            )
    ...
    @mock.patch("social_core.backends.base.BaseAuth.request")
    @mock.patch("markettor.api.authentication.get_instance_available_sso_providers")
    @mock.patch("markettor.tasks.user_identify.identify_task")
    @pytest.mark.ee
    ...
    def test_social_signup_with_allowed_domain_on_cloud_reverse(self, mock_sso_providers, mock_request):
        with self.is_cloud(True):
            # user already exists
            User.objects.create(email="jane@hogflix.markettor.com", distinct_id=str(uuid.uuid4()))
    ...
    def test_social_signup_to_existing_org_without_allowed_domain_on_cloud(self, mock_sso_providers, mock_request):
        ...
        self.assertRedirects(
            response,
            "/organization/confirm-creation?organization_name=&first_name=jane&email=jane%40hogflix.markettor.com",
        )  # page where user will create a new org
...
class TestInviteSignupAPI(APIBaseTest):
    ...
    def test_api_invite_sign_up_prevalidate(self):
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+19@markettor.com", organization=self.organization
        )
        ...
        self.assertEqual(
            ...
            {
                "id": str(invite.id),
                "target_email": "test+19@markettor.com",
                "first_name": "",
    ...
    def test_api_invite_sign_up_with_first_name_prevalidate(self):
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+58@markettor.com",
    ...
    def test_api_invite_sign_up_prevalidate_for_existing_user(self):
        user = self._create_user("test+29@markettor.com", VALID_TEST_PASSWORD)
        new_org = Organization.objects.create(name="Test, Inc")
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+29@markettor.com", organization=new_org
        )
        ...
        self.assertEqual(
            ...
            {
                "id": str(invite.id),
                "target_email": "test+29@markettor.com",
                "first_name": "",
    ...
    def test_existing_user_cant_claim_invite_if_it_doesnt_match_target_email(self):
        user = self._create_user("test+39@markettor.com", VALID_TEST_PASSWORD)
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+49@markettor.com", organization=self.organization
        )
    ...
    def test_api_invite_sign_up_prevalidate_expired_invite(self):
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+59@markettor.com", organization=self.organization
        )
    ...
    def test_api_invite_sign_up(self, mock_capture):
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+99@markettor.com", organization=self.organization
        )
        ...
        self.assertEqual(user.email, "test+99@markettor.com")
        ...
        response = self.client.get("/api/users/@me/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["email"], "test+99@markettor.com")
    ...
    def test_api_invite_sign_up_where_there_are_no_default_non_private_projects(self):
        self.client.logout()
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+private@markettor.com", organization=self.organization
        )
    ...
    def test_api_invite_sign_up_where_default_project_is_private(self):
        ...
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+privatepublic@markettor.com",
            organization=self.organization,
        )
        response = self.client.post(
            f"/api/signup/{invite.id}/",
    ...
    def test_api_invite_sign_up_member_joined_email_is_not_sent_for_initial_member(self):
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+100@markettor.com", organization=self.organization
        )

        with self.settings(
            EMAIL_ENABLED=True,
            EMAIL_HOST="localhost",
            SITE_URL="http://test.markettor.com",
        ):
            response = self.client.post(
                f"/api/signup/{invite.id}/",
                {
    ...
    def test_api_invite_sign_up_member_joined_email_is_sent_for_next_members(self):
        with override_instance_config("EMAIL_HOST", "localhost"):
            initial_user = User.objects.create_and_join(self.organization, "test+420@markettor.com", None)

            invite: OrganizationInvite = OrganizationInvite.objects.create(
                target_email="test+100@markettor.com", organization=self.organization
            )

            with self.settings(EMAIL_ENABLED=True, SITE_URL="http://test.markettor.com"):
                response = self.client.post(
                    f"/api/signup/{invite.id}/",
                    {
                        "first_name": "Alice",
    ...
    def test_api_invite_sign_up_member_joined_email_is_not_sent_if_disabled(self):
        ...
        User.objects.create_and_join(self.organization, "test+420@markettor.com", None)
    ...
    def test_existing_user_can_sign_up_to_a_new_organization(
        ...
        user = self._create_user("test+159@markettor.com", VALID_TEST_PASSWORD)
        new_org = Organization.objects.create(name="TestCo")
        new_team = Team.objects.create(organization=new_org)
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+159@markettor.com", organization=new_org
        )
        ...
        self.assertEqual(user.email, "test+159@markettor.com")
        self.assertFalse(user.is_staff)  # Not first user in the instance
    ...
    def test_cannot_use_claim_invite_endpoint_to_update_user(self, mock_capture):
        ...
        """
        new_org = Organization.objects.create(name="TestCo")
        user = self._create_user("test+189@markettor.com", VALID_TEST_PASSWORD)
        user2 = self._create_user("test+949@markettor.com")
        user2.join(organization=new_org)

        Team.objects.create(organization=new_org)
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+189@markettor.com", organization=new_org
        )
    ...
    def test_cant_claim_sign_up_invite_without_required_attributes(self):
        ...
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+799@markettor.com", organization=self.organization
        )
        ...
        session.update(
            {
                ...
                "email": "test_api_social_invite_sign_up@markettor.com",
            }
        )
        session.save()
    ...
    def test_api_social_invite_sign_up_if_email_verification_on(self, email_mock, email_available_mock):
        ...
        session.update(
            {
                "backend": "google-oauth2",
                "email": "test_api_social_invite_sign_up_with_verification@markettor.com",
            }
        )
        session.save()
