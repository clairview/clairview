...
from markettor.api.email_verification import EmailVerifier
from markettor.api.organization import OrganizationSerializer
from markettor.api.shared import OrganizationBasicSerializer, TeamBasicSerializer
from markettor.api.utils import (
...
from markettor.auth import (
    PersonalAPIKeyAuthentication,
    SessionAuthentication,
    TemporaryTokenAuthentication,
    authenticate_secondarily,
)
from markettor.constants import PERMITTED_FORUM_DOMAINS
from markettor.email import is_email_available
from markettor.event_usage import (
    report_user_logged_in,
    report_user_updated,
    report_user_verified_email,
)
from markettor.middleware import get_impersonated_session_expires_at
from markettor.models import Dashboard, Team, User, UserScenePersonalisation
from markettor.models.organization import Organization
from markettor.models.user import NOTIFICATION_DEFAULTS, Notifications
from markettor.permissions import APIScopePermission
from markettor.rate_limit import UserAuthenticationThrottle, UserEmailVerificationThrottle
from markettor.tasks import user_identify
from markettor.tasks.email import send_email_change_emails
from markettor.user_permissions import UserPermissions
from markettor.utils import get_js_url
...
def redirect_to_site(request):
    ...
    # see https://github.com/MarketTor/markettor/issues/9671
    state = urllib.parse.quote(json.dumps(params), safe="")

    return redirect("{}#__markettor={}".format(app_url, state))
...
def redirect_to_website(request):
    ...
    if request.user.strapi_id is None:
        response = requests.request(
            "POST",
            "https://squeak.markettor.cc/api/auth/local/register",
            json={
        ...
        if response.status_code == 200:
            ...
            if response.text and error_message == "Email or Username are already taken":
                return redirect("https://markettor.com/auth?error=emailIsTaken")
    else:
        token = jwt.encode(
            {
    ...
    return redirect("{}?userData={}&redirect={}".format("https://markettor.com/auth", userData, app_url))
...
def test_slack_webhook(request):
    ...
    message = {"text": "_Greetings_ from MarketTor!"}
    try:
