...
from markettor.constants import EventDefinitionType
from markettor.exceptions import (
    RequestParsingError,
    UnspecifiedCompressionFallbackParsingError,
    generate_exception_response,
)
from markettor.models import Entity, EventDefinition
from markettor.models.entity import MathType
...
from markettor.utils import load_data_from_request
        ...
        if request.POST.get("api_key"):
            ...
            if isinstance(data, dict):
                if data.get("$token"):
                    ...
                    token = data["api_key"]  # server-side libraries like markettor-python and markettor-ruby
...
# keep in sync with markettor/plugin-server/src/utils/db/utils.ts::safeClickhouseString
...
def create_event_definitions_sql(
    ...
    enterprise_join = (
        "FULL OUTER JOIN ee_enterpriseeventdefinition ON markettor_eventdefinition.id=ee_enterpriseeventdefinition.eventdefinition_ptr_id"
        if is_enterprise
        else ""
    )

    if event_type == EventDefinitionType.EVENT_CUSTOM:
        conditions += " AND markettor_eventdefinition.name NOT LIKE %(is_markettor_event)s"
    if event_type == EventDefinitionType.EVENT_MARKETTOR:
        conditions += " AND markettor_eventdefinition.name LIKE %(is_markettor_event)s"
