...
class AsyncMigration(models.Model):
    ...
    markettor_min_version = models.CharField(max_length=20, null=True, blank=True)
    markettor_max_version = models.CharField(max_length=20, null=True, blank=True)
    ...
    def get_name_with_requirements(self) -> str:
        return (
            f"{self.name} - must be ran on MarketTor version {self.markettor_min_version} up to {self.markettor_max_version}"
        )
