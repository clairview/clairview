...
from markettor.redis import redis, get_client
import time
...
REDIS_LOCK_TOKEN = "markettor:decide_analytics:lock"
...
def get_team_request_key(team_id: int, request_type: FlagRequestType) -> str:
    if request_type == FlagRequestType.DECIDE:
        return f"markettor:decide_requests:{team_id}"
    elif request_type == FlagRequestType.LOCAL_EVALUATION:
        return f"markettor:local_evaluation_requests:{team_id}"
    else:
