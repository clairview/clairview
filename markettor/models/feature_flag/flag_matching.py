...
from markettor.models.person import Person, PersonDistinctId
from markettor.models.property import GroupTypeIndex, GroupTypeName
from markettor.models.property.property import Property
from markettor.models.cohort import Cohort, CohortOrEmpty
from markettor.models.utils import execute_with_timeout
from markettor.queries.base import match_property, properties_to_Q, sanitize_property_key
from markettor.database_healthcheck import (
    postgres_healthcheck,
    DATABASE_FOR_FLAG_MATCHING,
)
from markettor.utils import label_for_team_id_to_track
                ...
                with execute_with_timeout(FLAG_MATCHING_QUERY_TIMEOUT_MS, DATABASE_FOR_FLAG_MATCHING) as cursor:
                    ...
                    query = """
                        WITH target_person_ids AS (
                            SELECT team_id, person_id FROM markettor_persondistinctid WHERE team_id = %(team_id)s AND
                        ...
                        SELECT key FROM markettor_featureflag WHERE team_id = %(team_id)s AND ensure_experience_continuity = TRUE AND active = TRUE AND deleted = FALSE
            ...
            with execute_with_timeout(FLAG_MATCHING_QUERY_TIMEOUT_MS) as cursor:
                query = """
                    ...
                    INSERT INTO markettor_featureflaghashkeyoverride (team_id, person_id, feature_flag_key, hash_key)
                        ...
                        WHERE EXISTS (SELECT 1 FROM markettor_person WHERE id = person_id AND team_id = %(team_id)s)
