...
from markettor.constants import INVITE_DAYS_VALIDITY, MAX_SLUG_LENGTH, AvailableFeature
...
class Organization(UUIDModel):
    ...
    members = models.ManyToManyField(
        "markettor.User",
        through="markettor.OrganizationMembership",
        related_name="organizations",
        related_query_name="organization",
    )
    name = models.CharField(max_length=64)
    slug: LowercaseSlugField = LowercaseSlugField(unique=True, max_length=MAX_SLUG_LENGTH)
    logo_media = models.ForeignKey("markettor.UploadedMedia", on_delete=models.SET_NULL, null=True, blank=True)
...
def organization_membership_saved(sender: Any, instance: OrganizationMembership, **kwargs: Any) -> None:
    from markettor.event_usage import report_user_organization_membership_level_changed
