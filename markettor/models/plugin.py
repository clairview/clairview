...
from markettor.plugins.access import can_configure_plugins, can_install_plugins
from markettor.plugins.plugin_server_api import populate_plugin_capabilities_on_workers, reload_plugins_on_workers
...
def update_validated_data_from_url(validated_data: dict[str, Any], url: str) -> dict[str, Any]:
    ...
    if url.startswith("file:"):
        ...
        markettor_version = plugin_json.get("markettorVersion", None)
    ...
    if markettor_version and not is_cloud():
        # Legacy: MarketTor is no longer versioned
        try:
            spec = SimpleSpec(markettor_version.replace(" ", ""))
        except ValueError:
            raise ValidationError(f'Invalid MarketTor semantic version requirement "{markettor_version}"!')
        if FROZEN_MARKETTOR_VERSION not in spec:
            raise ValidationError(
                f'Currently running MarketTor version {FROZEN_MARKETTOR_VERSION} does not match this plugin\'s semantic version requirement "{markettor_version}".'
            )
...
class Plugin(models.Model):
    ...
    # DEPRECATED: this was used when syncing markettor.json with the db on app start
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                ...
                name="markettor_unique_plugin_storage_key",
            )
        ]
...
def transpile(input_string: str, type: Literal["site", "frontend"] = "site") -> Optional[str]:
    from markettor.settings.base_variables import BASE_DIR
