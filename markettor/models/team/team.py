...
from markettor.helpers.dashboard_templates import create_dashboard_from_template
...
from markettor.settings.utils import get_list
from markettor.utils import GenericEmails
...
# keep in sync with markettor/frontend/src/scenes/project/Settings/ExtraTeamSettings.tsx
...
class TeamManager(models.Manager):
    ...
    def increment_id_sequence(self) -> int:
        ...
        cursor.execute("SELECT nextval('markettor_team_id_seq')")
...
class Team(UUIDClassicModel):
    ...
    # This is meant to be used as a stopgap until https://github.com/MarketTor/meta/pull/39 gets implemented
    ...
    def _person_on_events_person_id_no_override_properties_on_events(self) -> bool:
        ...
        # on MarketTor Cloud, use the feature flag
        if is_cloud():
            return markettoranalytics.feature_enabled(
    ...
    def reset_token_and_save(self, *, user: "User", is_impersonated_session: bool):
        from markettor.models.activity_logging.activity_log import Change, Detail, log_activity
    ...
    def kick_off_demo_data_generation(self, initiating_user: "User") -> None:
        from markettor.tasks.demo_create_data import create_data_for_demo_team
