...
from markettor.demo.matrix.taxonomy_inference import infer_taxonomy_for_team
...
class MatrixManager:
        ...
        *,
        ...
        if existing_user is None:
            ...
            if is_staff and not existing_user.is_staff:
                ...
                # and now are logging in securely as a MarketTor team member
    ...
    def _create_master_team(cls) -> Team:
        organization = Organization.objects.create(id=cls.MASTER_TEAM_ID, name="MarketTor")
    ...
    def _copy_analytics_data_from_master_team(self, target_team: Team):
        from markettor.models.event.sql import COPY_EVENTS_BETWEEN_TEAMS
        from markettor.models.group.sql import COPY_GROUPS_BETWEEN_TEAMS
    ...
    def _sync_postgres_with_clickhouse_data(cls, source_team_id: int, target_team_id: int):
        from markettor.models.group.sql import SELECT_GROUPS_OF_TEAM
        from markettor.models.person.sql import (
    ...
    def _save_sim_person(self, team: Team, subject: SimPerson):
        ...
        if subject.past_events:
            from markettor.models.person.util import (
                create_person,
                create_person_distinct_id,
            )

            create_person(
                uuid=str(subject.in_markettor_id),
                team_id=team.pk,
            ...
            for distinct_id in subject.distinct_ids_at_now:
                create_person_distinct_id(
                    ...
                    person_id=str(subject.in_markettor_id),
                )
    ...
    def _save_sim_group(
        ...
        from markettor.models.group.util import raw_create_group_ch
