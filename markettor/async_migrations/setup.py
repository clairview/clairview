...
from markettor.settings import TEST
...
ASYNC_MIGRATIONS_MODULE_PATH = "markettor.async_migrations.migrations"
ASYNC_MIGRATIONS_EXAMPLE_MODULE_PATH = "markettor.async_migrations.examples"

all_migrations = import_submodules(ASYNC_MIGRATIONS_MODULE_PATH)
reload_migration_definitions()


def setup_async_migrations(ignore_markettor_version: bool = False):
    """
    ...
    3. Check if all migrations necessary for this MarketTor version have completed (else don't start)
    ...
    for migration_name, migration in ALL_ASYNC_MIGRATIONS.items():
        ...
        if (
            (not ignore_markettor_version)
            and (migration_name in unapplied_migrations)
            and (FROZEN_MARKETTOR_VERSION > Version(migration.markettor_max_version))
        ):
            raise ImproperlyConfigured(
                f"Migration {migration_name} is required for MarketTor versions above {FROZEN_MARKETTOR_VERSION}."
            )
...
def setup_model(migration_name: str, migration: AsyncMigrationDefinition) -> AsyncMigration:
    ...
    sm.markettor_max_version = migration.markettor_max_version
    sm.markettor_min_version = migration.markettor_min_version
