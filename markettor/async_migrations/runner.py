...
from markettor.async_migrations.definition import AsyncMigrationDefinition
from markettor.async_migrations.setup import (
    FROZEN_MARKETTOR_VERSION,
    get_async_migration_definition,
    get_async_migration_dependency,
)
from markettor.async_migrations.utils import (
    complete_migration,
    execute_op,
...
from markettor.version_requirement import ServiceVersionRequirement
...
def start_async_migration(
    migration_name: str,
    ignore_markettor_version=False,
    ...
    2. The migration can be run with the current MarketTor version
    ...
    """
    send_analytics_to_markettor("Async migration start", {"name": migration_name})
    ...
    if not (
        ignore_markettor_version
        or is_markettor_version_compatible(
            migration_instance.markettor_min_version,
            migration_instance.markettor_max_version,
        )
    ):
        process_error(
            migration_instance,
            f"Migration is not available on this MarketTor version",
...
def is_markettor_version_compatible(markettor_min_version, markettor_max_version):
    return get_instance_setting("ASYNC_MIGRATIONS_IGNORE_MARKETTOR_VERSION") or FROZEN_MARKETTOR_VERSION in SimpleSpec(
        f">={markettor_min_version},<={markettor_max_version}"
    )


def run_next_migration(candidate: str):
    migration_instance = AsyncMigration.objects.get(name=candidate)
    migration_in_range = is_markettor_version_compatible(
        migration_instance.markettor_min_version, migration_instance.markettor_max_version
    )
