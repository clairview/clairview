        ...
        cursor.execute(
            ...
            SELECT markettor_team.id,
                COALESCE(
                    MIN(
                        CASE
                            WHEN markettor_dashboard.pinned THEN markettor_dashboard.id
                            ELSE NULL
                        END
                    ),
                    MIN(
                        CASE
                            WHEN NOT markettor_dashboard.pinned THEN markettor_dashboard.id
                            ELSE NULL
                        END
                    )
                ) AS primary_dashboard_id
            FROM markettor_team
            INNER JOIN markettor_dashboard ON markettor_dashboard.team_id = markettor_team.id
            WHERE markettor_team.primary_dashboard_id IS NULL
            GROUP BY markettor_team.id
            """
        )
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("markettor", "0219_migrate_tags_v2"),
    ]
