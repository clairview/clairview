...
DECLARE
    ...
    row TEXT;
BEGIN 
    new_table_name := 'new_markettor_event';

    EXECUTE ('SET TIME ZONE UTC ');
    EXECUTE format('CREATE TABLE %s (like markettor_event including defaults) partition by list (event)', new_table_name);

    -- Add dummy timestamp column so that constraints can be met between new partitioned table
    EXECUTE('ALTER TABLE markettor_action_events ADD COLUMN timestamp timestamp, ADD COLUMN event varchar(200);');
    EXECUTE('ALTER TABLE markettor_element ADD COLUMN timestamp timestamp, ADD COLUMN event varchar(200)');

    range_begin := (SELECT date_trunc('week', MIN(timestamp)) as range_begin from markettor_event);
    ...
    EXECUTE('CREATE TABLE markettor_event_partitions_manifest (event varchar(200) NOT NULL);');
    FOREACH row IN ARRAY $1 LOOP
        EXECUTE format('INSERT INTO markettor_event_partitions_manifest (event) VALUES (''%s'')', row);
        partition_name := 'markettor_event_' || row;
        IF NOT EXISTS
            (SELECT 1
            ...
            EXECUTE format('CREATE TABLE %I PARTITION OF public.new_markettor_event FOR VALUES IN ($var$%s$var$) partition by range (timestamp)', partition_name, row);
    ...
    partition_name := 'markettor_event_' || 'default';
    EXECUTE ('INSERT INTO markettor_event_partitions_manifest (event) VALUES (''default'')');
    IF NOT EXISTS
        (SELECT 1
        FROM   information_schema.tables 
        WHERE  table_name = partition_name)
        THEN
        RAISE NOTICE 'Partition created: %', partition_name;
        EXECUTE format('CREATE TABLE %I PARTITION OF public.new_markettor_event DEFAULT partition by range (timestamp)', partition_name);
        temp_range_begin := range_begin;
        WHILE temp_range_begin <= range_end
        LOOP
            partition_date := to_char(temp_range_begin,'YYYY_MM_DD');
    ...
    EXECUTE ('INSERT INTO public.new_markettor_event (id, event, properties, elements, timestamp, team_id, distinct_id, elements_hash)
    SELECT id, event, properties, elements, timestamp, team_id, distinct_id, elements_hash
    FROM public.markettor_event;');

    -- replace old table with new partitioned table
    EXECUTE ('ALTER TABLE markettor_event RENAME TO old_markettor_event');
    EXECUTE ('ALTER TABLE new_markettor_event RENAME TO markettor_event');

    EXECUTE ('ALTER SEQUENCE markettor_event_id_seq OWNED BY markettor_event."id"');
    EXECUTE ('DROP TABLE old_markettor_event CASCADE');

    EXECUTE ('CREATE UNIQUE INDEX markettor_event_pkey ON public.markettor_event USING btree (id, timestamp, event)');
    EXECUTE ('CREATE INDEX markettor_event_team_id_a8b4c6dc ON public.markettor_event USING btree (team_id)');
    EXECUTE ('CREATE INDEX markettor_event_idx_distinct_id ON public.markettor_event USING btree (distinct_id)');
    EXECUTE ('CREATE INDEX markettor_eve_element_48becd_idx ON public.markettor_event USING btree (elements_hash)');
    EXECUTE ('CREATE INDEX markettor_eve_timesta_1f6a8c_idx ON public.markettor_event USING btree ("timestamp", team_id, event)');
    
    EXECUTE ('ALTER TABLE markettor_event ADD CONSTRAINT markettor_event_team_id_a8b4c6dc_fk_markettor_team_id FOREIGN KEY (team_id) REFERENCES markettor_team(id) DEFERRABLE INITIALLY DEFERRED');
    EXECUTE ('ALTER TABLE markettor_action_events ADD CONSTRAINT markettor_action_events_event_id_7077ea70_fk_markettor_event_id FOREIGN KEY (event_id, timestamp, event) REFERENCES markettor_event(id, timestamp, event) DEFERRABLE INITIALLY DEFERRED');
    EXECUTE ('ALTER TABLE markettor_element ADD CONSTRAINT markettor_element_event_id_bb6549a0_fk_markettor_event_id FOREIGN KEY (event_id, timestamp, event) REFERENCES markettor_event(id, timestamp, event) DEFERRABLE INITIALLY DEFERRED');
END
$$
LANGUAGE plpgsql;
...
BEGIN 
    ...
    IF NOT EXISTS
        (SELECT 1
        FROM   information_schema.tables 
        WHERE  table_name = 'markettor_event')
    THEN
        RETURN;
    END IF;

    FOR row IN(SELECT * FROM markettor_event_partitions_manifest) LOOP
        event_table_name := 'markettor_event_' || row.event;
