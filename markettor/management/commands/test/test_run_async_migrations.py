...
from markettor.async_migrations.setup import ALL_ASYNC_MIGRATIONS
...
def test_plan_includes_all_migrations_except_past_max_version(caplog):
    ...
    not return migrations that are still within the markettor_min_version,
    markettor_max_version range. This is to ensure that the application is able to
    ...
    for migration_name, migration in ALL_ASYNC_MIGRATIONS.items():
        if FROZEN_MARKETTOR_VERSION > Version(migration.markettor_max_version):
