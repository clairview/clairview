...
from markettor.api.test.batch_exports.conftest import describe_schedule, start_test_worker
from markettor.api.test.test_organization import create_organization
from markettor.api.test.test_team import create_team
from markettor.management.commands.create_batch_export_from_app import (
    map_plugin_config_to_destination,
)
from markettor.models import Plugin, PluginAttachment, PluginConfig
from markettor.temporal.common.client import sync_connect
from markettor.temporal.common.codec import EncryptionCodec
...
def snowflake_plugin(organization) -> typing.Generator[Plugin, None, None]:
    plugin = Plugin.objects.create(
        ...
        url=append_random("https://github.com/MarketTor/snowflake-export-plugin"),
        plugin_type="custom",
...
def s3_plugin(organization) -> typing.Generator[Plugin, None, None]:
    plugin = Plugin.objects.create(
        ...
        url=append_random("https://github.com/MarketTor/s3-export-plugin"),
        plugin_type="custom",
        organization=organization,
    )
    yield plugin
...
def bigquery_plugin(organization) -> typing.Generator[Plugin, None, None]:
    plugin = Plugin.objects.create(
        ...
        url=append_random("https://github.com/MarketTor/bigquery-plugin"),
        plugin_type="custom",
        organization=organization,
    )
    yield plugin
...
def postgres_plugin(organization) -> typing.Generator[Plugin, None, None]:
    plugin = Plugin.objects.create(
        ...
        url=append_random("https://github.com/MarketTor/postgres-plugin"),
        plugin_type="custom",
        organization=organization,
    )
    yield plugin
