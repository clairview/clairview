...
from markettor.async_migrations.runner import (
    complete_migration,
...
def get_necessary_migrations() -> Sequence[AsyncMigration]:
    ...
    for migration_name, definition in sorted(ALL_ASYNC_MIGRATIONS.items()):
        ...
        if FROZEN_MARKETTOR_VERSION > Version(sm.markettor_max_version):
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        setup_async_migrations(ignore_markettor_version=True)
...
def handle_check(necessary_migrations: Sequence[AsyncMigration]):
            ...
            "\n".join(
                [
                    ...
                    *(f"- {migration.get_name_with_requirements()}" for migration in necessary_migrations),
                    "See more in Docs: https://markettor.com/docs/self-host/configure/async-migrations/overview",
                ]
            )
        )
        exit(1)
    ...
    if running_migrations.exists():
        logger.critical(
            "\n".join(
                [
                    "Stopping MarketTor!",
                    f"Async migration {running_migrations[0].name} is currently running. If you're trying to update MarketTor, wait for it to finish before proceeding",
                    "See more in Docs: https://markettor.com/docs/self-host/configure/async-migrations/overview",
                ]
            )
        )
    ...
    if errored_migrations.exists():
        logger.error(
            "\n".join(
                [
                    f"Stopping MarketTor!",
                    "Some async migrations are currently in an 'Errored' state. If you're trying to update MarketTor, please make sure they complete successfully first:",
...
def handle_run(necessary_migrations: Sequence[AsyncMigration]):
    for migration in necessary_migrations:
        ...
        started_successfully = start_async_migration(migration.name, ignore_markettor_version=True)
