...
def validate_migration_sql(sql) -> bool:
    ...
    for operation_sql in operations:
        # Extract table name from queries of this format: ALTER TABLE TABLE "markettor_feature"
        table_being_altered: Optional[str] = (
            re.findall(r"ALTER TABLE \"([a-z_]+)\"", operation_sql)[0] if "ALTER TABLE" in operation_sql else None
        )
        # Extract table name from queries of this format: CREATE TABLE "markettor_feature"
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not migrations:
            migrations = ["markettor/migrations/0339_add_session_recording_storage_version.py"]
