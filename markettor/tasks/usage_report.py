...
from markettor import version_requirement
...
from markettor.constants import FlagRequestType
from markettor.logging.timing import timed_log
from markettor.models import GroupTypeMapping, OrganizationMembership, User
...
from markettor.models.utils import namedtuplefetchall
from markettor.settings import CLICKHOUSE_CLUSTER, INSTANCE_TAG
...
from markettor.warehouse.models import ExternalDataJob
...
class TableSizes(TypedDict):
    markettor_event: int
    markettor_sessionrecordingevent: int
...
def get_instance_metadata(period: tuple[datetime, datetime]) -> InstanceMetadata:
    ...
    if realm != "cloud":
        ...
        metadata.table_sizes = {
            "markettor_event": fetch_table_size("markettor_event"),
            "markettor_sessionrecordingevent": fetch_table_size("markettor_sessionrecordingevent"),
        }
...
def get_teams_with_billable_event_count_in_period(
    ...
    if count_distinct:
        ...
        # https://github.com/MarketTor/markettor/blob/master/markettor/models/event/sql.py#L92
    ...
    results = sync_execute(
        ...
        SELECT
            ...
            multiIf(
                ...
                {lib_expression} = 'markettor-node', 'node_events',
                {lib_expression} = 'markettor-android', 'android_events',
                {lib_expression} = 'markettor-flutter', 'flutter_events',
                {lib_expression} = 'markettor-ios', 'ios_events',
                {lib_expression} = 'markettor-go', 'go_events',
                {lib_expression} = 'markettor-java', 'java_events',
                {lib_expression} = 'markettor-react-native', 'react_native_events',
                {lib_expression} = 'markettor-ruby', 'ruby_events',
                {lib_expression} = 'markettor-python', 'python_events',
                {lib_expression} = 'markettor-php', 'php_events',
                'other'
            ) AS metric,
            count(1) as count
        FROM events
...
def capture_report(
    ...
    try:
        ...
        logger.info(f"UsageReport sent to MarketTor for organization {org_id}")
    except Exception as err:
        logger.exception(
            f"UsageReport sent to MarketTor for organization {org_id} failed: {str(err)}",
        )
...
def send_all_org_usage_reports(
    ...
    try:
        ...
        logger.info("Sending usage reports to MarketTor and Billing...")  # noqa T201
        time_now = datetime.now()
        for org_report in org_reports.values():
            ...
            # First capture the events to MarketTor
        ...
        time_since = datetime.now() - time_now
        logger.debug(f"Sending usage reports to MarketTor and Billing took {time_since.total_seconds()} seconds.")  # noqa T201
    except Exception as err:
        capture_exception(err)
        raise
