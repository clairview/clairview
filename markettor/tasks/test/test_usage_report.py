...
from markettor.hogql_queries.events_query_runner import EventsQueryRunner
from markettor.models import Organization, Plugin, Team
from markettor.models.app_metrics2.sql import TRUNCATE_APP_METRICS2_TABLE_SQL
from markettor.models.dashboard import Dashboard
from markettor.models.event.util import create_event
...
from markettor.models.plugin import PluginConfig
from markettor.models.sharing_configuration import SharingConfiguration
from markettor.schema import EventsQuery
...
from markettor.test.fixtures import create_app_metric2
from markettor.utils import get_machine_id, get_previous_day
from markettor.warehouse.models import ExternalDataJob, ExternalDataSource
...
class UsageReport(APIBaseTest, ClickhouseTestMixin, ClickhouseDestroyTablesMixin):
    ...
    def _test_usage_report(self) -> list[dict]:
        with self.settings(SITE_URL="http://test.markettor.com"):
            ...
            assert report["table_sizes"]["markettor_event"] < 10**7  # <10MB
            assert report["table_sizes"]["markettor_sessionrecordingevent"] < 10**7  # <10MB
    ...
    @patch("markettor.tasks.usage_report.Client")
    @patch("requests.post")
    def test_unlicensed_usage_report(self, mock_post: MagicMock, mock_client: MagicMock) -> None:
        ...
        mock_markettor = MagicMock()
        mock_client.return_value = mock_markettor
        ...
        assert mock_markettor.capture.call_count == 2
        mock_markettor.capture.assert_has_calls(calls, any_order=True)
...
class TestFeatureFlagsUsageReport(ClickhouseDestroyTablesMixin, TestCase, ClickhouseTestMixin):
    ...
    def _setup_teams(self) -> None:
        self.analytics_org = Organization.objects.create(name="MarketTor")
    ...
    @snapshot_clickhouse_queries
    @patch("markettor.tasks.usage_report.Client")
    @patch("markettor.tasks.usage_report.send_report_to_billing_service")
    def test_usage_report_decide_requests(self, billing_task_mock: MagicMock, markettor_capture_mock: MagicMock) -> None:
        self._setup_teams()
        for i in range(10):
            _create_event(
                distinct_id="3",
    ...
    def test_usage_report_local_evaluation_requests(
        self, billing_task_mock: MagicMock, markettor_capture_mock: MagicMock
    ) -> None:
        self._setup_teams()
        for i in range(10):
            _create_event(
...
class TestSurveysUsageReport(ClickhouseDestroyTablesMixin, TestCase, ClickhouseTestMixin):
    ...
    @patch("markettor.tasks.usage_report.Client")
    @patch("markettor.tasks.usage_report.send_report_to_billing_service")
    def test_usage_report_survey_responses(self, billing_task_mock: MagicMock, markettor_capture_mock: MagicMock) -> None:
        self._setup_teams()
        for i in range(10):
            _create_event(
                distinct_id="3",
...
class TestHogFunctionUsageReports(ClickhouseDestroyTablesMixin, TestCase, ClickhouseTestMixin):
    ...
    @patch("markettor.tasks.usage_report.Client")
    @patch("markettor.tasks.usage_report.send_report_to_billing_service")
    def test_hog_function_usage_metrics(self, billing_task_mock: MagicMock, markettor_capture_mock: MagicMock) -> None:
        self._setup_teams()
...
class SendUsageTest(LicensedTestMixin, ClickhouseDestroyTablesMixin, APIBaseTest):
    ...
    def test_send_usage(self, mock_post: MagicMock, mock_client: MagicMock) -> None:
        ...
        mock_markettor.capture.assert_any_call(
            get_machine_id(),
            "organization usage report",
    ...
    @freeze_time("2021-10-10T23:01:00Z")
    @patch("markettor.tasks.usage_report.capture_exception")
    @patch("markettor.tasks.usage_report.sync_execute", side_effect=Exception())
    @patch("markettor.tasks.usage_report.Client")
    @patch("requests.post")
