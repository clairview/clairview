...
from markettor.batch_exports.models import (
    BatchExport,
    BatchExportDestination,
    BatchExportRun,
)
from markettor.models import Organization, Team, User
...
from markettor.models.plugin import Plugin, PluginConfig
from markettor.tasks.email import (
...
@patch("markettor.tasks.email.EmailMessage")
class TestEmail(APIBaseTest, ClickhouseTestMixin):
    ...
    def setUpTestData(cls) -> None:
        ...
        create_org_team_and_user("2022-01-01 00:00:00", "too_late_user@markettor.com")
        create_org_team_and_user(
            "2022-01-02 00:00:00",
            "ingested_event_in_range_user@markettor.com",
            ingested_event=True,
        )
        create_org_team_and_user("2022-01-03 00:00:00", "too_early_user@markettor.com")

    def test_send_invite(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)

        org, user = create_org_team_and_user("2022-01-02 00:00:00", "admin@markettor.com")
        invite = OrganizationInvite.objects.create(organization=org, created_by=user, target_email="test@markettor.com")
    ...
    def test_send_fatal_plugin_error_with_settings(self, MockEmailMessage: MagicMock) -> None:
        ...
        self._create_user("test2@markettor.com")
        ...
        assert mocked_email_messages[0].to == [{"recipient": "test2@markettor.com", "raw_email": "test2@markettor.com"}]
    ...
    def test_send_batch_export_run_failure(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)
        _, user = create_org_team_and_user("2022-01-02 00:00:00", "admin@markettor.com")
    ...
    def test_send_canary_email(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)
        send_canary_email("test@markettor.com")
    ...
    def test_send_async_migration_complete_email(self, MockEmailMessage: MagicMock) -> None:
        ...
        User.objects.create(email="staff-user@markettor.com", password="password", is_staff=True)
