...
import markettor.tasks.tasks
from markettor.redis import get_client
from markettor.tasks.poll_query_performance import query_manager_from_initial_query_id, poll_query_performance
from markettor.tasks.tasks import Polling
...
class TestPollQueryPerformance(SimpleTestCase):
    ...
    @patch("markettor.tasks.poll_query_performance.QueryStatusManager")
    @patch("markettor.tasks.poll_query_performance.sync_execute")
...
class TestPollQueryPerformanceTask(SimpleTestCase):
    @patch("markettor.tasks.tasks.logger.error")
    def test_poll_query_performance_does_not_run_if_last_update_does_not_match(
        ...
        markettor.tasks.tasks.poll_query_performance("DIFFERENT TIME")
        mock_logger_error.assert_called_once_with("Poll query performance task terminating: another poller is running")

    @patch("markettor.tasks.tasks.logger.error")
    @patch("markettor.tasks.tasks.poll_query_performance.apply_async")
    def test_poll_query_performance_runs_and_restarts_itself_with_delay(
        ...
        markettor.tasks.tasks.poll_query_performance(key)
    ...
    @patch("markettor.tasks.tasks.logger.error")
    @patch("markettor.tasks.tasks.poll_query_performance.delay")
    ...
    def test_start_poll_query_performance_does_nothing_for_14_seconds(
        ...
        for _ in range(3):
            markettor.tasks.tasks.start_poll_query_performance()
