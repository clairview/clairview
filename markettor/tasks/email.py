...
from markettor.email import EMAIL_TASK_KWARGS, EmailMessage, is_email_available
...
def send_invite(invite_id: str) -> None:
    ...
    message = EmailMessage(
        ...
        subject=f"{invite.created_by.first_name} invited you to join {invite.organization.name} on MarketTor",
...
def send_member_join(invitee_uuid: str, organization_id: str) -> None:
    ...
    message = EmailMessage(
        campaign_key=campaign_key,
        subject=f"{invitee.first_name} joined you on MarketTor",
...
def send_password_reset(user_id: int, token: str) -> None:
    ...
    message = EmailMessage(
        ...
        subject=f"Reset your MarketTor password",
...
def send_batch_export_run_failure(
    ...
    message = EmailMessage(
        campaign_key=campaign_key,
        subject=f"MarketTor: {batch_export_run.batch_export.name} batch export run failure",
...
def send_canary_email(user_email: str) -> None:
    message = EmailMessage(
        ...
        subject="This is a test email of your MarketTor instance",
...
def send_email_change_emails(now_iso: str, user_name: str, old_address: str, new_address: str) -> None:
    message_old_address = EmailMessage(
        ...
        subject="This is no longer your MarketTor account email",
    ...
    message_new_address = EmailMessage(
        ...
        subject="This is your new MarketTor account email",
