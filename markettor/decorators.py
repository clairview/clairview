...
from markettor.caching.utils import is_stale_filter
        ...
        if not refresh_requested_by_client(request):
            ...
            if cached_result_package and cached_result_package.get("result"):
                if not is_stale_filter(team, filter, cached_result_package):
                    ...
                    statsd.incr("markettor_cached_function_cache_hit", tags={"route": route})
                    return cached_result_package
                else:
                    statsd.incr("markettor_cached_function_cache_stale", tags={"route": route})
            else:
                statsd.incr("markettor_cached_function_cache_miss", tags={"route": route})
