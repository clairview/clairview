...
# (see https://github.com/clairview/clairview/issues/23822)
RUN chown node:node .
USER node
...
RUN <<EOF
	...
	pnpm --filter clairview deploy --prod dist
	cd dist
	# Regenerate package.json file with essential fields only
	# (see https://github.com/clairview/clairview/issues/20338)
	node -e '
...
ENV \
	DB_CLIENT="sqlite3" \
	DB_FILENAME="/clairview/database/database.sqlite" \
	NODE_ENV="production" \
	NPM_CONFIG_UPDATE_NOTIFIER="false"

COPY --from=builder --chown=node:node /clairview/ecosystem.config.cjs .
COPY --from=builder --chown=node:node /clairview/dist .
