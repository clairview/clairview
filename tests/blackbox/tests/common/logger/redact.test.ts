...
describe('Logger Redact Tests', () => {
	...
	beforeAll(async () => {
		...
		for (const vendor of vendors) {
			...
			clairviewInstances[vendor] = server;

			promises.push(awaitClairviewConnection(newServerPort));
		}
	...
	afterAll(async () => {
		for (const [vendor, connection] of databases) {
			clairviewInstances[vendor].kill();
			...
			TEST_USERS.forEach((userKey) => {
				describe(USER[userKey].NAME, () => {
					it.each(vendors)('%s', async (vendor) => {
						...
						// Action
						const logger = new TestLogger(clairviewInstances[vendor], '/auth/refresh', true);
						...
						const loggerGql = new TestLogger(clairviewInstances[vendor], '/graphql/system', true);
