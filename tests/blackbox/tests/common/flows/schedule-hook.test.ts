...
describe('Flows Schedule Hook Tests', () => {
	...
	const clairviewInstances = {} as Record<Vendor, ChildProcess[]>;
	...
	beforeAll(async () => {
		...
		for (const vendor of vendors) {
			...
			envRedis1[vendor]['SYNCHRONIZATION_NAMESPACE'] = `clairview-${vendor}`;
			...
			clairviewInstances[vendor] = [server1, server2, server3];
			envs[vendor] = [envRedis1, envRedis2, envMemory];

			promises.push(
				awaitClairviewConnection(newServerPort1),
				awaitClairviewConnection(newServerPort2),
				awaitClairviewConnection(newServerPort3),
			);
		}
