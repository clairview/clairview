...
import { awaitClairviewConnection } from '@utils/await-connection';
...
describe('App Caching Tests', () => {
	...
	const clairviewInstances = {} as { [vendor: string]: ChildProcess[] };
	...
	const cacheNamespacePrefix = 'clairview-app-cache';
	...
	beforeAll(async () => {
		...
		for (const vendor of vendors) {
			...
			envMem[vendor]['CACHE_AUTO_PURGE_IGNORE_LIST'] = `clairview_activity,clairview_presets,${collectionIgnored}`;
			...
			clairviewInstances[vendor] = [serverMem, serverMemPurge, serverRedis, serverRedisPurge];
			envs[vendor] = { envMem, envMemPurge, envRedis, envRedisPurge };

			promises.push(
				awaitClairviewConnection(newServerPortMem),
				awaitClairviewConnection(newServerPortMemPurge),
				awaitClairviewConnection(newServerPortRedis),
				awaitClairviewConnection(newServerPortRedisPurge),
			);
		}
	...
	afterAll(async () => {
		for (const [vendor, connection] of databases) {
			for (const instance of clairviewInstances[vendor]!) {
				instance.kill();
			}
