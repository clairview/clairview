...
describe('Schema Caching Tests', () => {
	describe('GET /collections/:collection', () => {
		describe('schema change propagates across nodes using messenger', () => {
			...
			const tzClairview = {} as { [vendor: string]: ChildProcess[] };
			...
			beforeAll(async () => {
				...
				for (const vendor of vendors) {
					...
					env1[vendor]['CACHE_NAMESPACE'] = 'clairview-schema-cache';
					...
					tzClairview[vendor] = [server1, server2];
					envs[vendor] = [env1, env2];

					promises.push(awaitClairviewConnection(newServerPort1), awaitClairviewConnection(newServerPort2));
				}
			...
			afterAll(async () => {
				for (const [vendor, connection] of databases) {
					for (const instance of tzClairview[vendor]!) {
						instance.kill();
					}
		...
		describe('schema change does not propagate across nodes without messenger', () => {
			...
			beforeAll(async () => {
				...
				for (const vendor of vendors) {
					...
					const cacheNamespacePrefix = `clairview-${vendor}`;
					...
					tzClairview[vendor] = [server3, server4];
					envs[vendor] = [env3, env4];

					promises.push(awaitClairviewConnection(newServerPort3), awaitClairviewConnection(newServerPort4));
				}
