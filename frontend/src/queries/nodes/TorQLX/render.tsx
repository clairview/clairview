...
export function parseTorQLX(value: any): any {
    ...
    if (value[0] === '__hx_tag') {
        ...
        for (let i = start; i < value.length; i += 2) {
            const key = parseTorQLX(value[i])
            object[key] = parseTorQLX(value[i + 1])
        }
        return object
    }
    return value.map((v) => parseTorQLX(v))
}
    ...
    return (
        <ErrorBoundary>
            <LemonButton
                ...
                data-attr="tor-ql-view-recording-button"
...
export function renderTorQLX(value: any): JSX.Element {
    const object = parseTorQLX(value)
    ...
    if (typeof object === 'object') {
        ...
        if (!tag) {
            ...
            return (
                <ErrorBoundary>
                    <Link to={href} target={target ?? '_self'}>
                        {source ? renderTorQLX(source) : href}
                    </Link>
                </ErrorBoundary>
            )
        } else if (tag === 'strong') {
            ...
            return (
                <ErrorBoundary>
                    <strong>{renderTorQLX(source)}</strong>
                </ErrorBoundary>
            )
        } else if (tag === 'em') {
            const { source } = rest
