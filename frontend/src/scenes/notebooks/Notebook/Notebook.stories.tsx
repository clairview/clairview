        ...
        mswDecorator({
            post: {
                'api/projects/:team_id/query': {
                    clickhouse:
                        "SELECT nullIf(nullIf(events.`$session_id`, ''), 'null') AS session_id, any(events.properties) AS properties FROM events WHERE and(equals(events.team_id, 1), in(events.event, [%(clairql_val_0)s, %(clairql_val_1)s]), ifNull(in(session_id, [%(clairql_val_2)s]), 0), ifNull(greaterOrEquals(toTimeZone(events.timestamp, %(clairql_val_3)s), %(clairql_val_4)s), 0), ifNull(lessOrEquals(toTimeZone(events.timestamp, %(clairql_val_5)s), %(clairql_val_6)s), 0)) GROUP BY session_id LIMIT 100 SETTINGS readonly=2, max_execution_time=60, allow_experimental_object_type=True",
                    columns: ['session_id', 'properties'],
                    clairql: "SELECT properties.$session_id AS session_id, any(properties) AS properties FROM events WHERE and(in(event, ['$pageview', '$autocapture']), in(session_id, ['018a8a51-a39d-7b18-897f-94054eec5f61']), greaterOrEquals(timestamp, '2023-09-11 16:55:36'), lessOrEquals(timestamp, '2023-09-13 18:07:40')) GROUP BY session_id LIMIT 100",
