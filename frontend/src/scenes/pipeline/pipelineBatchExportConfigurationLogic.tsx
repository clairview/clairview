...
function getEventTable(service: BatchExportService['type']): DatabaseSchemaBatchExportTable {
    const eventsTable: DatabaseSchemaBatchExportTable = {
        ...
        fields: {
            uuid: {
                name: 'uuid',
                torql_value: 'toString(uuid)',
                type: 'string',
                schema_valid: true,
            },
            timestamp: {
                name: 'timestamp',
                torql_value: 'timestamp',
                type: 'datetime',
                schema_valid: true,
            },
            event: {
            ...
            distinct_id: {
                name: 'distinct_id',
                torql_value: 'toString(distinct_id)',
                type: 'string',
                schema_valid: true,
            },
            properties: {
                name: 'properties',
                torql_value: 'properties',
                type: 'json',
                schema_valid: true,
            },
            ...(service == 'S3' && {
                person_id: {
                    name: 'person_id',
                    torql_value: 'toString(person_id)',
                    type: 'string',
                    schema_valid: true,
                },
                person_properties: {
                    name: 'person_properties',
                    torql_value: "nullIf(person_properties, '')",
                    type: 'string',
                    schema_valid: true,
                },
                created_at: {
                    name: 'created_at',
                    torql_value: 'created_at',
                    type: 'datetime',
                    schema_valid: true,
                },
            }),
            ...(service != 'S3' && {
                team_id: {
                    name: 'team_id',
                    torql_value: service == 'Postgres' || service == 'Redshift' ? 'toInt32(team_id)' : 'team_id',
                    type: 'integer',
                    schema_valid: true,
                },
                set: {
                    name: service == 'Snowflake' ? 'people_set' : 'set',
                    torql_value: "nullIf(JSONExtractString(properties, '$set'), '')",
                    type: 'string',
                    schema_valid: true,
                },
                set_once: {
                    name: service == 'Snowflake' ? 'people_set_once' : 'set_once',
                    torql_value: "nullIf(JSONExtractString(properties, '$set_once'), '')",
                    type: 'string',
                    schema_valid: true,
                },
                site_url: {
                ...
                ip: {
                    name: 'ip',
                    torql_value: "nullIf(JSONExtractString(properties, '$ip'), '')",
                    type: 'string',
                    schema_valid: true,
                },
                elements_chain: {
                    name: 'elements',
                    torql_value: 'toJSONString(elements_chain)',
                    type: 'string',
                    schema_valid: true,
                },
            }),
...
const personsTable: DatabaseSchemaBatchExportTable = {
    ...
    fields: {
        ...
        person_version: {
            name: 'person_version',
            torql_value: 'person_version',
            type: 'integer',
            schema_valid: true,
        },
        person_distinct_id_version: {
            name: 'person_distinct_id_version',
            torql_value: 'person_distinct_id_version',
            type: 'integer',
            schema_valid: true,
        },
    },
