...
function getEventTable(service: BatchExportService['type']): DatabaseSchemaBatchExportTable {
    const eventsTable: DatabaseSchemaBatchExportTable = {
        ...
        fields: {
            uuid: {
                name: 'uuid',
                clairql_value: 'toString(uuid)',
                type: 'string',
                schema_valid: true,
            },
            timestamp: {
                name: 'timestamp',
                clairql_value: 'timestamp',
                type: 'datetime',
                schema_valid: true,
            },
            event: {
            ...
            distinct_id: {
                name: 'distinct_id',
                clairql_value: 'toString(distinct_id)',
                type: 'string',
                schema_valid: true,
            },
            properties: {
                name: 'properties',
                clairql_value: 'properties',
                type: 'json',
                schema_valid: true,
            },
            ...(service == 'S3' && {
                person_id: {
                    name: 'person_id',
                    clairql_value: 'toString(person_id)',
                    type: 'string',
                    schema_valid: true,
                },
                person_properties: {
                    name: 'person_properties',
                    clairql_value: "nullIf(person_properties, '')",
                    type: 'string',
                    schema_valid: true,
                },
                created_at: {
                    name: 'created_at',
                    clairql_value: 'created_at',
                    type: 'datetime',
                    schema_valid: true,
                },
            }),
            ...(service != 'S3' && {
                team_id: {
                    name: 'team_id',
                    clairql_value: service == 'Postgres' || service == 'Redshift' ? 'toInt32(team_id)' : 'team_id',
                    type: 'integer',
                    schema_valid: true,
                },
                set: {
                    name: service == 'Snowflake' ? 'people_set' : 'set',
                    clairql_value: "nullIf(JSONExtractString(properties, '$set'), '')",
                    type: 'string',
                    schema_valid: true,
                },
                set_once: {
                    name: service == 'Snowflake' ? 'people_set_once' : 'set_once',
                    clairql_value: "nullIf(JSONExtractString(properties, '$set_once'), '')",
                    type: 'string',
                    schema_valid: true,
                },
                site_url: {
                ...
                ip: {
                    name: 'ip',
                    clairql_value: "nullIf(JSONExtractString(properties, '$ip'), '')",
                    type: 'string',
                    schema_valid: true,
                },
                elements_chain: {
                    name: 'elements',
                    clairql_value: 'toJSONString(elements_chain)',
                    type: 'string',
                    schema_valid: true,
                },
            }),
...
const personsTable: DatabaseSchemaBatchExportTable = {
    ...
    fields: {
        team_id: {
            name: 'team_id',
            clairql_value: 'team_id',
            type: 'integer',
            schema_valid: true,
        },
        distinct_id: {
            name: 'distinct_id',
            clairql_value: 'distinct_id',
            type: 'string',
            schema_valid: true,
        },
        person_id: {
        ...
        person_version: {
            name: 'person_version',
            clairql_value: 'person_version',
            type: 'integer',
            schema_valid: true,
        },
        person_distinct_id_version: {
            name: 'person_distinct_id_version',
            clairql_value: 'person_distinct_id_version',
            type: 'integer',
            schema_valid: true,
        },
    },
