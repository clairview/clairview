...
import { TorQLQuery, TorQLQueryResponse } from '~/queries/schema'

import { QueryTabs } from './QueryTabs'

interface TorQLDebugProps {
    queryKey: `new-${string}`
    query: TorQLQuery
    setQuery: (query: TorQLQuery) => void
}

export function TorQLDebug({ query, setQuery, queryKey }: TorQLDebugProps): JSX.Element {
    ...
    const response = _response as TorQLQueryResponse | null

    return (
        <BindLogic logic={dataNodeLogic} props={dataNodeLogicProps}>
            <div className="space-y-2">
                <TorQLQueryEditor query={query} setQuery={setQuery} />
