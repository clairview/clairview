...
export function loadClairViewJS(): void {
    if (window.JS_CLAIRVIEW_API_KEY) {
        clairview.init(
            window.JS_CLAIRVIEW_API_KEY,
            configWithSentry({
                ...
                api_host: window.JS_CLAIRVIEW_HOST,
                ui_host: window.JS_CLAIRVIEW_UI_HOST,
                rageclick: true,
                persistence: 'localStorage+cookie',
                bootstrap: window.CLAIRVIEW_USER_IDENTITY_WITH_FLAGS ? window.CLAIRVIEW_USER_IDENTITY_WITH_FLAGS : {},
        ...
        if (Cypress) {
            Object.entries(Cypress.env()).forEach(([key, value]) => {
                if (key.startsWith('CLAIRVIEW_PROPERTY_')) {
                    clairview.register_for_session({
                        [key.replace('CLAIRVIEW_PROPERTY_', 'E2E_TESTING_').toLowerCase()]: value,
                    })
                }
            })
        }
