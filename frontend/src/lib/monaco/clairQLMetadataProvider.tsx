...
export const clairQLMetadataProvider: () => languages.CodeActionProvider = () => ({
    provideCodeActions: (model, _range, context) => {
        ...
        if (logic?.isMounted()) {
            ...
            // We have a list of _all_ markers returned from the ClairQL metadata query
            ...
            for (const activeMarker of markersFromMonaco) {
                ...
                for (const rawMarker of markersFromMetadata) {
                    if (
                        rawMarker.clairQLFix &&
                        // if ranges overlap
                        rawMarker.start <= end &&
                        rawMarker.end >= start
                    ) {
                        quickFixes.push({
                            title: `Replace with: ${rawMarker.clairQLFix}`,
