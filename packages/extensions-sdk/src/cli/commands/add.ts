...
import { isIn, isTypeIn } from '@clairview/utils';
...
export default async function add(options: AddOptions): Promise<void> {
	...
	if (!(await fse.pathExists(packagePath))) {
		log(`Current directory is not a valid Clairview extension:`, 'error');
	...
	if (extensionOptions.type === 'bundle') {
		...
		const spinner = ora(chalk.bold('Modifying Clairview extension...')).start();
		...
		const newExtensionManifest = {
			...extensionManifest,
			name: extensionName,
			keywords: ['clairview', 'clairview-extension', 'clairview-extension-bundle'],
			[EXTENSION_PKG_KEY]: newExtensionOptions,
			devDependencies: {
				...extensionManifest.devDependencies,
				...(await getExtensionDevDeps(
