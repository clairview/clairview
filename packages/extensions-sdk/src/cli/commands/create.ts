...
async function createBundleExtension({
	...
	const spinner = ora(chalk.bold('Scaffolding Clairview extension...')).start();
...
function getPackageManifest(name: string, options: ExtensionOptions, deps: Record<string, string>) {
	const packageManifest: Record<string, any> = {
		...
		icon: 'extension',
		version: '1.0.0',
		keywords: ['clairview', 'clairview-extension', `clairview-extension-${options.type}`],
		type: 'module',
		files: ['dist'],
		[EXTENSION_PKG_KEY]: options,
		scripts: {
			build: 'clairview-extension build',
			dev: 'clairview-extension build -w --no-minify',
			link: 'clairview-extension link',
		},
		devDependencies: deps,
	};

	if (options.type === 'bundle') {
		packageManifest['scripts']['add'] = 'clairview-extension add';
	}
