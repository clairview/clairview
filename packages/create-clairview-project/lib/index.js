...
program
	.version(pkg.version)
	.arguments('<directory>')
	.description('Create a new Clairview project')
	.action(create)
	.parse(process.argv);
...
async function create(directory) {
	...
	// Let's get into the Clairview mood while waiting
	const bunnyFrames = [];
	const numOfSpaces = 3;
	...
	spinner.start(chalk.bold('Installing Clairview'));
	...
	try {
		await execa('npm', ['install', 'clairview', '--omit=dev'], {
			cwd: rootPath,
			stdin: 'ignore',
		});
	} catch (err) {
	...
	try {
		await execa('npx', ['clairview', 'init'], {
			cwd: rootPath,
			stdio: 'inherit',
		});
	} catch {
