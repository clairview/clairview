...
async function run() {
	...
	if (githubOutput) {
		const outputs = [
			`CLAIRVIEW_VERSION=${mainVersion}`,
			`CLAIRVIEW_PRERELEASE=${isPrerelease}`,
			...(prereleaseId ? [`CLAIRVIEW_PRERELEASE_ID=${prereleaseId}`] : []),
			`CLAIRVIEW_RELEASE_NOTES<<EOF_RELEASE_NOTES\n${markdown}\nEOF_RELEASE_NOTES`,
		];
