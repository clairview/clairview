import type { Field, Filter } from '@clairview/types';
...
test('should append $thumbnail to fetched items when collection is clairview_files', async () => {
	vi.mocked(useCollection).mockReturnValueOnce({ primaryKeyField: computed(() => null) } as any);

	const collection = ref('clairview_files');
