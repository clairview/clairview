import type { Item, Query } from '@clairview/types';
import { getEndpoint, moveInArray } from '@clairview/utils';
...
export function useItems(collection: Ref<string | null>, query: ComputedQuery): UsableItems {
	...
	async function getItems() {
		...
		try {
			 ...
			 * able to render out the clairview_files collection (file library) using regular layouts
			 *
			 * Layouts expect the file to be a m2o of a `file` type, however, clairview_files is the
			 ...
			 */
			if (collection.value === 'clairview_files') {
