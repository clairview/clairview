...
import { isClairviewVariable } from '../utils/is-clairview-variable.js';
...
vi.mock('../utils/is-clairview-variable.js');
...
describe('File based configuration', () => {
	beforeEach(() => {
		...
		vi.mocked(isClairviewVariable).mockImplementation((key) => {
			return key === 'PROCESS_FILE';
		});
...
test('Passthrough file variables that are not Clairview configuration flags', () => {
	vi.mocked(readConfigurationFromFile).mockReturnValue({
		TEST_FILE: './test/path',
	});

	vi.mocked(isClairviewVariable).mockImplementation(() => {
		return false;
	});
