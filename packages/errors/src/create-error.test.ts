import { randomAlpha, randomInteger } from '@clairview/random';
...
test('Returns enhanced error object', () => {
	...
	expect(error.name).toBe('ClairviewError');
...
test('Overrides toString', () => {
	...
	expect(error.toString()).toBe(`ClairviewError [${sample.code.toUpperCase()}]: ${sample.message}`);
});
