...
import { useCollection, useFilterFields, useItems, useSync } from '@clairview/composables';
import { defineLayout } from '@clairview/extensions';
import { User } from '@clairview/types';
import { getEndpoint, getRelationType, moveInArray } from '@clairview/utils';
	...
	setup(props, { emit }) {
				...
				if (junction !== undefined) {
					const related = relationsStore.relations.find(
						(relation) =>
							...
							relation.related_collection === 'clairview_users',
					);
			...
			file: (field) => {
				...
				const relation = relationsStore.relations.find((relation) => {
					return (
						relation.collection === props.collection &&
						relation.field === field.field &&
						relation.related_collection === 'clairview_files'
					);
				});
		...
		function useLayoutQuery() {
			...
			const fields = computed<string[]>(() => {
				...
				if (props.collection === 'clairview_files' && imageSource.value === '$thumbnail') {
