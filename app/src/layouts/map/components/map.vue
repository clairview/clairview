...
import type { ShowSelect } from '@clairview/extensions';
...
const boxSelectControl = new BoxSelectControl({
	...
	layers: ['__clairview_polygons', '__clairview_points', '__clairview_lines'],
});
...
function setupMap() {
	...
	map.on('load', () => {
		...
		const activeLayers = ['__clairview_polygons', '__clairview_points', '__clairview_lines'];
		...
		map.on('click', '__clairview_clusters', expandCluster);
		map.on('mousemove', '__clairview_clusters', hoverCluster);
		map.on('mouseleave', '__clairview_clusters', hoverCluster);
...
function updateData(newData: any) {
	const source = map.getSource('__clairview');
...
function updateSource(newSource: GeoJSONSource) {
	...
	if (map.getStyle().sources?.['__clairview']) {
		map.removeSource('__clairview');
	}

	map.addSource('__clairview', { ...newSource, data: props.data });
...
function updateSelection(newSelection?: (string | number)[], previousSelection?: (string | number)[]) {
	previousSelection?.forEach((id) => {
		map.setFeatureState({ id, source: '__clairview' }, { selected: false });
		map.removeFeatureState({ id, source: '__clairview' });
	});

	newSelection?.forEach((id) => {
		map.setFeatureState({ id, source: '__clairview' }, { selected: true });
	});
}
...
function updatePopup(event: MapLayerMouseEvent) {
	...
	if (previousId && featureChanged) {
		map.setFeatureState({ id: previousId, source: '__clairview' }, { hovered: false });
	}
	...
	if (feature && feature.properties) {
		...
		if (featureChanged) {
			map.setFeatureState({ id: feature.id, source: '__clairview' }, { hovered: true });
...
function expandCluster(event: MapLayerMouseEvent) {
	const features = map.queryRenderedFeatures(event.point, {
		layers: ['__clairview_clusters'],
	});

	const clusterId = features[0]?.properties?.cluster_id;
	const source = map.getSource('__clairview') as GeoJSONSource;
