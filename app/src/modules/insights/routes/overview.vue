...
const { createAllowed, updateAllowed, deleteAllowed } = useCollectionPermissions('clairview_dashboards');
...
function exportDasboard(ids: string[]) {
	const endpoint = getEndpoint('clairview_dashboards');
...
<template>
	<private-view :title="t('insights')">
		...
		<template #sidebar>
			...
			<basic-import-sidebar-detail collection="clairview_dashboards" @refresh="refresh" />
		</template>
