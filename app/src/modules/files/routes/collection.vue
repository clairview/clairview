...
import { useLayout } from '@clairview/composables';
import { mergeFilters } from '@clairview/utils';
...
const { layout, layoutOptions, layoutQuery, filter, search, resetPreset } = usePreset(ref('clairview_files'));
...
const {
	...
	createAllowed,
} = useCollectionPermissions('clairview_files');

const { createAllowed: createFolderAllowed } = useCollectionPermissions('clairview_folders');
...
<template>
	...
	>
		...
		>
			...
			<template #actions>
				<search-input v-model="search" v-model:filter="filter" collection="clairview_files" />
