...
const { layout, layoutOptions, layoutQuery, filter, search, resetPreset } = usePreset(ref('clairview_users'));
...
} = useCollectionPermissions('clairview_users');

const { readAllowed: rolesReadAllowed } = useCollectionPermissions('clairview_roles');
...
<template>
	...
	>
		...
		>
			...
			<template #actions>
				<search-input v-model="search" v-model:filter="filter" collection="clairview_users" />
