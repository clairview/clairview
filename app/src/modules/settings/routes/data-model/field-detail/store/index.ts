...
import type { DisplayConfig, InterfaceConfig } from '@clairview/extensions';
import type { Collection, DeepPartial, Field, LocalType, Relation } from '@clairview/types';
		...
		async startEditing(collection: string, field: string, localType?: LocalType) {
			...
			if (field !== '+') {
				...
				if (['files', 'm2m', 'translations', 'm2a'].includes(this.localType)) {
					// These types rely on clairview_relations fields being said, so meta should exist for these particular relations
