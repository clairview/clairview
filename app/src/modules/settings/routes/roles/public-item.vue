...
import { useApi } from '@clairview/composables';
import { Alterations, Item, Policy } from '@clairview/types';
...
const policiesField = cloneDeep(fieldsStore.getField('clairview_roles', 'policies'));
...
policiesField!.meta!.options = {
	...
	filter: {
		'$FOLLOW(clairview_access,policy)': {
			_none: {
				_and: [
					{
						role: {
