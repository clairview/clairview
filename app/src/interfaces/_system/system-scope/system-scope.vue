...
const options = computed(() => {
	let options: any[] = [
		...
		},
		{
			...
			value: 'clairview_users',
		},
		{
			text: t('role') + ': ' + t('select'),
			value: 'clairview_roles',
		},
	];
...
function onSelectItem(value: (string | number)[] | null) {
	if (collection.value === 'clairview_users') return emit('input', 'user_' + value![0]);
	if (collection.value === 'clairview_roles') return emit('input', 'role_' + value![0]);
}
