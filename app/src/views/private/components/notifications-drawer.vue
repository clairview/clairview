...
import { useItems } from '@clairview/composables';
import { useAppStore } from '@clairview/stores';
import { Filter, Notification } from '@clairview/types';
...
watch(notificationsDrawerOpen, (open) => {
	...
	collection.value = open ? 'clairview_notifications' : null;
});
...
<template>
	...
	>
		...
		<template #actions>
			<search-input v-model="search" v-model:filter="filter" collection="clairview_notifications" />
			<v-button
