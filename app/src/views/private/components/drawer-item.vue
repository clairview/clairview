...
import { useCollection } from '@clairview/composables';
import { isSystemCollection } from '@clairview/system-data';
import { Field, PrimaryKey, Relation } from '@clairview/types';
import { getEndpoint } from '@clairview/utils';
...
function useFile() {
	const isClairviewFiles = computed(() => {
		return props.collection === 'clairview_files' || relatedCollection.value === 'clairview_files';
	});

	const file = computed(() => {
		if (isClairviewFiles.value === false || !initialValues.value) return null;
