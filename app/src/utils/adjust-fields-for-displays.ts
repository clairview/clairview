...
import { Field } from '@clairview/types';
		...
		.map((fieldKey) => {
			...
			if (fieldKeys) {
				return fieldKeys.map((fieldKey) => {
					 ...
					 * clairview_files. The thumbnail itself isn't a real field, but shows the thumbnail based
					 * on the other available fields (like ID, title, and type).
					 */
					if (fieldKey.includes('$thumbnail') && field.collection === 'clairview_files') {
						return fieldKey
							.split('.')
