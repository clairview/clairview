...
import { Alterations, Field, Item, PrimaryKey, Query, Relation } from '@clairview/types';
...
export function useItem<T extends Item>(
	...
	function setItemValueToResponse(response: AxiosResponse) {
		if (
			(isSystemCollection(collection.value) && collection.value !== 'clairview_collections') ||
			(collection.value === 'clairview_collections' && isSystemCollection(response.data.data.collection ?? ''))
		) {
