import { APP_SHARED_DEPS } from '@clairview/extensions';
import { generateExtensionsEntrypoint, resolveFsExtensions, resolveModuleExtensions } from '@clairview/extensions/node';
...
export default defineConfig({
	plugins: [
		...
		{
			name: 'watch-clairview-dependencies',
			configureServer: (server) => {
				server.watcher.options = {
					...server.watcher.options,
					ignored: [/node_modules\/(?!@clairview\/).*/, '**/.git/**'],
				};
			},
		},
	],
...
function clairviewExtensions() {
	const virtualExtensionsId = '@clairview-extensions';

	let extensionsEntrypoint = null;

	return [
		{
			name: 'clairview-extensions-serve',
			apply: 'serve',
			config: () => ({
				optimizeDeps: {
		...
		{
			name: 'clairview-extensions-build',
			apply: 'build',
			config: () => ({
				build: {
					rollupOptions: {
