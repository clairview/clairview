...
import type { SchemaInspector } from '@clairview/schema';
...
export async function isInstalled(): Promise<boolean> {
	...
	// The existence of a clairview_collections table alone isn't a "proper" check to see if everything
	// is installed correctly of course, but it's safe enough to assume that this collection only
	// exists when Clairview is properly installed.
	return await inspector.hasTable('clairview_collections');
}
...
export async function validateMigrations(): Promise<boolean> {
	...
	try {
		...
		const completedVersions = (await database.select('version').from('clairview_migrations')).map(
			({ version }) => version,
		);
