...
export async function up(knex: Knex): Promise<void> {
	...
	await knex.schema.alterTable('clairview_shares', (table) => {
...
/**
 * Temporarily drop foreign key constraint for MySQL instances, see https://github.com/clairview/clairview/issues/19399
 */
async function dropConstraint(knex: Knex) {
	...
	const foreignKeys = await inspector.foreignKeys('clairview_shares');
	...
	if (constraintName && collectionForeignKeys.length === 1) {
		...
		logger.warn(`Unexpected number of foreign key constraints on 'clairview_shares.collection':`);
...
async function recreateConstraint(knex: Knex) {
	return knex.schema.alterTable('clairview_shares', async (table) => {
		table.foreign('collection').references('clairview_collections.collection').onDelete('CASCADE');
	});
}
