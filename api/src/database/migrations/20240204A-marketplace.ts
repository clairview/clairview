import { resolvePackage } from '@clairview/utils/node';
...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.alterTable('clairview_extensions', (table) => {
	...
	const installedExtensions = await knex.select('name').from('clairview_extensions');
	...
	for (const { name } of installedExtensions) {
		...
		if (name.includes(':')) {
			await knex('clairview_extensions').delete().where({ name });
		} else {
			const id = randomUUID();
			...
			await knex('clairview_extensions').update({ id, source, folder: name }).where({ name });
	...
	for (const { name } of installedExtensions) {
		...
		await knex('clairview_extensions')
			.update({
	...
	await knex.transaction(async (trx) => {
		await trx.schema.alterTable('clairview_extensions', (table) => {
			table.dropPrimary();
			table.primary(['id']);
		});
	});
...
export async function down(knex: Knex): Promise<void> {
	...
	const installedExtensions = await knex.select(['id', 'folder', 'bundle']).from('clairview_extensions');
	...
	for (const { id, folder, bundle, source } of installedExtensions) {
		if (source === 'registry') {
			await knex('clairview_extensions').delete().where({ id });
			continue;
		}
		...
		await knex('clairview_extensions').update({ name }).where({ id });
	}
