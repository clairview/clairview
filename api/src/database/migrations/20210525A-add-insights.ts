...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.createTable('clairview_dashboards', (table) => {
	...
	await knex.schema.createTable('clairview_panels', (table) => {
		table.uuid('id').primary().notNullable();
		table.uuid('dashboard').notNullable().references('id').inTable('clairview_dashboards').onDelete('CASCADE');
		table.string('name');
...
export async function down(knex: Knex): Promise<void> {
	await knex.schema.dropTable('clairview_panels');
	await knex.schema.dropTable('clairview_dashboards');
}
