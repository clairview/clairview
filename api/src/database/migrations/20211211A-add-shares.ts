...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.createTable('clairview_shares', (table) => {
		table.uuid('id').primary().notNullable();
		table.string('name');
		table.string('collection', 64).references('collection').inTable('clairview_collections').onDelete('CASCADE');
		table.string('item');
		table.uuid('role').references('id').inTable('clairview_roles').onDelete('CASCADE');
	...
	await knex.schema.alterTable('clairview_sessions', (table) => {
		...
		table.uuid('share').references('id').inTable('clairview_shares').onDelete('CASCADE');
	});
}
...
export async function down(knex: Knex): Promise<void> {
	...
	await knex.schema.dropTable('clairview_shares');
}
