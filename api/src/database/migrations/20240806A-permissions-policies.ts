import { processChunk, toBoolean } from '@clairview/utils';
...
import type { LogicalFilterAND, LogicalFilterOR, Permission } from '@clairview/types';
...
// Adapted from https://github.com/clairview/clairview/blob/141b8adbf4dd8e06530a7929f34e3fc68a522053/api/src/utils/merge-permissions.ts#L4
...
export async function up(knex: Knex) {
	...
	if (await knex.schema.hasTable('clairview_policies')) {
		return;
	}
	...
	await knex.schema.createTable('clairview_policies', (table) => {
	...
	const roles = await knex
		...
		.from('clairview_roles');

	if (roles.length > 0) {
		await processChunk(roles, 100, async (chunk) => {
			await knex('clairview_policies').insert(chunk);
		});
	}
	...
	try {
		...
		const foreignKeys = await inspector.foreignKeys('clairview_permissions');

		const foreignConstraint =
			foreignKeys.find((foreign) => foreign.foreign_key_table === 'clairview_roles' && foreign.column === 'role')
		...
		logger.warn('Failed to drop foreign key constraint on `role` column in `clairview_permissions` table');
	}

	await knex('clairview_permissions')
		.update({
			role: PUBLIC_POLICY_ID,
		})
		.whereNull('role');

	await knex('clairview_permissions').update({
	...
	await knex.schema.alterTable('clairview_permissions', (table) => {
		...
		table.foreign('policy').references('clairview_policies.id').onDelete('CASCADE');
	});
	...
	// well, which would require the O2A type to exist in Clairview which currently doesn't.
	// Shouldn't be the end of the world here, as we know we're only attaching policies to two other
	// collections.

	await knex.schema.createTable('clairview_access', (table) => {
		table.uuid('id').primary();
		table.uuid('role').references('clairview_roles.id').nullable().onDelete('CASCADE');
		table.uuid('user').references('clairview_users.id').nullable().onDelete('CASCADE');
		table.uuid('policy').references('clairview_policies.id').notNullable().onDelete('CASCADE');
		table.integer('sort');
	});
	...
	await processChunk(policyAttachments, 100, async (chunk) => {
		await knex('clairview_access').insert(chunk);
	});

	await knex('clairview_access').insert({
		id: randomUUID(),
		role: null,
		user: null,
...
export async function down(knex: Knex) {
	/////////////////////////////////////////////////////////////////////////////////////////////////
	// Reinstate access control fields on clairview roles
	...
	// fetch all roles
	const roles: Array<{ id: string | null }> = await knex.select('id').from('clairview_roles');
	...
	for (const role of roles) {
		...
		if (role.id !== null) {
			...
			await knex('clairview_roles').update(roleAccess).where({ id: role.id });
		}
	...
	//  as soon as we reference clairview_roles in clairview_permissions again

	/////////////////////////////////////////////////////////////////////////////////////////////////
	// Drop policy attachments

	await knex.schema.dropTable('clairview_access');
	...
	await processChunk(originalPermissions, 100, async (chunk) => {
		await knex('clairview_permissions').delete(chunk);
	});

	// insert all v10 permissions
	await processChunk(rolePermissions, 100, async (chunk) => {
		await knex('clairview_permissions').insert(chunk);
	});

	await knex.schema.alterTable('clairview_permissions', (table) => {
		table.uuid('role').references('clairview_roles.id').alter();
	...
	await knex.schema.dropTable('clairview_policies');
}
