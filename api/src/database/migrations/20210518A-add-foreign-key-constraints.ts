...
import type { RelationMeta } from '@clairview/types';
...
export async function up(knex: Knex): Promise<void> {
	...
	if (corruptedRelations.length > 0) {
		logger.warn(
			`Encountered one or more corrupted relationships. Please check the following rows in "clairview_relations": ${corruptedRelations.join(
				', ',
			)}`,
		);
	}
