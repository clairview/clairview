...
export async function up(knex: Knex): Promise<void> {
	...
	await knex('clairview_users').update({ appearance: 'dark' }).where({ theme: 'dark' });
	await knex('clairview_users').update({ appearance: 'light' }).where({ theme: 'light' });
	...
	await knex('clairview_settings').update({ project_color: '#6644ff' }).whereNull('project_color');

	await knex.schema.alterTable('clairview_settings', (table) => {
		table.string('project_color').defaultTo('#6644FF').notNullable().alter();
		table.uuid('public_favicon').references('clairview_files.id');
