...
import type { Filter, Permission, Query } from '@clairview/types';
...
export function getDBQuery(
	...
	if (sortRecords) {
		...
		if (needsInnerQuery) {
			...
			if (hasMultiRelationalSort) {
				...
				// Start order by with clairview_row_number. The clairview_row_number is derived from a window function that
				...
				// Since the only relevant result is the first row of this partition, adding the clairview_row_number to the
				// order by here ensures that all rows with a clairview_row_number = 1 show up first in the inner query result,
				// and are correctly truncated by the limit, but not earlier.
				orderByString = `?? asc, ${orderByString}`;
				orderByFields.unshift(knex.ref('clairview_row_number'));
			}
	...
	if (sortRecords) {
		...
		if (hasMultiRelationalSort) {
			wrapperQuery.where('inner.clairview_row_number', '=', 1);
