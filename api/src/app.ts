...
import { InvalidPayloadError, ServiceUnavailableError } from '@clairview/errors';
...
export default async function createApp(): Promise<express.Application> {
	...
	if ((await isInstalled()) === false) {
		logger.error(`Database doesn't have Clairview tables installed.`);
		process.exit(1);
	}
	...
	app.use((_req, res, next) => {
		res.setHeader('X-Powered-By', 'Clairview');
		next();
	});
	...
	if (env['SERVE_APP']) {
		const adminPath = require.resolve('@clairview/app');
		...
		const htmlWithVars = html
			...
			.replace('<!-- clairview-embed-head -->', embeds.head)
			.replace('<!-- clairview-embed-body -->', embeds.body);
