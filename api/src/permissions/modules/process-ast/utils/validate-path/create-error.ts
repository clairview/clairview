import { type ClairviewError, ForbiddenError } from '@clairview/errors';

export function createCollectionForbiddenError(path: string, collection: string): ClairviewError<any> {
...
export function createFieldsForbiddenError(path: string, collection: string, fields: string[]): ClairviewError<any> {
	const pathSuffix = path === '' ? 'root' : `"${path}"`;
