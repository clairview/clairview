...
export function validateSnapshot(snapshot: Snapshot, force = false) {
	...
	if (snapshot.clairview !== version) {
		throw new InvalidPayloadError({
			reason: `Provided snapshot's clairview version ${snapshot.clairview} does not match the current instance's version ${version}. You can bypass this check by passing the "force" query parameter`,
		});
	}
