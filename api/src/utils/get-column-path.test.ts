import type { DeepPartial } from '@clairview/types';
import { expect, test } from 'vitest';
import { InvalidQueryError } from '@clairview/errors';
...
/*
{
  ...
  collection: 'articles',
  aliasMap: {
    author: { alias: 'grenv', collection: 'clairview_users' },
	'author.role': { alias: 'ljnsv', collection: 'clairview_roles' },
  },
  relations: []
...
test('Returns correct alias for m2o', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		collection: 'articles',
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'clairview_users' },
			'author.role': { alias: 'grenv', collection: 'clairview_roles' },
		},
		relations: [
			{
				collection: 'articles',
				field: 'author',
				related_collection: 'clairview_users',
				meta: null,
				schema: null,
			},
			{
				...
				field: 'role',
				related_collection: 'clairview_roles',
				meta: null,
				schema: null,
			},
		],
	...
	expect(result.targetCollection).toBe('clairview_roles');
});
...
test('Returns correct alias when there are multiple joins to the same table', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'clairview_users' },
			'author.role': { alias: 'grenv', collection: 'clairview_roles' },
			'author.secondary_role': { alias: 'psgwn', collection: 'clairview_roles' },
		},
		relations: [
			{
				collection: 'articles',
