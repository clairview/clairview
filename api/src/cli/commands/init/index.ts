...
export default async function init(): Promise<void> {
	...
	await db('clairview_roles').insert({ ...defaultAdminRole, id: role });
	await db('clairview_policies').insert({ ...defaultAdminPolicy, id: policy });
	await db('clairview_access').insert({ id: randomUUID(), role, policy });

	await db('clairview_users').insert({
		...defaultAdminUser,
		id: randomUUID(),
	...
	process.stdout.write(`\nStart Clairview by running:\n`);
	process.stdout.write(`  ${chalk.blue('cd')} ${rootPath}\n`);
	process.stdout.write(`  ${chalk.blue('npx clairview')} start\n`);
