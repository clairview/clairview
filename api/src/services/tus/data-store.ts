...
import type { TusDriver, ChunkedUploadContext } from '@clairview/storage';
import type { Accountability, File, SchemaOverview } from '@clairview/types';
...
export class TusDataStore extends DataStore {
	...
	public override async create(upload: Upload): Promise<Upload> {
		...
		const itemsService = new ItemsService<File>('clairview_files', {
		...
		if ('folder' in fileData === false) {
			const settings = await knex.select('storage_default_folder').from('clairview_settings').first();
	...
	public override async write(readable: stream.Readable, tus_id: string, offset: number): Promise<number> {
		...
		const sudoService = new ItemsService<File>('clairview_files', {
			schema: this.schema,
		});
