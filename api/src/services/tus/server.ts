...
import type { Driver, TusDriver } from '@clairview/storage';
import { supportsTus } from '@clairview/storage';
...
export async function createTusServer(context: Context): Promise<[Server, () => void]> {
	...
	const server = new Server({
		...
		async onUploadFinish(req: any, res, upload) {
			const service = new ItemsService<File>('clairview_files', {
				schema: req.schema,
			});
	...
	server.on(EVENTS.POST_CREATE, async (_req, res, upload) => {
		res.setHeader('Clairview-File-Id', upload.metadata!['id']!);
	});
