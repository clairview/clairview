...
import { ForbiddenError, InvalidPayloadError, LimitExceededError, UnprocessableContentError } from '@clairview/errors';
import type { ApiOutput, BundleExtension, ExtensionSettings } from '@clairview/extensions';
import { describe, type DescribeOptions } from '@clairview/extensions-registry';
import type { Accountability, DeepPartial, SchemaOverview } from '@clairview/types';
import { isObject } from '@clairview/utils';
...
export class ExtensionsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.extensionsItemService = new ItemsService('clairview_extensions', {
			knex: this.knex,
			schema: this.schema,
	...
	private async checkBundleAndSyncStatus(trx: Knex, extensionId: string, extension: ApiOutput) {
		if (extension.bundle === null && extension.schema?.type === 'bundle') {
			...
			await trx('clairview_extensions')
		...
		const hasEnabledChildren = !!(await trx('clairview_extensions')
			.where({ bundle: parentId })
			.where({ enabled: true })
			.first());

		if (hasEnabledChildren) {
			await trx('clairview_extensions').update({ enabled: true }).where({ id: parentId });
		} else {
			await trx('clairview_extensions').update({ enabled: false }).where({ id: parentId });
		}
	}
}
