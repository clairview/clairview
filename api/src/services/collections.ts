...
import { ForbiddenError, InvalidPayloadError } from '@clairview/errors';
import type { SchemaInspector, Table } from '@clairview/schema';
import { createInspector } from '@clairview/schema';
import { systemCollectionRows, type BaseCollectionMeta } from '@clairview/system-data';
import type { Accountability, FieldMeta, RawField, SchemaOverview } from '@clairview/types';
import { addFieldFlag } from '@clairview/utils';
...
export class CollectionsService {
	...
	async createOne(payload: RawCollection, opts?: MutationOptions): Promise<string> {
		...
		if (payload.collection.startsWith('clairview_')) {
			throw new InvalidPayloadError({ reason: `Collections can't start with "clairview_"` });
		}
		...
		try {
			const existingCollections: string[] = [
				...((await this.knex.select('collection').from('clairview_collections'))?.map(({ collection }) => collection) ??
					[]),
			...
			await transaction(this.knex, async (trx) => {
				if (payload.schema) {
					// Clairview heavily relies on the primary key of a collection, so we have to make sure that
					...
					const fieldItemsService = new ItemsService('clairview_fields', {
						knex: trx,
				...
				if (payload.meta) {
					const collectionItemsService = new ItemsService('clairview_collections', {
						knex: trx,
						accountability: this.accountability,
						schema: this.schema,
					});
	...
	async deleteOne(collectionKey: string, opts?: MutationOptions): Promise<string> {
		...
		try {
			...
			await transaction(this.knex, async (trx) => {
				...
				await trx('clairview_collections').update({ group: null }).where({ group: collectionKey });
				...
				if (collectionToBeDeleted!.schema) {
					...
					await trx('clairview_fields').delete().where('collection', '=', collectionKey);
					await trx('clairview_presets').delete().where('collection', '=', collectionKey);

					const revisionsToDelete = await trx
						.select('id')
						.from('clairview_revisions')
						.where({ collection: collectionKey });
					...
					if (revisionsToDelete.length > 0) {
						...
						for (const keys of chunks) {
							await trx('clairview_revisions').update({ parent: null }).whereIn('parent', keys);
						}
					}

					await trx('clairview_revisions').delete().where('collection', '=', collectionKey);

					await trx('clairview_activity').delete().where('collection', '=', collectionKey);
					await trx('clairview_permissions').delete().where('collection', '=', collectionKey);
					await trx('clairview_relations').delete().where({ many_collection: collectionKey });
