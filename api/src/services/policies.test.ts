...
// import { ForbiddenError, UnprocessableContentError } from '@clairview/errors';
// import { randomUUID } from '@clairview/random';
...
// 	beforeEach(() => {
// 		tracker.on.any('clairview_roles').response({});

// 		tracker.on
// 			.select(/"clairview_roles"."id" from "clairview_roles" order by "clairview_roles"."id" asc limit .*/)
// 			.response([]);

// 		tracker.on
// 			.select(
// 				/(select count\("clairview_users"\."id"\) as "count", "clairview_roles"\."admin_access", "clairview_roles"\."app_access" from "clairview_users").*(where "clairview_roles"\."id" = \?).*/,
// 			)
...
// 							tracker.on.select('select "admin_access" from "clairview_roles"').responseOnce({ admin_access });
// 							tracker.on.select('select "id" from "clairview_users" where "role" = ?').responseOnce([{ id: userId1 }]);
...
// 							tracker.on
// 								.select('select "id" from "clairview_users" where "role" = ?')
// 								.responseOnce([{ id: userId1 }, { id: userId2 }]);

// 							tracker.on.select('select count(*) as "count" from "clairview_users"').responseOnce({ count: 1 });
...
// 							tracker.on.select('select "admin_access" from "clairview_roles"').responseOnce({ admin_access });
// 							tracker.on.select('select "id" from "clairview_users" where "role" = ?').responseOnce([{ id: userId1 }]);
// 							tracker.on.select('select count(*) as "count" from "clairview_users"').responseOnce({ count: 0 });
...
// 							tracker.on
// 								.select('select count(*) as "count" from "clairview_users" where "id" in')
...
// 										sql.startsWith('select count(*) as "count" from "clairview_users"') && bindings.includes(userId2),
// 								)
// 								.response({ count: 1 });
