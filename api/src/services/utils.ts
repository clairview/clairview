...
import type { Accountability, PrimaryKey, SchemaOverview } from '@clairview/types';
...
export class UtilsService {
	...
	async sort(collection: string, { item, to }: { item: PrimaryKey; to: PrimaryKey }): Promise<void> {
		const sortFieldResponse =
			(await this.knex.select('sort_field').from('clairview_collections').where({ collection }).first()) ||
