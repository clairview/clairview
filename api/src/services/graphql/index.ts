import { FUNCTIONS } from '@clairview/constants';
import { useEnv } from '@clairview/env';
import { ErrorCode, ForbiddenError, InvalidPayloadError, isClairviewError, type ClairviewError } from '@clairview/errors';
...
import { parseFilterFunctionPath, toBoolean } from '@clairview/utils';
...
import isClairviewJWT from '../../utils/is-clairview-jwt.js';
...
const READ_ONLY = ['clairview_activity', 'clairview_revisions'];
...
export class GraphQLService {
					...
					fields: Object.values(collection.fields).reduce(
						(acc, field) => {
							...
							if (collection.primary === field.field && fieldIsInconsistent === false) {
								// permissions IDs need to be nullable https://github.com/clairview/clairview/issues/20509
								if (collection.collection === 'clairview_permissions') {
									type = GraphQLID;
	 ...
	 * Clairview' query structure which is then executed by the services.
	 */
	async resolveQuery(info: GraphQLResolveInfo): Promise<Partial<Item> | null> {
		let collection = info.fieldName;
		if (this.scope === 'system') collection = `clairview_${collection}`;
	 ...
	 * whole nested tree into one big query using Clairview' own query resolver, we want to have a nested structure of
	 * arguments for the whole resolving tree, which can later be transformed into Clairview' AST using `deep`.
	...
	/**
	 * Get a Clairview Query object from the parsed arguments (rawQuery) and GraphQL AST selectionSet. Converts SelectionSet into
	 * Clairview' `fields` query for use in the resolver. Also applies variables where appropriate.
	 */
	getQuery(
		rawQuery: Query,
	...
	/**
	 * Replace functions from GraphQL format to Clairview-Filter format
	 */
	...
	/**
	 * Convert Clairview-Exception into a GraphQL format, so it can be returned by GraphQL properly.
	 */
	formatError(error: ClairviewError | ClairviewError[]): GraphQLError {
	...
	injectSystemResolvers(
			...
			auth_password_request: {
				...
				resolve: async (_, args, { req }) => {
					...
					try {
						...
						if (isClairviewError(err, ErrorCode.InvalidPayload)) {
							throw err;
						}
					}
		...
		if ('clairview_collections' in schema.read.collections) {
			Collection.addFields({
				collection: GraphQLString,
				meta: schemaComposer.createObjectTC({
					name: 'clairview_collections_meta',
					fields: Object.values(schema.read.collections['clairview_collections']!.fields).reduce(
						(acc, field) => {
							acc[field.field] = {
								type: field.nullable
				...
				schema: schemaComposer.createObjectTC({
					name: 'clairview_collections_schema',
					fields: {
						name: GraphQLString,
		...
		if ('clairview_fields' in schema.read.collections) {
			Field.addFields({
				collection: GraphQLString,
				field: GraphQLString,
				type: GraphQLString,
				meta: schemaComposer.createObjectTC({
					name: 'clairview_fields_meta',
					fields: Object.values(schema.read.collections['clairview_fields']!.fields).reduce(
						(acc, field) => {
							acc[field.field] = {
								type: field.nullable
									? getGraphQLType(field.type, field.special)
		...
		if ('clairview_relations' in schema.read.collections) {
			Relation.addFields({
				collection: GraphQLString,
				field: GraphQLString,
				related_collection: GraphQLString,
				schema: schemaComposer.createObjectTC({
					name: 'clairview_relations_schema',
					fields: {
				...
				meta: schemaComposer.createObjectTC({
					name: 'clairview_relations_meta',
					fields: Object.values(schema.read.collections['clairview_relations']!.fields).reduce(
						(acc, field) => {
							acc[field.field] = {
		...
		if (this.accountability?.admin === true) {
			schemaComposer.Mutation.addFields({
				create_collections_item: {
					type: Collection,
					args: {
						data: toInputObjectType(Collection.clone('create_clairview_collections'), {
							postfix: '_input',
						}).addFields({
							fields: [
								toInputObjectType(Field.clone('create_clairview_collections_fields'), { postfix: '_input' }).NonNull,
							],
						}).NonNull,
					},
					resolve: async (_, args) => {
				...
				update_collections_item: {
					...
					args: {
						collection: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(Collection.clone('update_clairview_collections'), {
							postfix: '_input',
			...
			schemaComposer.Mutation.addFields({
				create_fields_item: {
					...
					args: {
						collection: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(Field.clone('create_clairview_fields'), { postfix: '_input' }).NonNull,
					},
					resolve: async (_, args) => {
						const service = new FieldsService({
							accountability: this.accountability,
				...
				update_fields_item: {
					...
					args: {
						collection: new GraphQLNonNull(GraphQLString),
						field: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(Field.clone('update_clairview_fields'), { postfix: '_input' }).NonNull,
					},
					resolve: async (_, args) => {
						const service = new FieldsService({
							accountability: this.accountability,
			...
			schemaComposer.Mutation.addFields({
				create_relations_item: {
					type: Relation,
					args: {
						data: toInputObjectType(Relation.clone('create_clairview_relations'), { postfix: '_input' }).NonNull,
					},
					resolve: async (_, args) => {
				...
				update_relations_item: {
					...
					args: {
						collection: new GraphQLNonNull(GraphQLString),
						field: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(Relation.clone('update_clairview_relations'), { postfix: '_input' }).NonNull,
					},
					resolve: async (_, args) => {
						const relationsService = new RelationsService({
							accountability: this.accountability,
			...
			Extension.addFields({
				...
				name: new GraphQLNonNull(GraphQLString),
				schema: schemaComposer.createObjectTC({
					name: 'clairview_extensions_schema',
					fields: {
						type: GraphQLString,
				...
				meta: schemaComposer.createObjectTC({
					name: 'clairview_extensions_meta',
					fields: {
					...
					args: {
						...
						data: toInputObjectType(
							schemaComposer.createObjectTC({
								name: 'update_clairview_extensions_input',
								fields: {
									meta: schemaComposer.createObjectTC({
										name: 'update_clairview_extensions_input_meta',
										fields: {
											enabled: GraphQLBoolean,
										},
									}),
		...
		if ('clairview_users' in schema.read.collections) {
			schemaComposer.Query.addFields({
				users_me: {
					type: ReadCollectionTypes['clairview_users']!,
		...
		if ('clairview_permissions' in schema.read.collections) {
			schemaComposer.Query.addFields({
				permissions_me: {
		...
		if ('clairview_roles' in schema.read.collections) {
			schemaComposer.Query.addFields({
				roles_me: {
					type: ReadCollectionTypes['clairview_roles']!.List,
					resolve: async (_, args, __, info) => {
						if (!this.accountability?.user && !this.accountability?.role) return null;
		...
		if ('clairview_policies' in schema.read.collections) {
			schemaComposer.Query.addFields({
				policies_me_globals: {
					type: schemaComposer.createObjectTC({
		...
		if ('clairview_users' in schema.update.collections && this.accountability?.user) {
			schemaComposer.Mutation.addFields({
				update_users_me: {
					type: ReadCollectionTypes['clairview_users']!,
					args: {
						data: toInputObjectType(UpdateCollectionTypes['clairview_users']!),
					},
					resolve: async (_, args, __, info) => {
						if (!this.accountability?.user) return null;
						...
						if ('clairview_users' in ReadCollectionTypes) {
							const selections = this.replaceFragmentsInSelections(
								info.fieldNodes[0]?.selectionSet?.selections,
								info.fragments,
							);
		...
		if ('clairview_activity' in schema.create.collections) {
			schemaComposer.Mutation.addFields({
				create_comment: {
					type: ReadCollectionTypes['clairview_activity'] ?? GraphQLBoolean,
					args: {
						collection: new GraphQLNonNull(GraphQLString),
						item: new GraphQLNonNull(GraphQLID),
					...
					resolve: async (_, args, __, info) => {
						...
						if ('clairview_activity' in ReadCollectionTypes) {
							const selections = this.replaceFragmentsInSelections(
								info.fieldNodes[0]?.selectionSet?.selections,
								info.fragments,
							);
		...
		if ('clairview_activity' in schema.update.collections) {
			schemaComposer.Mutation.addFields({
				update_comment: {
					type: ReadCollectionTypes['clairview_activity'] ?? GraphQLBoolean,
					args: {
		...
		if ('clairview_activity' in schema.delete.collections) {
			schemaComposer.Mutation.addFields({
				delete_comment: {
					type: DeleteCollectionTypes['one']!,
					args: {
		...
		if ('clairview_files' in schema.create.collections) {
			schemaComposer.Mutation.addFields({
				import_file: {
					type: ReadCollectionTypes['clairview_files'] ?? GraphQLBoolean,
					args: {
						url: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(CreateCollectionTypes['clairview_files']!).setTypeName(
							'create_clairview_files_input',
						),
					},
					resolve: async (_, args, __, info) => {
						...
						if ('clairview_files' in ReadCollectionTypes) {
							const selections = this.replaceFragmentsInSelections(
								info.fieldNodes[0]?.selectionSet?.selections,
								info.fragments,
							);
		...
		if ('clairview_users' in schema.create.collections) {
			schemaComposer.Mutation.addFields({
				users_invite: {
					type: GraphQLBoolean,
					args: {
