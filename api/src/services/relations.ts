...
import type { ForeignKey, SchemaInspector } from '@clairview/schema';
import { createInspector } from '@clairview/schema';
import { systemRelationRows } from '@clairview/system-data';
import type { Accountability, Query, Relation, RelationMeta, SchemaOverview } from '@clairview/types';
...
export class RelationsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.relationsItemService = new ItemsService('clairview_relations', {
			knex: this.knex,
			schema: this.schema,
			// We don't set accountability here. If you have read access to certain fields, you are
			// allowed to extract the relations regardless of permissions to clairview_relations. This
	...
	async createOne(relation: Partial<Relation>, opts?: MutationOptions): Promise<void> {
		...
		try {
			...
			await transaction(this.knex, async (trx) => {
				...
				const relationsItemService = new ItemsService('clairview_relations', {
					knex: trx,
					schema: this.schema,
					// We don't set accountability here. If you have read access to certain fields, you are
					// allowed to extract the relations regardless of permissions to clairview_relations. This
	...
	async deleteOne(collection: string, field: string, opts?: MutationOptions): Promise<void> {
		...
		try {
			await transaction(this.knex, async (trx) => {
				...
				if (existingRelation.meta) {
					await trx('clairview_relations').delete().where({ many_collection: collection, many_field: field });
				}
	...
	/**
	 * Combine raw schema foreign key information with Clairview relations meta rows to form final
	 * Relation objects
	 */
