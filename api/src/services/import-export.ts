...
import type { Accountability, File, Query, SchemaOverview } from '@clairview/types';
import { parseJSON, toArray } from '@clairview/utils';
import { createTmpFile } from '@clairview/utils/node';
...
export class ExportService {
	...
	async exportToFile(
		...
		const { createTmpFile } = await import('@clairview/utils/node');
		const tmpFile = await createTmpFile().catch(() => null);
