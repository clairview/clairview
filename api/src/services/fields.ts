...
} from '@clairview/constants';
import { useEnv } from '@clairview/env';
import { ForbiddenError, InvalidPayloadError } from '@clairview/errors';
import type { Column, SchemaInspector } from '@clairview/schema';
import { createInspector } from '@clairview/schema';
import type { Accountability, Field, FieldMeta, RawField, SchemaOverview, Type } from '@clairview/types';
import { addFieldFlag, toArray } from '@clairview/utils';
...
export class FieldsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.itemsService = new ItemsService('clairview_fields', options);
		this.payloadService = new PayloadService('clairview_fields', options);
	...
	async readAll(collection?: string): Promise<Field[]> {
		...
		const nonAuthorizedItemsService = new ItemsService('clairview_fields', {
			knex: this.knex,
			schema: this.schema,
		});
		...
		const aliasQuery = this.knex.select<any[]>('*').from('clairview_fields');
	...
	async readOne(collection: string, field: string): Promise<Record<string, any>> {
		...
		let fieldInfo = await this.knex.select('*').from('clairview_fields').where({ collection, field }).first();
	...
	async createField(
		...
		try {
			const exists =
				...
				isNil(
					await this.knex.select('id').from('clairview_fields').where({ collection, field: field.field }).first(),
				) === false;

			// Check if field already exists, either as a column, or as a row in clairview_fields
			if (exists) {
			...
			await transaction(this.knex, async (trx) => {
				const itemsService = new ItemsService('clairview_fields', {
					knex: trx,
					accountability: this.accountability,
					schema: this.schema,
				});
		...
		try {
			...
			await transaction(this.knex, async (trx) => {
				...
				if (Object.keys(collectionMetaUpdates).length > 0) {
					await trx('clairview_collections').update(collectionMetaUpdates).where({ collection });
				}

				// Cleanup clairview_fields
				...
				if (metaRow) {
					...
					await trx('clairview_fields')
						.update({ group: null })
						.where({ group: metaRow.field, collection: metaRow.collection });
				}

				await trx('clairview_fields').delete().where({ collection, field });
			});
