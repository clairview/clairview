...
import { ContentTooLargeError, ForbiddenError, InvalidPayloadError, ServiceUnavailableError } from '@clairview/errors';
import formatTitle from '@clairview/format-title';
import type { BusboyFileStream, File, PrimaryKey, Query } from '@clairview/types';
...
export class FilesService extends ItemsService<File> {
	...
	async uploadOne(
		...
		if ('folder' in payload === false) {
			const settings = await this.knex.select('storage_default_folder').from('clairview_settings').first();
		...
		const sudoService = new ItemsService('clairview_files', {
			knex: this.knex,
			schema: this.schema,
		});
