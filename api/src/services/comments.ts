...
import { ErrorCode, ForbiddenError, InvalidPayloadError, isClairviewError } from '@clairview/errors';
import type { Filter } from '@clairview/system-data';
import type { Accountability, Comment, Item, PrimaryKey, Query } from '@clairview/types';
...
export class CommentsService extends ItemsService {
	...
	constructor(
		...
		super('clairview_comments', options);
...
`;
				...
				if (isClairviewError(err, ErrorCode.Forbidden)) {
	...
	async migrateLegacyComment(activityPk: PrimaryKey): Promise<PrimaryKey> {
		...
		return transaction(this.knex, async (trx) => {
			let primaryKey;
			const legacyComment = await trx('clairview_activity').select('*').where('id', '=', activityPk).first();

			// Legacy comment
			if (legacyComment['action'] === Action.COMMENT) {
				primaryKey = randomUUID();

				await trx('clairview_comments').insert({
					id: primaryKey,
			...
			// Migrated comment
			else if (legacyComment.collection === 'clairview_comment' && legacyComment.action === Action.CREATE) {
