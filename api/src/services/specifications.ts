...
import { spec } from '@clairview/specs';
import { isSystemCollection } from '@clairview/system-data';
import type { Accountability, FieldOverview, Permission, SchemaOverview, Type } from '@clairview/types';
	...
	async generate(host?: string) {
		...
		const spec: OpenAPIObject = {
			...
			servers: [
				{
					url,
					description: 'Your current Clairview instance.',
				},
			],
			paths,
		};
