...
import {
	...
	UserSuspendedError,
} from '@clairview/errors';
import type { Accountability, SchemaOverview } from '@clairview/types';
...
import type { AbstractServiceOptions, ClairviewTokenPayload, LoginResult, Session, User } from '../types/index.js';
...
export class AuthenticationService {
	...
	async login(
		...
		if (allowedAttempts !== null) {
			...
			try {
				...
				if (error instanceof RateLimiterRes && error.remainingPoints === 0) {
					await this.knex('clairview_users').update({ status: 'suspended' }).where({ id: user.id });
		...
		const tokenPayload: ClairviewTokenPayload = {
			id: user.id,
			role: user.role,
		...
		await this.knex('clairview_sessions').insert({
			token: refreshToken,
			user: user.id,
		...
		await this.knex('clairview_sessions').delete().where('expires', '<', new Date());
		...
		await this.knex('clairview_users').update({ last_access: new Date() }).where({ id: user.id });
	...
	async refresh(refreshToken: string, options?: Partial<{ session: boolean }>): Promise<LoginResult> {
		...
		const record = await this.knex
			...
			.from('clairview_sessions AS s')
			.leftJoin('clairview_users AS u', 's.user', 'u.id')
			.leftJoin('clairview_shares AS d', 's.share', 'd.id')
		...
		if (record.user_id && record.user_status !== 'active') {
			await this.knex('clairview_sessions').where({ token: refreshToken }).del();
		...
		if (options?.session) {
			...
			await this.knex('clairview_sessions')
				.update({
		...
		if (record.user_id) {
			await this.knex('clairview_users').update({ last_access: new Date() }).where({ id: record.user_id });
		}
	...
	private async updateStatefulSession(
		...
		const updatedSession = await this.knex('clairview_sessions')
			.update(
				{
		...
		if (updatedSession.length === 0) {
			...
			const { next_token } = await this.knex('clairview_sessions')
				.select('next_token')
	...
	async logout(refreshToken: string): Promise<void> {
		const record = await this.knex
			...
			.from('clairview_sessions as s')
			.innerJoin('clairview_users as u', 's.user', 'u.id')
			.where('s.token', refreshToken)
			.first();
		...
		if (record) {
			...
			await this.knex.delete().from('clairview_sessions').where('token', refreshToken);
		}
	}
