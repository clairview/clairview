...
import { ForbiddenError, InvalidPayloadError, RecordNotUniqueError } from '@clairview/errors';
import type { Item, PrimaryKey, RegisterUserInput, User } from '@clairview/types';
import { getSimpleHash, toArray, validatePayload } from '@clairview/utils';
import { FailedValidationError, joiValidationErrorItemToErrorExtensions } from '@clairview/validation';
import Joi from 'joi';
...
export class UsersService extends ItemsService {
	 ...
	 * clairview_settings.auth_password_policy
	 */
	...
	override async deleteMany(keys: PrimaryKey[], opts: MutationOptions = {}): Promise<PrimaryKey[]> {
		...
		// Manual constraint, see https://github.com/clairview/clairview/pull/19912
		await this.knex('clairview_comments').update({ user_updated: null }).whereIn('user_updated', keys);
		await this.knex('clairview_notifications').update({ sender: null }).whereIn('sender', keys);
		await this.knex('clairview_versions').update({ user_updated: null }).whereIn('user_updated', keys);
	...
	async requestPasswordReset(email: string, url: string | null, subject?: string | null): Promise<void> {
		...
		const token = jwt.sign(payload, getSecret(), { expiresIn: '1d', issuer: 'clairview' });
	...
	async resetPassword(token: string, password: string): Promise<void> {
		const { email, scope, hash } = jwt.verify(token, getSecret(), { issuer: 'clairview' }) as {
			email: string;
			scope: string;
			hash: string;
		};
