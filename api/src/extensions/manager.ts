...
import type {
	...
	HybridExtension,
	OperationApiConfig,
} from '@clairview/extensions';
import { APP_SHARED_DEPS, HYBRID_EXTENSION_TYPES } from '@clairview/extensions';
import { generateExtensionsEntrypoint } from '@clairview/extensions/node';
import type {
	ActionHandler,
	EmbedHandler,
	FilterHandler,
...
import { isTypeIn, toBoolean } from '@clairview/utils';
import { pathToRelativeUrl, processId } from '@clairview/utils/node';
...
export class ExtensionManager {
	...
	private async registerSandboxedApiExtension(extension: ApiExtension | HybridExtension) {
		...
		await module.instantiate(context, (specifier) => {
			if (specifier !== 'clairview:api') {
				throw new Error('Imports other than "clairview:api" are prohibited in API extension sandboxes');
			}
