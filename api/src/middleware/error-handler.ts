import { ErrorCode, InternalServerError, isClairviewError } from '@clairview/errors';
import type { DeepPartial } from '@clairview/types';
import { isObject } from '@clairview/utils';
import { getNodeEnv } from '@clairview/utils/node';
...
export const errorHandler = asyncErrorHandler(async (err, req, res) => {
	...
	for (const error of receivedErrors) {
		...
		if (isClairviewError(error)) {
			...
			if (isClairviewError(error, ErrorCode.MethodNotAllowed)) {
