---
description: A guide on how to build custom hooks in Clairview.
readTime: 7 min read
---
...
Below is an example of canceling a `create` event by throwing a Clairview error.
...
Init hooks execute at a defined point within the life cycle of Clairview. Use init hook objects to inject logic into
internal services.
...
Schedule hooks execute at certain points in time rather than when Clairview performs a specific action. This is supported
...
Clairview reads system collection data to perform correctly, both in the Data Studio and the generated APIs. Be careful
when modifying the output of system collection read/query events, as this can cause Clairview core functionality to
break.
...
- `emitter` — [Event emitter](https://github.com/clairview/clairview/blob/main/api/src/emitter.ts) instance that can be
