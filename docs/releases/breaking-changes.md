---
description: A list of any actions you may need to take on upgrades of Clairview.
---

# Breaking Changes

As we continue to build Clairview, we occasionally make changes that change how certain features works. We try and keep
...
Starting with Clairview 10.0, here is a list of potential breaking changes with remedial action you may need to take.

## Version 11.1.2

### New Comment Endpoints

We've introduced a dedicated `clairview_comments` collection, replacing the previous system that used `clairview_activity`
...
The SDK's internal comment endpoints have been updated to reflect this change. To avoid errors, ensure your Clairview
...
The SendGrid abstraction for `nodemailer` is no longer supported, so we have dropped it's usage from Clairview. Users of
SendGrid should update their configuration to use their SendGrid account's SMTP Relay configuration instead.

## Version 11.0.0

Clairview 11 introduces policies, a new concept within access control configuration. Permissions are no longer held in
...
```
GET https://example.clairview.app/items/example?fields=items.item.m2a_field
```

This no longer works and you must specify which collection the field is located in:

```
GET https://example.clairview.app/items/example?fields=items.item:m2a_collection.m2a_field
```

[Understand the M2A field syntax in our global query parameter page](https://docs.clairview.io/reference/query).
...
extensions folder prefixed with `clairview-extension-`) no action is required at this point. If you're currently relying
...
Clairview will ignore extensions that use the legacy format starting in this version.
...
If your extension does not already have one, add a `clairview:extension` object to your `package.json` file:

```json
{
	"name": "clairview-extension-hello-world",
	"version": "1.0.0",
	"type": "module",
	"clairview:extension": {
		"type": "endpoint",
...
- Make sure `clairview:extension.type` matches the type of extension. This should match the legacy type folder name.
- Make sure `clairview:extension.path`points to your extensions’ `dist` file.
- Make sure `clairview:extension.source` points to your extensions’ source code entry point or set to an empty string
  `""` when the source code is not stored alongside the `package.json` file.
- Make sure `clairview:extension.host` is set to a Clairview version range your extension is compatible with (for example:
  `^10.0.0`)
...
resolved rather than returned as a raw changes object (see [#20890](https://github.com/clairview/clairview/issues/20890)
...
applications, you can set `AUTH_<PROVIDER>_MODE=cookie`. This will however not work with the Clairview app.
...
Previously, the [Send Email](https://docs.clairview.io/app/flows/operations.html#send-email) Flow Operation has waited
...
Exif tags stored in Clairview Files to not change name.
...
Any scoped imports of `@clairview/sdk` will need updating to import functions from the root.
...
import { rest } from '@clairview/sdk/rest';
```
...
Environment handling has been moved to a new `@clairview/env` package. With this new package, ESM config files are still
...
While this is not a breaking change, you are advised to regenerate the snapshot after the version update of Clairview,
...
The `ExtensionItem` type has been renamed to `ClairviewExtension` to be inline with other system collections.
...
Prior to this release, Clairview relied on `vm2` to run code from **Run Script** operations in Flows - our automation
feature. `vm2` is now unmaintained with critical security issues that could potentially allow code to escape the sandbox
and potentially access the machine which hosts your Clairview project. We have migrated to `isolated-vm` to allow Flows
...
Clairview had various different functionalities that required you to use Redis when running Clairview in a horizontally
...
Clairview used to support either memory, Redis, or Memcached for caching and rate-limiting storage. Given a deeper
integration with Redis, and the low overall usage/adoption of Memcached across Clairview installations, we've decided to
sunset Memcached in favor of focusing on Redis as the primary solution for pub/sub and hot-storage across load-balanced
Clairview installations.
