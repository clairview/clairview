---
description: REST and GraphQL API documentation on the Settings collection in Clairview.
readTime: 3 min read
...
```js
import { createClairview, rest, readSettings } from '@clairview/sdk';
...
```graphql
type Mutation {
	update_settings(data: update_clairview_settings_input!): clairview_settings
}
```
...
```js
import { createClairview, rest, updateSettings } from '@clairview/sdk';
