---
description: REST and GraphQL API documentation on the Revisions collection in Clairview.
readTime: 2 min read
...
> Revisions are individual changes to items made. Clairview keeps track of changes made, so you're able to revert to a
...
List all revisions that exist in Clairview.
...
```js
import { createClairview, rest, readRevisions } from '@clairview/sdk';
...
```graphql
type Query {
	revisions_by_id(id: ID!): clairview_revisions
}
```
...
```js
import { createClairview, rest, readRevision } from '@clairview/sdk';
