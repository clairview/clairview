---
description: REST and GraphQL API documentation on the Webhooks collection in Clairview.
readTime: 5 min read
...
```js
import { createClairview, rest, readWebhooks } from '@clairview/sdk';
...
```graphql
type Query {
	webhooks_by_id(id: ID!): clairview_webhooks
}
```
...
```js
import { createClairview, rest, readWebhook } from '@clairview/sdk';
...
```graphql
type Mutation {
	create_webhooks_item(data: create_clairview_webhooks_input!): clairview_webhooks
}
```
...
```js
import { createClairview, rest, createWebhook } from '@clairview/sdk';
...
const result = await client.request(
	createWebhook({
		...
		url: 'https://clairview.example.com/articles_activity',
	})
);
```
...
```graphql
type Mutation {
	create_webhooks_items(data: [create_clairview_webhooks_input!]!): [clairview_webhooks]
}
```
...
```js
import { createClairview, rest, createWebhooks } from '@clairview/sdk';
...
const result = await client.request(
	createWebhooks([
		...
		},
		{
			...
			collections: 'authors',
			actions: ['update'],
			url: 'https://clairview.example.com/authors_changes',
		},
	])
);
```
...
```graphql
type Mutation {
	update_webhooks_item(id: ID!, data: update_clairview_webhooks_input!): clairview_webhooks
}
```
...
```js
import { createClairview, rest, updateWebhook } from '@clairview/sdk';
...
```graphql
type Mutation {
	update_webhooks_items(ids: [ID!]!, data: update_clairview_webhooks_input!): [clairview_webhooks]
}
```
...
```js
import { createClairview, rest, updateWebhooks } from '@clairview/sdk';
...
```js
import { createClairview, rest, deleteWebhook } from '@clairview/sdk';
...
```js
import { createClairview, rest, deleteWebhooks } from '@clairview/sdk';
