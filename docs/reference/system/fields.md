---
description: REST and GraphQL API documentation on the Fields collection in Clairview.
readTime: 7 min read
...
`type` **string**\
The Clairview data type of the field. See [Types](/user-guide/overview/glossary#types) for possible options.

#### Meta

Clairview metadata, primarily used in the Data Studio. Meta is optional.

`id` **integer**\
Primary key of the metadata row in `clairview_fields`.
...
are available for all drivers. Note: schema is optional. If a field exist in clairview_fields, but not in the database,
...
`is_nullable` **boolean**\
Whether or not the column is nullable. This is what is used as the "required" state in Clairview.
...
```js
import { createClairview, rest, readFields } from '@clairview/sdk';
...
```graphql
type Query {
	fields_in_collection(collection: String!): clairview_fields
}
```
...
```js
import { createClairview, rest, readFieldsByCollection } from '@clairview/sdk';
...
```graphql
type Query {
	fields_by_name(collection: String!, field: String!): clairview_fields
}
```
...
```js
import { createClairview, rest, readField } from '@clairview/sdk';
...
```graphql
type Mutation {
	create_fields_item(collection: String!, data: create_clairview_fields_input!): clairview_fields
}
```
...
```js
import { createClairview, rest, createField } from '@clairview/sdk';
...
`type` **Required**\
One of the Clairview types. This in turn controls what datatype is used for the column in the database. Setting the type to
...
```graphql
type Mutation {
	update_fields_item(collection: String!, field: String!, data: update_clairview_fields_input!): clairview_fields
}
```
...
```js
import { createClairview, rest, updateField } from '@clairview/sdk';
...
```js
import { createClairview, rest, deleteField } from '@clairview/sdk';
