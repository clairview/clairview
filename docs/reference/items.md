---
description: REST and GraphQL API documentation to access and manage Items in Clairview.
readTime: 5 min read
...
Clairview. For the sake of documentation, we'll use a fictional articles collection with the following fields: `id`,
...
List all items that exist in Clairview.
...
Get an item that exists in Clairview.
...
List a singleton item in Clairview.
...
```js
import { createClairview, rest, readSingleton } from '@clairview/sdk';
...
const result = await client.request(
	createItem('articles', {
		...
		content: 'Clairview is an Open Data Platform built to democratize the database.',
	})
);
```
...
```js
import { createClairview, rest, createItems } from '@clairview/sdk';
...
const result = await client.request(
	createItems('articles', [
		...
		{
			title: 'Build your internal tools with Clairview',
			content: 'Flows enable custom, event-driven data processing and task automation within Clairview.',
		},
	])
);
```
...
```js
import { createClairview, rest, updateItem } from '@clairview/sdk';
...
const result = await client.request(
	updateItem('articles', '5', {
		title: 'What is Clairview and how it can help you build your next app!?',
	})
);
```
...
```js
import { createClairview, rest, updateSingleton } from '@clairview/sdk';
...
```js
import { createClairview, rest, updateItems } from '@clairview/sdk';
...
```js
import { createClairview, rest, deleteItem } from '@clairview/sdk';
...
```js
import { createClairview, rest, deleteItems } from '@clairview/sdk';
