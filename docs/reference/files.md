---
description: REST and GraphQL API documentation for file access and management in Clairview.
...
> tracked within the `clairview_files` system collection. Any requested file transformations are handled on the fly, and
> are only saved to storage.

## File Security

Data and permissions around files are associated to the `clairview_files` collection.
...
example.com/assets/1ac73658-8b62-4dea-b6da-529fbc9d01a4/clairview-logo.png
```
...
there, it is recommended that you always use the Clairview API. This is the only way that you can take advantage of file
permissions and other built-in features.

:::

![Original File](https://cdn.clairview.io/docs/v9/reference/files/original-20220216A.jpg) _Original File Used — 602KB and
1800x1200_
...
```html
<a href="https://your-clairview.com/assets/<file-id>?download" target="_blank" download="Your File.pdf">Download</a>
```
...
For even more advanced control over the file generation, Clairview exposes
...
| 25%                                                                                             | 50%                                                                                             | 75%                                                                                             | 100%                                                                                               |
| ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| ![25%](https://cdn.clairview.io/docs/v9/reference/files/200-200-cover-25-20220216A.jpg)<br>_4KB_ | ![50%](https://cdn.clairview.io/docs/v9/reference/files/200-200-cover-50-20220216A.jpg)<br>_6KB_ | ![75%](https://cdn.clairview.io/docs/v9/reference/files/200-200-cover-75-20220216A.jpg)<br>_8KB_ | ![100%](https://cdn.clairview.io/docs/v9/reference/files/200-200-cover-100-20220216A.jpg)<br>_38KB_ |
...
```js
import { createClairview, rest, readAssetRaw } from '@clairview/sdk';
...
Clairview will crop assets when requested with a `width` or `height` query parameter. By default, images are cropped
...
`metadata` **object**\
Any additional metadata Clairview was able to scrape from the file. For images, this includes Exif, IPTC, and ICC information.
...
List all files that exist in Clairview.
...
```js
import { createClairview, rest, readFiles } from '@clairview/sdk';
...
```graphql
type Query {
	files_by_id(id: ID!): clairview_files
}
```
...
```js
import { createClairview, rest, uploadFiles } from '@clairview/sdk';
...
```js-vue [index.js]
import { createClairview, rest, uploadFiles } from 'https://unpkg.com/@clairview/sdk@{{ packages['@clairview/sdk'].version.major }}';
...
```graphql
type Mutation {
	import_file(url: String!, data: create_clairview_files_input!): clairview_files
}
```
...
```js
import { createClairview, rest, importFile } from '@clairview/sdk';
...
```graphql
type Mutation {
	update_files_item(id: ID!, data: update_clairview_files_input!): clairview_files
}
```
...
```js
import { createClairview, rest, updateFile } from '@clairview/sdk';
...
```graphql
type Mutation {
	update_files_items(ids: [ID!]!, data: update_clairview_files!): [clairview_files]
}
```
...
```js
import { createClairview, rest, updateFiles } from '@clairview/sdk';
...
```js
import { createClairview, rest, deleteFile } from '@clairview/sdk';
...
```js
import { createClairview, rest, deleteFiles } from '@clairview/sdk';
