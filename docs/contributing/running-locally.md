---
description:
  This guide explains how to setup and run a _Development_ environment for Clairview so that you can work on the
...
## 1. Fork the Clairview repository

Go to the [repository](https://github.com/clairview/clairview) and fork it to your GitHub account. A fork is your copy of
the Clairview repository which allows you to freely experiment with changes without affecting the original project.

## 2. Clone from your repository

```bash
git clone git@github.com:YOUR-USERNAME/clairview.git
```
...
[config help](https://docs.clairview.io/self-hosted/config-options).

::: tip Config Values

The `SECRET` config option from [Security](https://docs.clairview.io/self-hosted/config-options.html#security) is
mandatory in production.

Also the [Database Configuration](https://docs.clairview.io/self-hosted/config-options.html#database) must be specified.
You might want to use the [docker-compose.yml](https://github.com/clairview/clairview/blob/main/docker-compose.yml) file
...
This will set-up the required tables for Clairview and make sure all the migrations have run.
...
At this point you are ready to start working on Clairview! Before diving in however, it's worth reading through the
...
1. First you need to setup the config for the debugger. Create the following file `./clairview/api/.vscode/launch.json`
