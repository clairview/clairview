...
export default {
	...
	'/plus/': await sidebarClairviewPlus(),
...
function sidebarDeveloperReference() {
	return [
		...
		},
		{
			...
			items: [
				{
					link: 'https://github.com/clairview/clairview/releases',
					text: 'Release Notes',
				},
				{
...
function typeDocSidebarFormat(item: DefaultTheme.SidebarItem) {
	...
	if (item.text?.startsWith('@clairview/')) {
		item.items?.unshift({
			text: 'Overview',
		...
		item.text = formatTitle(item.text.replace('@clairview/', ''));
...
async function sidebarClairviewPlus() {
