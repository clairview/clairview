...
Modules are an empty canvas in Clairview with an empty navigation panel on the left, page header at the top and the
sidebar on the right. This guide will help you set up a multi-page module with navigation in the navigation bar and link
breadcrumbs.

![A custom module has three items in the navigation - Home, Hello World, and Contact Us. The homepage displays an image, three navigation tiles, and some copy.](https://marketing.clairview.app/assets/e1d33578-4ebc-4294-9b7c-2cc9ee3a1bfb.png)
...
`clairview-extension-module-landing-page`). For this guide, select JavaScript.
...
As it stands, this module will load an empty page wrapped by the Clairview UI:

![An empty module.](https://marketing.clairview.app/assets/976ebe2d-4d62-4136-a238-ae2107a8e417.png)
...
import { useApi } from '@clairview/extensions-sdk';
```
...
Clairview has a header element at the top of the module that uses the title attribute of the private view as the page
...
![Breadcrumb showing only Home](https://marketing.clairview.app/assets/53bd50fc-a247-42d6-9d40-93a97d689eec.png)

When the page changes to `/admin/landing-page/hello-world`, the page title changes and the breadcrumbs are updated:

![Breadcrumb showing both Home and Hello World as a second level item](https://marketing.clairview.app/assets/e9aa47d8-0e2e-409a-81a8-f715be739917.png)
...
for a built-in Clairview component called `v-list`. Ideally this function will use an API to fetch this information:
...
Here is an example of the above code as an API using a collection in Clairview called `pages`:
...
![A module is empty but shows the navigation with three items.](https://marketing.clairview.app/assets/aaed210a-7706-42c2-b1e4-558cf481c2a1.png)
...
Since [Clairview version 10.10.0](/releases/breaking-changes.html#version-10-10-0) the query parameter authentication is
...
`use-clairview-token.js` and copy the following code:
...
setup(props) {
	...
	const api = useApi();
	const { addTokenToURL } = useClairviewToken(api);
...
instead prefix each class with a unique reference to prevent changing other components in Clairview. In this example, use
the following SCSS:
...
This will format the banner, cards and the container. It’s a good idea to make use of the native CSS of Clairview as much
as possible so your module appears part of Clairview.
...
with `clairview-extension`. In this case, you may choose to use `clairview-extension-module-landing-page`.
...
To use your new module in Clairview, you need to enable it in the
...
images from within Clairview to surface on the page. From here you can create content rich modules driven by the features
of the Clairview platform.
...
import PageNavigation from './components/navigation.vue';
import useClairviewToken from './use-clairview-token.js';
