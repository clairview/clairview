...
![A dropdown showing a number of values](https://marketing.clairview.app/assets/46b4070d-fd77-4ee3-b08b-654519dda031)
...
`clairview-interface-m2o-searchable-dropdown`). For this guide, select JavaScript.
...
Setting `relational: true` will force Clairview to only offer this interface on a relational field. Finally,
...
the Clairview Extensions SDK right before `export default`:
...
Inside the `template` tag, add the following code which uses Clairview' provided components. For the `v-input`, pass the
...
with `clairview-extension`. In this case, you may choose to use `clairview-extension-interface-m2o-searchable-dropdown`.
...
![An interface configuration showing the new M2O Dropdown with Search option](https://marketing.clairview.app/assets/95195049-82f4-4494-8ed7-9a1a2dcd184d)
