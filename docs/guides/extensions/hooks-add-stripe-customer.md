...
account when an item is created in Clairview and write the customer ID back to the record.
...
`clairview-hook-create-stripe-customer`). For this guide, select JavaScript.
...
customer is created in Clairview.
...
Inside the function, define the internal `ItemsService` Clairview API function from the `services` scope. Also include
...
`env` looks inside the Clairview environment variables for `STRIPE_TOKEN`. In order to start using this hook, this
...
stripe.customers
	...
	.catch((error) => {
		...
		mailService.send({ // [!code ++]
			to: 'sharedmailbox@clairview.io', // [!code ++]
			from: 'noreply@clairview.io', // [!code ++]
...
with `clairview-extension`. In this case, you may choose to use `clairview-extension-hook-create-stripe-customer`.
...
and write back the customer ID to Clairview. Now that you know how to interact with the Stripe API, you can investigate
	...
	action('items.create', async ({ key, collection, payload }, { schema }) => {
		...
		stripe.customers
			...
			.catch((error) => {
				const mailService = new MailService({ schema });
				mailService.send({
					to: 'sharedmailbox@clairview.io',
					from: 'noreply@clairview.io',
