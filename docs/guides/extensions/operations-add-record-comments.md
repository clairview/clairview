...
![An Add Comment operation in a Flow](https://marketing.clairview.app/assets/5b20c836-39f1-4905-95ad-e829f1a65efc.png)
...
`clairview-operation-add-comment`). For this guide, select JavaScript.
...
<img src="https://marketing.clairview.app/assets/7033ae7b-178f-489a-aa7f-170d498e6c83.png" alt="Add comment - add a comment to a record. A chat icon is displayed in the box." style="padding:2px 6px;"/>
...
- `comment_key` - field where the ID of the item is entered. This will accept a string, guid, int or a variable from the
  Flow. This must be an existing record in Clairview.
...
<img src="https://marketing.clairview.app/assets/000d7d2a-8bdf-400f-8e46-2a6c018a58b3.png" alt="Add comment operation is selected. There are three fields - collection, IDs, and comment. Collection is a dropdown and IDs is a key." style="padding:6px 8px;"/>
...
<img src="https://marketing.clairview.app/assets/1eaa545e-05a0-4b1c-b921-1d56902ac10d.png" alt="The flow overview card shows a collection and comment value." style="max-width: 400px;"/>
...
Comments are stored inside the `clairview_activity` table so the `ActivityService` is required to perform the action.
...
<img src="https://marketing.clairview.app/assets/4155f7fa-41af-4af1-8b96-d3e7b26f20db.png" alt="Inside of the sidebar, the comment pane shows one new comment from Tim Butterfield saying 'reminder sent'" style="max-width: 300px; padding: 1em 1em 1em 0;"/>
...
with `clairview-extension`. In this case, you may choose to use `clairview-extension-operation-add-comment`.
...
In the Clairview Data Studio, open the Flows section in Settings. Create a new flow with an event trigger such as
...
<img src="https://marketing.clairview.app/assets/0bc1022b-2da7-4064-8c80-0fc32da59159.png" alt="Select Add Comment. Inside of Collection, type {{$trigger.payload.collection}}. Inside of IDs, type {{$trigger.payload.keys}}. For the comment, type any that you would like to add as a comment." style="padding:6px 8px;"/>
...
an array of activity IDs for the comments. This operation requires setting up fields on the front-end and using Clairview
