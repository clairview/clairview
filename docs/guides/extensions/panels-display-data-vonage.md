...
in their Clairview collections. In this guide, you will instead fetch data from an external API and display it in a table
as part of a panel.

<img src="https://marketing.clairview.app/assets/31ac7437-99ed-44fe-9b75-b21c92198fda.png" alt="Table with header Messages shows several items with status, sent delative date, a recipient ID, and a provider">

Panels can only talk to internal Clairview services, and can't reliably make external web requests because browser
...
You will need a Clairview project - check out our [quickstart guide](https://docs.clairview.io/getting-started/quickstart)
...
`clairview-extension-bundle-vonage-activity`).

Now the boilerplate bundle has been created, navigate to the directory with
`cd clairview-extension-bundle-vonage-activity` and open the directory in your code editor.
...
endpoint), and type a name for your extension (for example, `clairview-endpoint-vonage`). For this guide, select
JavaScript.

This will add an entry to the `clairview:extension` metadata in your `package.json` file.
...
Open the `src/clairview-endpoint-vonage/index.js` file and replace it with the following:
...
In your terminal, run `npm run add` to create a new extension in this bundle. A list of options will appear (choose
panel), and type a name for your extension (for example, `clairview-panel-vonage-activity`). For this guide, select
JavaScript.
...
<img src="https://marketing.clairview.app/assets/053298df-a5ff-4fd5-85ec-ff0f78bb64ca.png" alt="Form shows product type dropdown, direction dropdown, date range dropdown, included message checkbox, and status dropdown." style="padding: 2px 12px;">
...
import { adjustDate } from '@clairview/shared/utils';
...
directory into the Clairview extensions `directory`. Make sure the directory with your bundle has a name that starts with
`clairview-extension`. In this case, you may choose to use `clairview-extension-bundle-vonage-activity`.

Restart Clairview to load the extensions.
...
<script>
import { useApi } from '@clairview/extensions-sdk';
import { adjustDate } from '@clairview/utils';
import { formatISO, formatDistanceToNow, parseISO } from 'date-fns';
import { ref, watch } from 'vue';
export default {
	props: {
