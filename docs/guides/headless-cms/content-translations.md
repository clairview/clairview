...
![A Clairview item detail page is shown using the Translations interface. Editable fields for the English and French translations for an article are displayed side-by-side. Fields include Title, Slug, Summary, and Content. ](https://marketing.clairview.app/assets/64eee60e-605b-4d2c-809f-83557ed0d5ef.png?key=doc)
...
- Familiarity with [creating collections](/app/data-model/collections#create-a-collection) and
  [creating fields](/app/data-model/fields#create-a-field-standard) inside Clairview.
...
1. Go to Settings > Data Model and create a new collection for `languages`.
   ...
   ![The Creating New Collection form overlay is shown. The active tab is Collection Setup. Name, Singleton, Primary Key Field, and Type fields are shown and editable by the user.](https://marketing.clairview.app/assets/e4fe9a63-9cfd-48d9-a1a9-7aedff6279a9.png?key=doc)
...
2. Create a new collection for `articles`.
   ...
   - image (Type: Image)
   - author (Type: M2O Relation, Related Collection: clairview_users)
   - date_published (Type: Timestamp, Interface: Datetime)
   ```

   ![Data Model settings screen for the Articles collection is displayed. The following fields are shown: id, status, date_published, author, image.](https://marketing.clairview.app/assets/fdeb07d9-d11a-42e9-a39a-14f0a196ecc6.png?key=doc)
...
By default Clairview will name this junction collection `articles_translations` following the convention
...
4. Navigate to the newly created `articles_translations` collection and create the fields for the content you'll
   ...
   ![Data Model settings screen for the Articles Translations collection is displayed. The following fields are shown: id, articles_id, languages_code, title, slug, summary, content.](https://marketing.clairview.app/assets/e2ee5c61-3449-40cf-9cd4-24f9edb2ae6a.png?key=doc)
...
5. Before you create your first article, you'll need to add a few items to the `languages` collection. Navigate to the
   ...
   ![The table layout for the Languages collection is shown. There are two items displayed. One for English and one for French.](https://marketing.clairview.app/assets/9afe044e-405c-46e7-851f-2a400f3672ea.png?key=doc)
...
6. In the Articles collection, create a new article and scroll down to the Translations field and start translating your
   ...
   ![A dropdown menu with a progress indicator for English and French languages are shown. The progress is less than half and shown in red.](https://marketing.clairview.app/assets/78a895e9-744e-4d33-9d87-3c5ca4c784e2.png?key=doc)
...
// Call the Clairview API using the SDK using the locale of the frontend and the slug.
...
Over the course of this guide, you've setup the basics for translating your content within Clairview. Nice work!
