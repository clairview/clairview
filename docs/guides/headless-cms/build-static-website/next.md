---
description: This guide shows you how build a website with Next.js and Clairview as a Headless CMS.
clairview_version: 10
author: Kevin Lewis
---

# Build a Website With Next.js and the Clairview JavaScript SDK

> {{ $frontmatter.description }}

[Next.js](https://nextjs.org/) is a popular JavaScript framework based on React.js. In this tutorial, you will learn how
to build a website using Clairview as a [Headless CMS](https://clairview.io/solutions/headless-cms). You will store,
...
scratch and install the Clairview JavaScript SDK:
...
To share a single instance of the Clairview JavaScript SDK between multiple pages in this project, create a single helper
file that can be imported later. Create a new directory called `lib` and a new file called `clairview.js` inside of it.
...
```js
const clairview = createClairview('https://clairview.example.com').with(
  rest({
...
Ensure your Project URL is correct when initializing the Clairview JavaScript SDK.
...
![A form named "Global" has two inputs - a title and a description, each filled with some text.](https://cdn.clairview.io/docs/v9/headless-cms/how-to-packet-20220222A/next-global-config.webp)
...
You should see data from your Clairview Global collection in your page. Some additional files will be created by Next
...
async function getPage(slug) {
	try {
		const page = await clairview.request(readItem('pages', slug));
		return page;
	} catch (error) {
		notFound();
	}
...
![A page with a title of "Blog". On it is a list of three items - each with a title, author, and date. The title is a link.](https://cdn.clairview.io/docs/v9/headless-cms/how-to-packet-20220222A/next-blog-listing.webp)
...
async function getPost(slug) {
	try {
		const post = await clairview.request(
			readItem('posts', slug, {
...
![A blog post page shows an image, a title, and a number of paragraphs.](https://cdn.clairview.io/docs/v9/headless-cms/how-to-packet-20220222A/next-blog-single.webp)

## Add Navigation

While not strictly Clairview-related, there are now several pages that aren't linked to each other. Create the file
...
Through this guide, you have set up a Next project, created a Clairview helper, and used it to query data. You have used
...
[on GitHub](https://github.com/clairview/examples/blob/main/website-next13).
