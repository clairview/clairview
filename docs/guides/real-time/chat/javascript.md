...
Inside of the `<script>`, create a `url` variable being sure to replace `your-clairview-url` with your project’s URL:
...
<html>
	<body>
		<script>
			import { createClairview, authentication, realtime } from 'https://www.unpkg.com/@clairview/sdk/dist/index.js'; // [!code ++]
...
Within the `#new` form submit event handler, send a new message to create the item in your Clairview collection:
