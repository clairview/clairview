...
static int parser_modexec(PyObject* module) {
  ...
  state->ast_module = PyImport_ImportModule("markettor.hogql.ast");
  if (!state->ast_module) {
    return -1;
  }
  state->base_module = PyImport_ImportModule("markettor.hogql.base");
  if (!state->base_module) {
    return -1;
  }
  state->errors_module = PyImport_ImportModule("markettor.hogql.errors");
